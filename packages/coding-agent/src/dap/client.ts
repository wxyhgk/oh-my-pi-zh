...
import { isEnoent, logger, ptree } from "@wxyhgk/pi-utils";
...
export class DapClient {
	...
	static async #spawnSocketClientAddr({
		...
		const connectTimeout = setTimeout(
			() => rejectTimeout(new Error(`${adapter.name} 未在 ${timeoutMs}ms 内建立连接`)),
			timeoutMs,
		);
		try {
	...
	async waitForEvent<TBody>(
		...
		timeout = setTimeout(() => {
			cleanup();
			reject(new Error(`DAP 事件 ${event} 在 ${timeoutMs}ms 后超时`));
		}, timeoutMs);
		return promise;
	}
	...
	async sendRequest<TBody = unknown>(
		...
		if (this.#disposed) {
			throw new Error(`DAP 适配器 ${this.adapter.name} 未在运行`);
		}
		...
		timeout = setTimeout(() => {
			...
			this.#pendingRequests.delete(requestSeq);
			cleanup();
			reject(new Error(`DAP 请求 ${command} 在 ${timeoutMs}ms 后超时`));
		}, timeoutMs);
		if (signal) {
			signal.addEventListener("abort", abortHandler, { once: true });
		}
	...
	async #writeMessage(message: DapRequestMessage | DapResponseMessage): Promise<void> {
		...
		if (this.#adapterExited) {
			throw new Error(`DAP 适配器 ${this.adapter.name} 在写入完成前已退出`);
		}
		...
		const rejectOnExit = () => {
			guardReject(new Error(`DAP 适配器 ${this.adapter.name} 在写入完成前已退出`));
		};
	...
	async dispose(): Promise<void> {
		...
		this.#disposed = true;
		this.#rejectPendingRequests(new Error(`DAP 适配器 ${this.adapter.name} 已释放`));
		try {
			this.#socket?.end();
		} catch {
	...
	async #startMessageReader(): Promise<void> {
		...
		try {
			while (true) {
				...
				for (const messageText of framer.drain(headerText => {
					...
					logger.warn("DAP 帧重同步:头部缺少 Content-Length", {
						adapter: this.adapter.name,
			...
			closeError = new Error(`DAP 连接已关闭: ${toErrorMessage(error)}`);
		} finally {
		...
		this.#failConnection(closeError ?? new Error(`DAP 连接已关闭: ${this.adapter.name} 传输已结束`));
	}
	...
	#handleResponse(message: DapResponseMessage): void {
		...
		const errorMessage = message.message ?? `DAP 请求 ${pending.command} 失败`;
	...
	async #handleAdapterRequest(message: DapRequestMessage): Promise<void> {
		try {
			...
			const errorMessage = `不支持的 DAP 请求: ${message.command}`;
			await this.sendResponse(
				message,
				false,
				{
...
async function waitForCondition(
	...
	while (Date.now() < deadline) {
		...
		if (proc.exitCode !== null) {
			throw new Error("适配器进程在 socket 就绪前已退出");
		}
		await Bun.sleep(50);
	}
	throw new Error(`socket 在 ${timeoutMs}ms 后仍未就绪`);
}
	...
	void Bun.connect({
		...
		socket: {
			...
			close() {
				onClose?.();
				if (!opened) {
					reject(new Error(`与 TCP 端口 ${host}:${port} 的连接在建立前已关闭`));
				}
				try {
...
async function waitForTcpTransport(
	...
	while (Date.now() < deadline) {
		if (proc.exitCode !== null) {
			throw new Error(`适配器进程在 TCP 端口 ${host}:${port} 就绪前已退出`);
		}
		try {
	...
	throw new Error(`TCP 端口 ${host}:${port} 在 ${timeoutMs}ms 后仍未就绪`);
}
...
export async function connectSocket(options: { unix: string }, timeoutMs: number): Promise<SocketTransport> {
	...
	const timer = setTimeout(() => {
		reject(new Error(`连接 unix socket ${options.unix} 超时(超过 ${timeoutMs}ms)`));
	}, timeoutMs);
	...
	Bun.connect({
		...
		socket: {
			...
			close() {
				if (!opened) {
					reject(new Error(`Unix socket ${options.unix} 在建立连接前已关闭`));
				}
				try {
					streamController.close();
				} catch {
