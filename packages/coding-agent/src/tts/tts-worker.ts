...
import { ensureRuntimeInstalled, getTinyModelsCacheDir, resolveRuntimeModule } from "@wxyhgk/pi-utils";
import {
	errorMessage,
	errorText,
...
function loadKokoroRuntime(
	...
	return kokoroRuntime.load(async () => {
		...
		if (!kokoroEntry) throw new Error(`无法在运行时 ${nodeModules} 中解析 ${KOKORO_PACKAGE}`);
		const transformersEntry = resolveRuntimeModule(nodeModules, TRANSFORMERS_PACKAGE);
		if (!transformersEntry) throw new Error(`无法在运行时 ${nodeModules} 中解析 ${TRANSFORMERS_PACKAGE}`);
...
async function loadModelWithDeviceFallback(
	...
	if (order[0] !== ttsDevicePreference.device) {
		sendLog(transport, "warn", "tts: 请求的设备在 worker 中不安全,改用 CPU", {
			modelKey,
			repo: spec.repo,
	...
	for (let i = 0; i < devices.length; i += 1) {
		...
		try {
			...
			sendLog(transport, "warn", "tts: 加速设备加载失败,正在回退", {
				modelKey,
				repo: spec.repo,
				device,
				fallbackDevice,
...
async function loadModel(
	...
	if (!spec) throw new Error(`未知的本地 TTS 模型:${modelKey}`);
	...
	const loaded = loadModelWithDeviceFallback(runtime, spec, modelKey, transport, requestId).then(
		({ model, device }) => {
			sendLog(transport, "debug", "tts: 本地模型加载完成", {
				modelKey,
				repo: spec.repo,
				device,
				requestedDevice: ttsDevicePreference.device,
...
async function synthesize(
	...
	if (!audio) throw new Error("Kokoro 语音合成未返回音频样本");
