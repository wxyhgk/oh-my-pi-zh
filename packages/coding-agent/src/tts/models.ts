...
export const KOKORO_VOICES: readonly TtsLocalVoiceSpec[] = [
	...
	{ id: "af_nicole", label: "Nicole(美式女声)" },
	{ id: "af_aoede", label: "Aoede(美式女声)" },
	{ id: "af_kore", label: "Kore(美式女声)" },
	{ id: "af_sarah", label: "Sarah(美式女声)" },
	{ id: "am_michael", label: "Michael(美式男声)" },
	{ id: "am_fenrir", label: "Fenrir(美式男声)" },
	{ id: "am_puck", label: "Puck(美式男声)" },
	{ id: "bf_emma", label: "Emma(英式女声)" },
	{ id: "bm_george", label: "George(英式男声)" },
	{ id: "bm_fable", label: "Fable(英式男声)" },
] as const;
...
export const TTS_LOCAL_MODELS = [
	{
		...
		sampleRate: 24_000,
		label: "Kokoro-82M",
		description: "Kokoro-82M 神经 TTS——端侧顶尖质量,多音色,完全本地运行",
...
export function resolveTtsRepo(modelKey: string | undefined): string {
	...
	if (!spec) throw new Error(`未注册 key 对应的本地 TTS 模型:${modelKey ?? DEFAULT_TTS_LOCAL_MODEL_KEY}`);
	return spec.repo;
}
