 ...
 *   model.
 */
import { type AssistantMessage, completeSimple } from "@wxyhgk/pi-ai";
	...
	async rewrite(block: string, signal?: AbortSignal): Promise<string | null> {
		try {
			...
			if (response.stopReason === "error") {
				logger.debug("speech-enhancer: 改写出错", { error: response.errorMessage });
				return null;
			}
			return extractText(response.content);
		} catch (error) {
			if (!signal?.aborted) {
				logger.debug("speech-enhancer: 改写失败", {
