 ...
 * `@wxyhgk/pi-utils` (or anything that transitively does). That package's
 ...
 * runtime imports from `@wxyhgk/pi-utils`, which is the whole reason it can
...
function parseMaxTimeSeconds(value: string): number {
	...
	throw new CliUsageError(
		`无效的 --max-time 值:${JSON.stringify(value)}。应为正数的秒数或类似 "5s"、"10m"、"1h" 的时长。`,
	);
}
...
export const STRING_SETTERS: Record<string, StringSetter> = {
	...
	"--service-tier": (result, value) => {
		if (!isServiceTierOpenAISettingValue(value)) {
			throw new CliUsageError(
				`无效的 --service-tier 值:${JSON.stringify(value)}。可选值:${SERVICE_TIER_OPENAI_VALUES.join(", ")}。`,
			);
		}
	...
	"--tools": (result, value, deps) => {
		...
		if (unknown.length > 0) {
			throw new CliUsageError(
				`--tools 中存在未知工具:${unknown.join(", ")}。有效工具:${deps.builtinToolNames.join(", ")}。`,
			);
		}
		result.tools = names;
	},
	"--thinking": (result, value, deps) => {
		...
		if (thinking !== undefined) {
			...
			deps.logger.warn("传给 --thinking 的思考级别无效", {
				level: value,
	...
	"--approval-mode": (result, value, deps) => {
		if (value === "always-ask" || value === "write" || value === "yolo") {
			...
			deps.logger.warn("传给 --approval-mode 的值无效", {
				value,
