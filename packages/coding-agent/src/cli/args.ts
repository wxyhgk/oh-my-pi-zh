 ...
 */
import { $env, APP_NAME, logger } from "@wxyhgk/pi-utils";
 ...
 * setter table itself can stay free of `@wxyhgk/pi-utils` runtime imports
...
export function reportUnrecognizedFlags(
	...
	write(`${chalk.red(`错误:未知标志: ${flags.join(", ")}`)}\n`);
	write(`运行 \`${APP_NAME} --help\` 可查看可用标志。\n`);
	return true;
}
...
export function printHelp(): void {
	process.stdout.write(
		`${chalk.bold(APP_NAME)} - AI 编程助手\n\n` +
			...
			`运行 ${APP_NAME} <command> --help 查看指定命令的帮助。\n\n` +
			`${getExtraHelpText()}\n`,
	);
}
