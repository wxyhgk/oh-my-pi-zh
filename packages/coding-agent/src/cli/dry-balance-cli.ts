...
function formatBenchAccount(account: string | undefined): string {
	return account ? sanitizeBenchText(account, BENCH_ACCOUNT_WIDTH) : chalk.dim("(无账户)");
}
...
async function runOneAttempt(
	...
	try {
		...
		if (!access) return { ok: false, reason: "未解析到 OAuth 访问权限" };
...
export async function runDryBalanceCommand(
	...
	try {
		...
		if (isBench) {
			...
			if (targets.length === 0) throw new Error(`提供商 ${model.provider} 未解析到任何 OAuth 账户`);
