...
import { normalizeProfileName } from "@wxyhgk/pi-utils/dirs";
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "omp-zh") {
		throw new Error("无效的别名“omp-zh”。拒绝覆盖基础 omp-zh 命令。");
	}
	if (getReservedAliasNames(shell).has(normalized.toLowerCase())) {
		throw new Error(`无效的别名“${aliasName}”。拒绝创建 ${shell} 的保留字。`);
	}
	return normalized;
}
...
function renderAliasBlock(
	...
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps omp-zh --description 'OMP profile ${profile}'`,
...
function upsertBlock(content: string, aliasName: string, block: string): string {
	...
	if (startIndex !== -1) {
		...
		if (endIndex === -1) {
			throw new Error(
				`在 shell 配置中找到了“${start}”但没有与之匹配的“${end}”。` +
					`受管理的别名块已损坏;请删除过期的标记行并重新运行 --alias。`,
			);
		}
...
export async function installProfileAlias(options: ProfileAliasInstallOptions): Promise<ProfileAliasInstallResult> {
	...
	if (!profile) {
		throw new Error("--alias 需要一个已命名的 --profile 值。");
	}
