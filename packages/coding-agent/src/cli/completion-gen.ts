 ...
 */
import type { ArgDescriptor, CliConfig, CommandMetadata, FlagDescriptor } from "@wxyhgk/pi-utils/cli";
...
${cmdRows}
	...
	for (const c of spec.commands) {
		const specs = ["'(-h --help)'{-h,--help}'[显示帮助]'", ...c.flags.map(zshFlagSpec), ...c.args.map(zshArgSpec)];
		parts.push(`_omp_cmd_${bashFn(c.name)}() {
	_arguments -s \\
		${specs.join(" \\\n\t\t")}
}`);
