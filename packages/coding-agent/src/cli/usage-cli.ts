...
import { formatDuration, formatNumber, sanitizeText } from "@wxyhgk/pi-utils";
...
function describeAmount(limit: UsageLimit): string {
	...
	if (fraction !== undefined) {
		parts.push(`${(fraction * 100).toFixed(1)}% 已用`);
	} else if (amount.remainingFraction !== undefined) {
		parts.push(`${(amount.remainingFraction * 100).toFixed(1)}% 剩余`);
	}
...
function accountIdentityLabel(account: UsageAccountIdentity, redaction?: Map<string, string>): string {
	...
	const base = account.email ?? account.accountId ?? account.projectId ?? account.enterpriseUrl ?? "OAuth 账户";
...
function formatAccountHeader(
	...
	if (savedResets > 0) {
		header += chalk.cyan(` · ✦ ${savedResets} 个已保存的重置`);
		...
		if (credits) {
			...
			if (upcoming) {
				header += chalk.dim(
					` · 最近到期时间:${formatDuration(upcoming.ms - nowMs)}(${upcoming.date.slice(0, 10)})`,
				);
			} else {
	...
	if (report.fetchedAt && nowMs - report.fetchedAt > 90_000) {
		header += chalk.dim(` · ${formatDuration(nowMs - report.fetchedAt)} 前获取`);
	}
	return header;
}
...
function formatLimitLine(limit: UsageLimit, labelWidth: number, nowMs: number): string[] {
	...
	if (resetsAt !== undefined && resetsAt > nowMs) {
		details.push(`${limit.window?.resetLabel ?? "重置"}于 ${formatDuration(resetsAt - nowMs)} 后`);
	}
	const lines = [
...
function formatMissingLimitLine(template: ProviderLimitTemplate, labelWidth: number): string {
	...
	return `      ${chalk.dim("○")} ${padded}  ${chalk.dim("·".repeat(BAR_WIDTH))}  ${chalk.dim("未报告")}`;
}
...
function formatReloginDeadline(
	...
	return `  ${chalk.yellow(`⚠ ${label} — 请在 ${formatDuration(remaining)} 内重新登录(Anthropic 的 OAuth 授权在登录后约 30 天过期)`)}`;
}
...
function disabledIdentityLabel(summary: DisabledCredentialSummary, redaction?: Map<string, string>): string {
	const base = summary.email ?? summary.accountId ?? "OAuth 账户";
	const masked = redaction?.get(base) ?? base;
...
export function formatUsageBreakdown(
	...
	const headerSuffix = latestFetchedAt ? chalk.dim(` · ${formatDuration(nowMs - latestFetchedAt)} 前获取`) : "";
	...
	for (const provider of providers) {
		...
		for (const summary of disabledByProvider.get(provider) ?? []) {
			...
			lines.push(
				`  ${chalk.red(`✗ ${label} — 已禁用${ago}:${sanitizeText(shortDisableCause(summary.cause))}`)} ${chalk.dim("(重新登录以恢复)")}`,
			);
		}
		...
		if (stats.length > 0) {
			const parts = stats.map(
				stat =>
					`${stat.window} → ${stat.usedAccounts.toFixed(2)}/${stat.accounts} 个账户已用(剩余 ${stat.remainingAccounts.toFixed(2)}× 配额)`,
			);
	...
	for (const provider of [...providers.keys()].sort((a, b) => a.localeCompare(b))) {
		...
		for (const account of sortedAccounts) {
			...
			for (const series of sortedSeries) {
				...
				details.push(`${series.entries.length} 条快照`);
				lines.push(
...
export async function runUsageCommand(cmd: UsageCommandArgs): Promise<void> {
	...
	try {
		if (cmd.action === "invalidate") {
			...
			if (provider) {
				process.stdout.write(`已使提供商“${provider}”的用量报告缓存失效。\n`);
			} else {
				process.stdout.write("已使所有提供商的用量报告缓存失效。\n");
			}
			return;
		}
		if (cmd.history) {
		...
		if (filteredReports.length === 0 && accounts.length === 0) {
			...
			const message =
				storedAccounts.length > 0
					? `没有用量数据${scope}。已存储的凭据属于没有用量端点的提供商。\n`
					: `未找到凭据${scope}。请运行 \`omp-zh\` 并使用 /login 添加账户。\n`;
