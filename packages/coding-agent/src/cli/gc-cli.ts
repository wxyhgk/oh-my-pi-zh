...
export function collectGcErrors(result: GcResult): string[] {
	return [
		...(result.blobs?.errors ?? []).map(error => `blob:${error}`),
		...(result.archive?.errors ?? []).map(error => `归档:${error}`),
	];
}
...
async function cleanupHistoryRowsForArchivedSessions(
	...
	try {
		...
		result.errors.push(`历史清理扫描:${errorMessage(error)}`);
	}
	...
	try {
		...
		result.errors.push(`历史清理:${errorMessage(error)}`);
	}
}
...
async function collectArchivedStatsSessions(
	...
	for (const file of await collectCompressedJsonlFiles(archiveRoot)) {
		...
		try {
			...
			if (!header) throw new Error("归档缺少有效的会话头部");
			sessions.push({
				path: sourcePath,
				id: header.id,
			...
			const retainedStatsSessions = await Promise.all(
				retainedSessions.map(async session => {
					...
					if (!header || header.id !== session.id) {
						throw new Error(`统计清理期间会话头部发生了变化:${session.path}`);
					}
					return {
						path: session.path,
						id: session.id,
...
async function checkpointWal(dbPath: string, apply: boolean): Promise<WalCheckpointResult> {
	...
	if (checkpointAttempted && (result.busy > 0 || result.walBytes > 0)) {
		throw new Error(`WAL 检查点对 ${dbPath} 失败:busy=${result.busy}, walBytes=${result.walBytes}`);
	}
...
function renderText(result: GcResult): string {
	const lines = [`GC ${result.apply ? "已应用" : "演练"}(${result.agentDir})`];
	if (result.blobs) {
		lines.push(
			`blob:已删除 ${result.blobs.deleted}/${result.blobs.wouldDelete} 个文件,${formatBytes(result.blobs.bytes)},${result.blobs.referenced} 个引用`,
		);
		if (result.blobs.errors.length > 0) lines.push(`blob 错误:${result.blobs.errors.length}`);
	}
	if (result.archive) {
		lines.push(
			`sessions:已归档 ${result.archive.archived}/${result.archive.wouldArchive} 个,移除 ${result.archive.historyRowsDeleted} 行历史记录和 ${result.archive.statsRowsDeleted} 行统计记录`,
		);
		if (result.archive.skippedActive > 0) lines.push(`跳过的活动会话:${result.archive.skippedActive}`);
		if (result.archive.errors.length > 0) lines.push(`会话错误:${result.archive.errors.length}`);
	}
	if (result.wal) {
		const state = result.wal.checkpointed ? "已检查点" : "检查点演练";
		lines.push(`wal:${state},${formatBytes(result.wal.walBytes)},跨 ${result.wal.databases.length} 个数据库`);
	}
	return `${lines.join("\n")}\n`;
}
