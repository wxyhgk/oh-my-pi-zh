...
import type * as MnemopiNs from "@wxyhgk/pi-mnemopi";
import type { Mnemopi, RecallResult } from "@wxyhgk/pi-mnemopi";
import type * as MnemopiCoreNs from "@wxyhgk/pi-mnemopi/core";
import type { LocalModelInitializer } from "@wxyhgk/pi-mnemopi/core";
import { logger, toError } from "@wxyhgk/pi-utils";
import {
	composeRecallQuery,
	formatCurrentTime,
...
function installLocalModelInitializer(setInitializer: (initializer: LocalModelInitializer) => void): void {
	...
	setInitializer(({ model, cacheDir }) =>
		mnemopiEmbedClient.initialize(model, cacheDir).then(handle => {
			...
			throw new Error("mnemopi 嵌入子进程不可用");
		}),
	);
}

/**
 * Lazily load `@wxyhgk/pi-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@wxyhgk/pi-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `@wxyhgk/pi-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@wxyhgk/pi-mnemopi/core");
...
export function requireMnemopi(): typeof MnemopiNs {
	if (!mnemopiMod) throw new Error("Mnemopi 模块未加载;请先 await loadMnemopi()。");
	return mnemopiMod;
}
...
export function requireMnemopiCore(): typeof MnemopiCoreNs {
	if (!mnemopiCoreMod) throw new Error("Mnemopi 核心模块未加载;请先 await loadMnemopiCore()。");
	return mnemopiCoreMod;
}
...
export class MnemopiSessionState {
	...
	formatScopedRecallWithIds(results: readonly RecallResult[]): string {
		...
		const lines = results.map(result => {
			const id = result.id ? ` (id: ${result.id})` : " (id 不可用)";
	...
	async collectScopedRecallResults(query: string): Promise<RecallResult[]> {
		...
		if (successfulTargets === 0 && failures.length > 0) {
			...
			throw new AggregateError(
				...
				`Mnemopi 对所有作用域目标的召回均失败(${details})`,
			);
		}
	...
	async maybeRecallOnAgentStart(): Promise<void> {
		...
		try {
			...
			if (this.config.debug) logger.debug("Mnemopi:召回后提示词刷新失败", { error: String(error) });
		}
	}
	...
	async dispose(options: { consolidate?: boolean; timeoutMs?: number } = {}): Promise<void> {
		...
		const consolidatePromise = this.consolidate({ full: false, extract: false, sleep: false }).catch(
			(error: unknown) => {
				logger.warn("Mnemopi:dispose 时合并失败。", { error: String(error) });
			},
		);
		if (deadline !== undefined) {
			...
			const completed =
				remainingMs > 0
					...
					: false;
			if (!completed) {
				logger.warn("Mnemopi:dispose 时合并超出关机预算;转后台执行。", {
					timeoutMs,
				});
