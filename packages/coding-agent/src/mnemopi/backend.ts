...
import { type ApiKeyResolver, completeSimple } from "@wxyhgk/pi-ai";
import { hostMatchesUrl } from "@wxyhgk/pi-catalog/hosts";
import type { Mnemopi } from "@wxyhgk/pi-mnemopi";
import type * as MnemopiDiagnoseNs from "@wxyhgk/pi-mnemopi/diagnose";
import type { DiagnosticSummary } from "@wxyhgk/pi-mnemopi/diagnose";
...
async function loadMnemopiDiagnose(): Promise<typeof MnemopiDiagnoseNs> {
	if (!mnemopiDiagnoseMod) {
		mnemopiDiagnoseMod = await import("@wxyhgk/pi-mnemopi/diagnose");
	}
...
export const mnemopiBackend: MemoryBackend = {
	...
	async start(options: MemoryBackendStartOptions): Promise<void> {
		...
		try {
			...
			logger.warn("Mnemopi:后端启动失败;记忆后端已停用。", { error: String(error) });
		}
	},
	...
	async clear(agentDir, _cwd, session): Promise<void> {
		...
		try {
			...
			logger.warn("Mnemopi:清除后重建失败;记忆后端已停用。", { error: String(error) });
		}
	},
	...
	async enqueue(agentDir, _cwd, session): Promise<void> {
		try {
			...
			logger.warn("Mnemopi:入队失败。", { error: String(error) });
		}
	},
		...
		try {
			if (targets.length === 0) {
				return {
					...
					writable: false,
					searchable: false,
					message: "Mnemopi 后端已配置但未为本会话初始化。",
				};
			}
	...
	async search({ session }, query, options) {
		...
		if (options?.signal?.aborted) {
			return { backend: "mnemopi", query, count: 0, items: [], message: "搜索已中止。" };
		}
	...
	async save({ cwd, session }, input: MemoryBackendSaveInput) {
		...
		if (!content) return { backend: "mnemopi", stored: 0, message: "记忆内容为空。" };
		...
		return {
			...
			stored: id ? 1 : 0,
			ids: id ? [id] : [],
			message: id ? undefined : "Mnemopi 未返回已存储的记忆 ID。",
		};
	},
...
function renderMnemopiStats(targets: readonly MnemopiStatsTarget[]): string {
	...
	for (const target of targets) {
		...
		lines.push(
			...
			)} | ${stats.beam.triples.total} | ${escapeMarkdownTableCell(stats.last_memory ?? "从未")} | ${escapeMarkdownTableCell(shortenPath(stats.database))} |`,
		);
	}
	return lines.join("\n");
}
...
function renderMnemopiDiagnostics(entries: readonly { bank: string; summary: DiagnosticSummary }[]): string {
	...
	lines.push("", "## 主要发现");
	lines.push(...(findings.length > 0 ? findings : ["- 无"]));
	return lines.join("\n");
}
...
async function resolveMnemopiProviderOptions(
	...
	try {
		...
		if (!model) {
			logger.warn("Mnemopi:llmMode=smol 但未解析到 tiny/smol 模型;继续在无 LLM 状态下运行。");
			return base;
		}
		return {
			...base,
			llm: async (prompt, opts) => {
				...
				if (!hasApiKey) {
					logger.warn("Mnemopi:请求了 smol 补全但当前没有可用的 API 密钥。", {
		...
		logger.warn("Mnemopi:smol LLM 解析失败;继续在无 LLM 状态下运行。", { error: String(error) });
		return base;
	}
}
	...
	for (const dbPath of dbPaths) {
		for (const suffix of ["", "-wal", "-shm"]) {
			await removeWithRetries(`${dbPath}${suffix}`).catch(error => {
				...
				if (code !== "ENOENT") {
					logger.warn("Mnemopi:重试后仍无法删除 DB 文件", { path: `${dbPath}${suffix}`, code });
				}
			});
		}
	}
