...
import type { MnemopiOptions } from "@wxyhgk/pi-mnemopi";
import { getMemoriesDir, logger } from "@wxyhgk/pi-utils";
...
function bankOnlyHasCwd(dbPath: string, cwd: string): boolean {
	...
	try {
		...
		logger.debug("Mnemopi:遗留记忆库探测失败", { dbPath, error: String(error) });
		return false;
	} finally {
		try {
			db?.close();
