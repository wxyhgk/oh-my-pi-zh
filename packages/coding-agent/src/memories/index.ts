...
import { type ApiKey, completeSimple, Effort, type Model } from "@wxyhgk/pi-ai";
import { clampThinkingLevelForModel } from "@wxyhgk/pi-catalog/model-thinking";
import { getAgentDbPath, getMemoriesDir, isEnoent, logger, parseJsonlLenient, prompt } from "@wxyhgk/pi-utils";
...
export function startMemoryStartupTask(options: {
	...
	try {
		...
		logger.debug("记忆启动已跳过:状态数据库不可用", { error: String(error) });
		return;
	}
	...
	void runMemoryStartup({ session, settings, modelRegistry, agentDir, config: cfg, signal })
		.catch(error => {
			if (!signal.aborted) logger.warn("记忆启动失败", { error: String(error) });
		})
	...
	try {
		...
		await runWithConcurrency(claims, config.stage1Concurrency, async claim => {
			...
			if (result.kind === "failed") {
				logger.error("记忆 phase1 的 stage1 任务失败", {
...
async function runPhase2(options: MemoryStartupOptions): Promise<void> {
	...
	try {
		...
		if (outputs.length === 0) {
			...
			if (!marked) {
				logger.warn("Phase2 空输入完成时丢失了任务所有权", { memoryRoot });
			}
			return;
		}
...
function markPhase2FailureWithFallback(
	...
	if (!unownedFailed) {
		logger.warn("Phase2 无法标记失败(所有权丢失且无主回退被跳过)", {
...
export async function saveLearnedLesson(
	...
	if (!content) {
		return { backend: "local", stored: 0, message: "经验教训为空;未存储任何内容。" };
	}
	...
	return { backend: "local", stored: 1, message: `经验教训已保存到 ${LEARNED_LESSONS_FILE}。` };
}
