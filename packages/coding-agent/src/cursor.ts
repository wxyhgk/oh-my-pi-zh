...
import {
	...
	piReadPath,
	piTimeout,
} from "@wxyhgk/pi-ai/providers/cursor/exec-modern";
...
function formatMcpToolErrorMessage(toolName: string, availableTools: string[]): string {
	const list = availableTools.length > 0 ? availableTools.join(", ") : "无";
	return `未找到 MCP 工具 “${toolName}”。可用工具:${list}`;
}
...
function formatTodoSyncSummary(phases: TodoPhase[]): string {
	...
	if (tasks.length === 0) return "没有待办事项";
	const done = tasks.filter(task => task.status === "completed").length;
	return `${done}/${tasks.length} 个任务已完成`;
}
