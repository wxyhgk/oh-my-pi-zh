...
export function formatMiddleElisionMarker(elidedLines: number, elidedBytes: number): string {
	...
	if (elidedLines <= 1) return `[…已省略 ${elidedBytes}B…]`;
	return `[…已省略 ${elidedLines} 行…]`;
}
...
export class OutputSink {
	...
	#flushArtifactTailIfCapped(): void {
		...
		if (droppedBytes > 0) {
			...
			const notice =
				`${headSep}[产物已截断：保留前 ${formatBytes(headWritten)} 与后 ${formatBytes(tailBytes)} ` +
				`（共 ${formatBytes(totalCapped)}）；中段已省略 ${formatBytes(droppedBytes)}]${tailSep}`;
...
export function formatTailTruncationNotice(
	...
	const fullOutputPart = fullOutputPath ? `。完整输出：${fullOutputPath}` : "";
	...
	if (truncation.lastLinePartial) {
		...
		if (originalContent) {
			...
			lastLineSizePart = `（行大小为 ${formatBytes(Buffer.byteLength(lastLine, "utf-8"))}）`;
		}
		notice = `[显示第 ${endLine} 行的最后 ${formatBytes(truncation.outputBytes ?? truncation.totalBytes)}${lastLineSizePart}${fullOutputPart}${suffix}]`;
	} else {
		notice = `[显示第 ${startLine}-${endLine} 行（共 ${truncation.totalLines} 行）${fullOutputPart}${suffix}]`;
	}
...
export function formatHeadTruncationNotice(
	...
	const notice = `[显示第 ${startLineDisplay}-${endLineDisplay} 行（共 ${totalFileLines} 行）。使用 :${nextOffset} 继续]`;
	return `\n\n${notice}`;
}
