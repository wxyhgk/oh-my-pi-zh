...
export function parseCompactArgs(args: string): ParsedCompactArgs | { error: string } {
	...
	if (mode.rejectsFocus && focus) {
		return {
			error: `/compact ${mode.name} 不接受聚焦说明（它在无 LLM 摘要的情况下归档历史）。`,
		};
	}
