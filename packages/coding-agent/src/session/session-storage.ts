...
import { hasFsCode, isEnoent, logger, peekFileEnds, Snowflake, toError } from "@wxyhgk/pi-utils";
	...
	#replaceSessionFileAfterEpermSync(
		...
		try {
			...
			try {
				...
				throw new Error(
					`EPERM 后替换会话文件失败（原始：${toError(renameError).message}；重试：${
						toError(replaceError).message
					}；回滚：${rollbackError.message}）`,
...
export class MemorySessionStorage implements SessionStorage {
	...
	readText(path: string): Promise<string> {
		const entry = this.#files.get(path);
		if (!entry) return Promise.reject(new Error(`找不到文件：${path}`));
