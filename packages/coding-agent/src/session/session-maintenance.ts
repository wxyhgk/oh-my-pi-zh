...
} from "@wxyhgk/pi-agent-core/compaction";
import {
	DEFAULT_PRUNE_CONFIG,
	pruneSupersededToolResults,
	pruneToolOutputs,
	readToolSupersedeKey,
} from "@wxyhgk/pi-agent-core/compaction/pruning";
import type { ProtectedToolMatcher } from "@wxyhgk/pi-agent-core/compaction/tool-protection";
import type { AssistantMessage, CodexCompactionContext, Message, Model, ProviderSessionState } from "@wxyhgk/pi-ai";
...
import * as snapcompact from "@wxyhgk/snapcompact";
...
function compactionDeadEndWarning(remedies: string): string {
	return (
		"压缩释放的上下文太少，无法取得进展——为避免压缩循环，暂停自动维护。 " +
		`仅最近一轮就过大而无法进一步缩减；${remedies}，或切换到上下文更大的模型。`
	);
}
...
export class SessionMaintenance {
	...
	#shakeElidePlaceholder(region: ShakeRegion, index: number, artifactId: string | undefined): string {
		if (artifactId) {
			return `[已清理约 ${region.tokens} token——恢复：artifact://${artifactId}（区域 ${index + 1}）]`;
		}
		return `[已清理约 ${region.tokens} token]`;
	}
	...
	async #saveShakeArtifact(regions: ShakeRegion[]): Promise<string | undefined> {
		...
		for (let i = 0; i < regions.length; i++) {
			...
			parts.push(`### 区域 ${i + 1}（${region.label}，约 ${region.tokens} token）`, "", region.originalText, "");
		}
		try {
	...
	async compact(customInstructions?: string, options?: CompactOptions): Promise<CompactionResult> {
		...
		if (compactMode?.rejectsFocus && (customInstructions || options?.internalGuidance)) {
			throw new Error(`/compact ${compactMode.name} 不接受聚焦说明。`);
		}
		...
		try {
			...
			if (requireProviderRemote && compactionCandidates.length === 0) {
				this.#host.emitNotice(
					"warning",
					`远程压缩对 ${this.#model.id} 不可用（未配置远程端点，回退链中也没有支持远程的提供商原生压缩模型）——改用本地摘要`,
					"compaction",
				);
			...
			if (wantsSnapcompact && !this.#model.input.includes("image")) {
				if (explicitSnapcompact) {
					this.#host.emitNotice(
						"warning",
						`snapcompact 需要支持视觉的模型（${this.#model.id} 仅支持文本）`,
						"compaction",
					);
					throw new Error(`snapcompact 无法在本地运行：${this.#model.id} 仅支持文本。`);
				}
				this.#host.emitNotice(
					"warning",
					`snapcompact 需要支持视觉的模型（${this.#model.id} 仅支持文本）；回退到 LLM 压缩`,
					"compaction",
				);
				...
				if (!renderScan.isSafe) {
					...
					this.#host.emitNotice(
						"warning",
						`snapcompact 已禁用：所选 snapcompact 字体包含不支持的字符（${percent}%）。未尝试 LLM 回退。`,
						"compaction",
					);
					throw new Error(
						`snapcompact 无法在本地渲染此对话：所选 snapcompact 字体包含不支持的字符（${percent}%）。`,
					);
				}
			}
			...
			if (snapcompactReady) {
				...
				if (maxFrames < 1) {
					...
					this.#host.emitNotice(
						"warning",
						"snapcompact：仅保留的历史记录就超出上下文预算。未尝试 LLM 回退。",
						"compaction",
					);
					throw new Error("snapcompact 无法在本地运行：仅保留的历史记录就超出上下文预算。");
				} else {
					const shape = snapcompactShape;
					if (!shape) {
						throw new Error("snapcompact 形状在渲染前未解析。");
					}
					...
					if (this.#projectSnapcompactContextTokens(preparation, snapcompactResult) > budget) {
						...
						this.#host.emitNotice(
							"warning",
							"snapcompact 未能将上下文压缩到限制以内。未尝试 LLM 回退。",
							"compaction",
						);
						throw new Error("snapcompact 无法在本地将上下文压缩到限制以内。");
					}
				}
			}
	...
	#buildCompactionAuthError(): Error {
		...
		return new Error(
			`压缩需要 ${currentModel.provider}/${currentModel.id} 的可用凭据。 ` +
				`请配置 ${currentModel.provider} 的凭据，或指定一个已认证的回退角色，例如 modelRoles.smol。`,
		);
	}
	...
	async #rescueCompactionDeadEnd(
		...
		if (!options.skipElide) {
			try {
				...
				if (result.artifactId) elideSink = "一个产物";
				if (elided > 0) {
			...
			if (elided > 0 && options.hasProgress()) {
				this.#host.emitNotice(
					"info",
					`压缩死胡同恢复：${this.#describeElideRescue(elided, elidedTokens, elideSink)}，以便维护能够取得进展。`,
					"compaction",
				);
				return true;
			}
		...
		if (imagesDropped > 0 && options.hasProgress()) {
			const elidedPart = elided > 0 ? `${this.#describeElideRescue(elided, elidedTokens, elideSink)}，并 ` : "";
			this.#host.emitNotice(
				"info",
				`压缩死胡同恢复：${elidedPart}移除了 ${imagesDropped} 张附加图片，以便维护能够取得进展。`,
				"compaction",
			);
			return true;
		}
	...
	#describeElideRescue(elided: number, tokensFreed: number, sink: string): string {
		return `将 ${elided} 个重量级块（约 ${tokensFreed.toLocaleString()} token）省略为 ${sink}`;
	}
	...
	async #rescueSnapcompactFrameOverflow(
		...
		this.#host.emitNotice(
			"info",
			`压缩死胡同恢复：以更小的帧预算重建了尾部的 snapcompact 归档（${archive.frames.length} → ${rebuilt.frames.length} 帧），以便维护能够取得进展。`,
			"compaction",
		);
		return result;
	}
	...
	async runAutoCompaction(
		...
		if (action === "snapcompact" && this.#model && !this.#model.input.includes("image")) {
			this.#host.emitNotice(
				"warning",
				`snapcompact 需要支持视觉的活动模型（${this.#model.id} 仅支持文本）；改用 context-full 自动压缩。`,
				"compaction",
			);
			action = "context-full";
		}
		...
		try {
			...
			if (!preparation) {
				...
				if (!preparation) {
					...
					const deadEndWarning = noProgressDeadEnd
						? compactionDeadEndWarning("缩小它（例如清除大量工具输出）")
						: undefined;
			...
			if (action === "snapcompact" && compactionPrep.kind !== "fromHook") {
				...
				if (!renderScan.isSafe) {
					...
					snapcompactBlocker = `snapcompact 已禁用：所选 snapcompact 字体包含不支持的字符（${percent}%）；改用 context-full 自动压缩。`;
				} else {
			...
			const deadEndWarning = noProgressDeadEnd ? compactionDeadEndWarning("清除大量工具输出") : undefined;
			if (deadEndWarning) {
			...
			await this.#host.emitSessionEvent({
				...
				errorMessage:
					reason === "overflow"
						? `上下文溢出恢复失败：${errorMessage}`
						: reason === "incomplete"
							? `不完整响应恢复失败：${errorMessage}`
							: `自动压缩失败：${errorMessage}`,
			});
		} finally {
		...
		try {
			...
			if (shouldFallBack) {
				const errorMessage = reclaimed
					? `自动 shake 回收了约 ${result.tokensFreed} token，但上下文仍高于阈值；回退到 context-full 压缩。`
					: "自动 shake 未找到可移除的内容；回退到 context-full 压缩。";
				await this.#host.emitSessionEvent({
					type: "auto_compaction_end",
					action,
					result: undefined,
			...
			const message = error instanceof Error ? error.message : "shake 失败";
			await this.#host.emitSessionEvent({
				type: "auto_compaction_end",
				action,
				result: undefined,
