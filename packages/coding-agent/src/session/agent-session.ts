...
import type { Clipboard, InMemorySnapshotStore } from "@wxyhgk/hashline";
import {
...
import { type Effort, streamSimple } from "@wxyhgk/pi-ai";
import * as AIError from "@wxyhgk/pi-ai/error";
import { resetOpenAICodexHistoryAfterCompaction } from "@wxyhgk/pi-ai/providers/openai-codex-responses";
import { toolWireSchema } from "@wxyhgk/pi-ai/utils/schema";
import { modelsAreEqual } from "@wxyhgk/pi-catalog/models";
import { MacOSPowerAssertion } from "@wxyhgk/pi-natives";
import {
	$env,
	escapeXmlText,
	formatDuration,
...
export class AgentSession {
	...
	async preparePlanForReview(title: string): Promise<AgentToolResult<PlanApprovalDetails>> {
		...
		return {
			content: [{ type: "text", text: "计划已就绪，等待审核。" }],
	...
	async #formatAsyncResultForFollowUp(result: string): Promise<string> {
		...
		const preview = `${result.slice(0, ASYNC_PREVIEW_MAX_CHARS)}\n\n[输出已截断。显示前 ${ASYNC_PREVIEW_MAX_CHARS.toLocaleString()} 个字符。]`;
		try {
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "assistant") {
				...
				if (
					...
					this.emitNotice(
						"warning",
						"该模型拒绝了优先/快速模式；已在无该模式的情况下重试。快速模式现已关闭。",
						"priority",
					);
				}
	...
	async #beforeToolCall(ctx: BeforeToolCallContext): Promise<BeforeToolCallResult | undefined> {
		...
		if (callResult?.block) {
			return { block: true, reason: callResult.reason || "工具执行被扩展阻止" };
		}
	...
	async #maybeApplyUsageAwareFallback(signal: AbortSignal): Promise<void> {
		...
		if (reservePolicy === "fail-closed") {
			const condition = health.state === "reserve" ? "已达到储备" : "用量已耗尽";
			throw new Error(`${condition}，针对 ${currentSelector}；储备策略为失败即关闭。`);
		}
	...
	#assertVibeSessionTransitionAllowed(action: string): void {
		if (this.#vibeModeState?.enabled) {
			throw new Error(`vibe 模式激活期间无法执行 ${action}。请先退出 vibe 模式。`);
		}
	}
	...
	async #promptWithMessage(
		...
		try {
			...
			if (!this.model) {
				throw new Error(
					"未选择模型。\n\n" +
						`使用 /login 登录、设置 API 密钥环境变量，或创建 ${getAgentDbPath()}\n\n` +
						"然后使用 /model 选择模型。",
				);
			}
			...
			if (!apiKey) {
				throw new Error(
					`未找到 ${this.model.provider} 的 API 密钥。\n\n` +
						`使用 /login 登录、设置 API 密钥环境变量，或创建 ${getAgentDbPath()}`,
				);
			}
	...
	queueLaunchCompletion(notification: DaemonCompletionNotification): Promise<void> {
		if (this.#isDisposed) return Promise.reject(new Error("会话在启动完成通知送达前已销毁"));
	...
	async runEphemeralTurn(args: {
		...
		for await (const event of stream) {
			...
			if (event.type === "error") {
				throw new Error(event.error.errorMessage || "临时轮次失败");
			}
		}

		if (!assistantMessage) {
			throw new Error("临时轮次结束但未产生最终消息");
		}
	...
	async branchFromBtw(
		...
		if (
			...
			throw new Error("会话维护或用户操作仍在运行时无法进行 /btw 分支");
		}
	...
	async navigateTree(
		...
		if (!targetEntry) {
			throw new Error(`条目 ${targetId} 不存在`);
		}
	...
	async #confirmCodexAutoRedeem(
		...
		if (!runner?.hasUI()) {
			if (!coordinator.notifiedKeys.has(first.attemptKey)) {
				...
				this.emitNotice(
					"warning",
					"已保存的 Codex 重置可用于消费，但未设置自动兑换，且没有可用的提示 UI。请运行 `/usage reset` 或设置 codexResets.autoRedeem。",
					"codex-auto-reset",
				);
			}
			return false;
		...
		const lines = actions.map(action =>
			action.reason === "blocked-account"
				? `${action.label} 被 Codex ${(action.blockedWindows ?? []).join(" + ") || "用量"}限制阻止约 ${formatDuration(action.remainingMs ?? 0)}。`
				: `${action.label}：已保存的重置将在 ${formatDuration(action.expiresInMs ?? 0)} 后过期（${action.salvageWindow ?? "weekly"} 窗口已使用 ${Math.round((action.salvageUsedFraction ?? action.weeklyUsedFraction ?? 0) * 100)}%）。`,
		);
		const question =
			actions.length === 1
				? `要消耗一个已保存的 Codex 速率限制重置吗？\n${lines[0]}`
				: `要消耗 ${actions.length} 个已保存的 Codex 速率限制重置吗？\n${lines.join("\n")}`;
		try {
			const choice = await runner.getUIContext().select(question, [
				{
					label: "Yes",
					description: "立即兑换，并记住对未来的符合条件 Codex 重置也自动兑换。",
				},
				{
					label: "No",
					description: "不自动兑换已保存的 Codex 重置。",
				},
			]);
			if (choice === "Yes") {
		...
		for (const action of actions) {
			...
			switch (outcome.code) {
				case "reset": {
					...
					this.emitNotice(
						"info",
						`已为 ${action.label} 自动兑换一个已保存的 Codex 速率限制重置${left ?? ""}；${detail}。`,
						"codex-auto-reset",
					);
					break;
				}
				case "already_redeemed":
					this.emitNotice(
						"warning",
						`针对 ${action.label} 的已保存 Codex 重置已在其他位置兑换。`,
						"codex-auto-reset",
					);
					break;
				case "no_credit":
				...
				case "nothing_to_reset":
					...
					if (action.reason === "blocked-account") {
						this.emitNotice(
							"warning",
							`针对 ${action.label} 的 Codex 重置报告无可重置内容；稍后将重试。`,
							"codex-auto-reset",
						);
					} else {
				...
				default:
					if (action.reason === "blocked-account") {
						this.emitNotice(
							"warning",
							`针对 ${action.label} 的 Codex 自动兑换失败（${outcome.code}）；稍后将重试。`,
							"codex-auto-reset",
						);
					} else {
