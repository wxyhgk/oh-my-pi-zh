 ...
 */
import type { AgentTool, AgentToolContext, AgentToolUpdateCallback } from "@wxyhgk/pi-agent-core";
import type { Static, TSchema } from "@wxyhgk/pi-ai";
	...
	async execute(
		...
		if (this.hookRunner.hasHandlers("tool_call")) {
			try {
				...
				if (callResult?.block) {
					const reason = callResult.reason || "工具执行已被钩子阻止";
				...
				throw new Error(`钩子失败,已阻止执行:${String(err)}`);
			}
		}
