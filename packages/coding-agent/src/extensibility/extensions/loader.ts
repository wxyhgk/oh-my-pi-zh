...
import { hasFsCode, isEacces, isEnoent, logger } from "@wxyhgk/pi-utils";
...
export class ExtensionRuntimeNotInitializedError extends Error {
	constructor() {
		super("扩展运行时尚未初始化。扩展加载期间无法调用操作方法。");
	}
}
...
class ConcreteExtensionAPI implements ExtensionAPI, IExtensionRuntime {
	...
	setServiceTier(family: ServiceTierFamily, tier: ServiceTier | undefined): void {
		if (!isServiceTierFamily(family) || (tier !== undefined && !isServiceTierForFamily(family, tier))) {
			throw new TypeError(`服务层级 “${String(tier)}” 对模型系列 “${String(family)}” 无效`);
		}
...
async function loadExtension(
	...
	try {
		...
		if (typeof factory !== "function") {
			return {
				extension: null,
				error: `扩展未导出有效的工厂函数:${extensionPath}`,
			};
		}
