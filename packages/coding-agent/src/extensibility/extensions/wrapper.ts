...
import type { ComputerSafetyCheck, ImageContent, Static, TextContent, TSchema } from "@wxyhgk/pi-ai";
	...
	implements AgentTool<TParameters, TDetails>
{
	...
	async execute(
		...
		if (resolveApproval(this.tool, approvalArgs(params, context), approvalMode, userPolicies).policy === "deny") {
			throw new Error(
				`工具 “${this.tool.name}” 已被用户策略阻止。\n` +
					`如需允许:请从配置中移除 “tools.approval.${this.tool.name}: deny”。`,
			);
		}
		...
		if (!loopEmittedToolCall && this.runner.hasHandlers("tool_call")) {
			try {
				...
				if (callResult?.block) {
					const reason = callResult.reason || "工具执行已被扩展阻止";
				...
				throw new Error(`扩展失败,已阻止执行:${String(err)}`);
			}
		}
		...
		if (approvalCheck.required) {
			...
			if (!this.runner.hasUI()) {
				...
				throw new Error(
					`工具 “${this.tool.name}” 需要批准,但没有可用的交互式 UI。\n` +
						`选项:\n` +
						`  1. 在 /settings 中将 tools.approvalMode 设为 yolo\n` +
						`  2. 在配置中添加 tools.approval.${this.tool.name}: allow\n` +
						`  3. 使用交互式 UI 批准该工具调用`,
				);
			}
			...
			try {
				choice = await uiContext.select(safetyPrompt, ["批准", "拒绝"]);
			} catch (err) {
				await emitApprovalResolved(false, err instanceof Error ? err.message : "approval aborted");
				throw err;
			}
			const approved = choice === "批准";
			await emitApprovalResolved(approved, approved ? undefined : "denied by user");
			if (!approved) {
				throw new Error(`工具调用已被用户拒绝:${this.tool.name}`);
			}
			if (pendingSafetyChecks.length > 0) {
				if (!context) throw new Error("提供商安全批准上下文不可用");
