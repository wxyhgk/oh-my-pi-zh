...
import { getAgentDir, getProjectDir, isEnoent, logger } from "@wxyhgk/pi-utils";
...
async function loadCommandModule(
	...
	try {
		...
		if (typeof factory !== "function") {
			return { commands: null, error: "命令必须导出默认函数" };
		}
		...
		for (const cmd of commands) {
			if (!cmd.name || typeof cmd.name !== "string") {
				return { commands: null, error: "命令必须包含名称" };
			}
			if (!cmd.description || typeof cmd.description !== "string") {
				return { commands: null, error: `命令 “${cmd.name}” 必须包含描述` };
			}
			if (typeof cmd.execute !== "function") {
				return { commands: null, error: `命令 “${cmd.name}” 必须包含 execute 函数` };
			}
		}
		...
		return { commands: null, error: `命令加载失败:${message}` };
	}
}
			...
			for (const command of loadedCommands) {
				...
				if (existingIdx !== -1) {
					...
					if (existing.source === "bundled") {
						...
						errors.push({
							path: commandPath,
							error: `命令名 “${command.name}” 与现有命令冲突`,
						});
						continue;
					}
				}
