...
async function buildPrReviewPrompt(
	...
	try {
		...
		const failure = `获取 PR ${ref.repo}#${ref.number} 的差异失败:${message}`;
		if (ctx.hasUI) {
	...
	const promptText = buildReviewPromptFromDiff(
		...
		`PR ${ref.repo}#${ref.number} 没有可用的差异内容`,
	...
	return `无法审查 PR ${ref.repo}#${ref.number}:没有可用的差异内容。`;
}
	...
	async execute(args: string[], ctx: HookCommandContext): Promise<string | undefined> {
		...
		const choices: Array<{ label: string; value: ReviewMenuChoice }> = [
			...findRecentPrRefs(ctx, REVIEW_CONTEXT_PR_LIMIT).map(ref => ({
				label: `从对话中审查 PR ${ref.repo}#${ref.number}`,
				value: { kind: "detected-pr" as const, ref },
			})),
			{
				label: "1. 对照基础分支审查(PR 风格)",
		...
		switch (selectedChoice.kind) {
			...
			case "base-branch": {
				...
				if (branches.length === 0) {
					ctx.ui.notify("未找到 git 分支", "error");
					return undefined;
				}

				const baseBranch = await ctx.ui.select("选择要对照的基础分支", branches);
				...
				try {
					...
					ctx.ui.notify(`获取差异失败:${err instanceof Error ? err.message : String(err)}`, "error");
					return undefined;
				}
				...
				return buildReviewPromptFromDiff(
					...
					diffText,
					extraInstructions,
					`在 ${baseBranch} 与 ${currentBranch} 之间没有更改`,
				);
			}
			...
			case "uncommitted": {
				...
				return buildReviewPromptFromDiff(
					...
					reviewDiff.diffText,
					extraInstructions,
					reviewDiff.emptyMessage ?? "未找到差异内容",
			...
			case "commit": {
				...
				const selectedCommit = await ctx.ui.select("选择要审查的提交", commits);
				...
				try {
					...
					ctx.ui.notify(`获取提交失败:${err instanceof Error ? err.message : String(err)}`, "error");
					return undefined;
				}
				...
				return buildReviewPromptFromDiff(
					...
					extraInstructions,
					"该提交没有差异内容",
					{ filteredMessage: "提交中没有可审查的文件(所有更改均被过滤)" },
				);
			}
