...
import { isCompiledBinary, stripWindowsExtendedLengthPathPrefix } from "@wxyhgk/pi-utils";
...
function parseExtensionSource(source: string, importerPath: string): ParseResult {
	...
	try {
		...
		throw new Error(
			`解析扩展源码以重写依赖失败:${importerPath}:${error instanceof Error ? error.message : String(error)}`,
			{ cause: error },
		);
	}
}
...
function ensureBundledModuleLoadersLoaded(): Promise<BundledModuleLoaders> {
	if (!IS_COMPILED_BINARY) {
		return Promise.reject(new Error("omp:legacy-pi-shim:捆绑模块仅在编译模式下可用"));
	}
...
async function loadBundledModule(moduleKey: string): Promise<void> {
	...
	if (!loader) {
		throw new Error(`omp:legacy-pi-shim:未为 ${moduleKey} 注册捆绑模块`);
	}
...
export function resolveBundledVirtualSpecifier(specifier: string): BundledVirtualResolveResult {
	const registryKey = isBundledVirtualSpecifier(specifier)
		...
		: specifier;
	if (!registryKey) {
		throw new Error("omp:legacy-pi-shim:捆绑虚拟标识符没有注册表键");
	}
	return { path: registryKey, namespace: BUNDLED_VIRTUAL_NAMESPACE };
}
 ...
 * bare `@wxyhgk/pi-coding-agent` here: from a global install Bun can pick an
...
// imports such as `@wxyhgk/pi-ai/oauth` continue to resolve directly
...
/**
 * Resolve a canonical `@wxyhgk/*` specifier to a filesystem path, preferring
...
function evaluateGraphCommonJs(modulePath: string): unknown {
	...
	if (!definition) {
		throw new Error(`缺少图属 CommonJS 定义:${modulePath}`);
	}
	...
	const graphRequire: NodeJS.Require = Object.assign(
		(specifier: string) => {
			if (isBundledVirtualSpecifier(specifier)) {
				...
				if (!bundledModule) {
					throw new Error(`缺少捆绑的 CommonJS 宿主模块:${moduleKey}`);
				}
		...
		Bun.plugin({
			...
			setup(build) {
				build.onLoad({ filter, namespace: "file" }, args => {
					...
					if (source === undefined) {
						throw new Error(`缺少预重写的同步扩展源码:${sourcePath}`);
					}
					return { contents: source, loader: getLoader(sourcePath) };
				});
			},
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): LegacyPiResolveResult | undefined {
	...
	// Primary: resolve the canonical @wxyhgk/* specifier from the host binary
