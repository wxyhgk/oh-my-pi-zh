...
export function classifySource(source: string): MarketplaceSourceType {
	...
	throw new Error(`无法识别的源格式。您的意思是 './${source}' (本地)或 'owner/repo' (GitHub)吗?`);
}
...
function assertField(condition: boolean, field: string, filePath: string): void {
	if (!condition) {
		throw new Error(`目录中字段 “${field}” 缺失或无效:${filePath}`);
	}
}
...
export function parseMarketplaceCatalog(content: string, filePath: string): MarketplaceCatalog {
	...
	try {
		...
		throw new Error(`解析 ${filePath} 处的市场目录失败:${(err as Error).message}`);
	}

	if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
		throw new Error(`${filePath} 处的市场目录必须是 JSON 对象`);
	}
...
async function readMarketplaceCatalog(
	...
	throw new Error(
		`未在 ${tried.map(p => `"${p}"`).join(" 或 ")} 处找到市场目录。` +
			`请确保目录存在且包含以下文件之一:${CATALOG_RELATIVE_PATHS.join(", ")}。`,
	);
}
...
async function cloneAndReadCatalog(url: string, source: string, cacheDir: string): Promise<FetchResult> {
	...
	try {
		...
		throw new Error(`克隆仓库 ${url} 时出错:${(err as Error).message}(来源:${source})`, { cause: err });
	}
}
