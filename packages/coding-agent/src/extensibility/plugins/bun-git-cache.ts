...
export async function refreshBunGitCache(source: GitSource, cwd: string): Promise<void> {
	...
	if (cacheResult.exitCode !== 0) {
		throw new Error(`bun pm cache 执行失败:${cacheResult.stderr}`);
	}
	const cacheDir = cacheResult.stdout.trim();
	if (!cacheDir) {
		throw new Error("bun pm cache 返回了空的缓存路径");
	}
	...
	for (const entry of entries) {
		...
		if (fetchResult.exitCode !== 0) {
			throw new Error(`刷新 Bun 的 git 缓存失败(${source.host}/${source.path}):${fetchResult.stderr}`);
		}
	}
}
