 ...
 * `@wxyhgk/pi-coding-agent` (or one of its aliased scopes like
 ...
 * the same module identity as a direct `@wxyhgk/pi-coding-agent` import.
 */

import { Database } from "bun:sqlite";
import * as fs from "node:fs";
import * as path from "node:path";
import type { AgentToolResult, AgentToolUpdateCallback } from "@wxyhgk/pi-agent-core";
import { type AuthCredential, SqliteAuthCredentialStore, type TSchema } from "@wxyhgk/pi-ai";
import { piEscapeRegexLiteral, piJoinPath } from "@wxyhgk/pi-ai/providers/cursor-pi-args";
import { getKeybindings, type Keybinding, Text } from "@wxyhgk/pi-tui";
import {
	getAgentDbPath,
	getAgentDir,
	getProjectDir,
...
const legacyReadSchema = Type.Object({
	...
	offset: Type.Optional(Type.Number({ description: "从 1 开始的行偏移" })),
	limit: Type.Optional(Type.Number({ description: "最大读取行数" })),
});

const legacyGrepSchema = Type.Object({
	pattern: Type.String({ description: "搜索模式" }),
	path: Type.Optional(Type.String({ description: "要搜索的目录或文件" })),
	glob: Type.Optional(Type.String({ description: "Glob 筛选器" })),
	ignoreCase: Type.Optional(Type.Boolean({ description: "不区分大小写搜索" })),
	literal: Type.Optional(Type.Boolean({ description: "将模式视为字面字符串" })),
	context: Type.Optional(Type.Number({ description: "上下文行数" })),
});

const legacyFindSchema = Type.Object({
	pattern: Type.String({ description: "匹配文件的 Glob 模式" }),
	path: Type.Optional(Type.String({ description: "要搜索的目录" })),
...
function legacyBashSnapshot(output: string): { text: string; details?: { truncation: TruncationResult } } {
	...
	const note =
		truncation.truncatedBy === "lines"
			? `显示第 ${startLine}-${truncation.totalLines} 行,共 ${truncation.totalLines} 行`
			: `显示第 ${startLine}-${truncation.totalLines} 行,共 ${truncation.totalLines} 行(${formatBytes(DEFAULT_MAX_BYTES)} 限制)`;
	return {
...
async function executeLegacyBashOperations(
	...
	try {
		...
		if (err instanceof Error && err.message === "aborted") {
			throw new Error(appendStatus(text, "命令已取消"));
		}
		if (err instanceof Error && err.message.startsWith("timeout:")) {
			throw new Error(appendStatus(text, `命令在 ${err.message.slice("timeout:".length)} 秒后超时`));
		}
		throw err;
	}
}
...
export function createGrepToolDefinition(cwd: string, options?: GrepToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"不支持旧版 GrepToolOptions.operations:内置 grep 工具原生搜索本地文件系统,不提供可插拔的文件系统接口(历史接口仅自定义上下文行读取;搜索本身始终在本地运行)。请通过 defineTool() 注册自定义 grep 工具,而不要在 createGrepTool()/createGrepToolDefinition() 中传入 operations。",
		);
	}
...
export function createFindToolDefinition(cwd: string, options?: FindToolOptions): ToolDefinition {
	...
	return markToolDefinition({
		...
		renderCall: (params, optionsArg, themeArg) => {
			...
			const pattern = stringField(params, "pattern") ?? "";
			const searchPath = stringField(params, "path") ?? ".";
			return new Text(`${themedTitle(theme, "find")} ${themedMuted(theme, `${pattern} 在 ${searchPath} 中`)}`, 0, 0);
		},
		renderResult: legacyRenderResult,
		execute: async (toolCallId, params, signal, onUpdate) => {
			...
			if (options?.operations) {
				if (!(await options.operations.exists(absolutePath))) {
					throw new Error(`路径未找到:${absolutePath}`);
				}
				...
				return {
					content: [{ type: "text", text: truncation.content || "未找到匹配模式的文件" }],
...
export function createLsToolDefinition(cwd: string, options?: LsToolOptions): ToolDefinition {
	return markToolDefinition({
		...
		execute: async (_toolCallId, params, _signal, _onUpdate) => {
			...
			if (!exists) throw new Error(`路径未找到:${absolutePath}`);
...
export function createEditToolDefinition(cwd: string, options?: EditToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"不支持旧版 EditToolOptions.operations:OMP 内置 edit 工具原生写入本地文件系统,不提供可插拔的 operations 接口。请通过 defineTool() 注册自定义 edit 工具,而不要在 createEditTool()/createEditToolDefinition() 中传入 operations。",
		);
	}
...
export function createWriteToolDefinition(cwd: string, options?: WriteToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"不支持旧版 WriteToolOptions.operations:OMP 内置 write 工具原生写入本地文件系统,不提供可插拔的 operations 接口。请通过 defineTool() 注册自定义 write 工具,而不要在 createWriteTool()/createWriteToolDefinition() 中传入 operations。",
		);
	}
	...
	async #loadAdditionalSkills(): Promise<AdditionalSkillLoadResult> {
		...
		for (const resourcePath of this.#state.additionalSkillPaths) {
			...
			try {
				...
				diagnostics.push({
					type: "warning",
					message: `加载附加技能路径失败:${err instanceof Error ? err.message : String(err)}`,
					path: resolvedPath,
				});
			}
		}
		...
		for (const resourcePath of this.#state.additionalPromptTemplatePaths) {
			...
			try {
				...
				if (stat.isDirectory()) {
					...
					diagnostics.push({
						type: "warning",
						message: "附加提示词模板路径既不是目录也不是 Markdown 文件",
						path: resolvedPath,
					});
				}
			} catch (err) {
				diagnostics.push({
					type: "warning",
					message: `检查附加提示词模板路径失败:${err instanceof Error ? err.message : String(err)}`,
					path: resolvedPath,
				});
				continue;
			}
			...
			for (const filePath of files) {
				try {
					...
					diagnostics.push({
						type: "warning",
						message: `加载附加提示词模板失败:${err instanceof Error ? err.message : String(err)}`,
						path: filePath,
					});
				}
			}
...
// `getAgentDir`; `getProjectDir` (a `@wxyhgk/pi-utils` helper) and
...
export { getProjectDir } from "@wxyhgk/pi-utils";
...
// `@wxyhgk/pi-agent-core/compaction`, and the coding-agent barrel below does
...
export { compact, estimateTokens, serializeConversation } from "@wxyhgk/pi-agent-core/compaction";
...
// `CONFIG_DIR_NAME` lives in `@wxyhgk/pi-utils` and `parseArgs` in
...
export { CONFIG_DIR_NAME } from "@wxyhgk/pi-utils";
