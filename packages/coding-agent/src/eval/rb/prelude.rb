...
unless defined?($__omp_prelude_loaded) && $__omp_prelude_loaded
  ...
  def __omp_resolve_path(path)
    ...
    raise "此辅助函数不支持协议路径: #{path}" if root.nil? || root.to_s.empty?
    ...
    if relative.start_with?("/") || relative.split("/").include?("..")
      raise "不安全的 #{scheme}:// 路径(绝对路径或路径穿越): #{path}"
    end
  ...
  def output(*ids, format: "raw", query: nil, offset: nil, limit: nil)
    ...
    if artifacts_dir.nil? || artifacts_dir.empty?
      ...
      if session_file.nil? || session_file.empty?
        __omp_emit_status("output", "error" => "没有可用的会话文件")
        raise "没有会话文件——输出产物不可用"
      end
      artifacts_dir = session_file.sub(/\.[^.]*\z/, "")
    end
    unless File.directory?(artifacts_dir)
      __omp_emit_status("output", "error" => "未找到产物目录", "path" => artifacts_dir)
      raise "未找到产物目录: #{artifacts_dir}"
    end
    raise ArgumentError, "至少需要一个输出 ID" if ids.empty?
    if query && (!offset.nil? || !limit.nil?)
      __omp_emit_status("output", "error" => "query 不能与 offset/limit 组合使用")
      raise ArgumentError, "query 不能与 offset/limit 组合使用"
    end
      ...
      if query
        json_value =
          begin
            ...
            __omp_emit_status("output", "id" => output_id, "error" => "不是有效的 JSON: #{e.message}")
            raise "输出 #{output_id} 不是有效的 JSON: #{e.message}"
          end
        ...
        if start_line > total_lines
          __omp_emit_status("output", "id" => output_id, "error" => "偏移量 #{start_line} 超出末尾 (#{total_lines} 行)")
          raise "偏移量 #{start_line} 超出输出末尾 (#{total_lines} 行),针对 #{output_id}"
        end
    ...
    unless not_found.empty?
      ...
      msg = "未找到输出: #{not_found.join(", ")}"
      unless available.empty?
        msg += "\n\n可用输出: #{available.first(20).join(", ")}"
        msg += " (还有 #{available.length - 20} 个)" if available.length > 20
      end
    ...
    def call(name, args)
      ...
      data =
        begin
          ...
          raise "桥接调用 #{name.inspect}:非 JSON 响应: #{resp.body.to_s[0, 200].inspect}"
        end
      unless data.is_a?(Hash) && data["ok"]
        raise((data.is_a?(Hash) ? data["error"] : nil) || "桥接调用 #{name.inspect} 失败")
      end
      data["value"]
    end
    ...
    def tool_call(name, positional, kwargs)
      merged =
        if positional.nil?
          ...
          raise ArgumentError, "tool.#{name}(...) 需要参数字典(实际为 #{positional.class})"
        end
      merged.merge!(stringify_keys(kwargs)) if kwargs && !kwargs.empty?
      merged[INTENT_FIELD] = "rb 预加载" unless merged.key?(INTENT_FIELD)
      call(name, merged)
    end
  end
  ...
  class OmpToolProxy < BasicObject
    ...
    def inspect
      session = ::ENV["PI_TOOL_BRIDGE_SESSION"]
      session ? "#<工具代理 session=#{session}>" : "#<工具代理不可用>"
    end
  end
  ...
  def parallel(thunks)
    list = thunks.to_a
    list.each do |t|
      raise TypeError, "parallel() 需要零参数可调用对象的可迭代集合" unless t.respond_to?(:call)
    end
  ...
  def pipeline(items, *stages)
    ...
    stages.each do |stage|
      raise TypeError, "pipeline() 的各阶段必须是可调用对象" unless stage.respond_to?(:call)
  ...
  class OmpBudget
    ...
    def inspect
      ...
      "#<预算 total=#{snap["total"].inspect} spent=#{snap["spent"].inspect}>"
    rescue StandardError
      "#<预算不可用>"
    end
  end
