...
import { $flag, isBunTestRuntime, logger, Snowflake } from "@wxyhgk/pi-utils";
import { $ } from "bun";
...
async function probeRubyKernelAvailability(cwd: string, interpreter?: string): Promise<RubyKernelAvailability> {
	try {
		...
		if (runtimes.length === 0) {
			return { ok: false, reason: "未在 PATH 中找到 Ruby 可执行文件" };
		}
		...
		for (const runtime of runtimes) {
			try {
				...
				failures.push(`${runtime.rubyPath}(退出码 ${probe.exitCode})`);
			} catch (err) {
		...
		return {
			...
			reason: `未找到可用的 Ruby 解释器。尝试过:${failures.join("; ")}`,
		};
	} catch (err) {
...
export class RubyKernel extends BaseKernel<KernelExecuteOptions> {
	...
	static async start(options: KernelStartOptions): Promise<RubyKernel> {
		...
		if (!availability.ok) {
			throw new Error(availability.reason ?? "Ruby 内核不可用");
		}
		...
		try {
			...
			await kernel.executeWithBudget(initScript, startup.signal, startupBudget, "Ruby 内核初始化");
			await kernel.executeWithBudget(RUBY_PRELUDE, startup.signal, startupBudget, "Ruby 内核预加载");
			return kernel;
		} catch (err) {
