...
async function probePythonKernelAvailability(cwd: string, interpreter?: string): Promise<PythonKernelAvailability> {
	try {
		...
		if (runtimes.length === 0) {
			return { ok: false, reason: "未在 PATH 中找到 Python 可执行文件" };
		}
		...
		for (const runtime of runtimes) {
			try {
				...
				failures.push(`${runtime.pythonPath}(退出码 ${probe.exitCode})`);
			} catch (err) {
		...
		return {
			...
			reason: `未找到可用的 Python 解释器。尝试过:${failures.join("; ")}`,
		};
	} catch (err) {
...
export class PythonKernel extends BaseKernel {
	...
	static async start(options: KernelStartOptions): Promise<PythonKernel> {
		...
		try {
			...
			await kernel.executeWithBudget(initScript, startup.signal, startupBudget, "Python 内核初始化");
			await kernel.executeWithBudget(PYTHON_PRELUDE, startup.signal, startupBudget, "Python 内核预加载");
			return kernel;
		} catch (err) {
