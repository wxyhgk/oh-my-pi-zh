...
if "__omp_prelude_loaded__" not in globals():
    ...
    def _resolve_omp_path(path: str | Path) -> Path:
        ...
        if not root:
            raise ValueError(f"此辅助函数不支持协议路径:{path}")
        ...
        if rel_path.is_absolute() or ".." in rel_path.parts:
            raise ValueError(f"不安全的 {scheme}:// 路径(绝对路径或路径穿越):{path}")
        ...
        if resolved != root_path and not resolved.startswith(root_path + os.sep):
            raise ValueError(f"{scheme}:// 路径越出其根目录:{path}")
    ...
    def output(
        *ids: str,
        ...
        if not artifacts_dir:
            ...
            if not session_file:
                _emit_status("output", error="没有可用的会话文件")
                raise RuntimeError("没有会话文件——输出产物不可用")
            artifacts_dir = session_file.rsplit(".", 1)[0]  # Strip .jsonl extension
        if not Path(artifacts_dir).exists():
            _emit_status(
                "output", error="未找到产物目录", path=artifacts_dir
            )
            raise RuntimeError(f"未找到产物目录:{artifacts_dir}")

        if not ids:
            _emit_status("output", error="未提供任何 ID")
            raise ValueError("至少需要一个输出 ID")

        if query and (offset is not None or limit is not None):
            _emit_status("output", error="query 不能与 offset/limit 组合使用")
            raise ValueError("query 不能与 offset/limit 组合使用")
        ...
        for output_id in ids:
            ...
            if query:
                try:
                    ...
                    _emit_status("output", id=output_id, error=f"不是有效的 JSON:{e}")
                    raise ValueError(f"输出 {output_id} 不是有效的 JSON:{e}")
            ...
            # Handle offset/limit
                ...
                if start_line > total_lines:
                    _emit_status(
                        "output",
                        id=output_id,
                        error=f"偏移量 {start_line} 超出末尾({total_lines} 行)",
                    )
                    raise ValueError(
                        f"偏移量 {start_line} 超出输出末尾({total_lines} 行),针对 {output_id}"
                    )
        ...
        if not_found:
            ...
            error_msg = f"未找到输出:{', '.join(not_found)}"
            if available:
                error_msg += f"\n\n可用输出:{', '.join(available[:20])}"
                if len(available) > 20:
                    error_msg += f" (还有 {len(available) - 20} 个)"
    ...
    def _tool_proxy_from_env() -> tuple[str, str, str]:
        ...
        if not base or not token or not session:
            raise RuntimeError("此内核中工具桥接不可用")
    ...
    def _bridge_call(name: str, args: dict):
        ...
        try:
            ...
            raise RuntimeError(
                f"桥接调用 {name!r}:非 JSON 响应:{body[:200]!r}"
            ) from None
        if not isinstance(data, dict) or not data.get("ok"):
            msg = (data or {}).get("error") if isinstance(data, dict) else None
            raise RuntimeError(msg or f"桥接调用 {name!r} 失败")
    ...
    class _ToolCallable:
        ...
        def __call__(self, args=None, /, **kwargs):
            if args is None:
                ...
                raise TypeError(
                    f"tool.{self._name}(...) 需要参数字典(实际为 {type(args).__name__})"
                )
            merged.update(kwargs)
            if INTENT_FIELD not in merged:
                merged[INTENT_FIELD] = "py 预加载"
    ...
    def parallel(thunks):
        ...
        for t in thunks:
            if not callable(t):
                raise TypeError("parallel() 需要零参数可调用对象的可迭代集合")
    ...
    def pipeline(items, *stages):
        ...
        for stage in stages:
            if not callable(stage):
                raise TypeError("pipeline() 的各阶段必须是可调用对象")
    ...
    class _Budget:
        ...
        def __repr__(self):
            try:
                snap = _bridge_call("__budget__", {}) or {}
                return f"<预算 total={snap.get('total')} spent={snap.get('spent')}>"
            except Exception:
                return "<预算不可用>"
