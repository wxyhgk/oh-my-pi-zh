...
export abstract class BaseKernel<TExecuteOptions extends KernelExecuteOptions = KernelExecuteOptions> {
	...
	setProcess(proc: Subprocess<"pipe", "pipe", "pipe">) {
		...
		void this.#exitedPromise.then(code => {
			this.#alive = false;
			this.#abortPendingExecutions(`${this.#options.languageName} 内核已退出,退出码 ${code}`, {
				kernelKilled: true,
			});
		});
	...
	async execute(code: string, options?: TExecuteOptions): Promise<KernelExecuteResult> {
		if (!this.isAlive()) {
			throw new Error(`${this.#options.languageName} 内核未在运行`);
		}
	...
	async interrupt(): Promise<void> {
		...
		try {
			...
			logger.warn(`中断 ${this.#options.languageName.toLowerCase()} runner 失败`, {
	...
	async shutdown(options?: KernelShutdownOptions): Promise<KernelShutdownResult> {
		...
		this.#alive = false;
		this.#abortPendingExecutions(`${this.#options.languageName} 内核已关闭`, { kernelKilled: true });
	...
	#abortPendingExecutions(reason: string, options?: { kernelKilled?: boolean }): void {
		...
		for (const entry of pending) {
			...
			entry.settled = true;
			void entry.options?.onChunk?.(`[内核] ${reason}\n`);
			entry.resolve({
				status: "error",
				cancelled: true,
	...
	async #writeLine(line: string): Promise<void> {
		if (!this.#stdin) {
			throw new Error(`${this.#options.languageName} 内核 stdin 未打开`);
		}
	...
	async executeWithBudget(
		...
		const timer =
			timeoutMs > 0
				? setTimeout(() => controller.abort(createAbortError("TimeoutError", `${label} 超时`)), timeoutMs)
				: undefined;
		...
		try {
			...
			if (result.cancelled) {
				throw createAbortError(result.timedOut ? "TimeoutError" : "AbortError", `${label} 已取消`);
			}
			if (result.status === "error") {
				const reason = result.error?.value ?? `${this.#options.languageName} 内核初始化失败`;
