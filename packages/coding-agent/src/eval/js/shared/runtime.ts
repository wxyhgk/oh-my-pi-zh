...
export class JsRuntime {
	...
	#activateGlobals(action: string): void {
		if (this.#disposed) throw new Error(`无法在已释放的 JS 运行时上${action}`);
	...
	setCwd(cwd: string): void {
		if (this.#disposed) throw new Error("无法在已释放的 JS 运行时上设置 cwd");
	...
	async run(
		...
		this.#activateGlobals("运行代码");
		const leaveRun = enterGlobalRun(this.#globalOwner, "运行代码");
	...
	displayValue(value: unknown, hooks: RuntimeHooks | undefined = this.#als.getStore()?.hooks): void {
		...
		if (!hooks) {
			logger.warn("在活动运行之外调用 js 运行时显示");
			return;
		}
		if (value && typeof value === "object") {
			...
			if (record.type === "image" && typeof record.mimeType === "string") {
				...
				logger.warn("js displayValue:丢弃无法识别数据形状的图片", {
					mimeType: record.mimeType,
					dataType: describeDataType(record.data),
				});
				hooks.onText(
					`[显示:已丢弃图片——\`data\` 必须是 base64 字符串、Uint8Array/Buffer 或 ArrayBuffer;实际为 ${describeDataType(record.data)}]\n`,
				);
				return;
			}
			try {
				...
				logger.debug("js displayValue:值不可结构化克隆,回退为文本", {
	...
	#activeHooks(action: string): RuntimeHooks | undefined {
		...
		if (!hooks) {
			logger.warn("在活动运行之外调用 js 运行时辅助函数", { action });
		}
		return hooks;
	}
	...
	#install(extraGlobals: Record<string, unknown> | undefined): void {
		...
		assertCanUseGlobalOwner(this.#globalOwner, "初始化 JS 运行时");
		const injected: Record<string, unknown> = {
			...
			__omp_log__: (level: string, ...args: unknown[]) => {
				const prefix = level === "error" ? "[错误] " : level === "warn" ? "[警告] " : "";
			...
			__omp_set_final_expr__: (value: unknown) => {
				...
				if (!context) {
					logger.warn("在活动运行之外设置 js 运行时最终表达式");
					return;
				}
...
function assertCanUseGlobalOwner(owner: symbol, action: string): void {
	...
	throw new Error(`无法${action}:另一个同 realm 的 JS 运行时正在运行`);
}
...
function activateGlobalOwner(owner: symbol, keys: Iterable<string>, action: string): void {
	...
	for (const key of keys) {
		...
		if (!stack || index === -1) throw new Error(`无法在已释放的 JS 运行时上${action}`);
