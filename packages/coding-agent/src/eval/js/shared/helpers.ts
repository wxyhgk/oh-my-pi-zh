...
export function createHelpers(ctx: HelperContext): HelperBundle {
	return {
		...
		writeFile: async (rawPath, data) => {
			if (!isWriteData(data)) {
				throw new ToolError("write() 需要字符串、Blob、ArrayBuffer 或 TypedArray 数据");
			}
...
function resolveHelperPath(ctx: HelperContext, rawPath: string, op: "read" | "write"): string {
	...
	if (!root) {
		throw new ToolError(`${op}() 不支持协议路径:${rawPath}`);
	}
...
function resolveUnderRoot(scheme: string, root: string, rawRelative: string, rawPath: string): string {
	...
	try {
		...
		throw new ToolError(`${scheme}:// 路径中的 URL 编码无效:${rawPath}`);
	}
	const rootPath = path.resolve(root);
	if (relative === "") return rootPath;
	if (path.isAbsolute(relative)) {
		throw new ToolError(`${scheme}:// URL 不允许绝对路径:${rawPath}`);
	}
	const normalized = path.normalize(relative);
	if (normalized.startsWith("..") || normalized.includes("/../") || normalized.includes("/..")) {
		throw new ToolError(`${scheme}:// URL 不允许路径穿越(..):${rawPath}`);
	}
	const resolved = path.resolve(rootPath, normalized);
	if (resolved !== rootPath && !resolved.startsWith(`${rootPath}${path.sep}`)) {
		throw new ToolError(`${scheme}:// 路径越出其根目录:${rawPath}`);
	}
	return resolved;
}
...
async function resolveRegularFile(
	...
	if (stat.isDirectory()) {
		throw new ToolError(`read() 不支持目录路径:${filePath}`);
	}
