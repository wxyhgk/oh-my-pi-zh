...
function foldFloatingRejections(active: ActiveRun, result: RunResult, hooks: RuntimeHooks): RunResult {
	...
	if (result.ok) {
		...
		error.message = `未处理的拒绝(缺少 await?):${error.message}`;
	...
	for (const reason of reported) {
		...
		hooks.onText(`[未处理的拒绝] ${payload.name ?? "Error"}: ${payload.message}\n`);
	}
	return folded;
}
	...
	#consumeRejection(reason: unknown): boolean {
		...
		if (stack) {
			...
			if (recent) {
				this.#transport.send({
					type: "log",
					level: "warn",
					msg: "来自已结束 eval 单元的未处理拒绝(缺少 await?)",
		...
		if (this.#options.mode === "isolated" && this.#runs.size > 0) {
			...
			this.#transport.send({
				type: "log",
				level: "warn",
				msg: "并发 eval 运行期间出现未处理的拒绝;无法归属到某个单元",
	...
	#syncProcessCwd(cwd: string, currentRunId?: string): void {
		...
		for (const runId of this.#runs.keys()) {
			...
			this.#transport.send({
				type: "log",
				level: "warn",
				msg: "JS eval 子进程保留了其进程 cwd:其他单元正在运行中",
				meta: { cwd },
			});
			return;
		}
	...
	#close(): void {
		for (const active of this.#runs.values()) {
			for (const pending of active.pendingTools.values()) {
				pending.reject(new ToolError("JS Worker 已关闭"));
			}
