import { logger, postmortem, Snowflake, workerHostEntry } from "@wxyhgk/pi-utils";
import {
	createWorkerHandle,
...
export async function resetVmContext(sessionKey: string): Promise<void> {
	...
	await killSession(session, new ToolError("JS 上下文已重置"), { force: false });
}
...
export async function disposeAllVmContexts(): Promise<void> {
	...
	sessions.clear();
	await Promise.all(all.map(session => killSession(session, new ToolError("JS 上下文已释放"), { force: false })));
}
...
export async function smokeTestJsEvalWorker(): Promise<void> {
	...
	try {
		...
		if (worker.mode !== "process") {
			throw new Error("JS eval Worker 冒烟测试回退到了非隔离子进程");
		}
	} finally {
...
async function runOnce(
	...
	const onAbort = (): void => {
		...
		const abortError = reasonToError(reason, "执行已中止");
		...
		while (true) {
			try {
				...
				if (failed.mode === "process") {
					logger.warn("JS eval 子进程初始化失败;改用 Bun Worker 重试", {
						error: error instanceof Error ? error.message : String(error),
					});
					session.worker = spawnBunWorker();
				} else {
					logger.warn("JS eval Worker 初始化失败;改用内联 worker 重试(无同步循环保护)", {
						error: error instanceof Error ? error.message : String(error),
					});
...
async function initWorker(session: JsSession, snapshot: SessionSnapshot, timeoutMs: number): Promise<void> {
	...
	try {
		...
		await raceWithTimeout(readyPromise, timeoutMs, "初始化 JS eval Worker 超时");
	} catch (error) {
...
async function handleToolCall(session: JsSession, msg: Extract<WorkerOutbound, { type: "tool-call" }>): Promise<void> {
	...
	if (!pending) {
		safeSend(session, {
			type: "tool-reply",
			id: msg.id,
			reply: { ok: false, error: { message: "运行已不再活动" } },
		});
		return;
	}
	if (pending.aborted) {
		safeSend(session, {
			type: "tool-reply",
			id: msg.id,
			reply: { ok: false, error: { message: "运行已被中断" } },
		});
		return;
	}
...
function safeSend(session: JsSession, msg: WorkerInbound): void {
	...
	try {
		...
		logger.debug("js worker 发送失败", { error: err instanceof Error ? err.message : String(err) });
	}
}
...
function errorFromPayload(payload: RunErrorPayload): Error {
	if (payload.isAbort) {
		const err = new ToolAbortError(payload.message || "执行已中止");
...
function spawnJsWorker(): WorkerHandle {
	if (!useWorkerThreadForTests) {
		try {
			...
			// cannot.
			logger.warn("JS eval 子进程启动失败;回退到 Bun Worker", {
...
function spawnBunWorker(): WorkerHandle {
	try {
		...
		logger.warn("Bun Worker 启动失败;改用内联 JS eval worker(无同步循环保护)", {
			error: err instanceof Error ? err.message : String(err),
		});
	...
	return {
		...
		onError(handler) {
			...
			const onMessageError = (event: MessageEvent): void =>
				handler(new ToolError(`JS eval Worker 消息错误:${String(event.data)}`));
			const onClose = (): void => handler(new Error("JS eval Worker 已退出"));
...
function errorFromWorkerEvent(event: ErrorEvent): Error {
	...
	return new Error("未知的 JS eval Worker 错误");
}
