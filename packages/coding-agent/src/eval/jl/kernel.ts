...
import { $flag, Snowflake } from "@wxyhgk/pi-utils";
import { $ } from "bun";
...
async function probeJuliaKernelAvailability(cwd: string, interpreter?: string): Promise<JuliaKernelAvailability> {
	...
	if (runtimes.length === 0) {
		return {
			ok: false,
			reason: "未在 PATH 中找到 Julia 可执行文件。请安装 Julia(https://julialang.org/)。",
		};
	}
	...
	for (const runtime of runtimes) {
		try {
			...
			failures.push(`${runtime.juliaPath}(退出码 ${probe.exitCode})`);
		} catch (err) {
	...
	return {
		...
		reason: `未找到可用的 Julia 解释器。尝试过:${failures.join("; ")}`,
	};
}
...
export class JuliaKernel extends BaseKernel<KernelExecuteOptions> {
	...
	static async start(options: KernelStartOptions): Promise<JuliaKernel> {
		...
		if (!availability.ok) {
			throw new Error(availability.reason ?? "Julia 内核不可用");
		}
		...
		try {
			...
			await kernel.executeWithBudget(initScript, startup.signal, startupBudget, "Julia 内核初始化");
			await kernel.executeWithBudget(JULIA_PRELUDE, startup.signal, startupBudget, "Julia 内核预加载");
			return kernel;
		} catch (err) {
