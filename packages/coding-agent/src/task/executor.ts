...
function withAbortTimeout<T>(
	...
	const timeoutId = setTimeout(() => {
		if (settled) return;
		settled = true;
		timeoutController?.abort(new DOMException(`MCP 工具调用在 ${timeoutMs}ms 后超时`, "TimeoutError"));
		reject(new Error(`MCP 工具调用在 ${timeoutMs}ms 后超时`));
	}, timeoutMs);
...
function buildSchemaViolationOutcome(
	...
	const headline =
		missing.length > 0
			? `schema_violation: 缺少必填字段:${missing.join(", ")}`
...
export function finalizeSubprocessOutput(args: FinalizeSubprocessOutputArgs): FinalizeSubprocessOutputResult {
	...
	if (hasYield) {
		...
		if (lastYield?.status === "aborted") {
			...
			exitCode = 0;
			stderr = lastYield.error || "子代理已中止任务";
			try {
				rawOutput = JSON.stringify({ aborted: true, error: lastYield.error }, null, 2);
			} catch {
				rawOutput = `{"aborted":true,"error":"${lastYield.error || "未知错误"}"}`;
			}
		} else {
			...
			if (!assembled || assembled.missingData) {
				...
				const failure =
					validation && !validation.success
						...
						: assembled.schemaOverridden
							...
							: schemaError
								? { message: `无效的输出 schema:${schemaError}`, missingRequired: [] }
								: undefined;
					...
					structuredOutput =
						schemaError || normalized === undefined
							? {
									...
									status: "unavailable",
									data: completeData,
									error: schemaError ? `无效的输出 schema:${schemaError}` : undefined,
								}
							: failure
				...
				if (mustReject && failure) {
					...
					try {
						...
						rawOutput = `{"error":"序列化 yield 数据失败:${errorMessage}"}`;
					}
		...
		if (fallback) {
			...
			if (!result.success) {
				...
				try {
					...
					const errorMessage = err instanceof Error ? err.message : String(err);
					rawOutput = `{"error":"序列化回退完成数据失败:${errorMessage}"}`;
				}
				exitCode = 0;
				stderr = "";
			}
	...
	return mcpManager.getTools().map(tool => {
		...
		return {
			...
			execute: async (toolCallId, params, onUpdate, ctx, signal) => {
				...
				if (!source?.execute) {
					return {
						content: [{ type: "text" as const, text: `MCP 错误:工具 ${mcpToolName} 不再可用` }],
				...
				try {
					...
					return {
						content: [
							{
								type: "text" as const,
								text: `MCP 错误:${error instanceof Error ? error.message : String(error)}`,
							},
						],
						details: { serverName, mcpToolName, isError: true },
					};
...
function createSubagentRunMonitor(args: RunMonitorArgs): SubagentRunMonitor {
	...
	const requestYieldTurnStop = () => {
		...
		yieldTurnStopPromise = session
			? session.abort().catch(error => {
					logger.debug("子代理 yield 轮次停止中止失败", {
						error: error instanceof Error ? error.message : String(error),
					});
				})
			: Promise.resolve();
	...
	const resolveAbortReasonText = (): string => {
		if (runtimeLimitExceeded) {
			return `子代理运行时长超限(task.maxRuntimeMs=${maxRuntimeMs})`;
		}
		if (budgetLimitExceeded) {
			return `软请求预算超限(${progress.requests} 次请求;预算 ${softRequestBudget})——强制停止后 Agent 未提交 yield`;
		}
		if (budgetStopRequested) {
			return `软请求预算超限(${progress.requests} 次请求;预算 ${softRequestBudget})`;
		}
				...
				if (event.toolName === "yield") {
					if (event.isError && !abortSent) {
						...
						if (consecutiveYieldToolErrors >= MAX_YIELD_TOOL_ERRORS) {
							const suffix = yieldErrorText ? ` 最近一次 yield 错误:${yieldErrorText}` : "";
							failWithError(
								`子代理连续 ${consecutiveYieldToolErrors} 次提交无效的 yield 结果,为防止无限提交循环而停止。${suffix}`,
							);
						}
			...
			if (event.type === "auto_retry_end") {
				...
				if (!event.success) {
					progress.retryFailure = {
						attempt,
						errorMessage: event.finalError ?? "自动重试失败",
					};
				}
				scheduleProgress(true);
				return;
	...
	try {
			...
			while (!monitor.yieldCalled() && retryCount < MAX_YIELD_RETRIES && !abortSignal.aborted) {
				...
				try {
					...
					if (abortSignal.aborted || err instanceof ToolAbortError) {
						...
						logger.debug("子代理提示词已中止");
					} else {
						logger.error("子代理提示词失败", {
							error: err instanceof Error ? err.message : String(err),
						});
					}
				}
		...
		if (lastAssistant) {
			if (lastAssistant.stopReason === "aborted") {
				...
				exitCode = 1;
				error ??= lastAssistant.errorMessage || "子代理失败";
			}
		}
...
async function finalizeRunResult(args: FinalizeRunArgs): Promise<SingleResult> {
	...
	if (
		...
		rawOutput = `[已取消:${progress.requests} 次请求,${progress.tokens} token —— 最后活动:"${formatSalvageSnippet(salvageText)}"]`;
	}
	const lastYield = yieldItems?.[yieldItems.length - 1];
	const yieldAbortReason = lastYield?.status === "aborted" ? lastYield.error || "子代理已中止任务" : undefined;
		...
		return async turnError => {
			...
			const error =
				lastAssistant?.stopReason === "error"
					? lastAssistant.errorMessage || "子代理失败"
	...
	const runSubagent = async (): Promise<{
		...
		try {
			...
			if (!registryFromParent) {
				...
				logger.debug("runSubagent: 复用父级 modelRegistry,跳过刷新");
			}
			checkAbort();
			...
			if (modelResolutionWarning) {
				logger.warn("子代理模型解析警告", {
					warning: modelResolutionWarning,
					requested: modelPatterns,
				});
			}
			if (authFallbackUsed && model) {
				logger.warn("子代理模型没有可用凭据;回退到父会话模型", {
					requested: modelPatterns,
			...
			if (prewalkPattern) {
				...
				if (!target || !modelRegistry.hasConfiguredAuth(target)) {
					logger.warn("子代理 prewalk 目标不可用;跳过 prewalk", {
						agent: agent.name,
					...
					logger.debug("子代理 prewalk 目标与起始模型和思考级别一致;跳过 prewalk", {
						agent: agent.name,
						pattern: prewalkPattern,
					});
				} else {
			...
			if (extensionRunner) {
				extensionRunner.initialize(
					{
						sendMessage: (message, options) => {
							const sendPromise = session.sendCustomMessage(message, options).catch(e => {
								logger.error("扩展 sendMessage 失败", {
						...
						sendUserMessage: (content, options) => {
							const sendPromise = session.sendUserMessage(content, options).catch(e => {
								logger.error("扩展 sendUserMessage 失败", {
									error: e instanceof Error ? e.message : String(e),
								});
							});
							pendingExtensionMessages.push(sendPromise);
				...
				extensionRunner.onError(err => {
					logger.error("扩展错误", { path: err.extensionPath, error: err.error });
				});
			...
			if (jobManager) {
				...
				while (!(await jobManager.waitForOwnerJobs(id, { timeoutMs: 10_000 }))) {
					logger.warn("子代理异步任务仍在结算;延迟清理直到进程退出", { id });
				}
			}
		}
