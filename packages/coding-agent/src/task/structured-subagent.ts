...
import { $env, prompt, Snowflake } from "@wxyhgk/pi-utils";
...
function assertDepthAndSpawnAllowed(request: StructuredSubagentRequest, agentName: string): void {
	...
	if (!canSpawnAtDepth(maxDepth, taskDepth)) {
		throw new StructuredSubagentError(
			"preflight",
			`无法在任务深度 ${taskDepth} 处再派生 Agent;最大深度为 ${maxDepth}。`,
		);
	}
	...
	if (blockedAgent && blockedAgent === agentName) {
		throw new StructuredSubagentError(
			"preflight",
			`无法从 ${blockedAgent} Agent 自身内部再次派生(防止递归)。请改用其他 Agent 类型。`,
		);
	}
...
export async function resolveEffectiveSubagentPolicy(
	...
	if (!agent) {
		...
		throw new StructuredSubagentError("preflight", `未知 Agent "${agentName}"。可用:${available}`);
	}
	...
	if (disabledAgents.includes(agentName)) {
		...
		throw new StructuredSubagentError(
			"preflight",
			`Agent "${agentName}" 已在设置中禁用。请通过 /agents 启用,或改用其他 Agent 类型。${enabled.length > 0 ? ` 可用:${enabled.join(", ")}` : ""}`,
		);
	}
	...
	if (schema.source === "caller" || (schema.source !== "none" && schema.mode === "strict")) {
		...
		if (error) {
			...
			throw new StructuredSubagentError("preflight", `无效的 ${scope} 输出 schema:${error}`);
		}
	}
...
async function isolationRecoveryHint(result: SingleResult, artifactsDir: string): Promise<string> {
	const hints: string[] = [];
	if (result.patchPath) hints.push(`已保留捕获的补丁:${result.patchPath}。`);
	for (const nestedPath of await persistNestedPatches(artifactsDir, result.id, result.nestedPatches ?? [])) {
		hints.push(`已保留捕获的嵌套补丁:${nestedPath}。`);
	}
	if (result.branchName) hints.push(`已保留捕获的分支:${result.branchName}。`);
	...
	try {
		...
		if (
			...
			if (result.branchName)
				mergeSummary = `\n\n隔离:更改已捕获到分支 \`${result.branchName}\`(apply=false)。未合并。`;
			else if (result.patchPath)
				mergeSummary = `\n\n隔离:更改已捕获到 \`${result.patchPath}\`(apply=false)。未应用。`;
			else if ((result.nestedPatches?.length ?? 0) > 0)
				mergeSummary = `\n\n隔离:已为 ${result.nestedPatches?.length} 个嵌套仓库捕获更改(apply=false)。未应用。`;
			else mergeSummary = "\n\n隔离:未捕获任何更改。";
		}
