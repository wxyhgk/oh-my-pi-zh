...
import { getWorktreeDir, logger, Snowflake } from "@wxyhgk/pi-utils";
...
export async function getRepoRoot(cwd: string): Promise<string> {
	...
	if (await jj.isPureJjRepo(cwd)) {
		throw new Error(
			"隔离任务执行需要 Git 检出,但此工作区是纯 Jujutsu(`.jj/` 且没有并置的 `.git/`)。请运行 `jj git init --colocate` 添加 Git 检出,或将 `task.isolation.mode: none` 设置为禁用任务隔离。",
		);
	}
	...
	throw new Error("未找到 Git 仓库,无法执行隔离任务。");
}
		...
		try {
			...
			if ((await git.status(nestedDir)).trim().length > 0) {
				if (touchedFiles.length === 0) {
					throw new Error(`嵌套仓库 ${relativePath} 的补丁未包含可暂存的文件路径。`);
				}
				const msg = (await commitMessage?.(combinedDiff)) ?? "来自隔离任务的更改";
			...
			if (stashed) {
				...
				if (!restored) {
					logger.warn("嵌套仓库原有的脏状态无法自动恢复", {
						nestedDir,
					});
					warnings.push(
						`嵌套仓库 \`${relativePath}\` 中原有的脏状态在 Agent 提交后无法自动恢复;已保留 stash 条目。`,
					);
				}
			}
		}
...
export async function ensureIsolation(
	...
	throw new Error(fallbackReason ?? "没有可用的隔离后端。");
}
	...
	if (plainErr) {
		...
		if (threeWayErr) {
			...
			if (wipPatches.length === 0 || !baselineWip) {
				...
				logger.error("commitToBranch: git apply --3way 失败", {
					taskId,
				...
				throw new Error(`任务 ${taskId} 的 git apply --3way 失败:${stderr}`);
			}
			try {
				...
				logger.error("commitToBranch: 带基线 WIP 的 git apply 失败", {
					taskId,
				...
				throw new Error(`任务 ${taskId} 的带基线 WIP git apply 失败:${stderr}`);
			}
		}
	}
		...
		try {
			...
			if (didStash) {
				...
				if (!restored) {
					...
					logger.warn("任务合并后无法恢复 stash 的更改;已保留 stash 条目");
					const stashConflict =
						"stash pop:cherry-pick 的更改与未提交的编辑冲突。已合并的提交位于 HEAD;请运行 `git stash pop` 并手动解决。";
					if (conflictResult) {
