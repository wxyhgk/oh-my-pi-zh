...
import type * as natives from "@wxyhgk/pi-natives";
	...
	try {
		...
		if (opts.mergeMode === "branch" && result.exitCode === 0) {
			try {
				...
				try {
					...
					return { ...result, error: `合并失败:${msg};补丁捕获失败:${patchMsg}` };
				}
			}
		}
		if (result.exitCode === 0) {
			try {
				...
				return { ...result, error: `补丁捕获失败:${msg}` };
			}
		}
		return result;
	} catch (err) {
	...
	try {
		if (mergeMode === "branch") {
			if (!result.branchName && result.exitCode === 0 && !result.aborted && result.error) {
				...
				return {
					summary: `\n\n<system-notification>在任务分支创建之前分支合并失败:${result.error}\n任务输出已保留,但更改未应用。${patchList}</system-notification>`,
			...
			let summary: string;
			if (changesApplied) {
				summary = hadAnyChanges ? `\n\n已合并分支:${result.branchName}` : "\n\n没有需要应用的更改。";
			} else {
				const conflictPart = mergeResult.conflict ? `\n冲突:${mergeResult.conflict}` : "";
				summary = `\n\n<system-notification>分支合并失败:${result.branchName}。${conflictPart}\n未合并的分支保留,供手动解决。</system-notification>`;
			}
		...
		const msg = mergeErr instanceof Error ? mergeErr.message : String(mergeErr);
		return {
			summary: `\n\n<system-notification>合并阶段失败:${msg}\n任务输出已保留,但更改未应用。</system-notification>`,
			changesApplied: false,
			hadAnyChanges: false,
			mergedBranchForNestedPatches: false,
		};
...
export async function applyEligibleNestedPatches(opts: NestedPatchApplyOptions): Promise<string> {
	...
	try {
		...
		return "\n\n<system-notification>部分嵌套仓库补丁应用失败。</system-notification>";
	}
}
