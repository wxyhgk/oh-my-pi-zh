...
import { Container, Markdown, Text } from "@wxyhgk/pi-tui";
import { formatNumber, sanitizeText } from "@wxyhgk/pi-utils";
...
function renderNestedCycleLine(theme: Theme): string {
	return theme.fg("dim", "… 嵌套任务进度已在上方显示");
}
...
function formatFindingSummary(findings: FindingDetails[], theme: Theme): string {
	if (findings.length === 0) return theme.fg("dim", "发现:无");
	...
	return `${theme.fg("dim", "发现:")} ${parts.join(theme.sep.dot)}`;
}
...
function formatYieldPreview(item: RenderYieldItem): string {
	if (item.useLastTurn === true && item.data === undefined) return "最后助手轮次";
	if (item.data === undefined) return "最后助手轮次";
...
function renderTypedYieldSections(value: unknown, continuePrefix: string, expanded: boolean, theme: Theme): string[] {
	...
	if (typedItems.length > displayCount) {
		lines.push(`${continuePrefix}${theme.fg("dim", formatMoreItems(typedItems.length - displayCount, "结果"))}`);
	}
	return lines;
}
...
const MISSING_YIELD_WARNING_PREFIX = "SYSTEM WARNING: 子代理在 3 次提醒后仍未调用 yield 工具即退出";
...
function renderJsonTreeLines(
	...
	const renderNode = (val: unknown, key: string | undefined, ancestors: boolean[], isLast: boolean, depth: number) => {
		...
		if (scalar) {
			const label = key ? theme.fg("muted", sanitizeText(key)) : theme.fg("muted", "值");
			pushLine(`${prefix}${iconScalar} ${label}: ${theme.fg("dim", scalar)}`);
			return;
		}

		if (Array.isArray(val)) {
			const header = key ? theme.fg("muted", sanitizeText(key)) : theme.fg("muted", "数组");
		...
		if (val && typeof val === "object") {
			const header = key ? theme.fg("muted", sanitizeText(key)) : theme.fg("muted", "对象");
...
function renderOutputSection(
	...
	if (warning) {
		lines.push(`${continuePrefix}${theme.fg("dim", "输出")}`);
		lines.push(
...
function renderTaskSection(
	...
	lines.push(`${continuePrefix}${theme.fg("dim", "任务")}`);
...
function formatOutputInline(data: unknown, theme: Theme, maxWidth = 80): string {
	if (data === null || data === undefined) return "输出:无";

	// For scalars, show directly
	if (typeof data !== "object") {
		return `输出:${formatScalarInline(data, 60, theme)}`;
	}

	// For arrays, show count and first element preview
	if (Array.isArray(data)) {
		if (data.length === 0) return "输出:[]";
		const preview = formatScalarInline(data[0], 40, theme);
		return `输出:[${data.length} 项] ${preview}${data.length > 1 ? "…" : ""}`;
	}

	// For objects, show key=value pairs inline
	const entries = Object.entries(data as Record<string, unknown>);
	if (entries.length === 0) return "输出:{}";

	const pairs: string[] = [];
	let totalLen = "输出:".length;
...
function renderAgentProgress(
	...
	if (progress.retryState && progress.status === "running") {
		...
		const waitLabel = remainingMs > 0 ? `在 ${formatDuration(remainingMs)} 后` : "现在";
		const summary =
			`重试 ${progress.retryState.attempt}/${progress.retryState.maxAttempts} ${waitLabel}:` +
		...
		const summary = `自动重试在 ${progress.retryFailure.attempt} 次尝试后放弃:${previewLine(sanitizeText(progress.retryFailure.errorMessage), 80)}`;
				...
				if ((dataArray as unknown[]).length > displayCount) {
					lines.push(
						`${continuePrefix}${theme.fg(
							"dim",
							formatMoreItems((dataArray as unknown[]).length - displayCount, "条目"),
						)}`,
					);
				}
			}
...
function renderReviewResult(
	...
	lines.push(
		`${continuePrefix} 补丁判定:${theme.fg(verdictColor, summary.overall_correctness)} ${verdictIcon} ${theme.fg(
			"dim",
			`(${(summary.confidence * 100).toFixed(0)}% 置信度)`,
		)}`,
	);
	...
	if (summary.explanation) {
		if (expanded) {
			lines.push(`${continuePrefix}${theme.fg("dim", "摘要")}`);
...
function renderFindings(findings: FindingDetails[], continuePrefix: string, expanded: boolean, theme: Theme): string[] {
	...
	for (let i = 0; i < displayCount; i++) {
		...
		const rawTitle = sanitizeText(finding.title?.replace(/^\[P\d\]\s*/, "") ?? "未命名");
	...
	if (!expanded && findings.length > 3) {
		lines.push(`${continuePrefix}${theme.fg("dim", formatMoreItems(findings.length - 3, "发现"))}`);
	}
...
function renderAgentResult(
	...
	if (result.extractedToolData) {
		for (const toolName in result.extractedToolData) {
			...
			if (handler?.renderFinal && (dataArray as unknown[]).length > 0) {
				...
				if (!isTaskTool) {
					hasCustomRendering = true;
					target.push(`${continuePrefix}${theme.fg("dim", `工具:${toolName}`)}`);
				}
				if (component instanceof Text) {
	...
	if (result.patchPath && !aborted && result.exitCode === 0) {
		lines.push(`${continuePrefix}${theme.fg("dim", `补丁:${result.patchPath}`)}`);
	} else if (result.branchName && !aborted && result.exitCode === 0) {
		lines.push(`${continuePrefix}${theme.fg("dim", `分支:${result.branchName}`)}`);
	}
...
function formatHiddenProgressLine(hidden: readonly AgentProgress[], theme: Theme): string {
	...
	const parts: string[] = [];
	if (counts.completed > 0) parts.push(theme.fg("dim", `${counts.completed} 已完成`));
	if (counts.running > 0) parts.push(theme.fg("dim", `${counts.running} 运行中`));
	if (counts.pending > 0) parts.push(theme.fg("dim", `${counts.pending} 等待中`));
	if (counts.failed > 0) parts.push(theme.fg("error", `${counts.failed} 失败`));
	if (counts.aborted > 0) parts.push(theme.fg("error", `${counts.aborted} 已中止`));
	const breakdown =
		parts.length > 0
...
export function renderResult(
	...
	if (!details) {
		...
		const header = errored
			? renderStatusLine({ icon: "error", title: "任务", description: agentLabel }, theme)
			: renderStatusLine(
					{
	...
	const countLabel = agentCount > 0 ? `${agentCount} 个 Agent` : undefined;
	...
	return framedBlock(theme, width => {
		...
		if (shouldRenderProgress && details.progress) {
			...
			if (abortedCount > 0) summaryParts.push(theme.fg("error", `${abortedCount} 已中止`));
			if (successCount > 0) summaryParts.push(theme.fg("success", `${successCount} 已完成`));
			if (mergeFailedCount > 0) summaryParts.push(theme.fg("warning", `${mergeFailedCount} 合并失败`));
			if (failCount > 0) summaryParts.push(theme.fg("error", `${failCount} 失败`));
			const totalRequests = requestTotal;
			if (totalRequests > 0) summaryParts.push(theme.fg("dim", `${formatNumber(totalRequests)} 次请求`));
		...
		if (lines.length === 0) {
			const text = fallbackText.trim() ? fallbackText : "无结果";
			return {
				header,
				sections: [
					...(contextSection ? [contextSection(width)] : []),
		...
		if (fallbackText.trim()) {
			...
			const markerIndex = summaryLines.findIndex(
				line =>
					...
					line.startsWith("已应用的补丁:") ||
					line.startsWith("没有需要应用的更改。"),
			);
			if (markerIndex >= 0) {
...
function renderNestedTaskResults(
	...
	for (const details of detailsList) {
		...
		if (depth >= MAX_NESTED_TASK_RENDER_DEPTH) {
			lines.push(theme.fg("dim", "… 达到嵌套任务深度上限"));
			continue;
		}
		seen.add(details);
