...
export class IrcBus {
	...
	async send(
		...
		if (!ref) {
			return {
				to: message.to,
				outcome: "failed",
				error: `未知 Agent "${message.to}" — 请用 \`irc list\` 查看当前在线的同伴。`,
			};
		}
		if (ref.status === "aborted") {
			return {
				to: message.to,
				outcome: "failed",
				error: `Agent "${message.to}" 已被硬中止,无法向其发送消息或将其恢复。其记录仍可在 history://${message.to} 查看。`,
			};
		}
		...
		if (ref.kind === "advisor") {
			return {
				to: message.to,
				outcome: "failed",
				error: `Agent "${message.to}" 是只读的 advisor 记录,无法向其发送消息。`,
			};
		}
		...
		if (!session) {
			return { to: message.to, outcome: "failed", error: `Agent "${message.to}" 没有活跃会话。` };
		}
	...
	async wait(
		...
		if (signal?.aborted) {
			throw signal.reason instanceof Error ? signal.reason : new Error("IRC 等待已中止");
		}
		...
		const livenessReason = filter.from
			? `IRC 等待已中止:Agent "${filter.from}" 未在运行`
			: "IRC 等待已中止:没有仍在运行的同伴";
		...
		if (signal) {
			onAbort = () =>
				settle({
					kind: "abort",
					error: signal.reason instanceof Error ? signal.reason : new Error("IRC 等待已中止"),
				});
	...
	unreadCount(agentId: string): number {
		...
		if (mailbox.length > MAILBOX_CAP) {
			...
			logger.debug("IrcBus: 邮箱已满,已丢弃最早的消息", {
				agentId: message.to,
	...
	#relayToMainUi(message: IrcMessage): void {
		...
		try {
			...
			logger.debug("IrcBus: 主界面转发失败", { to: message.to, error: String(error) });
		}
	}
}
