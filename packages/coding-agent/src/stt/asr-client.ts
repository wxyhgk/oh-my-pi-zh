...
export class SttClient {
	...
	async transcribe(modelKey: SttModelKey, audio: Float32Array, options: SttTranscribeOptions = {}): Promise<string> {
		...
		const abort = (): void => {
			...
			pending.reject(new DOMException("操作已中止。", "AbortError"));
		};
	...
	async terminate(): Promise<void> {
		...
		for (const pending of this.#pending.values()) {
			...
			if (pending.kind === "transcribe") pending.reject(new Error("STT 工作进程已终止"));
			else pending.resolve({ ok: false });
		}
		this.#pending.clear();
		this.#refed = false;
		this.#failStreams(new Error("STT 工作进程已终止"));
		try {
...
export async function smokeTestSttWorker({
	...
	await smokeTestWorker(wrapSubprocess(createSttSubprocess()), "STT 工作进程", timeoutMs);
}
