...
export async function downloadSttModel(
	...
	if (!result.ok) {
		const detail = result.error ? `: ${result.error}` : "。请检查网络连接。";
		throw new Error(`下载语音模型失败(${spec.repo})${detail}`);
	}
	if (!(await isSttModelCached(spec.key))) {
		throw new Error(`语音模型下载完成但缺少必需文件(${spec.repo})。`);
	}
}
		...
		progress => {
			const stage =
				progress.status === "ready" || progress.status === "done"
					? `语音模型 ${progress.label} 已就绪`
					: `正在下载语音模型 ${progress.label}`;
