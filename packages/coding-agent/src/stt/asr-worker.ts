...
function resolveSherpaVersionSpec(): string {
	...
	if (!versionSpec) throw new Error(`${SHERPA_PACKAGE} 在 package.json 的 optionalDependencies 中缺失`);
	return versionSpec;
}
...
function loadSherpaRuntime(transport: SttTransport, requestId: string, modelKey: SttModelKey): Promise<SherpaRuntime> {
	return sherpaRuntime.load(async () => {
		...
		if (!entry) throw new Error(`无法在编译运行时 ${nodeModules} 中解析 ${SHERPA_PACKAGE}`);
...
async function loadPipelineWithDeviceFallback(
	...
	if (devices[0] !== sttModelDevicePreference.device) {
		sendLog(transport, "warn", "stt:请求的设备在工作进程中不安全,改用 CPU", {
			modelKey,
			repo: spec.repo,
	...
	for (let i = 0; i < devices.length; i += 1) {
		...
		try {
			...
			sendLog(transport, "warn", "stt:加速设备加载失败,正在回退", {
				modelKey,
				repo: spec.repo,
				device,
				fallbackDevice,
	...
	throw new Error("未配置任何 STT 模型设备");
}
...
async function loadTransformersModel(
	...
	sendLog(transport, "debug", "stt:本地模型已加载", {
		modelKey,
		repo: spec.repo,
		engine: "transformers",
		device,
...
async function downloadSherpaFile(
	...
	if (!response.ok || !response.body) {
		throw new Error(`下载 ${filename}(${repo})失败:HTTP ${response.status}`);
	}
...
async function loadModel(modelKey: SttModelKey, transport: SttTransport, requestId: string): Promise<LoadedModel> {
	...
	if (!spec) throw new Error(`未知的 STT 模型:${modelKey}`);
