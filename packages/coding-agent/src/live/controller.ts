...
import { AudioCapture } from "@wxyhgk/pi-natives";
...
export class LiveSessionController {
	...
	async start(): Promise<void> {
		...
		if (this.#stopped) {
			throw this.#failure ?? new Error("实时会话在启动期间已停止。");
		}
		...
		try {
			...
			if (this.#stopped) {
				throw this.#failure ?? new Error("实时会话在连接期间已停止。");
			}
			...
			if (this.#stopped) {
				throw this.#failure ?? new Error("实时会话在录音开始前已停止。");
			}
			...
			if (this.#stopped) {
				...
				throw this.#failure ?? new Error("实时会话在录音开始时已停止。");
			}
