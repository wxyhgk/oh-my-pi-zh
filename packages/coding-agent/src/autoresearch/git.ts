...
export async function ensureAutoresearchBranch(
	...
	if (await jj.isPureJjRepo(workDir)) {
		return {
			ok: false,
			error: "Autoresearch 需要 Git 检出来实现分支隔离和基线提交,但此工作区是纯 Jujutsu(`.jj/` 而没有共置的 `.git/`)。请先运行 `jj git init --colocate` 添加 Git 检出,再开始 autoresearch。",
		};
	}
	...
	try {
		...
		return {
			ok: false,
			error: `开始 autoresearch 前无法检查 git 状态:${err instanceof Error ? err.message : String(err)}`,
		};
	}
	...
	if (dirtyPaths.length > 0) {
		...
		return {
			ok: false,
			error: `工作区不干净(${preview})。开始 autoresearch 前请先提交或暂存这些更改 — 新的 autoresearch/* 分支需要干净的基线。`,
		};
	}
	...
	try {
		...
		return {
			ok: false,
			error: `创建 autoresearch 分支 ${branchName} 失败:${err instanceof Error ? err.message : String(err)}`,
		};
	}
...
function formatDirtyPaths(paths: string[]): string {
	...
	return paths.length > 5 ? `${preview}(另加 ${paths.length - 5} 个)` : preview;
}
