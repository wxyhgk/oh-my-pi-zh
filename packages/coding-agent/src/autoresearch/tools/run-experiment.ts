...
const runExperimentSchema = type({
	"timeout_seconds?": type("number").describe("超时秒数(默认 600)"),
});
	...
	return {
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (abandonedPriorRun !== null) {
				headerLines.push(`注意:在开始本次运行之前已放弃之前待处理的运行 #${abandonedPriorRun}。`);
			}
		...
		renderResult(result, options, theme): Text {
			if (isProgressDetails(result.details)) {
				const header = theme.fg("warning", `运行中 ${result.details.elapsed}…`);
			...
			const suffix =
				options.expanded && details.truncation && details.fullOutputPath
					? `\n${theme.fg("warning", `完整输出:${shortenPath(details.fullOutputPath)}`)}`
					: "";
...
function buildRunText(details: RunDetails, outputPreview: string, bestMetric: number | null): string {
	const lines: string[] = [];
	lines.push(`运行 #${details.runNumber} 目录:${details.runDirectory}`);
	if (details.timedOut) {
		lines.push(`在 ${details.durationSeconds.toFixed(1)}s 后超时`);
	} else if (details.exitCode !== 0) {
		lines.push(`失败,退出码 ${details.exitCode},耗时 ${details.durationSeconds.toFixed(1)}s`);
	} else {
		lines.push(`通过,耗时 ${details.durationSeconds.toFixed(1)}s`);
	}
	if (bestMetric !== null) {
		lines.push(`当前基线 ${details.metricName}:${formatNum(bestMetric, details.metricUnit)}`);
	}
	if (details.parsedPrimary !== null) {
		lines.push(`已解析 ${details.metricName}:${details.parsedPrimary}`);
		lines.push(`下一次 log_experiment 指标:${details.parsedPrimary}`);
	}
	if (details.parsedMetrics) {
		...
		if (secondary.length > 0) {
			lines.push(`已解析指标:${secondary.join(", ")}`);
			lines.push(`下一次 log_experiment 指标:${JSON.stringify(Object.fromEntries(secondaryEntries))}`);
		}
	}
	if (details.parsedAsi) {
		lines.push(`已解析 ASI 键:${Object.keys(details.parsedAsi).join(", ")}`);
	}
	lines.push("");
...
function renderStatus(details: RunDetails, theme: Theme): string {
	if (details.timedOut) {
		return theme.fg("error", `超时 ${details.durationSeconds.toFixed(1)}s`);
	}
	if (details.exitCode !== 0) {
		return theme.fg("error", `失败 exit=${details.exitCode} ${details.durationSeconds.toFixed(1)}s`);
	}
	const metric =
		details.parsedPrimary !== null
			? ` ${details.metricName}=${formatNum(details.parsedPrimary, details.metricUnit)}`
			: "";
	return theme.fg("success", `通过 ${details.durationSeconds.toFixed(1)}s${metric}`);
}
