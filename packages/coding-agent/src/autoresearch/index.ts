...
import type { AutocompleteItem } from "@wxyhgk/pi-tui";
...
export const createAutoresearchExtension: ExtensionFactory = api => {
	...
	api.registerCommand("autoresearch", {
		description: "切换内置 autoresearch 模式,或传入 off / clear,或传入目标消息。",
		getArgumentCompletions(argumentPrefix: string): AutocompleteItem[] | null {
			if (argumentPrefix.includes(" ")) return null;
			const normalized = argumentPrefix.trim().toLowerCase();
			if (normalized.length === 0) return null;
			const completions: AutocompleteItem[] = [
				{ label: "off", value: "off", description: "退出 autoresearch 模式" },
				{
					label: "clear",
					value: "clear",
					description: "将工作区重置到基线并关闭活动会话",
				},
			];
		...
		async handler(args, ctx): Promise<void> {
			...
			if (trimmed === "" && runtime.autoresearchMode) {
				...
				ctx.ui.notify("Autoresearch 模式已禁用", "info");
				return;
			}
			...
			if (goalArg !== null) {
				...
				ctx.ui.notify("Autoresearch 已启用 — 在下一消息中描述要优化的目标。", "info");
			}
		},
	});

	api.registerShortcut("ctrl+x", {
		description: "切换 autoresearch 仪表盘",
		handler(ctx): void {
			const runtime = getRuntime(ctx);
			if (runtime.state.results.length === 0 && !runtime.runningExperiment) {
				ctx.ui.notify("尚无 autoresearch 结果", "info");
				return;
			}
	...
	api.registerShortcut("ctrl+shift+x", {
		description: "显示 autoresearch 仪表盘浮层",
	...
	async function handleClear(
		...
		const experimentTools = new Set(EXPERIMENT_TOOL_NAMES);
		await api.setActiveTools(api.getActiveTools().filter(name => !experimentTools.has(name)));
		ctx.ui.notify("Autoresearch 会话已清除。", "info");
	}
};
...
function removeLegacyArtifacts(workDir: string): void {
	for (const name of LEGACY_ARTIFACTS) {
		...
		try {
			...
			logger.warn("移除遗留 autoresearch 产物失败", {
				path: target,
