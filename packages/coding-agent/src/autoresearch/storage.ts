...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@wxyhgk/pi-utils";
...
export class AutoresearchStorage {
	...
	openSession(params: OpenSessionParams): SessionRow {
		...
		if (!row) throw new Error("插入 autoresearch 会话失败");
		const session = this.getSessionById(row.id);
		if (!session) throw new Error(`读取已插入的 autoresearch 会话 ${row.id} 失败`);
		return session;
	}
	...
	updateSession(sessionId: number, updates: UpdateSessionParams): SessionRow {
		...
		if (!session) throw new Error(`更新后未找到会话 ${sessionId}`);
		return session;
	}
	...
	bumpSegment(sessionId: number): SessionRow {
		...
		const session = this.getSessionById(sessionId);
		if (!session) throw new Error(`推进分段后未找到会话 ${sessionId}`);
		return session;
	}
	...
	insertRun(params: InsertRunParams): RunRow {
		...
		if (!row) throw new Error("插入运行记录失败");
	...
	getRunByIdRequired(runId: number): RunRow {
		...
		if (!run) throw new Error(`未找到运行记录 ${runId}`);
		return run;
	}
...
export function closeAllAutoresearchStorages(): void {
	for (const storage of storageCache.values()) {
		try {
			...
			logger.warn("关闭 autoresearch 存储失败", {
