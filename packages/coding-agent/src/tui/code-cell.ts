...
export function renderCodeCell(options: CodeCellOptions, theme: Theme): string[] {
	...
	if (hiddenCodeLines > 0) {
		...
		if (tail) {
			...
			const moreLine = `${formatMoreItems(hiddenCodeLines, "行")}${hint ? ` ${hint}` : ""}`;
	...
	if (output?.trim()) {
		...
		if (remaining > 0) {
			...
			const moreLine = `${formatMoreItems(remaining, "行")}${hint ? ` ${hint}` : ""}`;
...
export function renderMarkdownCell(options: MarkdownCellOptions, theme: Theme): string[] {
	...
	if (hiddenContentLines > 0) {
		...
		const moreLine = `${formatMoreItems(hiddenContentLines, "行")}${hint ? ` ${hint}` : ""}`;
