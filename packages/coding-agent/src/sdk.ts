...
import { resolveApiKeyOnce } from "@wxyhgk/pi-ai/auth-retry";
import type { Dialect } from "@wxyhgk/pi-ai/dialect";
import {
	getOpenAICodexTransportDetails,
	prewarmOpenAICodexResponses,
} from "@wxyhgk/pi-ai/providers/openai-codex-responses";
import { FALLBACK_DIALECT, preferredDialect } from "@wxyhgk/pi-catalog/identity";
import type { Component } from "@wxyhgk/pi-tui";
import { $env, $flag, getAgentDir, getProjectDir, logger, postmortem, prompt, Snowflake } from "@wxyhgk/pi-utils";
...
function createPendingMCPTool(name: string): Tool {
	...
	const message = serverName
		? `MCP 服务器 “${serverName}” 仍在连接中;工具 “${name}” 暂不可用。请在 MCP 连接完成后重试。`
		: `MCP 发现仍在进行中;工具 “${name}” 暂不可用。请在 MCP 连接完成后重试。`;
	const tool: Tool & { mcpServerName?: string; mcpToolName?: string } = {
		name,
		label,
		description: `待连接的 MCP 工具。${message}`,
		parameters: {
			type: "object",
			properties: {},
 ...
 *
 * // With explicit model
 * import { getModel } from '@wxyhgk/pi-ai';
 * const { session } = await createAgentSession({
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			queueLaunchCompletion: notification =>
				...
				Promise.reject(new Error("会话不可用,无法投递启动完成事件")),
				...
				if (
					...
					if (usageHealth?.state === "depleted") {
						if (usageReservePolicy === "fail-closed") {
							throw new Error(
								`“${primary.model.provider}/${primary.model.id}”的用量已耗尽;储备策略为 fail-closed。`,
							);
						}
					...
					if (usageHealth?.state === "reserve") {
						if (usageReservePolicy === "fail-closed") {
							throw new Error(
								`“${primary.model.provider}/${primary.model.id}”已达用量储备线;储备策略为 fail-closed。`,
							);
						}
			...
			if (model) {
				...
				modelFallbackMessage =
					patterns && patterns.length > 0
						? `没有与 enabledModels (${patterns.join(", ")}) 匹配且凭据可用的模型。请为允许的提供商配置认证,或调整 enabledModels。`
						: "没有可用模型。请使用 /login 登录或设置 API key 环境变量,然后使用 /model 选择模型。";
			}
		}
		...
		if (!registeredAgentRef) {
			throw new Error(`Agent “${resolvedAgentId}” 已被另一会话代次占有。`);
		}
		...
		const convertToLlmWithBlockImages = (messages: AgentMessage[]): Message[] => {
			...
			if (settings.get("images.blockImages")) {
				return replaceLlmImagesWithText(converted, "图像读取已禁用。");
			}
		...
		if (
			...
			throw new Error(`Agent “${resolvedAgentId}” 在会话初始化期间被替换。`);
		}
		hasRegistered = true;
		...
		const runAutoLearnCapture = createAutoLearnCaptureRunner({
			...
			createAgent: captureOptions => {
				...
				if (!captureModel || !captureSessionId) throw new Error("自动学习捕获身份信息不完整");
				return new Agent({
					...captureOptions,
