...
import { EventLoopKeepalive } from "@wxyhgk/pi-agent-core";
...
async function checkForNewVersion(currentVersion: string): Promise<string | undefined> {
	...
	try {
		const response = await fetch("https://registry.npmjs.org/@wxyhgk/pi-coding-agent/latest", {
...
export async function readPipedInput(): Promise<string | undefined> {
	...
	const notice = setTimeout(() => {
		process.stderr.write(`${chalk.dim("正在从管道 stdin 读取提示词(等待 EOF;ctrl+c 可中止)…")}\n`);
	}, 1000);
	notice.unref?.();
	try {
...
function armStartupWatchdog(): void {
	...
	startupWatchdogTimer = setInterval(() => {
		...
		process.stderr.write(
			`${chalk.yellow(`仍在启动中(已耗时 ${elapsed} 秒)`)}${chalk.dim(` — 阶段:${phase}`)}\n` +
				`${chalk.dim(`  日志:${getLogPath()} · 使用 PI_DEBUG_STARTUP=1 重新运行可查看流式阶段标记`)}\n`,
		);
...
export function buildModelScopeNotification(
	...
	const modelList = scopedModelsForDisplay
		...
		.join(", ");
	return { kind: "info", message: `模型范围:${modelList}(Ctrl+P 切换)` };
}
...
function resolveForeignSessionSource(
	...
	if (parsed.fromClaude && parsed.fromCodex) {
		throw new SessionResolutionError("--from-claude 与 --from-codex 不能同时使用");
	}
	const source = parsed.fromClaude ? "claude" : parsed.fromCodex ? "codex" : undefined;
	if (!source) return undefined;
	if (parsed.noSession) {
		throw new SessionResolutionError(`--from-${source} 需要会话持久化`);
	}
	if (parsed.continue || parsed.resume || parsed.fork) {
		throw new SessionResolutionError(`--from-${source} 不能与 --continue、--resume 或 --fork 同时使用`);
	}
	return source;
}
...
async function moveMissingCwdSessionIfNeeded(
	...
	if (movePromptResult === "unavailable") {
		throw new SessionResolutionError(
			`会话 “${sessionArg}” 所属的目录已不存在(${sourceCwd});请以交互模式运行以将其移动到当前项目。`,
		);
	}
...
export async function createSessionManager(
	...
	if (parsed.fork) {
		if (parsed.noSession) {
			throw new SessionResolutionError("--fork 需要会话持久化");
		}
		...
		if (!match) {
			throw new SessionResolutionError(
				`未找到会话 “${forkSource}”。`,
				"不带参数运行 `omp-zh --resume` 可从最近的会话中选择,或运行 `omp-zh` 开始一个新会话。",
			);
		}
...
export async function buildSessionOptions(
	...
	if (parsed.model) {
		...
		if (resolved.warning) {
			process.stderr.write(`${chalk.yellow(`警告:${resolved.warning}`)}\n`);
		}
	...
	if (parsed.noPrewalk && (parsed.prewalk || parsed.prewalkInto !== undefined)) {
		throw new Error("--no-prewalk 不能与 --prewalk 或 --prewalk-into 同时使用");
	}
	...
	if (prewalkEnabled) {
		...
		if (resolved.error || !resolved.model) {
			...
			process.stderr.write(
				`${chalk.yellow(`警告:已禁用 prewalk — ${resolved.error ?? `未找到模型 “${target}”`}`)}\n`,
			);
		} else if (!modelRegistry.hasConfiguredAuth(resolved.model)) {
			process.stderr.write(
				`${chalk.yellow(`警告:已禁用 prewalk — 没有 ${resolved.model.provider}/${resolved.model.id} 的 API key`)}\n`,
			);
		} else {
	...
	if (parsed.planYoloInto !== undefined && !parsed.planYolo) {
		throw new Error("--plan-yolo-into 需要 --plan-yolo");
	}
	if (parsed.planYolo) {
		...
		if (resolved.error || !resolved.model) {
			throw new Error(resolved.error ?? `未找到模型 “${parsed.planYoloInto ?? "@smol"}”`);
		}
		if (!modelRegistry.hasConfiguredAuth(resolved.model)) {
			throw new Error(`没有 ${resolved.model.provider}/${resolved.model.id} 的 API key`);
		}
...
export async function runRootCommand(
	...
	if (parsedArgs.export) {
		...
		try {
			...
			const message = error instanceof Error ? error.message : "会话导出失败";
			process.stderr.write(`${chalk.red(`错误:${message}`)}\n`);
			process.exit(1);
		}
		writeStartupNotice(parsedArgs, `已导出到:${result}\n`);
		process.exit(0);
	}

	if ((parsedArgs.mode === "rpc" || parsedArgs.mode === "rpc-ui") && parsedArgs.fileArgs.length > 0) {
		process.stderr.write(`${chalk.red("错误:RPC 模式不支持 @file 参数")}\n`);
		process.exit(1);
	}
	...
	try {
		...
		if (foreignSource) {
			if (isProtocolMode) {
				throw new SessionResolutionError(`--from-${foreignSource} 在 ${mode} 模式下不受支持`);
			}
			...
			try {
				...
				throw new SessionResolutionError(`列出 ${sourceName} 会话失败:${message}`);
			}
			if (foreignSessions.length === 0) {
				writeStartupNotice(parsedArgs, `${chalk.dim(`未找到 ${sourceName} 会话`)}\n`);
				stopStartupWatchdog();
				process.exit(0);
			}
			...
			if (!selected) {
				writeStartupNotice(parsedArgs, `${chalk.dim(`未选择 ${sourceName} 会话`)}\n`);
				stopStartupWatchdog();
				process.exit(0);
			}
			...
			if (!foreignSession) {
				throw new SessionResolutionError(`所选 ${sourceName} 会话已不可用`);
			}
			try {
				...
				const message = error instanceof Error ? error.message : String(error);
				throw new SessionResolutionError(`导入 ${sourceName} 会话失败:${message}`);
			}
		} else {
			sessionManager = await logger.time(
		...
		if (error instanceof SessionResolutionError) {
			process.stderr.write(`${chalk.red(`错误:${error.message}`)}\n`);
			if (error.hint) {
	...
	if (typeof parsedArgs.resume === "string" && !sessionManager) {
		writeStartupNotice(parsedArgs, `${chalk.dim("已取消恢复:会话未被移动。")}\n`);
		stopStartupWatchdog();
		process.exit(0);
	}
	...
	if (parsedArgs.resume === true && !parsedArgs.fork) {
		...
		if (folderSessions.length === 0) {
			...
			if (preloadedAllSessions.length === 0) {
				writeStartupNotice(parsedArgs, `${chalk.dim("未找到会话")}\n`);
				stopStartupWatchdog();
				process.exit(0);
			}
		}
		...
		resumeStartupWatchdog();
		if (!selected) {
			writeStartupNotice(parsedArgs, `${chalk.dim("未选择会话")}\n`);
	...
	if (mode === "acp") {
		...
		if (!isInteractive && !session.model) {
			...
			if (modelFallbackMessage) {
				...
				process.stderr.write(`${chalk.red("没有可用模型。")}\n`);
			}
			process.stderr.write(`${chalk.yellow("\n设置 API key 环境变量:")}\n`);
			process.stderr.write("  ANTHROPIC_API_KEY、OPENAI_API_KEY、GEMINI_API_KEY 等\n");
			process.stderr.write(`${chalk.yellow(`\n或创建 ${ModelsConfigFile.path()}`)}\n`);
			process.exit(1);
		}
