...
function applyCharacterMatch(
	...
	if (!matchOutcome.match) {
		...
		throw new ApplyPatchError(`在 ${path} 中找不到预期的行:\n${oldText}`);
	}
	...
	if (matchOutcome.dominantFuzzy && matchOutcome.match) {
		...
		warnings.push(
			`已选择 ${path} 中第 ${matchOutcome.match.startLine} 行附近的主导模糊匹配(相似度 ${similarity}%)。`,
		);
	}
...
async function readExistingPatchFile(fileSystem: FileSystem, absolutePath: string, path: string): Promise<string> {
	try {
		...
		if (isEnoent(error) || (error instanceof Error && error.message.startsWith("找不到文件:"))) {
			throw new ApplyPatchError(`找不到文件: ${path}`);
		}
		throw error;
	}
}
	...
	for (let j = 0; j < pattern.length; j++) {
		...
		for (const part of discardedParts) {
			...
			if (!newLinesNorm.includes(part)) {
				throw new ApplyPatchError(
					`拒绝 ${path} 第 ${matchStartIndex + j + 1} 行的部分行匹配:` +
						`文件行还包含 ${JSON.stringify(part)},替换将静默丢弃该内容。` +
						`请在区块中提供完整的行。`,
				);
			}
		}
	...
	for (const hunk of hunks) {
		...
		if (hunk.changeContext !== undefined) {
			...
			if (idx === undefined || (result.matchCount !== undefined && result.matchCount > 1)) {
				...
				if (fallback !== undefined) {
					...
					const strategyHint = result.strategy ? ` 匹配策略: ${result.strategy}。` : "";
					const previewText = previews ? `\n\n${previews}` : "";
					throw new ApplyPatchError(
						`在 ${path} 中找到 ${result.matchCount} 处与上下文 '${displayContext}' 的匹配。${strategyHint}` +
							`${previewText}\n\n添加更多周围上下文或额外的 @@ 锚点以使其唯一。`,
					);
				} else {
					const displayContext = hunk.changeContext.includes("\n")
						? hunk.changeContext.split("\n").join(" > ")
						: hunk.changeContext;
					throw new ApplyPatchError(`在 ${path} 中找不到上下文 '${displayContext}'`);
				}
			} else {
			...
			if (hunk.changeContext !== undefined) {
				...
				if (lineHintForInsertion !== undefined) {
					...
					if (lineHintForInsertion > originalLines.length + 1) {
						throw new ApplyPatchError(
							`插入 ${path} 时的行提示 ${lineHintForInsertion} 超出范围(文件共 ${originalLines.length} 行)`,
						);
					}
		...
		if (searchResult.index === undefined) {
			if (searchResult.matchCount !== undefined && searchResult.matchCount > 1) {
				...
				const strategyHint = searchResult.strategy ? ` 匹配策略: ${searchResult.strategy}。` : "";
				const previewText = previews ? `\n\n${previews}` : "";
				throw new ApplyPatchError(
			...
			if (closest.index !== undefined && closest.confidence > 0) {
				...
				throw new ApplyPatchError(
					`在 ${path} 中找不到预期的行:\n${hunk.oldLines.join("\n")}\n\n` +
						`最接近的匹配(相似度 ${similarity}%)位于第 ${closest.index + 1} 行附近:\n${preview}`,
				);
			}
			throw new ApplyPatchError(`在 ${path} 中找不到预期的行:\n${hunk.oldLines.join("\n")}`);
		}
		...
		if (searchResult.strategy === "fuzzy-dominant") {
			...
			warnings.push(`已选择 ${path} 中第 ${found + 1} 行附近的主导模糊匹配(相似度 ${similarity}%)。`);
		} else if (
		...
		if (searchResult.matchCount !== undefined && searchResult.matchCount > 1) {
			...
			const previewText = previews ? `\n\n${previews}` : "";
			throw new ApplyPatchError(
				`在 ${path} 中找到 ${searchResult.matchCount} 处与文本的匹配。${strategyHint}` +
					`${previewText}\n\n添加更多周围上下文或额外的 @@ 锚点以使其唯一。`,
			);
		}
		...
		if (next.startIndex < prevEnd) {
			const formatRange = (replacement: Replacement): string => {
				if (replacement.oldLen === 0) {
					return `${replacement.startIndex + 1} (插入)`;
				}
			...
			throw new ApplyPatchError(
				`检测到 ${path} 中第 ${prevRange} 行和第 ${nextRange} 行处的区块重叠。` +
					`请拆分区块或添加更多上下文以避免重叠。`,
			);
		}
	}
...
async function applyNormalizedPatch(input: PatchInput, options: ApplyPatchOptions): Promise<ApplyPatchResult> {
	...
	if (input.rename) {
		...
		if (destPath === absolutePath) {
			throw new ApplyPatchError("重命名路径与源路径相同");
		}
		...
		if (await fs.exists(destPath)) {
			throw new ApplyPatchError(`无法将 ${input.path} 重命名为 ${input.rename}:目标已存在。`);
		}
	}
	...
	if (op === "create") {
		if (!input.diff) {
			throw new ApplyPatchError("创建操作需要 diff(文件内容)");
		}
	...
	if (!input.diff) {
		throw new ApplyPatchError("更新操作需要 diff(区块)");
	}
	...
	if (hunks.length === 0) {
		throw new ApplyPatchError("差异不包含任何区块");
	}
...
function mergeDiagnosticsWithWarnings(
	...
	const warningMessages = warnings.map(warning => `补丁: ${warning}`);
	if (!diagnostics) {
		return {
			server: "patch",
			messages: warningMessages,
			summary: `补丁警告: ${warnings.length}`,
			errored: false,
		};
	}
	return {
		...
		summary: `${diagnostics.summary};补丁警告: ${warnings.length}`,
	};
}
...
export async function executePatchSingle(
	...
	if (
		...
		if (unchanged) {
			throw new ToolError(`编辑看似成功,但磁盘上的文件内容未发生更改: ${path}`, {
				path: resolvedPath,
			});
		}
	}
	...
	switch (result.change.type) {
		...
		case "delete":
			...
			break;
		case "update":
			resultText = effectiveRename ? `已更新并将 ${path} 移动到 ${effectiveRename}` : `已更新 ${path}`;
			break;
	}
