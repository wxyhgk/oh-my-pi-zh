	...
	static formatMessage(
		...
		if (!closest) {
			return options.allowFuzzy
				? `在 ${path} 中找不到足够接近的匹配。`
				: `在 ${path} 中找不到完全匹配的文本。旧文本必须完全匹配,包括所有空白和换行符。`;
		}
		...
		const hint = options.allowFuzzy
			? options.fuzzyMatches && options.fuzzyMatches > 1
				? `找到 ${options.fuzzyMatches} 处高置信度匹配。请提供更多上下文使其唯一。`
				: `最接近的匹配低于 ${thresholdPercent}% 的相似度阈值。`
...
function formatOccurrenceError(path: string, matchOutcome: MatchOutcome): string {
	...
	const moreMsg =
		matchOutcome.occurrences && matchOutcome.occurrences > MAX_RECORDED_MATCHES
			? `(仅显示前 ${MAX_RECORDED_MATCHES} 处,共 ${matchOutcome.occurrences} 处)`
			: "";
	return `在 ${path} 中找到 ${matchOutcome.occurrences} 处匹配${moreMsg}:\n\n${previews}\n\n添加更多上下文行以消除歧义。`;
}
...
export async function executeReplace(
	...
	if (normalizedContent === result.content) {
		throw new Error(`对 ${path} 的编辑未产生任何更改。`);
	}
