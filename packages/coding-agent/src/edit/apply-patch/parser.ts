	...
	while (remaining.length > 0) {
		...
		if (firstLine.startsWith(UPDATE_FILE_MARKER)) {
			...
			if (diffLines.length === 0) {
				...
				throw new ParseError(`路径 '${path}' 的更新文件区块为空`, lineNumber);
			}
		...
		throw new ParseError(
			`'${firstLine}' 不是有效的区块头。有效的区块头:'*** Add File: {path}'、'*** Delete File: {path}'、'*** Update File: {path}'`,
			lineNumber,
		);
	}
