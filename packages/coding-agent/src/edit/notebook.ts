...
function validateNotebook(value: unknown, displayPath: string): NotebookDocument {
	if (!isRecord(value)) {
		throw new Error(`notebook 结构无效(应为对象): ${displayPath}`);
	}
	if (!Array.isArray(value.cells)) {
		throw new Error(`notebook 结构无效(缺少 cells 数组): ${displayPath}`);
	}
	for (let index = 0; index < value.cells.length; index++) {
		const cell = value.cells[index];
		if (!isRecord(cell) || !isCellType(cell.cell_type)) {
			throw new Error(`${displayPath} 中的 notebook 单元格 ${index} 无效`);
		}
	}
...
export async function readNotebookDocument(absolutePath: string, displayPath: string): Promise<NotebookDocument> {
	try {
		...
		if (isEnoent(error)) throw new Error(`找不到文件: ${displayPath}`);
		if (error instanceof SyntaxError) throw new Error(`notebook 中的 JSON 无效: ${displayPath}`);
		throw error;
	}
}
...
function parseNotebookEditableText(text: string, displayPath: string): ParsedVirtualCell[] {
	...
	for (const line of lines) {
		...
		if (!current) {
			throw new Error(
				`用于 ${displayPath} 的 notebook 可编辑表示无效:第一行应为 "# %% [code] cell:0"、"# %% [markdown] cell:0" 或 "# %% [raw] cell:0"。`,
			);
		}
...
export async function serializeEditedNotebookText(
	...
	try {
		...
		if (error instanceof Error && error.message === `找不到文件: ${displayPath}`) {
