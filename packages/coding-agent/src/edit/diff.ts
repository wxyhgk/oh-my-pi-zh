...
export class ParseError extends Error {
	constructor(
		...
		super(lineNumber !== undefined ? `第 ${lineNumber} 行: ${message}` : message);
...
function parseOneHunk(lines: string[], lineNumber: number, allowMissingContext: boolean): ParseHunkResult {
	if (lines.length === 0) {
		throw new ParseError("差异不包含任何行", lineNumber);
	}
	...
	if (isHeaderLine && (headerTrimmed === EMPTY_CHANGE_CONTEXT_MARKER || isEmptyContextMarker)) {
		...
		if (unifiedHeader.oldStartLine < 1 || unifiedHeader.newStartLine < 1) {
			throw new ParseError("@@ 头中的行号必须 >= 1", lineNumber);
		}
		...
		if (lineHintMatch) {
			...
			if (oldStartLine < 1) {
				throw new ParseError("行提示必须 >= 1", lineNumber);
			}
		...
		if (!allowMissingContext) {
			throw new ParseError(`预期区块以 @@ 上下文标记开头,实际为:'${lines[0]}'`, lineNumber);
		}
		startIndex = 0;
	}

	if (oldStartLine !== undefined && oldStartLine < 1) {
		throw new ParseError(`行号必须 >= 1(实际为 ${oldStartLine})`, lineNumber);
	}
	if (newStartLine !== undefined && newStartLine < 1) {
		throw new ParseError(`行号必须 >= 1(实际为 ${newStartLine})`, lineNumber);
	}
	...
	if (startIndex >= lines.length) {
		throw new ParseError("区块不包含任何行", lineNumber + 1);
	}
		...
		if (firstChar === undefined || firstChar === "") {
			...
			if (parsedLines === 0) {
				throw new ParseError(
					`区块中出现意外行:'${line}'。行必须以 ' '(上下文)、'+'(新增)或 '-'(删除)开头`,
					lineNumber + 1,
				);
			}
			break;
	...
	if (parsedLines === 0) {
		throw new ParseError("区块不包含任何行", lineNumber + startIndex);
	}
...
export async function computeEditDiff(
	...
	try {
		...
		try {
			...
			return { error: message || `无法读取 ${path}` };
		}
		...
		if (normalizedContent === result.content) {
			return {
				error: `对 ${path} 不会产生任何更改。替换产生的内容与原文相同。`,
			};
		}
