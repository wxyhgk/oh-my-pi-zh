...
import { Filesystem, NotFoundError, type PreflightWriteOptions, type WriteResult } from "@wxyhgk/hashline";
...
export class HashlineFilesystem extends Filesystem {
	...
	async readText(relativePath: string): Promise<string> {
		...
		try {
			...
			if (error instanceof Error && error.message === `找不到文件: ${relativePath}`) {
