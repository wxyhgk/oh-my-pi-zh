...
async function readSectionText(absolutePath: string, sectionPath: string): Promise<string> {
	try {
		...
		throw new Error(message || `无法读取 ${sectionPath}`);
	}
}
...
function buildStreamingSectionDiff(
	...
	if (resolved.length === 0) {
		...
		return { error: `对 ${section.path} 不会产生任何更改。` };
	}
	...
	if (rows.length === 0) return { error: `对 ${section.path} 不会产生任何更改。` };
...
export async function computeHashlineDiff(
	...
	if (patch.sections.length !== 1) {
		return { error: "流式差异预览仅支持单个 hashline 区块。" };
	}
