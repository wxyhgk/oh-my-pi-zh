import { MismatchError as HashlineMismatchError } from "@wxyhgk/hashline";
import hashlineGrammar from "@wxyhgk/hashline/grammar.lark" with { type: "text" };
import hashlineDescription from "@wxyhgk/hashline/prompt.md" with { type: "text" };
...
import { isEnoent, isEnotdir, prompt } from "@wxyhgk/pi-utils";
...
function resolveConfiguredEditMode(rawEditMode: string): EditMode | undefined {
	...
	if (!editMode) {
		throw new Error(`无效的 PI_EDIT_VARIANT: ${rawEditMode}`);
	}
...
function resolveAllowFuzzy(session: ToolSession, rawValue: string): boolean {
	switch (rawValue) {
		...
		default:
			throw new Error(`无效的 PI_EDIT_FUZZY: ${rawValue}`);
	}
}
...
function resolveFuzzyThreshold(session: ToolSession, rawValue: string): number {
	...
	if (Number.isNaN(threshold) || threshold < 0 || threshold > 1) {
		throw new Error(`无效的 PI_EDIT_FUZZY_THRESHOLD: ${rawValue}`);
	}
...
async function executeApplyPatchPerFile(
	...
	for (let i = 0; i < fileEntries.length; i++) {
		...
		try {
			...
			contentTexts.push(`编辑 ${path} 出错: ${errorText}`);
			hasError = true;
			...
			if (i > 0) {
				const appliedPaths = fileEntries
					...
					.map(e => e.path)
					.join(", ");
				contentTexts.push(`已应用的文件: ${appliedPaths}。`);
			}
...
async function executeSinglePathEntries(
	...
	for (let i = 0; i < runs.length; i++) {
		...
		try {
			...
			const errorText = err instanceof Error ? err.message : String(err);
			contentTexts.push(`编辑 ${path} 出错(第 ${i + 1}/${runs.length} 项): ${errorText}`);
			if (i > 0) {
				contentTexts.push(i === 1 ? `第 1 项已应用。` : `第 1-${i} 项已应用。`);
			}
			if (i + 1 < runs.length) {
				contentTexts.push(
					(i + 2 === runs.length
...
function extractApprovalPath(args: unknown): string {
	...
	return typeof targetPath === "string" && targetPath.length > 0 ? targetPath : "(未知)";
}

export class EditTool implements AgentTool<TInput> {
	readonly approval = (args: unknown) => {
		const targetPath = extractApprovalPath(args);
		return targetPath !== "(未知)" && isInternalUrlPath(targetPath) ? "read" : "write";
	};
	readonly formatApprovalDetails = (args: unknown): string[] => [
		`文件: ${truncateForPrompt(extractApprovalPath(args))}`,
	];
	readonly name = "edit";
	readonly label = "编辑";
		...
		return {
			patch: {
				...
				examples: [
					...
					{
						caption: "多个条目",
						note: "一次调用中的所有条目都应用于顶层 `path`;不同文件请使用单独调用。",
					},
