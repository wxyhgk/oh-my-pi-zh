...
import { HL_FILE_PREFIX, HL_FILE_SUFFIX, HL_MOVE_KEYWORD, HL_REM_KEYWORD } from "@wxyhgk/hashline";
import type { Component } from "@wxyhgk/pi-tui";
import { sliceWithWidth, visibleWidth, wrapTextWithAnsi } from "@wxyhgk/pi-tui";
...
function getOperationTitle(op: Operation | undefined): string {
	return op === "create" ? "创建" : op === "delete" ? "删除" : "编辑";
}
...
function renderPlainTextPreview(text: string, uiTheme: Theme, _filePath?: string): string {
	...
	if (previewLines.length > CALL_TEXT_PREVIEW_LINES) {
		preview += uiTheme.fg("dim", `… 还有 ${previewLines.length - CALL_TEXT_PREVIEW_LINES} 行`);
	}
...
function formatStreamingDiff(
	...
	let text = cachedRenderedString(cache, uiTheme, expanded, `${rawPath}:${innerWidth}:${budget}`, diff, () => {
		...
		if (hiddenLines > 0) {
			...
			if (hiddenHunks > 0) remainder.push(`${hiddenHunks} 个区块`);
			remainder.push(`${hiddenLines} 行`);
			rendered += `${uiTheme.fg("dim", `… (上方 ${remainder.join(", ")})`)}\n`;
		}
	...
	const hideLabel = expanded && label === "预览";
...
function formatMultiFileStreamingDiff(
	...
	for (let index = 0; index < previews.length; index++) {
		...
		if (preview.diff) {
			...
			parts.push(
				`${header}${formatStreamingDiff(preview.diff, preview.path, width, uiTheme, expanded, "预览", isLast ? spinnerFrame : undefined, cache)}`,
			);
		}
	}
	return parts.join("");
...
function getCallPreview(
	...
	if (args.previewDiff) {
		return formatStreamingDiff(args.previewDiff, rawPath, width, uiTheme, expanded, "预览", spinnerFrame, cache);
	}
	if (args.diff && args.op) {
		return formatStreamingDiff(args.diff, rawPath, width, uiTheme, expanded, "流式", spinnerFrame, cache);
	}
	if (args.diff) {
...
const MISSING_APPLY_PATCH_END_ERROR = "补丁的最后一行必须是 '*** End Patch'";
...
function renderDiffSection(
	...
	return cachedRenderedString(cache, uiTheme, expanded, rawPath, diff, () => {
		...
		if (!expanded && (hiddenHunks > 0 || hiddenLines > 0)) {
			const remainder: string[] = [];
			if (hiddenHunks > 0) remainder.push(`${hiddenHunks} 个区块`);
			if (hiddenLines > 0) remainder.push(`${hiddenLines} 行`);
	...
	renderCall(
		...
		return framedBlock(uiTheme, width => {
			...
			const header = renderEditHeader(width, uiTheme, {
				...
				rawPath,
				rename,
				extraSuffix: fileCount > 1 ? uiTheme.fg("dim", ` (+${fileCount - 1} 个文件)`) : undefined,
			});
	...
	return framedBlock(uiTheme, width => {
		...
		if (isError) {
			...
			if (op !== "delete" && op !== "create" && !rename) {
				...
				body = noChangePath
					? uiTheme.fg("dim", `未对 ${noChangePath} 产生任何更改。`)
					: uiTheme.fg("dim", "未产生任何更改。");
			}
					...
					renderStatusLine(
						{
							...
							title: "编辑",
							description: uiTheme.fg("dim", `还有 ${remaining} 个文件待处理…`),
						},
						uiTheme,
					),
				);
