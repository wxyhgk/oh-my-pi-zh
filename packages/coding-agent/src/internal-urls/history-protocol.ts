...
export class HistoryProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl): Promise<InternalResource> {
		...
		if (!ref) {
			...
			const knownStr = known.length > 0 ? known.join(", ") : "无";
		...
		if (ref.session) {
			...
			notes.push(`来源:会话文件(只读,${ref.status})`);
		} else {
			...
			if (disk) return { ...disk, url: url.href };
			throw new Error(`Agent ${ref.id} 没有记录:会话已消失且未保留会话文件`);
		}
	...
	async #resolveFromDisk(agentId: string): Promise<InternalResource | undefined> {
		...
		const content = formatSessionHistoryMarkdown(messages, { title: `${matchedId}(磁盘)` });
		return {
			url: "",
			content,
			contentType: "text/markdown",
	...
	async #renderIndex(refs: AgentRef[]): Promise<string> {
		...
		for (const id of disk.keys()) {
			...
			entries.push({ id, status: "磁盘", kind: "—", parent: "—", lastActivity: "—" });
		}

		const lines: string[] = ["# Agent 列表", ""];
		if (entries.length === 0) {
			lines.push("尚未注册任何 Agent。");
			return `${lines.join("\n")}\n`;
		}
		lines.push("| id | 状态 | 类型 | 父级 | 最后活动 |", "|---|---|---|---|---|");
		for (const entry of entries) {
			lines.push(`| ${entry.id} | ${entry.status} | ${entry.kind} | ${entry.parent} | ${entry.lastActivity} |`);
		}
		lines.push("", "使用 `read history://<id>` 读取记录。");
		return `${lines.join("\n")}\n`;
	}
	...
	async complete(): Promise<UrlCompletion[]> {
		...
		for (const ref of AgentRegistry.global().list()) {
			...
			completions.push({
				value: ref.id,
				description: `${ref.status} · ${ref.kind}${ref.parentId ? ` · 父级 ${ref.parentId}` : ""}`,
			});
		}
		const disk = await sessionFilesFromDisk();
		for (const id of disk.keys()) {
			...
			seen.add(id);
			completions.push({ value: id, description: "磁盘" });
		}
		return completions;
	}
}
