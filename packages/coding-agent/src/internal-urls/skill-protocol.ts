...
export function validateRelativePath(relativePath: string): void {
	if (path.isAbsolute(relativePath)) {
		throw new Error("skill:// URL 不允许使用绝对路径");
	}
	...
	if (
		...
		throw new Error("skill:// URL 不允许路径穿越 (..)");
	}
}
...
export class SkillProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl, context?: ResolveContext): Promise<InternalResource> {
		...
		if (!skillName) {
			throw new Error("skill:// URL 需要技能名称:skill://<name>");
		}
		...
		if (!stats.isFile()) {
			throw new Error(`skill:// URL 必须解析为文件或目录:${url.href}`);
		}
