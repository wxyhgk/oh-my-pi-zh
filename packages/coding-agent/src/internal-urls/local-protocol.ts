...
function ensureWithinRoot(targetPath: string, rootPath: string): void {
	if (targetPath !== rootPath && !targetPath.startsWith(`${rootPath}${path.sep}`)) {
		throw new Error("local:// URL 越出了本地根目录");
	}
}
...
function buildNonTextLocalResource(url: InternalUrl, filePath: string, size: number, reason: string): InternalResource {
	const content = `[无法读取二进制 local:// 文件 '${url.href}'(${formatLocalByteSize(size)}):${reason}。该资源不是文本。请改用元数据/关键帧/视频专用工作流。]`;
	return {
		url: url.href,
		content,
...
function buildLargeLocalTextResource(url: InternalUrl, filePath: string, size: number): InternalResource {
	const content = `[无法将 local:// 文件 '${url.href}' 物化为内部文本资源(${formatLocalByteSize(size)} 超过 ${formatLocalByteSize(LOCAL_TEXT_RESOURCE_MAX_BYTES)})。请使用 read 工具的文件系统路径处理或行选择器,以便在文件大小保护下流式读取内容。]`;
	return {
		url: url.href,
		content,
		contentType: "text/plain",
...
async function buildFileResource(
	...
	if (BINARY_FILE_EXTENSIONS.has(path.extname(resolved.path).toLowerCase())) {
		return buildNonTextLocalResource(url, resolved.path, resolved.size, "扩展名属于已知的二进制/容器类型");
	}
	...
	if (!isUtf8Text(sniffBytes)) {
		return buildNonTextLocalResource(url, resolved.path, resolved.size, "内容不是有效的 UTF-8 文本");
	}
...
async function buildListing(url: InternalUrl, localRoot: string): Promise<InternalResource> {
	...
	const listing = files.length === 0 ? "(空)" : files.map(file => `- [${file}](local://${file})`).join("\n");
	const content =
		`# 本地\n\n` +
		`会话级临时空间,用于大型中间数据、子 Agent 交接与可复用的规划产物。\n\n` +
		`根目录:${localRoot}\n\n` +
		`可用文件 ${files.length} 个:\n\n` +
		`${listing}\n`;
...
function extractRelativePath(url: InternalUrl): string {
	...
	try {
		...
		throw new Error(`local:// 路径中的 URL 编码无效:${url.href}`);
	}
	try {
...
const LOCAL_WRITE_NOTE = "使用写入路径 local://<file> 可跨轮次保留大型中间产物。";
...
async function resolveLocalTarget(url: InternalUrl, opts: LocalProtocolOptions): Promise<ResolvedLocalTarget> {
	...
	try {
		...
		if (isEnoent(error)) {
			throw new Error("无法初始化 local:// 根目录");
		}
		throw error;
	}
	...
	try {
		...
		if (isEnoent(error)) {
			throw new Error(`未找到本地文件:${url.href}`);
		}
		throw error;
	}
	...
	if (!stat.isFile()) {
		throw new Error(`local:// URL 必须解析为文件或目录:${url.href}`);
	}
