...
} from "@wxyhgk/pi-natives/clipboard";
import * as logger from "@wxyhgk/pi-utils/logger";
import { SUPPORTED_IMAGE_MIME_TYPES } from "@wxyhgk/pi-utils/mime";
...
async function spawnCapture(
	...
	try {
		...
		await proc.exited;
		if (timedOut) {
			throw new Error(`${cmd[0]} 超时(超过 ${timeoutMs}ms)`);
		}
		if (proc.exitCode !== 0) {
			throw new Error(`${cmd[0]} 以退出码 ${proc.exitCode} 退出`);
		}
		return stdout;
	} finally {
		clearTimeout(timer);
