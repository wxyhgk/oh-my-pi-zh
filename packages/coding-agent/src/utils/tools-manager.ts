...
import { $which, APP_NAME, getToolsDir, logger, ptree, TempDir } from "@wxyhgk/pi-utils";
...
async function downloadTool(tool: ToolName, signal?: AbortSignal): Promise<string> {
	...
	try {
		...
		try {
			...
			throw new Error(`解压 ${assetName} 失败: ${err instanceof Error ? err.message : String(err)}`);
		}
		...
		if (fs.existsSync(extractedBinary)) {
			...
			throw new Error(`归档中未找到二进制文件: ${extractedBinary}`);
		}
