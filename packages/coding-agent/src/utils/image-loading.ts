...
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@wxyhgk/pi-utils";
...
export class ImageInputTooLargeError extends Error {
	...
	constructor(bytes: number, maxBytes: number) {
		super(`图片文件过大: ${formatBytes(bytes)} 超过 ${formatBytes(maxBytes)} 的限制。`);
...
export async function loadImageInput(options: LoadImageInputOptions): Promise<LoadedImageInput | null> {
	...
	let textNote = `已读取图片文件 [${outputMimeType}]`;
	if (dimensionNote) {
		textNote += `\n${dimensionNote}`;
	}
...
export async function loadImageAttachmentInput(
	...
	let textNote = `已读取图片附件 ${options.label} [${outputMimeType}]`;
	if (dimensionNote) {
		textNote += `\n${dimensionNote}`;
	}
