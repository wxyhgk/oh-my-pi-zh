...
export function renderSampleProfile(text: string): string | null {
	...
	const out: string[] = [];
	out.push(`macOS 采样分析: ${header.process}(pid ${header.pid}),每 ${header.intervalMs} ms 采样一次`);
	const meta: string[] = [];
	...
	let statLine = `时长: 约 ${durationSec.toFixed(1)} s(${maxSamples} 个样本/线程)`;
	if (header.footprint) {
		statLine += ` | 内存占用: ${header.footprint}${header.footprintPeak ? `(峰值 ${header.footprintPeak})` : ""}`;
	}
	out.push(statLine);
	out.push("");
	out.push(
		`进程总计: ${threads.length} 个线程中共 ${processCpu} 个 on-CPU 样本。下方计数与百分比均为 on-CPU 样本(不含阻塞时间)。`,
	);
	...
	for (const { thread, roots, cpu } of active) {
		...
		out.push(`## ${title} — ${thread.total} 个样本,${cpu} on-CPU(${formatPct(cpu, thread.total)})`);
		...
		if (kept.length === 0) out.push(`  (没有超过 ${ctx.minValue} 个 on-CPU 样本的调用路径)`);
	}

	if (idle.length > 0) {
		out.push("");
		out.push(`## 空闲/可忽略的线程(${idle.length})`);
		for (const { thread, cpu } of idle) {
			const title = thread.name ? `${thread.name} (Thread_${thread.id})` : `Thread_${thread.id}`;
			const wait = dominantWait(thread);
			const state = wait ? `阻塞于 ${wait}(${cpu} on-CPU)` : `${cpu}/${thread.total} 个 on-CPU 样本`;
	...
	out.push("");
	out.push("[macOS `sample` 调用树报告的摘要视图。使用 ':raw' 读取原始文件。]");
	return out.join("\n");
}
