...
export function renderCpuProfile(text: string): string | null {
	...
	const out: string[] = [];
	let header = `V8 CPU 分析: ${(duration / 1e6).toFixed(2)} s 墙钟时间`;
	if (sampleCount > 0) header += `,${sampleCount} 个样本(平均间隔 ${Math.round(avgInterval)} µs)`;
	out.push(header);
	out.push(
		`CPU 总计: ${(totalCpu / 1e6).toFixed(2)} s(占墙钟时间 ${formatPct(totalCpu, duration)})。下方数值为 CPU 毫秒数(不含空闲时间)。`,
	);
	...
	if (kept.length === 0) out.push(`  (没有超过 ${formatMs(ctx.minValue)} ms 的 CPU 调用路径)`);
	...
	out.push("");
	out.push("[V8 .cpuprofile 的摘要视图。使用 ':raw' 读取原始 JSON。]");
	return out.join("\n");
}
