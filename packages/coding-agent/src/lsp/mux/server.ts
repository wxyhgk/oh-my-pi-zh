...
import { isRecord, logger, postmortem, ptree, setProcessName } from "@wxyhgk/pi-utils";
...
export class LspMuxServer {
	...
	async listen(endpoint: string): Promise<void> {
		if (this.#netServer) throw new Error("LSP mux 已在监听");
	...
	async #clearStaleSocket(endpoint: string): Promise<void> {
		...
		if (alive) throw new Error(`LSP mux 已在 ${endpoint} 监听`);
		try {
			await fs.unlink(endpoint);
		} catch (error) {
			logger.warn("无法移除过期的 LSP mux 套接字", { endpoint, error: String(error) });
			throw error;
		}
	}
	...
	#accept(socket: net.Socket): void {
		...
		socket.on("data", chunk => {
			...
			for (const text of session.framer.drain(header => {
				logger.warn("LSP mux 客户端帧同步重置", { header: header.slice(0, 200) });
			})) {
				try {
					const parsed: unknown = JSON.parse(text);
					if (!isRecord(parsed) || parsed.jsonrpc !== "2.0") throw new Error("无效的 JSON-RPC 消息");
					void this.#fromSession(session, parsed as unknown as RpcMessage).catch(error => {
						logger.warn("LSP mux 客户端消息处理失败", { error: String(error) });
					});
				} catch (error) {
					logger.warn("LSP mux 客户端发送了格式错误的 JSON", { error: String(error) });
				}
			}
		});
		socket.on("error", error => logger.warn("LSP mux 会话套接字错误", { error: error.message }));
		...
		try {
			while (true) {
				...
				for (const text of framer.drain(header => {
					logger.warn("LSP mux 服务器帧同步重置", { server: server.key, header: header.slice(0, 200) });
				})) {
					try {
						const parsed: unknown = JSON.parse(text);
						if (!isRecord(parsed) || parsed.jsonrpc !== "2.0") throw new Error("无效的 JSON-RPC 消息");
						...
						logger.warn("LSP mux 服务器消息处理失败", { server: server.key, error: String(error) });
					}
				}
			}
		} catch (error) {
			logger.warn("LSP mux 服务器读取器失败", { server: server.key, error: String(error) });
		} finally {
			reader.releaseLock();
		}
	}
	...
	#writeServer(server: ServerInstance, message: RpcMessage): Promise<void> {
		...
		server.writeQueue = write.catch(error => {
			logger.warn("LSP mux 服务器写入失败", { server: server.key, error: String(error) });
		});
		return write;
	}
	...
	async #stopServer(server: ServerInstance): Promise<void> {
		...
		try {
			...
			logger.warn("LSP mux 服务器优雅关闭失败", { server: server.key, error: String(error) });
		} finally {
			this.#killServer(server);
		}
	}
...
export async function startLspMuxFromEnvironment(): Promise<void> {
	...
	if (!endpoint || !projectDir) throw new Error("LSP mux 环境不完整");
