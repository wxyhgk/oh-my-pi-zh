...
export function sortAndValidateTextEdits(edits: TextEdit[]): TextEdit[] {
	...
	for (let i = 0; i < unique.length - 1; i++) {
		...
		if (comparePosition(earlier.end, later.start) > 0) {
			throw new ToolError(
				`LSP 编辑范围重叠:${formatRange(earlier)} 与 ${formatRange(later)} 冲突;LSP 返回了不一致的编辑`,
			);
		}
	}
	...
	if (edit.documentChanges) {
		...
		for (const op of ops) {
			if (op.kind === "text") {
				...
				applied.push(`已对 ${formatPathRelativeToCwd(filePath, cwd)} 应用 ${op.edits.length} 处编辑`);
			} else if (op.kind === "create") {
				const filePath = uriToFile(op.uri);
				await Bun.write(filePath, "");
				applied.push(`已创建 ${formatPathRelativeToCwd(filePath, cwd)}`);
				...
				applied.push(`已删除 ${formatPathRelativeToCwd(filePath, cwd)}`);
			}
		}
		...
		for (const uri in changes) {
			...
			applied.push(`已对 ${formatPathRelativeToCwd(filePath, cwd)} 应用 ${textEdits.length} 处编辑`);
		}
	}
