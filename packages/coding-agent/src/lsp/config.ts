...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@wxyhgk/pi-utils";
...
function normalizeServerConfig(name: string, config: RawServerConfig): ServerConfig | null {
	...
	if (!command || !fileTypes || !rootMarkers) {
		logger.warn("忽略无效的 LSP 服务器配置(缺少必填字段)。", { name });
		return null;
	}
	...
	for (const [name, config] of Object.entries(overrides)) {
		if (merged[name]) {
			...
			if (normalized) {
				...
				logger.warn("忽略无效的 LSP 覆盖配置(保留先前配置)。", { name });
			}
		} else {
			const normalized = normalizeServerConfig(name, config);
			if (normalized) {
		...
		if (marker.includes("*")) {
			if (entries === null) {
				try {
					...
					entries = [];
					logger.warn("无法列出用于 glob 根标记的目录。", { marker, cwd });
				}
			}
