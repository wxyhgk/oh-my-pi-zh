export { truncate } from "@wxyhgk/pi-utils";
...
export function formatDiagnosticsSummary(diagnostics: Diagnostic[]): string {
	...
	return parts.length > 0 ? parts.join(", ") : "无问题";
}
...
export function summarizeDiagnosticMessages(messages: string[]): { summary: string; errored: boolean } {
	...
	return {
		summary: parts.length > 0 ? parts.join(", ") : "无问题",
...
export function formatWorkspaceEdit(edit: WorkspaceEdit, cwd: string): string[] {
	...
	if (edit.changes) {
		for (const [uri, textEdits] of Object.entries(edit.changes)) {
			...
			results.push(`${file}:${textEdits.length} 处编辑`);
		}
	}
	...
	if (edit.documentChanges) {
		for (const change of edit.documentChanges) {
			if ("edits" in change && change.textDocument) {
				...
				results.push(`${file}:${change.edits.length} 处编辑`);
			} else if ("kind" in change) {
				switch (change.kind) {
					case "create":
						results.push(`创建:${formatPathRelativeToCwd(uriToFile(change.uri), cwd)}`);
						break;
					case "rename":
						results.push(
							`重命名:${formatPathRelativeToCwd(uriToFile(change.oldUri), cwd)} ${theme.nav.cursor} ${formatPathRelativeToCwd(uriToFile(change.newUri), cwd)}`,
						);
						break;
					case "delete":
						results.push(`删除:${formatPathRelativeToCwd(uriToFile(change.uri), cwd)}`);
						break;
				}
			}
		}
...
export function formatTextEdit(edit: TextEdit, maxLength = 50): string {
	...
	return `第 ${range} 行 ${theme.nav.cursor} "${preview}"`;
}
...
export function formatDocumentSymbol(symbol: DocumentSymbol, indent = 0): string[] {
	...
	const results = [`${prefix}${icon} ${symbol.name}${detail} @ 第 ${line} 行`];
...
export function formatCodeAction(action: CodeAction | Command, index: number): string {
	const kind = "kind" in action && action.kind ? action.kind : "操作";
	const preferred = "isPreferred" in action && action.isPreferred ? " (首选)" : "";
	const disabled = "disabled" in action && action.disabled ? ` (已禁用:${action.disabled.reason})` : "";
...
export async function resolveSymbolColumn(filePath: string, line: number, symbolSpec?: string): Promise<number> {
	...
	try {
		...
		if (fallbackIndexes.length === 0) {
			throw new Error(`在第 ${lineNumber} 行未找到符号 "${symbol}"`);
		}
		if (occurrence > fallbackIndexes.length) {
			throw new Error(
				`符号 "${symbol}" 的第 ${occurrence} 次出现超出范围(第 ${lineNumber} 行,共找到 ${fallbackIndexes.length} 次)`,
			);
		}
