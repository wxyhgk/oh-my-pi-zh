...
import { Process, type PtyRunResult, PtySession } from "@wxyhgk/pi-natives";
import { isEexist, isEnoent, logger, postmortem, procmgr, sanitizeText, setProcessName } from "@wxyhgk/pi-utils";
...
function reapRecoveredSnapshot(snapshot: DaemonSnapshot, now: number): boolean {
	...
	snapshot.exitReason = "之前的 broker 已退出";
	return true;
}
	...
	static async readFiles(
		...
		if (grep) {
			...
			try {
				...
				throw new Error(`无效的日志正则表达式: ${error instanceof Error ? error.message : String(error)}`);
			}
			text = text
				.split("\n")
...
class DaemonBroker {
	...
	#accept(socket: net.Socket): void {
		...
		socket.on("data", chunk => {
			...
			if (Buffer.byteLength(buffer, "utf8") > MAX_REQUEST_BYTES) {
				socket.destroy(new Error("守护进程 broker 请求超过大小限制"));
				return;
			}
			for (;;) {
	...
	async #handleLine(socket: net.Socket, line: string, onAuthenticated: () => void): Promise<void> {
		...
		try {
			...
			id = request.id;
			if (request.token !== this.#token) throw new Error("守护进程 broker 认证失败");
			onAuthenticated();
	...
	async #start(spec: DaemonSpec, owner?: string): Promise<DaemonRpcResult> {
		if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,47}$/.test(spec.name)) {
			throw new Error("守护进程名称必须为 1-48 个字母、数字、点、下划线或连字符");
		}
		if (spec.detached && spec.pty) {
			throw new Error("分离式守护进程无法分配 PTY");
		}
		if (
			spec.pty &&
			...
			throw new Error('Windows 批处理文件要求 application 为 "cmd.exe",并将批处理路径放在 "/c" 之后');
		}
		if (this.#startingNames.has(spec.name)) {
			throw new Error(`守护进程 ${spec.name} 正在启动中`);
		}
		...
		try {
			...
			if (existing && !terminalState(existing.snapshot.state)) {
				throw new Error(`守护进程 ${spec.name} 已处于 ${existing.snapshot.state} 状态`);
			}
			if (existing && existing.pendingCompletions.length > 0) {
				throw new Error(`守护进程 ${spec.name} 存在未确认的完成通知`);
			}
			if (spec.ready?.log) {
				try {
					new RegExp(spec.ready.log, "u");
				} catch (error) {
					throw new Error(`无效的就绪正则表达式: ${error instanceof Error ? error.message : String(error)}`);
				}
			}
			const stat = await fs.stat(spec.cwd);
			if (!stat.isDirectory()) throw new Error(`守护进程 cwd 不是目录: ${spec.cwd}`);
	...
	async #launch(record: ManagedDaemon): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			record.log?.append(`守护进程启动失败: ${message}\n`);
	...
	async #launchPty(record: ManagedDaemon, generation: number): Promise<void> {
		...
		const onChunk = (error: Error | null, chunk: string): void => {
			...
			if (error) record.log?.append(`PTY 输出错误: ${error.message}\n`);
		...
		const onStart = (error: Error | null, pid: number): void => {
			if (error) {
				record.log?.append(`PTY 启动回调失败: ${error.message}\n`);
	...
	async #onPtyExit(record: ManagedDaemon, generation: number, result: PtyRunResult): Promise<void> {
		return this.#settle(record, generation, result.exitCode, result.timedOut ? "超时" : undefined);
	}
	...
	async #settle(record: ManagedDaemon, generation: number, exitCode?: number, error?: string): Promise<void> {
		...
		if (shouldRestart && !this.#shuttingDown) {
			...
			record.log?.append(
				`\n[守护进程已退出${exitCode === undefined ? "" : `,退出码 ${exitCode}`};将在 ${delay}ms 后重启]\n`,
			);
			this.#persist(record);
	...
	async #wait(operation: Extract<DaemonOperation, { op: "wait" }>): Promise<DaemonRpcResult> {
		...
		if (operation.pattern) {
			try {
				...
				throw new Error(`无效的等待正则表达式: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
	...
	async #send(operation: Extract<DaemonOperation, { op: "send" }>): Promise<DaemonRpcResult> {
		...
		if (terminalState(record.snapshot.state) || record.snapshot.state === "stopping") {
			throw new Error(`守护进程 ${operation.name} 当前为 ${record.snapshot.state} 状态`);
		}
		if (operation.data === undefined && operation.signal === undefined) {
			throw new Error("send 操作需要 data 或 signal 参数");
		}
		if (operation.data !== undefined) {
			...
			} else throw new Error(`守护进程 ${operation.name} 的标准输入不可用`);
		}
		if (operation.signal) {
			if (process.platform === "win32" && record.pty) {
				...
				const processRef = record.snapshot.pid === undefined ? null : Process.fromPid(record.snapshot.pid);
				if (!processRef) throw new Error(`守护进程 ${operation.name} 的进程不可用`);
	...
	#record(name: string): ManagedDaemon {
		...
		throw new Error(`未知守护进程 ${name}${names.length ? `。可用: ${names.join(", ")}` : ""}`);
	}
					...
					pendingCompletions: (() => {
						if ("pendingCompletions" in decoded && Array.isArray(decoded.pendingCompletions)) {
							return decoded.pendingCompletions.map(value => {
								...
								if (!("event" in message)) throw new Error("待处理的守护进程完成通知不是事件");
								return message;
							});
						}
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!projectDir || !runtimeDir) throw new Error("守护进程 broker 环境不完整");
	...
	if (!token) throw new Error("守护进程 broker token 为空");
