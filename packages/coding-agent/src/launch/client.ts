...
import { getGlobalDaemonRuntimeDir, isEexist, isEisdir, isEnoent, logger, postmortem } from "@wxyhgk/pi-utils";
...
async function readOrCreateToken(runtimeDir: string): Promise<string> {
	...
	throw new Error(`初始化 ${runtimeDir} 中的守护进程 broker token 超时`);
}
...
function openSocket(endpoint: string, timeoutMs: number): Promise<net.Socket> {
	...
	const timer = setTimeout(() => {
		socket.destroy();
		reject(new Error(`连接 ${endpoint} 处的守护进程 broker 超时`));
	}, timeoutMs);
	const cleanup = (): void => {
		clearTimeout(timer);
...
class SocketDaemonClient implements DaemonBrokerClient {
	...
	async request(operation: DaemonOperation, signal?: AbortSignal): Promise<DaemonRpcResult> {
		if (this.#closed) throw new Error("守护进程 broker 客户端已关闭");
		if (signal?.aborted) throw new Error("守护进程 broker 请求已中止");
		await this.#connect();
		const socket = this.#socket;
		if (!socket || socket.destroyed) throw new Error("守护进程 broker 套接字不可用");
		...
		const timer = setTimeout(() => {
			...
			pending.removeAbort?.();
			reject(new Error(`守护进程 ${operation.op} 请求超时`));
		...
		if (signal) {
			const abort = (): void => {
				...
				clearTimeout(timer);
				reject(new Error("守护进程 broker 请求已中止"));
			};
	...
	close(): void {
		...
		this.#rejectPending(new Error("守护进程 broker 客户端已关闭"));
	}
	...
	async #connectOnce(): Promise<void> {
		...
		throw new Error(`启动守护进程 broker 失败: ${lastError?.message ?? "套接字不可用"}`);
	}
	...
	#bindSocket(socket: net.Socket): void {
		...
		socket.on("close", () => {
			...
			this.#rejectPending(new Error("守护进程 broker 连接已关闭"));
...
export async function smokeTestDaemonBroker(): Promise<void> {
	...
	try {
		...
		if (ping.op !== "ping" || ping.projectDir !== client.projectDir) throw new Error("守护进程 broker ping 不匹配");
