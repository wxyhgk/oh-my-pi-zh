...
function daemonState(value: unknown): DaemonState {
	...
	throw new Error(`未知守护进程状态: ${state}`);
}
...
function daemonSignal(value: unknown): DaemonSignal {
	...
	throw new Error(`未知守护进程信号: ${signal}`);
}

function readyPendingList(value: unknown): ("log" | "port")[] {
	if (!Array.isArray(value)) throw new Error("daemon.readyPending 必须是数组");
	const result: ("log" | "port")[] = [];
	for (const item of value) {
		if (item !== "log" && item !== "port") throw new Error(`未知就绪条件: ${String(item)}`);
		result.push(item);
	}
	return result;
}
...
function readySpec(value: unknown): DaemonReadySpec {
	...
	if (!log && port === undefined) throw new Error("ready 需要 log 或 port");
...
export function parseDaemonWireResponse(value: unknown): DaemonWireResponse {
	...
	throw new Error("response.ok 必须是布尔值");
}
...
function parseDaemonOperation(value: unknown): DaemonOperation {
	...
	switch (op) {
		...
		case "wait": {
			...
			if (target !== "ready" && target !== "exit") throw new Error("operation.for 必须是 ready 或 exit");
			return {
				op,
				name: stringValue(source.name, "operation.name"),
				for: target,
...
export function parseDaemonRpcResult(operation: DaemonOperation, value: unknown): DaemonRpcResult {
	...
	switch (operation.op) {
		...
		case "list": {
			if (!Array.isArray(source.daemons)) throw new Error("result.daemons 必须是数组");
