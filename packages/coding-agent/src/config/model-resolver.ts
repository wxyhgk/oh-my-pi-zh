...
import type { Api, Effort, KnownProvider, Model, ModelSpec } from "@wxyhgk/pi-ai";
import { buildModel } from "@wxyhgk/pi-catalog/build";
import { modelMatchesHost } from "@wxyhgk/pi-catalog/hosts";
import { buildModelProviderPriorityRank } from "@wxyhgk/pi-catalog/identity";
import { stripThinkingVariantToken } from "@wxyhgk/pi-catalog/identity/family";
...
import { DEFAULT_MODEL_PER_PROVIDER } from "@wxyhgk/pi-catalog/provider-models";
import { resolveBareVariantAlias, resolveVariantAlias } from "@wxyhgk/pi-catalog/variant-collapse";
import { fuzzyMatch } from "@wxyhgk/pi-tui";
...
export function resolveCliModel(options: {
	...
	if (cliProvider && !provider) {
		return {
			...
			selector: undefined,
			warning: undefined,
			error: `Unknown provider "${cliProvider}". Run "omp-zh models" to see available providers/models.`,
		};
	}
		...
		if (roleSelector) {
			...
			if (configuredPatterns && configuredPatterns.length > 0) {
				return {
					...
					thinkingLevel: undefined,
					warning: resolved.warning,
					error: `Model "${trimmedModel}" not found. Run "omp-zh models" to see available models.`,
				};
			}
		}
	}
	...
	if (!model) {
		...
		return {
			...
			thinkingLevel: undefined,
			warning,
			error: `Model "${display}" not found. Run "omp-zh models" to see available models.`,
		};
	}
