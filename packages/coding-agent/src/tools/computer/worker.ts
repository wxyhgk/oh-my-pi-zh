...
import { createDesktopSession } from "@wxyhgk/pi-natives/desktop";
import * as postmortem from "@wxyhgk/pi-utils/postmortem";
...
function replyError(payload: RunErrorPayload): Error {
	if (payload.isAbort) {
		const error = new ToolAbortError(payload.message || "工具调用已中止");
...
function guardRun(context: ComputerRunContext, method: string): void {
	if (context.readOnly) throw new ToolError(`只读运行:'${method}' 需要 read_only: false`);
...
async function captureScreenshot(
	...
	if (!options?.silent) {
		context.output.push({
			type: "text",
			text: scaled
				? `截图 ${frame.target} ${frame.width}×${frame.height} (由 ${frame.sourceWidth}×${frame.sourceHeight} 缩放) → ${destination}`
				: `截图 ${frame.target} ${frame.width}×${frame.height} → ${destination}`,
		});
		context.output.push({
			type: "image",
...
export class ComputerWorkerCore {
	...
	async #run(message: Extract<ComputerWorkerInbound, { type: "run" }>): Promise<void> {
		if (this.#closed) {
			this.#transport.send({
				...
				id: message.id,
				ok: false,
				error: errorPayload(new ToolError("computer worker 已关闭")),
			});
			return;
		}
		if (this.#active) {
			this.#transport.send({
				...
				id: message.id,
				ok: false,
				error: errorPayload(new ToolError("computer worker 正忙")),
			});
			return;
		}
		...
		try {
			...
			runtime.setRunScope({
				...
				assert: (condition: unknown, text?: string): void => {
					if (!condition) throw new ToolError(text ?? "断言失败");
				},
			...
			const onCancel = (): void => {
				...
				rejectCancel(
					timeoutSignal.aborted
						? new ToolError(`computer 代码执行超过 ${message.timeoutMs}ms 后超时`)
						: abortError,
				);
			...
			runAc.abort(postmortem.markExpectedCleanupError(new ToolAbortError("computer 运行已结束")));
	...
	#currentRunContext = (): ComputerRunContext => {
		...
		if (!context) throw new ToolError("没有活动的 computer 运行");
		return context;
	};
	...
	#createDesktopScope(session: NativeDesktopSession): object {
		...
		return {
			...
			window: async (selector: string | WindowFilter): Promise<Win> => {
				...
				if (matches.length === 0) throw new ToolError(`没有窗口匹配 ${JSON.stringify(selector)}`);
				if (matches.length > 1) {
					const candidates = matches
						.map(window => `${window.id} ${window.app} ${JSON.stringify(window.title)}`)
						.join("\n");
					throw new ToolError(`有多个窗口匹配 ${JSON.stringify(selector)}:\n${candidates}`);
				}
