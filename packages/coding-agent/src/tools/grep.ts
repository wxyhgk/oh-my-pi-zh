...
const searchPathEntry = type("string").describe(
	'要搜索的文件、目录、glob 或内部 URL,或 "<file>:<lines>" 选择器(例如 "src/foo.ts:50-100"、"src/foo.ts:50+10"、"src/foo.ts:50-100,200-300")',
);
const searchSchema = type({
	pattern: type("string").describe("正则模式"),
	"path?": searchPathEntry.describe(
		'要搜索的文件、目录、glob 或内部 URL,或 "<file>:<lines>" 选择器;多个目标用分号分隔的列表传入("src; tests")。省略时搜索工作区根目录(".")',
	),
	...
	for (const entry of rawEntries) {
		...
		if (internalSplit.sel !== undefined) {
			...
			if (!isReadSelectorGrammar(internalSplit.sel)) {
				throw new ToolError(
					`路径条目 "${entry}" 包含无效选择器 ":${internalSplit.sel}"——请使用 ":N-M" 行范围、":raw"/":conflicts"、范围加 ":raw",或将字面 ":" 百分号编码为 %3A`,
				);
			}
	...
	for (let idx = 0; idx < pathSpecs.length; idx++) {
		...
		if (!archive) {
			try {
				...
				unreadable.push(`${entry} (无法打开压缩包: ${(err as Error).message})`);
				continue;
			}
		...
		if (extracted.bytes.some(byte => byte === 0)) {
			unreadable.push(`${entry} (二进制压缩包条目)`);
			continue;
		}
		let text: string;
		try {
			...
			unreadable.push(`${entry} (非 UTF-8 压缩包条目)`);
			continue;
		}
...
function jsMatchedLineIndexes(
	...
	try {
		...
		throw new ToolError(`无效正则: ${message.replace(/^Invalid regular expression:\s*/i, "")}`);
	}
	if (!multiline) {
		const out: number[] = [];
...
async function resolveInternalSearchInputs(opts: {
	...
	for (let idx = 0; idx < paths.length; idx++) {
		...
		if (hasGlobPathChars(globTarget)) {
			throw new ToolError(`内部 URL 不支持 Glob 模式: ${rawPath}`);
		}
		...
		if (resource.isDirectory && !resource.sourcePath) {
			throw new ToolError(
				`无法对 ${rawPath} 的目录列表进行递归搜索;请搜索其下的具体文件(例如 ${rawPath.replace(/\/+$/, "")}/<file>),或使用 read ${rawPath} 列出其中的条目`,
			);
		}
...
export class GrepTool implements AgentTool<typeof searchSchema, GrepToolDetails> {
	...
	readonly summary = "使用 ripgrep 搜索文件内容(快速正则搜索)";
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			if (normalizedSkip < 0 || !Number.isFinite(normalizedSkip)) {
				throw new ToolError("skip 必须为非负数");
			}
			...
			try {
				...
				if (
					...
					throw new ToolError(
						`无法搜索压缩包成员: ${archiveUnreadable.join(", ")}。` +
							`请使用 \`read <archive>:<member>\` 读取该成员并检查返回的文本,` +
							`或传入 UTF-8 文本成员。`,
					);
				}
				...
				if (searchablePaths.length > 0) {
					const scope = await resolveToolSearchScope({
						...
						multipathStatHint: " (`path` 列表中的每个条目都必须相对于 cwd 存在)",
						settings: this.session.settings,
						signal,
						localProtocolOptions: this.session.localProtocolOptions,
						skills: this.session.skills,
				...
				if (
					...
					const archiveHint =
						archiveUnreadable.length > 0
							...
							: "";
					throw new ToolError(
						`路径未找到: ${missingPaths.join(", ")};请在分号分隔的 \`path\` 中列出每个目标${archiveHint}`,
					);
				}
				...
				try {
					...
					if (err instanceof Error && /^regex(?: parse)? error/i.test(err.message)) {
						throw new ToolError(err.message.replace(/^regex(?: parse)? error:?\s*/i, "无效正则: "));
					}
					if (err instanceof Error && err.message.includes("Aborted: Timeout")) {
						throw new ToolError(
							`Grep 在 ${SEARCH_GREP_TIMEOUT_MS / 1000} 秒后超时;请缩小路径或模式范围,或先用 \`glob\` 限定范围`,
						);
					}
					throw err;
				}
				...
				try {
					...
					if (err instanceof SyntaxError) {
						throw new ToolError(`无效正则: ${err.message}`);
					}
					throw err;
				}
				...
				const limitMessage = fileLimitReached
					? `正在显示文件 ${skipFiles + 1}-${nextSkip},共 ${totalFilesLabel} 个。使用 skip=${nextSkip} 查看下一页,或缩小路径/模式范围。`
					: "";
				...
				const oversizedNote = await (async (): Promise<string | undefined> => {
					...
					return `仅搜索了大文件的前 ${limitMb}MB(超过 ${limitMb}MB 窗口的匹配不显示;其余内容请使用 \`read\`): ${oversized.join(", ")}`;
				})();
				...
				const missingPathsNote =
					missingPathsForNote.length > 0 ? `已跳过缺失路径: ${missingPathsForNote.join(", ")}` : undefined;
				const warningNote =
				...
				if (selectedMatches.length === 0) {
					...
					const noMatchText = skipPastEnd
						? `没有更多结果(共 ${totalFilesLabel} 个文件;skip=${normalizedSkip} 已超出末尾)`
						: "未找到匹配项";
...
function renderBudgetedSearchGroups(
	...
	if (hasSummary) {
		const hiddenLabel =
			hiddenMatches > 0 ? formatMoreItems(hiddenMatches, "匹配") : formatMoreItems(hiddenLines, "行");
...
export const grepToolRenderer = {
	...
	renderCall(args: GrepRenderArgs, _options: RenderResultOptions, uiTheme: Theme): Component {
		...
		const meta: string[] = [];
		if (paths.length) meta.push(`在 ${paths.join(", ")} 中`);
		if (args.case === false) meta.push("不区分大小写");
		if (args.gitignore === false) meta.push("不遵循 gitignore");
		if (args.skip !== undefined && args.skip > 0) meta.push(`跳过 ${args.skip} 个`);

		const text = renderStatusLine(
			{ icon: "pending", title: "内容搜索", titleColor: "toolTitle", description: args.pattern || "?", meta },
			uiTheme,
		);
	...
	renderResult(
		...
		if (!hasDetailedData) {
			...
			if (!textContent || textContent === "未找到匹配项") {
				return new Text(formatEmptyMessage("未找到匹配项", uiTheme), 1, 0);
			}
		...
		const missingNote =
			missingPathsList.length > 0
				? uiTheme.fg("warning", `已跳过缺失路径: ${missingPathsList.join(", ")}`)
				: undefined;
		...
		if (matchCount === 0) {
			...
			const header = renderStatusLine(
				{ icon: "warning", title: "内容搜索", titleColor: "toolTitle", description: args?.pattern, meta },
				uiTheme,
			);
			const lines = [header, formatEmptyMessage("未找到匹配项", uiTheme)];
