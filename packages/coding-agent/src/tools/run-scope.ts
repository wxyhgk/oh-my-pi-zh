import { untilAborted } from "@wxyhgk/pi-utils/abortable";
...
export function waitForRun(
	...
	const promise = (async (): Promise<unknown> => {
		...
		if (typeof msOrPredicate !== "function") {
			throw new ToolError("wait(...) 需要毫秒数(number)或要轮询的谓词函数");
		}
		const timeout =
		...
		for (;;) {
			...
			if (Date.now() + interval > deadline) {
				throw new ToolError(`wait(predicate) 在 ${timeout}ms 后超时 — 谓词从未返回真值`);
			}
