...
import type { FetchImpl, ImageContent, TextContent } from "@wxyhgk/pi-ai";
import { htmlToMarkdown } from "@wxyhgk/pi-natives";
import { type Component, Text } from "@wxyhgk/pi-tui";
import { $which, ptree, truncate } from "@wxyhgk/pi-utils";
...
export function parseReadUrlTarget(readPath: string): ParsedReadUrlTarget | null {
	...
	for (const sel of embedded?.sels ?? []) {
		...
		if (parsed === null) {
			...
			throw new ToolError(`无效的 URL 行选择器: ${sel}`);
		}
		ranges = parsed;
	}
...
async function parseFeedToMarkdown(content: string, maxItems = 10): Promise<string> {
	...
	try {
		...
		if (channel) {
			const title = cleanFeedText(channel.querySelector("title")?.text || "RSS 订阅源");
			const items = channel.querySelectorAll("item").slice(0, maxItems);

			let md = `# ${title}\n\n`;
			for (const item of items) {
				const itemTitle = cleanFeedText(item.querySelector("title")?.text || "无标题");
		...
		if (feed) {
			const title = cleanFeedText(feed.querySelector("title")?.text || "Atom 订阅源");
			const entries = feed.querySelectorAll("entry").slice(0, maxItems);

			let md = `# ${title}\n\n`;
			for (const entry of entries) {
				const entryTitle = cleanFeedText(entry.querySelector("title")?.text || "无标题");
...
function buildBinaryNotice(finalUrl: string, mime: string, byteLength?: number): string {
	const size = byteLength === undefined ? "未知大小" : formatBytes(byteLength);
	return `[二进制内容: ${binaryContentType(mime)}, ${size}] ${finalUrl}`;
}
...
async function tryRenderBinaryPayload(
	...
	if (!binary.ok) {
		resultNotes.push(binary.error ? `二进制下载失败: ${binary.error}` : "二进制下载失败");
	...
	if (isNotebookHint(mime, binaryExtHint)) {
		try {
			...
			resultNotes.push(`Notebook 渲染失败: ${formatErrorMessage(error)}`);
			return buildBinaryPayloadResult(
				url,
				finalUrl,
				mime,
	...
	if (isSqliteHint(mime, binaryExtHint) || looksLikeSqlite(binary.buffer)) {
		try {
			...
			resultNotes.push(`SQLite 渲染失败: ${formatErrorMessage(error)}`);
			return buildBinaryPayloadResult(
				url,
				finalUrl,
				mime,
	...
	if (archiveFormat) {
		try {
			...
			resultNotes.push(`压缩包渲染失败: ${formatErrorMessage(error)}`);
			return buildBinaryPayloadResult(
				url,
				finalUrl,
				mime,
...
async function renderUrl(
	...
	if (!response.ok) {
		return {
			...
			truncated: false,
			notes: [
				response.status ? `获取 URL 失败(HTTP ${response.status})` : "获取 URL 失败",
				...(response.error ? [`原因: ${response.error}`] : []),
			],
		};
	}
	...
	if (response.truncated) {
		notes.push(`响应体超过 ${formatBytes(MAX_BYTES)},在传输中途被截断;内容不完整`);
	}
	...
	if (imageMimeType) {
		if (!isInlineImageMimeTypeSupported(imageMimeType)) {
			...
			if (binary.ok) {
				...
				if (!isDecodedImage) {
					notes.push(`获取到的负载无法解码为 ${imageMimeType};仅返回文本元数据`);
					const output = finalizeOutput(
						rawContent ?? `获取到的负载被标记为 ${imageMimeType},但字节内容不是有效图片。`,
					);
					return {
						url,
						finalUrl,
				...
				let imageSummary = `已获取图片内容(${resized.mimeType})。`;
				if (dimensionNote) {
					imageSummary += `\n${dimensionNote}`;
				}
			...
			notes.push(binary.error ? `二进制下载失败: ${binary.error}` : "二进制下载失败");
			notes.push("回退为使用初始响应进行文本渲染");
			skipConvertibleBinaryRetry = true;
		}
	}
	...
	if (!skipConvertibleBinaryRetry && isConvertible(mime, extHint)) {
		...
		if (binary.ok) {
			...
			if (converted.ok) {
				...
				notes.push("markit 转换未产生可用输出");
			} else if (converted.error) {
				notes.push(`markit 转换失败: ${converted.error}`);
			} else {
				notes.push("markit 转换失败");
			}
		} else if (binary.error) {
			notes.push(`二进制下载失败: ${binary.error}`);
		} else {
			notes.push("二进制下载失败");
		}
	}
	...
	if (isHtml && !raw) {
		...
		if (markdownAlt) {
			...
			if (altResult.ok && altResult.content.trim().length > 100 && !looksLikeHtml(altResult.content)) {
				notes.push(`已使用 markdown 替代: ${resolved}`);
		...
		if (negotiated) {
			notes.push(`内容协商返回 ${negotiated.type}`);
		...
		if (!htmlResult.ok) {
			notes.push("HTML 渲染失败(没有阅读器后端产生可用输出)");

			const llmResult = await tryLlmEndpoints(finalUrl, timeout, signal);
			if (llmResult) {
				notes.push(`已使用 llms.txt 回退: ${llmResult.endpoint}`);
		...
		if (isLowQualityOutput(htmlResult.content)) {
			...
			notes.push("页面似乎需要 JavaScript,或大部分是导航内容");
		}
...
async function materializeReadUrlContent(session: ToolSession, entry: ReadUrlEntry, raw: boolean): Promise<string> {
	...
	if (!root) {
		throw new ToolError("无法搜索 URL 输出,因为当前会话无法物化读取产物。");
	}
...
function buildUrlReadOutput(result: FetchRenderResult, content: string): string {
	...
	output += `方式: ${result.method}\n`;
	if (result.notes.length > 0) {
		output += `备注: ${result.notes.join("; ")}\n`;
	}
	output += `\n---\n\n`;
	output += content;
	return output;
...
export function renderReadUrlResult(
	...
	const metadataLines: string[] = [
		...
		`${uiTheme.fg("muted", "方式:")} ${details.method}`,
	];
	if (hasRedirect) {
		metadataLines.push(
