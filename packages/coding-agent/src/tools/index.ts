...
import type { AgentOptions, AgentTelemetryConfig, AgentTool } from "@wxyhgk/pi-agent-core";
import type { FetchImpl, ImageContent, Model, ServiceTierByFamily, ToolChoice } from "@wxyhgk/pi-ai";
...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	if (!skipEvalPreflight && !allowJs && evalRequested) {
		if (allowPython) {
			...
			if (!availability.ok) {
				logger.warn("Python 内核不可用,且 JS 后端已禁用", { reason: availability.reason });
			}
		}
		if (allowRuby) {
			...
			if (!availability.ok) {
				logger.warn("Ruby 内核不可用,且 JS 后端已禁用", { reason: availability.reason });
			}
		}
		if (allowJulia) {
			...
			if (!availability.ok) {
				logger.warn("Julia 内核不可用,且 JS 后端已禁用", { reason: availability.reason });
			}
		}
	}
