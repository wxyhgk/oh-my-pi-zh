...
export function formatFullOutputReference(artifactId: string): string {
	return `读取 artifact://${artifactId} 以查看完整输出`;
}
...
function isGeneratedOutputNoticeLine(line: string): boolean {
	...
	return (
		...
		/^\d+ 匹配数达到上限。如需更多,请使用 limit=\d+$/u.test(body) ||
		/^\d+ results limit reached\. Use limit=\d+ for more/u.test(body) ||
		/^\d+ 结果数达到上限。如需更多,请使用 limit=\d+$/u.test(body) ||
		body.startsWith("Some lines truncated to ") ||
		body.startsWith("部分行被截断至 ")
	);
}
...
export function formatTruncationMetaNotice(truncation: TruncationMeta): string {
	...
	if (truncation.direction === "middle") {
		...
		const headPart = head ? `第 ${head.start}-${head.end} 行` : "";
		const tailPart = tail ? `第 ${tail.start}-${tail.end} 行` : "";
		if (headPart && tailPart) {
			notice = `显示 ${headPart} 和${tailPart}(共 ${totalLines} 行);已省略中间 ${elidedLines.toLocaleString()} 行(${formatBytes(elidedBytes)})`;
		} else {
			notice = `显示 ${totalLines} 行中的 ${truncation.outputLines} 行;中间部分已省略`;
		}
		if (truncation.nextOffset != null) {
			notice += `。请使用 :${truncation.nextOffset} 继续`;
		}
	...
	if (range && range.end >= range.start) {
		notice = `显示第 ${range.start}-${range.end} 行(共 ${truncation.totalLines} 行)`;
	} else {
		notice = `显示 ${truncation.totalLines} 行中的 ${truncation.outputLines} 行`;
	}

	if (truncation.truncatedBy === "bytes") {
		const maxBytes = truncation.maxBytes ?? truncation.outputBytes;
		notice += ` (${formatBytes(maxBytes)} 限制)`;
	}
...
export function formatOutputNotice(meta: OutputMeta | undefined): string {
	...
	if (meta.limits?.matchLimit) {
		...
		parts.push(`${l.reached} 匹配数达到上限。如需更多,请使用 limit=${l.suggestion}`);
	}
	if (meta.limits?.resultLimit) {
		const l = meta.limits.resultLimit;
		parts.push(`${l.reached} 结果数达到上限。如需更多,请使用 limit=${l.suggestion}`);
	}
	...
	if (meta.limits?.columnTruncated) {
		parts.push(`部分行被截断至 ${meta.limits.columnTruncated.maxColumn} 个字符`);
	}
