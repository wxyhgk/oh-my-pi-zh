...
function getWatchHeader(watch: GhRunWatchViewDetails): string {
	if (watch.mode === "run" && watch.run) {
		if (watch.state === "watching") {
			return `正在监控 ${watch.repo} 上的运行 #${watch.run.id}`;
		}

		return `${watch.repo} 上的运行 #${watch.run.id}`;
	}

	const shortSha = formatShortSha(watch.headSha) ?? "此提交";
	if (watch.state === "watching") {
		return `正在监控 ${watch.repo} 上的 ${shortSha}`;
	}

	return `${watch.repo} 上 ${shortSha} 的工作流运行`;
}
...
function renderRunBlock(run: GhRunWatchRunDetails, width: number, theme: Theme): string[] {
	...
	if (run.jobs.length === 0) {
		lines.push(theme.fg("dim", "正在等待工作流任务..."));
		return lines;
	}
...
function buildWatchSections(
	...
	if (watch.mode === "run" && watch.run) {
		...
		if (runs.length === 0) {
			main.push(theme.fg("dim", "正在等待工作流运行..."));
		} else {
	...
	if (failed.length > 0) {
		sections.push({ label: "失败日志", lines: failed });
	}
...
function renderFallbackComponent(
	...
	return framedBlock(theme, width => {
		...
		if (!expanded && remaining > 0) {
			...
			const more = `${formatMoreItems(remaining, "行")}${hint ? ` ${hint}` : ""}`;
...
function renderWatchCall(args: GithubToolRenderArgs, options: RenderResultOptions, theme: Theme): Component {
	...
	const titleText = theme.fg("accent", "GitHub 运行监控");
	let metaText: string;
	if (runId) {
	...
	const wait = theme.fg("dim", "正在等待工作流数据...");
	return new Text(`${header}\n${wait}`, 0, 0);
}
