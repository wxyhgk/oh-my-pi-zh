...
const BASH_TIMEOUT_DESCRIPTION = `超时秒数;0 表示禁用命令截止时间;非零值将被限制在 ${TOOL_TIMEOUTS.bash.min}-${TOOL_TIMEOUTS.bash.max} 之间`;

const bashSchemaBase = type({
	command: type("string").describe("要执行的命令"),
	"env?": type({ "[string]": "string" }).describe("额外的环境变量"),
	"timeout?": type("number").describe(BASH_TIMEOUT_DESCRIPTION),
	"cwd?": type("string").describe("工作目录"),
	"pty?": type("boolean").describe("以 pty 模式运行"),
});
...
const bashSchemaWithAsync = type({
	...
	"cwd?": "string",
	"pty?": "boolean",
	"async?": type("boolean").describe("在后台运行"),
});
...
function normalizeBashEnv(env: Record<string, string> | undefined): Record<string, string> | undefined {
	...
	for (const [key, value] of Object.entries(env)) {
		if (!BASH_ENV_NAME_PATTERN.test(key)) {
			throw new ToolError(`无效的 bash 环境变量名:${key}`);
		}
...
function formatTimeoutClampNotice(
	...
	const limit = cappedByGlobal
		? `全局 tools.maxTimeout 上限 ${maxTimeout}s`
		: `允许范围 ${TOOL_TIMEOUTS.bash.min}-${TOOL_TIMEOUTS.bash.max}s`;
	return `超时已被限制为 ${effectiveTimeoutSec}s(请求 ${requestedTimeoutSec}s;${limit})。`;
}
...
export class BashTool implements AgentTool<typeof bashSchemaBase | typeof bashSchemaWithAsync, BashToolDetails> {
	...
	readonly approval = (args: unknown): ToolApprovalDecision => {
		...
		if (patternRule?.approval === "deny") {
			return {
				...
				override: true,
				policy: "deny",
				reason: `被 bash 模式阻止:${patternRule.match}`,
			};
		}
		if (command !== "" && CRITICAL_BASH_PATTERNS.some(pattern => pattern.test(command))) {
			return { tier: "exec", override: true, reason: "检测到关键危险模式" };
		}
		...
		if (patternRule?.approval === "prompt") {
			return {
				...
				override: true,
				policy: "prompt",
				reason: `bash 模式要求提示确认:${patternRule.match}`,
			};
		}
		return "exec";
	};
	readonly formatApprovalDetails = (args: unknown): string[] => {
		const rawCommand = (args as Partial<BashToolInput>).command;
		const command = typeof rawCommand === "string" ? rawCommand : "(缺失)";
		return [`命令:${truncateForPrompt(command)}`];
	};
	readonly label = "Bash";
	...
	#throwIfUnfinished(
		...
		if (result.cancelled) {
			...
			const annotated = out.startsWith("[命令已取消]") ? out : out ? `${out}\n\n[命令已中止]` : out;
			throw new ToolError(annotated || "命令已中止");
		}
	...
	#startManagedBashJob(options: {
		...
		if (!manager) {
			throw new ToolError("此会话的后台作业管理器不可用。");
		}
	...
	async execute(
		...
		if (asyncRequested && !this.#asyncEnabled) {
			throw new ToolError("异步 bash 执行已禁用。启用 async.enabled 即可使用异步模式。");
		}
		...
		if (this.session.settings.get("bashInterceptor.enabled")) {
			...
			for (const commandToCheck of commandsToCheck) {
				...
				if (interception.block) {
					throw new ToolError(interception.message ?? "命令已被阻止");
				}
			}
		}
		...
		try {
			...
			if (isEnoent(err)) {
				throw new ToolError(`工作目录不存在:${commandCwd}`);
			}
			throw err;
		}
		if (!cwdStat.isDirectory()) {
			throw new ToolError(`工作目录不是目录:${commandCwd}`);
		}
		...
		if (asyncRequested) {
			if (!this.session.asyncJobManager) {
				throw new ToolError("此会话的异步作业管理器不可用。");
			}
		...
		if (
			...
			if (waitResult.kind === "aborted") {
				...
				throw new ToolAbortError(job.getLatestText() || "命令已中止");
			}
			job.stopUpdates();
			...
			const notices =
				waitResult.kind === "steer"
					? [...pendingNotices, "已提前转入后台以处理传入消息;命令会继续运行。"]
					: pendingNotices;
		...
		if (clientBridge?.capabilities.terminal && clientBridge.createTerminal && !pty) {
			...
			if (signal?.aborted) {
				throw new ToolAbortError("命令已中止");
			}
			...
			try {
				...
				if (finalOutput.truncated) bridgeNotices.push("(输出已截断)");
		...
		if (pty && !interactiveUi) {
			pendingNotices.push("请求了 pty,但当前环境不可用;已在不带终端的情况下运行");
		}
		const wallTimeStart = performance.now();
			...
			return markFramedBlockComponent({
				render: (width: number): readonly string[] => {
					...
					if (details?.async?.state === "running") {
						statsParts.push(`后台作业:${details.async.jobId}`);
					}
					if (wallTimeMs !== undefined) {
						statsParts.push(`耗时:${formatWallTimeSeconds(wallTimeMs)}s`);
					}
					if (timeoutDisabled) {
						statsParts.push("超时:已禁用");
					}
					if (typeof timeoutSeconds === "number") {
						statsParts.push(
							requestedTimeoutSeconds !== undefined && requestedTimeoutSeconds !== timeoutSeconds
								? `超时:${timeoutSeconds}s(请求 ${requestedTimeoutSeconds}s,已限制)`
								: `超时:${timeoutSeconds}s`,
						);
					}
					if (rawOutputArtifact.artifactId) {
						statsParts.push(`产物:${rawOutputArtifact.artifactId}`);
					}
					if (isError && typeof details?.exitCode === "number") {
						statsParts.push(`退出码:${details.exitCode}`);
					}
					const timeoutLine =
						statsParts.length > 0
							? uiTheme.fg(
						...
						if (hasSixelOutput) {
							...
							if (result.skippedCount > 0) {
								outputLines.push(
									uiTheme.fg(
										"dim",
										`… (省略 ${result.skippedCount} 行较早输出,当前显示 ${result.visualLines.length} / ${result.skippedCount + result.visualLines.length})(ctrl+o 展开)`,
									),
								);
							}
					...
					const framed = outputBlock.render(
						{
							...
							sections: [
								...
								{ label: uiTheme.fg("toolTitle", "输出"), lines: outputLines },
							],
							width,
						},
						uiTheme,
