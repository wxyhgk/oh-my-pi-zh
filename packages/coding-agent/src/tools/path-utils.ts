...
import { glob } from "@wxyhgk/pi-natives";
import { isEnoent, isEnotdir, stripWindowsExtendedLengthPathPrefix, untilAborted } from "@wxyhgk/pi-utils";
...
export function parseLineRangeChunk(sel: string): LineRange | null {
	...
	if (rawStart < 1) {
		throw new ToolError("行选择器 0 无效;行号从 1 开始。请使用 :1。");
	}
	...
	if (sep === "+") {
		if (rhs === undefined || rhs < 1) {
			throw new ToolError(`范围 ${rawStart}+${rhs ?? 0} 无效:数量必须 >= 1。`);
		}
		...
		if (rhs !== undefined) {
			if (rhs < rawStart) {
				throw new ToolError(`范围 ${rawStart}-${rhs} 无效:结束必须 >= 开始。`);
			}
			rawEnd = rhs;
		}
	}
...
export function peelWriteUrlSelector(rawPath: string): string {
	...
	throw new ToolError(
		`write 不接受尾部选择器 ":${sel}" — 它会写入整个文件。` +
			`请移除 ":${sel}";如果文件名确实以它结尾,请将 ":" 百分号编码为 %3A。`,
	);
}
...
function assertNotInternalUrl(expanded: string, original: string): void {
	for (const prefix of TOP_LEVEL_INTERNAL_URL_PREFIXES) {
		if (expanded.startsWith(prefix)) {
			throw new Error(
				`路径 "${original}" 使用了内部协议 "${prefix}",必须通过相应的协议处理器解析,不能作为文件系统路径处理。`,
			);
		}
	}
}
...
export async function resolveToolSearchScope(opts: ToolScopeOptions): Promise<ToolScopeResolution> {
	...
	if (normalizedRawPaths.some(rawPath => rawPath.length === 0)) {
		throw new ToolError("搜索范围条目必须是非空路径或 glob");
	}
	...
	for (const rawPath of rawPaths) {
		...
		if (externalUrl) {
			...
			throw new ToolError(
				`无法${internalUrlAction}外部 URL:${rawPath}。请使用 \`read\` 获取网页内容,然后搜索返回的文本。`,
			);
		}
		...
		if (isSshUrl(rawPath)) {
			throw new ToolError(
				`无法${internalUrlAction}远程 ssh:// 路径(无本地文件):${rawPath}。请使用 \`read ${rawPath}\` 查看,或使用 \`search\` 工具搜索远程文件。`,
			);
		}
		...
		if (!resource.sourcePath) {
			throw new ToolError(`无法${internalUrlAction}没有后备文件的内部 URL:${rawPath}`);
		}
	...
	if (resolvedPathInputs.length > 1) {
		...
		if (partition.valid.length === 0) {
			throw new ToolError(`未找到路径:${partition.missing.join(", ")}`);
		}
	...
	if (effectivePaths.length === 1) {
		...
		if (!multiSearchPath) {
			throw new ToolError("`paths` 必须至少包含一个路径或 glob");
		}
	...
	try {
		...
		throw new ToolError(`未找到路径:${scopePath}${hint}`);
	}
