...
function buildAsciiTable(columns: string[], rows: SqliteRow[]): string {
	if (columns.length === 0) {
		return rows.length === 0 ? "(无行)" : "(返回了无列名的行)";
	}
...
function parseLimit(value: string | null, fallback: number): number {
	...
	if (!Number.isFinite(parsed) || parsed < 1) {
		throw new ToolError(`SQLite limit 必须是正整数;当前为 '${value}'`);
	}
...
function parseOffset(value: string | null): number {
	...
	if (!Number.isFinite(parsed) || parsed < 0) {
		throw new ToolError(`SQLite offset 必须是非负整数;当前为 '${value}'`);
	}
	return parsed;
}
...
function getTableMasterRow(db: Database, table: string): SqliteMasterRow {
	const row =
		db
			...
			.get(table) ?? null;
	if (!row) {
		throw new ToolError(`未找到 SQLite 表 '${table}'`);
	}
	return row;
}
...
function coerceIntegerKey(key: string, label: string): number | bigint {
	...
	if (!/^-?\d+$/.test(trimmed)) {
		throw new ToolError(`${label} 必须是整数;当前为 '${key}'`);
	}
...
function coerceLookupValue(key: string, type: string): SqliteBinding {
	...
	if (normalizedType.includes("INT")) {
		return coerceIntegerKey(key, `主键 '${key}'`);
	}
...
function resolveOrderClause(order: string | undefined, columns: string[]): string {
	...
	if (!columns.includes(column)) {
		throw new ToolError(`SQLite 排序列 '${column}' 未在表结构中找到`);
	}
	if (direction !== "asc" && direction !== "desc") {
		throw new ToolError(`SQLite 排序方向必须是 'asc' 或 'desc';当前为 '${direction}'`);
	}
...
function normalizeWriteValue(value: unknown, column: string): SqliteBinding {
	...
	throw new ToolError(`SQLite 列 '${column}' 只接受 JSON 标量值或 null`);
}
...
function validateWriteColumns(
	...
	return Object.entries(data).map(([column, value]) => {
		if (!columns.has(column)) {
			throw new ToolError(`SQLite 表 '${table}' 中没有名为 '${column}' 的列`);
		}
...
export function parseSqliteSelector(subPath: string, queryString: string): SqliteSelector {
	...
	if (rawQuery !== null) {
		...
		if (normalizedSubPath || otherKeys.length > 0) {
			throw new ToolError("SQLite 原始查询不能与表选择器或分页组合使用");
		}
		if (!rawQuery.trim()) {
			throw new ToolError("SQLite 查询参数 'q' 不能为空");
		}
	...
	if (!normalizedSubPath) {
		if (params.size > 0) {
			throw new ToolError("SQLite 查询参数需要表选择器或 q=SELECT...");
		}
		return { kind: "list" };
	}
	...
	if (!table) {
		throw new ToolError("SQLite 选择器必须包含表名");
	}

	if (key !== undefined && key.length > 0) {
		if (params.size > 0) {
			throw new ToolError("SQLite 行查询不能与查询参数组合使用");
		}
	...
	if (hasQueryParams) {
		...
		for (const keyName of params.keys()) {
			if (!knownKeys.has(keyName)) {
				throw new ToolError(`不支持的 SQLite 查询参数 '${keyName}'`);
			}
		}
		return {
			kind: "query",
...
export function getTableSchema(db: Database, table: string): string {
	...
	if (!row.sql) {
		throw new ToolError(`表 '${table}' 的 SQLite 结构不可用`);
	}
	return row.sql;
}
...
export function resolveTableRowLookup(db: Database, table: string): SqliteRowLookup {
	...
	if (primaryKeyColumns.length > 1) {
		throw new ToolError(`SQLite 表 '${table}' 具有复合主键;请改用 '?where='`);
	}

	const schema = getTableSchema(db, table);
	if (/\bWITHOUT\s+ROWID\b/i.test(schema)) {
		throw new ToolError(`SQLite 表 '${table}' 不提供 ROWID;请改用 '?where='`);
	}
...
export function executeReadQuery(
	...
	if (statement.paramsCount > 0) {
		throw new ToolError("SQLite 原始查询不支持绑定参数");
	}
...
export function updateRowByKey(
	...
	const entries = validateWriteColumns(db, table, data);
	if (entries.length === 0) {
		throw new ToolError("SQLite 更新至少需要一个列值");
	}
...
export function renderSchema(
	...
	const parts = [schemaLines.join("\n"), "", "示例行:", buildAsciiTable(sampleRows.columns, sampleRows.rows)];
	return parts.join("\n");
}
...
export function renderTable(
	...
	if (shown < meta.totalCount) {
		...
		parts.push(
			truncateToWidth(
				replaceTabs(
					`[还有 ${remaining} 行;请在数据库路径后追加 :${meta.table}?limit=${meta.limit}&offset=${nextOffset} 以继续]`,
				),
				MAX_RENDER_WIDTH,
			),
		);
