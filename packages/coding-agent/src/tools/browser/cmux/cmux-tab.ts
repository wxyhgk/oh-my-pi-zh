...
import { logger, postmortem, Snowflake, untilAborted } from "@wxyhgk/pi-utils";
...
function consumeCmuxRunRejection(reason: unknown): boolean {
	...
	logger.warn("已完成的 cmux 浏览器运行存在未处理的 rejection(是否缺少 await?)", {
		filename: recent,
		error: reason,
	});
	return true;
...
export class CmuxTab {
	...
	async extract(format: ReadableFormat = "markdown"): Promise<string> {
		...
		if (!readable) {
			throw new ToolError(`tab.extract(${JSON.stringify(format)}) 在 ${url} 上未找到可读内容`);
		}
		const content = format === "markdown" ? readable.markdown : readable.text;
		if (!content) {
			throw new ToolError(`tab.extract(${JSON.stringify(format)}) 为 ${url} 生成了空的 ${format} 内容`);
		}
		return content;
	}
	...
	async screenshot(opts: ScreenshotOptions = {}): Promise<string> {
		...
		if (opts.selector) {
			...
			captureNotes.push(
				`选择器 ${JSON.stringify(opts.selector)} 已滚动到视口内,但此 surface 无法裁剪到元素 — 图片为完整视口`,
			);
		}
		if (opts.fullPage) {
			captureNotes.push("此 surface 不支持 fullPage — 图片仅为视口");
		}
	...
	async waitForUrl(pattern: string | RegExp, opts?: { timeout?: number }): Promise<string> {
		...
		throw new ToolError(`tab.waitForUrl() 在 ${timeoutMs}ms 后超时`);
	}
	...
	async waitForNavigation(opts?: { waitUntil?: WaitUntil; timeout?: number }): Promise<null> {
		...
		throw new ToolError(`tab.waitForNavigation() 在 ${timeoutMs}ms 后超时`);
	}
	...
	async waitForResponse(
		...
		throw new ToolError(`tab.waitForResponse() 在 ${timeoutMs}ms 后超时`);
	}
	...
	async waitForFunction(
		...
		throw new ToolError(`page.waitForFunction() 在 ${timeoutMs}ms 后超时`);
	}
	...
	async #captureScreenshotPng(timeoutMs: number): Promise<CmuxScreenshotResult & { png_base64: string }> {
		...
		if (typeof result.png_base64 !== "string" || result.png_base64.length === 0) {
			throw new ToolError("cmux 浏览器截图响应未包含 png_base64");
		}
	...
	async #waitForSelector(selector: string, timeoutMs: number): Promise<void> {
		...
		throw new ToolError(`tab.waitFor(${JSON.stringify(selector)}) 在 ${timeoutMs}ms 后超时`);
	}
	...
	async #dragPoint(target: DragTarget): Promise<{ x: number; y: number }> {
		if (typeof target === "string") {
			...
			if (!box) throw new ToolError(`拖拽选择器未能解析到可见元素: ${target}`);
			return { x: box.x + box.width / 2, y: box.y + box.height / 2 };
		}
		if (Number.isFinite(target.x) && Number.isFinite(target.y)) {
			return { x: target.x, y: target.y };
		}
		throw new ToolError("拖拽目标必须是选择器字符串或 { x: number, y: number } 坐标点");
	}
	...
	#requireRunContext(operation: string): RunContext {
		if (!this.#runContext) {
			throw new ToolError(`${operation} 需要处于活动状态的 cmux 浏览器运行`);
		}
...
export async function runCmuxCode(tab: CmuxTab, opts: RunCmuxCodeOptions): Promise<RunResultOk> {
	...
	const onAbort = (): void => {
		if (timeoutSignal.aborted) {
			reject(new ToolError(`浏览器代码执行超时(超过 ${opts.timeoutMs}ms)`));
		} else {
			reject(
	...
	try {
		...
		if (runFailed) {
			for (const reason of activeRun.floatingRejections) {
				logger.warn("失败的 cmux 浏览器运行伴随未处理的 rejection", { filename, error: reason });
			}
			throw runError;
		}
		if (activeRun.floatingRejections.length > 0) {
			...
			throw new ToolError(`未处理的 rejection(是否缺少 await?): ${messages.join("\n[未处理的 rejection] ")}`, {
