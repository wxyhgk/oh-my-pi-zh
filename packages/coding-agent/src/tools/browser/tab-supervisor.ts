import { getPuppeteerDir, logger, postmortem, Snowflake, withTimeout, workerHostEntry } from "@wxyhgk/pi-utils";
...
async function waitForTabCleanup<T>(
	...
	const message = `关闭 ${tab.kindTag} 浏览器标签页 ${JSON.stringify(tab.name)} 在 ${timeoutMs}ms 后超时;待清理资源:${pendingResource}`;
	try {
...
async function acquireTabImpl(
	...
	if (opts.signal?.aborted) {
		throw new ToolAbortError("浏览器标签页打开已中止");
	}
	...
	try {
		...
		logger.warn("标签页 Worker 初始化失败;改用内联标签页 Worker 重试(无同步循环防护)", {
		...
		try {
			...
			const finalError = new ToolError(
				`启动浏览器标签页 Worker 失败(内联回退也失败):${inlineError instanceof Error ? inlineError.message : String(inlineError)}`,
			);
...
async function acquireCmuxTab(
	...
	if (attachedSurface?.startsWith("surface:")) {
		throw new ToolError(
			"app.surface 必须是 surface UUID(例如 CMUX_SURFACE_ID),不能是 'surface:N' 引用;省略它以打开新的分屏",
		);
	}
	...
	try {
		if (!surfaceId) {
			...
			if (typeof result.surface_id !== "string" || result.surface_id.length === 0) {
				throw new ToolError("cmux browser.open_split 未返回 surface_id");
			}
...
async function runInTabWithSnapshot(
	...
	if (!tab || tab.state === "dead") {
		...
		throw new ToolError(
			killed
				? `标签页 ${JSON.stringify(name)} 已被终止:${killed}。请重新打开它。`
				: `标签页 ${JSON.stringify(name)} 不存在或已失效。请先用 action:"open" 打开它。`,
		);
	}
	if (tab.pending.size > 0) throw new ToolError(`标签页 ${JSON.stringify(name)} 正忙`);
		...
		try {
			...
			if (runTimedOut || error instanceof RecoverableWorkerError) {
				try {
					...
					logger.warn("回收浏览器标签页 Worker 失败,正在终止标签页", {
						error: recycleError instanceof Error ? recycleError.message : String(recycleError),
					});
					await forceKillTab(name, "浏览器标签页 Worker 恢复失败,标签页已终止");
				}
			}
			throw error;
		}
...
export async function releaseTab(name: string, opts: ReleaseTabOptions = {}): Promise<boolean> {
	const tab = tabs.get(name);
	if (!tab) {
		logger.debug("releaseTab:未知标签页", { name });
		return false;
	}
	const wasAlive = tab.state === "alive";
	tab.state = "dead";
	const closeError = postmortem.markExpectedCleanupError(new ToolError(`标签页 ${JSON.stringify(name)} 已关闭`));
	...
	if (tab.backend === "cmux") {
		...
		if (wasAlive && tab.cmuxOwnsSurface) {
			try {
				...
				if (isLastSurfaceCloseError(err)) {
					logger.debug("由于它是工作区中的最后一个 surface,保留 cmux 浏览器 surface 不关闭", {
		...
		try {
			await releaseBrowser(tab.browser, {
				...
				timeoutMs,
				resource: `标签页 ${JSON.stringify(name)}`,
			});
		} catch (error) {
			closeError ??= error;
		} finally {
	...
	if (forced && tab.kindTag === "headless") {
		try {
			await waitForTabCleanup(
				tab,
				timeoutMs,
				`孤立 CDP target ${JSON.stringify(tab.targetId)} (Page.close)`,
...
async function buildInitPayload(browser: PuppeteerBrowserHandle, opts: AcquireTabOptions): Promise<WorkerInitPayload> {
	...
	if (!browserWSEndpoint) throw new ToolError("浏览器 WebSocket 端点不可用");
...
function safeSend(tab: WorkerTabSession, msg: WorkerInbound): void {
	...
	try {
		...
		logger.debug("标签页 Worker 发送失败", { error: err instanceof Error ? err.message : String(err) });
	}
}
...
async function recycleTimedOutWorkerTab(tab: WorkerTabSession, timeoutMs: number): Promise<void> {
	...
	try {
		...
		try {
			...
			await worker.terminate().catch(() => undefined);
			const finalError = new ToolError(
				`回收超时的浏览器标签页 Worker 失败(内联回退也失败):${inlineError instanceof Error ? inlineError.message : String(inlineError)}`,
			);
...
async function waitForClosed(tab: WorkerTabSession): Promise<void> {
	...
	try {
		await raceWithTimeout(promise, GRACE_MS, "关闭浏览器标签页 Worker 超时");
	} finally {
		unsubscribe();
	}
}
...
async function spawnTabWorker(): Promise<WorkerHandle> {
	try {
		...
		logger.warn("Bun Worker 启动失败;改用内联标签页 Worker(无同步循环防护)", {
			error: err instanceof Error ? err.message : String(err),
		});
	...
	return {
		...
		onError(handler) {
			...
			const onMessageError = (event: MessageEvent): void =>
				handler(new ToolError(`标签页 Worker 消息错误:${String(event.data)}`));
...
async function initializeTabWorker(
	...
	const unlistenError = worker.onError(error => {
		reject(new ToolError(`标签页 Worker 启动失败:${error.message}`));
	});
	try {
		worker.send({ type: "init", payload });
		return await raceWithTimeout(promise, timeoutMs, "初始化浏览器标签页 Worker 超时");
	} finally {
		unlisten();
		unlistenError();
	}
...
function errorFromWorkerEvent(event: ErrorEvent): Error {
	...
	return new Error("未知的标签页 Worker 错误");
}
