...
export async function ensureSharedBrowser(opts: {
	...
	for (let attempt = 0; attempt < ENSURE_ATTEMPTS; attempt++) {
		...
		const existing = await describeQuietly(client, name, "共享浏览器", opts.signal);
		if (existing && existing.state !== "exited" && existing.state !== "failed") {
			const settled =
				existing.readyAt !== undefined ? existing : await waitReady(client, name, "共享浏览器", opts.signal);
			...
			await stopQuietly(client, name, "共享浏览器", opts.signal);
			continue;
		}
		try {
