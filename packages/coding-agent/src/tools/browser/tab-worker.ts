...
import { postmortem, Snowflake, untilAborted, withTimeout } from "@wxyhgk/pi-utils";
...
export function normalizeSelector(selector: string): string {
	...
	if (
		...
		throw new ToolError(
			`浏览器工具不支持 Playwright 专用选择器 ${JSON.stringify(selector)}。` +
				`请使用 puppeteer 文本选择器("text/Allow all")、aria 选择器("aria/Name")、CSS 或 "xpath/..."。`,
		);
	}
	if (selector.startsWith("p-") && !LEGACY_SELECTOR_PREFIXES.some(prefix => selector.startsWith(prefix))) {
		throw new ToolError(
			`不支持的选择器前缀。请使用 CSS 或 puppeteer 查询处理器(aria/、text/、xpath/、pierce/)。收到:${selector}`,
		);
	}
...
function replyError(payload: RunErrorPayload): Error {
	if (payload.isAbort) {
		const err = new ToolAbortError(payload.message || "工具调用已中止");
...
async function targetIdForTarget(target: Target): Promise<string> {
	...
	try {
		...
		throw new ToolError("无法从 CDP target 信息中获取 targetId");
	} finally {
...
async function clickQueryHandlerText(
	...
	throw new ToolError(
		`点击 ${selector} 超时(看到 ${lastSeen} 个匹配;最后原因:${lastReason ?? "未知"})。` +
			"如果有多个匹配元素,请使用 observe + tab.id() 或更具体的选择器。",
	);
}
...
export function formatSelectorMatchHint(count: number): string {
	return count === 0
		? ";选择器当前未匹配到任何元素——请运行 tab.observe() 或 tab.ariaSnapshot() 检查页面"
		: `;选择器当前匹配到 ${count} 个元素,但操作始终未能进行——元素可能被隐藏或被遮挡(请尝试 tab.scrollIntoView() 或更具体的选择器)`;
}
...
export async function preparePageForScreenshot(
	...
	if (!visible) {
		throw new ToolError("附加的浏览器标签页不可见;截图前请先切换到该标签页");
	}
}
...
export class WorkerCore {
	...
	async #init(payload: WorkerInitPayload): Promise<void> {
		try {
			...
			if (payload.mode === "headless") {
				...
				if (!page) throw new ToolError(`附加的浏览器上已不存在 Target ${payload.targetId}`);
				this.#page = page;
	...
	async #recoverAttachedTarget(target: Target): Promise<void> {
		...
		try {
			...
			this.#log("debug", "恢复用 CDP 会话失败,继续执行连接", {
				error: error instanceof Error ? error.message : String(error),
			});
		} finally {
			await session?.detach().catch(() => undefined);
	...
	async #run(msg: Extract<WorkerInbound, { type: "run" }>): Promise<void> {
		if (this.#active) {
			this.#transport.send({
				...
				id: msg.id,
				ok: false,
				error: errorPayload(new ToolError("标签页 Worker 正忙")),
			});
			return;
		}
		...
		try {
			...
			runtime.setRunScope({
				...
				assert: (cond: unknown, text?: string): void => {
					if (!cond) throw new ToolError(text ?? "断言失败");
				},
			...
			const onCancel = (): void => {
				...
				if (timeoutSignal.aborted) {
					...
					const dialogNote = dialog
						? `;本次运行期间打开了 ${dialog.type}(${JSON.stringify(dialog.message.slice(0, 80))}) 对话框,可能仍在阻塞页面——请使用 dialogs:"accept"|"dismiss" 重新打开标签页,或通过 page.on('dialog') 处理`
						: "";
					rejectCancel(
						new ToolError(
							`浏览器代码执行超时(超过 ${msg.timeoutMs}ms${stalled ? `,卡在 ${stalled}` : ""})${dialogNote}`,
						),
					);
				} else {
			...
			try {
				...
				if (!hooks) throw new ToolError("浏览器运行时启动时没有活动运行");
			...
			runAc.abort(postmortem.markExpectedCleanupError(new ToolAbortError("浏览器运行已结束")));
			try {
	...
	async #runOp<T>(
		...
		try {
			...
			if (
				...
				throw new ToolError(`${label} 在 ${perOpTimeoutMs}ms 后超时${hint}`);
			}
			throw err;
		} finally {
			earlyAc.abort();
	...
	async #zeroMatchWatchdog(selector: string, label: string, afterMs: number, signal: AbortSignal): Promise<never> {
		...
		while (!signal.aborted) {
			...
			if (count === 0 && Date.now() >= deadline) {
				throw new ToolError(`${label} 在 ${afterMs}ms 后快速失败${formatSelectorMatchHint(0)}`);
			}
			try {
		...
		return {
				...
				op(`tab.goto(${JSON.stringify(url)})`, INF, async sig => {
					...
					try {
						...
						if (err instanceof Error && err.name === "TimeoutError") {
							...
							throw new ToolError(
								`tab.goto(${JSON.stringify(url)}) 在 ${budgetBound}ms 后超时;挂起的导航已停止——请使用更长的工具超时或 waitUntil:"domcontentloaded" 重试`,
							);
						}
						throw err;
					}
			...
			extract: (format = "markdown") =>
				op(`tab.extract(${JSON.stringify(format)})`, quickOpMs, async sig => {
					...
					if (!content) {
						throw new ToolError(
							`tab.extract(${JSON.stringify(format)}) 为 ${page.url()} 生成了空的 ${format} 内容`,
						);
					}
					return content;
				}),
	...
	async #collectObservation(options: {
		...
		if (!snapshot) throw new ToolError("无法获取无障碍快照");
	...
	async #captureScreenshot(
		...
		if (opts.selector) {
			...
			if (!handle) throw new ToolError("截图选择器未能解析到元素");
			try {
	...
	async #drag(from: DragTarget, to: DragTarget, signal: AbortSignal): Promise<void> {
		...
		const resolveDragPoint = async (
			...
			if (typeof target === "string") {
				...
				if (!handle) throw new ToolError(`拖拽 ${role} 选择器未能解析:${target}`);
				...
				if (!box) {
					await handle.dispose().catch(() => undefined);
					throw new ToolError(`拖拽 ${role} 元素没有边界框(可能不可见):${target}`);
				}
			...
			throw new ToolError(
				`拖拽 ${role} 必须是选择器字符串或 { x: number, y: number } 坐标点。收到:${typeof target}`,
			);
		};
		...
		try {
			return (await untilAborted(signal, () =>
				handle.evaluate((el, vals) => {
					...
					if (select?.tagName !== "SELECT") throw new Error("tab.select() 需要 <select> 元素");
					const EventCtor = (
	...
	async #uploadFile(
		...
		if (!filePaths.length) throw new ToolError("tab.uploadFile() 至少需要一个文件路径");
		const handle = await this.#resolveActionHandle(selector, timeoutMs, signal);
		try {
	...
	async #resolveCachedHandle(id: number): Promise<ElementHandle> {
		...
		if (!handle) throw new ToolError(`未知的元素 id ${id}。请运行 tab.observe() 刷新元素列表。`);
		try {
			const isConnected = (await handle.evaluate(el => el.isConnected)) as boolean;
			if (!isConnected) {
				this.#clearElementCache();
				throw new ToolError(`元素 id ${id} 已过期。请重新运行 tab.observe()。`);
			}
		} catch (err) {
	...
	async #resolveAriaRef(id: string): Promise<ElementHandle> {
		...
		if (!handle) {
			throw new ToolError(
				`未知的 ARIA ref ${JSON.stringify(ref)}。请运行 tab.ariaSnapshot() 刷新 refs(每次快照都会重新编号)。`,
			);
		}
		return handle;
	}
	...
	async #stopLoading(): Promise<void> {
		try {
			...
			this.#log("debug", "Page.stopLoading 失败", {
				error: error instanceof Error ? error.message : String(error),
			});
		}
	}
	...
	#requirePage(): Page {
		if (!this.#page) throw new ToolError("标签页 Worker 尚未初始化");
		return this.#page;
	}

	#requireBrowser(): Browser {
		if (!this.#browser) throw new ToolError("标签页 Worker 尚未初始化");
		return this.#browser;
	}
