...
export async function ensureChromiumExecutable(): Promise<string | undefined> {
	...
	chromiumExecutablePromise = (async () => {
		...
		if (!platform) {
			logger.warn("无法检测浏览器平台,将依赖 puppeteer 默认解析");
			return undefined;
		}
		...
		logger.warn("正在下载 puppeteer 所需 Chromium(首次使用浏览器)", {
			buildId,
			platform,
			cacheDir,
		});
		...
		await browsers.install({
			...
			downloadProgressCallback: (downloaded, total) => {
				...
				if (pct >= lastReportedPercent + 10 || downloaded === total) {
					...
					logger.debug(
						`Chromium 下载:${pct}% (${Math.round(downloaded / 1_000_000)} / ${Math.round(total / 1_000_000)} MB)`,
					);
				}
			},
		});
		...
		throw new ToolError(
			`为 puppeteer 安装 Chromium 失败:${(err as Error).message}。` +
				"请设置 PUPPETEER_EXECUTABLE_PATH 使用现有的 Chrome/Chromium 可执行文件,或手动安装一个。",
		);
	});
...
function resolveSystemChromium(): string | undefined {
	...
	for (const candidate of systemChromiumCandidates()) {
		...
		if (isExecutableFile(candidate)) {
			...
			logger.debug("使用系统 Chrome/Chromium", { path: candidate });
			return candidate;
		}
	}
...
export async function removeUserDataDir(dir: string): Promise<void> {
	try {
		...
		logger.warn("清理失败后保留 Chromium 配置文件目录", {
			dir,
...
async function sendUserAgentOverride(client: PuppeteerCdpClient, override: UserAgentOverride): Promise<void> {
	...
	try {
		...
		logger.debug("应用 Network user agent 覆盖失败", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
	try {
		...
		logger.debug("应用 Emulation user agent 覆盖失败", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
async function withSoftTimeout<T>(promise: Promise<T>, timeoutMs: number, label: string): Promise<T | undefined> {
	...
	try {
		return await Promise.race([
			promise.catch(error => {
				logger.debug(`应用 ${label} 失败`, { error: error instanceof Error ? error.message : String(error) });
				return undefined;
			}),
			timeoutPromise,
		]);
