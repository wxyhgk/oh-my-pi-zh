...
export class RelayBridge {
	...
	extClosed(socket: RelaySocket): void {
		...
		for (const pending of this.#pendingRpc.values()) {
			...
			pending.reject(new Error("relay 扩展已断开连接"));
		}
	...
	extMessage(socket: RelaySocket, raw: string): void {
		...
		switch (msg.t) {
			...
			case "rpcResult": {
				...
				else pending.reject(new Error(msg.error ?? "扩展 RPC 调用失败"));
				return;
			}
			case "cdpEvent":
	...
	#onHello(msg: Extract<ExtToRelayMessage, { t: "hello" }>): void {
		...
		this.#syncGrouping();
		this.#log("扩展已连接", { tabs: this.#tabs.size, version: msg.browserVersion });
	}
	...
	cdpConnected(socket: RelaySocket): number {
		...
		this.#log("CDP 客户端已连接", { conn: conn.id });
		return conn.id;
	}
	...
	cdpClosed(connId: number): void {
		...
		this.#log("CDP 客户端已断开", { conn: connId });
	}
	...
	async #handleCdpCommand(conn: CdpConnection, msg: CdpCommand): Promise<void> {
		...
		this.#replyError(conn, msg, `未知的 session id ${sessionId}`);
	}
	...
	#claimTab(conn: CdpConnection, tabId: number): void {
		...
		if (!conn.claims.has(tabId)) {
			...
			this.#log("标签页已被认领", { conn: conn.id, tabId });
		}
	...
	#handleTabSessionCommand(conn: CdpConnection, msg: CdpCommand, ref: SessionRef): void {
		switch (msg.method) {
			case "Target.setAutoAttach": {
				...
				if (!tab) {
					this.#replyError(conn, msg, `标签页 ${ref.tabId} 已不存在`);
					return;
				}
			...
			default:
				this.#replyError(conn, msg, `标签页目标不支持 '${msg.method}'`, CDP_ERROR_METHOD_NOT_FOUND);
		}
	}
	...
	async #handleBrowserCommand(conn: CdpConnection, msg: CdpCommand): Promise<void> {
		switch (msg.method) {
			...
			case "Target.attachToTarget": {
				...
				if (!parsed || !tab) {
					this.#replyError(conn, msg, `不存在 id 为 ${String(msg.params?.targetId)} 的目标`);
					return;
				}
				if (!(await this.#ensureAttached(tab))) {
					this.#replyError(conn, msg, `无法附加到标签页 ${tab.tabId} (${tab.url})`);
					return;
				}
			...
			case "Browser.close":
				...
				this.#log("拒绝来自下游客户端的 Browser.close", { conn: conn.id });
				this.#reply(conn, msg, {});
				return;
			...
			case "Target.createBrowserContext":
				this.#replyError(conn, msg, "omp browser relay 不支持浏览器上下文");
				return;
			default:
				this.#replyError(conn, msg, `未找到 '${msg.method}'`, CDP_ERROR_METHOD_NOT_FOUND);
		}
	}
		...
		try {
			while (this.#groupQueue.length > 0) {
				...
				try {
					...
					this.#log("标签页已分组", { tabIds, grouped });
				} catch (err) {
					this.#log("标签页分组失败", { error: err instanceof Error ? err.message : String(err) });
				} finally {
	...
	#rpc(req: RelayRpcRequest, timeoutMs = RPC_TIMEOUT_MS): Promise<unknown> {
		const ext = this.#ext;
		if (!ext) return Promise.reject(new Error("relay 扩展未连接"));
		const id = ++this.#rpcSeq;
		const { promise, resolve, reject } = Promise.withResolvers<unknown>();
		const timer = setTimeout(() => {
			this.#pendingRpc.delete(id);
			reject(new Error(`扩展 RPC '${req.op}' 在 ${timeoutMs}ms 后超时`));
		}, timeoutMs);
