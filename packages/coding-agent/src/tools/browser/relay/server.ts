	...
	const server = Bun.serve({
		...
		fetch(req, srv): Response | undefined {
			...
			if (path === "/cdp") {
				...
				if (req.headers.get("origin")) return new Response("禁止访问", { status: 403 });
			...
			if (req.method !== "GET") return new Response("不允许的方法", { status: 405 });
			if (path === "/json/version") {
				if (!bridge.ready) {
					return Response.json({ error: "relay 扩展未连接" }, { status: 503 });
				}
