...
export function assertSelectorString(selector: unknown): asserts selector is string {
	...
	if (selector !== null && typeof selector === "object") {
		kind =
			"then" in selector && typeof selector.then === "function" ? "一个 Promise(缺少 await?)" : "一个 ElementHandle";
	} else {
		kind = `一个 ${typeof selector}`;
	}
	throw new ToolError(
		`浏览器选择器必须是字符串;实际传入 ${kind}。` +
			"tab.click/type/fill/waitFor 只接受字符串选择器——" +
			'请直接调用句柄方法(例如 (await tab.id(n)).click()),或传入类似 "aria-ref=eN" 的字符串。',
	);
}
