...
import { Process, ProcessStatus } from "@wxyhgk/pi-natives";
...
export async function findFreeCdpPort(): Promise<number> {
	...
	server.listen(0, "127.0.0.1", () => {
		...
		if (addr && typeof addr === "object" && typeof addr.port === "number") {
			...
			server.close();
			reject(new Error("无法分配临时 CDP 端口"));
		}
	});
	return promise;
}
...
export async function pickElectronTarget(
	...
	if (!fallbackPages.length) {
		throw new ToolError("所附加的浏览器上没有可用的页面目标");
	}
...
async function pickPageFromList(pages: Page[], options: { matcher?: string; preferVisible?: boolean }): Promise<Page> {
	...
	if (options.matcher) {
		...
		const summary = enriched.map(p => `- ${p.title || "(无标题)"}  ${p.url}`).join("\n");
		throw new ToolError(`没有与 ${JSON.stringify(options.matcher)} 匹配的页面目标。可用页面:\n${summary}`);
	}
