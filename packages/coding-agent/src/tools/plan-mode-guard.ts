...
import { HL_FILE_HASH_LENGTH, HL_FILE_HASH_SEP, HL_FILE_PREFIX, HL_FILE_SUFFIX } from "@wxyhgk/hashline";
import {
...
export function enforcePlanModeWrite(
	...
	if (options?.move) {
		throw new ToolError("计划模式:不允许重命名文件。");
	}

	if (options?.op === "delete") {
		throw new ToolError("计划模式:不允许删除文件。");
	}
