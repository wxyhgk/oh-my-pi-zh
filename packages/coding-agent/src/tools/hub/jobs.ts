...
function describeAgents(agents: AgentActivitySnapshot[]): string[] {
	const lines = [`## 运行中的 Agent (${agents.length}) — 无 job 支撑\n`];
	for (const agent of agents) {
		const parent = agent.parentId ? ` (由 \`${agent.parentId}\` 派生)` : "";
		const activity = agent.activity ? ` — ${agent.activity}` : "";
		lines.push(`- \`${agent.id}\`${parent} — 已运行 ${formatDuration(agent.ageMs)}${activity}`);
	}
	lines.push("", "这些 Agent 没有对应的 job;可通过 `hub` send 给它们发消息,转录记录在 `history://<id>`。");
	return lines;
}
...
export function buildJobResult(
	...
	if (cancelOutcomes.length > 0) {
		lines.push(`## 已取消 (${cancelOutcomes.length})\n`);
	...
	if (completed.length > 0) {
		...
		for (const j of completed) {
			...
			lines.push(`标签: ${j.label}`);
			if (j.resultText) {
				lines.push("```", j.resultText, "```");
			}
			if (j.errorText) {
				lines.push(`错误: ${j.errorText}`);
			}
			lines.push("");
		}
	}
	...
	if (running.length > 0) {
		lines.push(`## 仍在运行 (${running.length})\n`);
...
export function noMatchingJobsResult(session: ToolSession, ids: string[]): AgentToolResult<CoordinationDetails> {
	...
	const lines: string[] = [`未找到与 ID 匹配的 job: ${ids.join(", ")}`];
	const registry = session.agentRegistry;
	for (const id of ids) {
		...
		lines.push(
			ref.status === "running"
				? `- \`${id}\` 是运行中的 Agent,但没有对应的 job —— 可通过 \`hub\` send 给它发消息;转录记录在 history://${id}`
				: `- \`${id}\` 是状态为 ${ref.status} 的 Agent(其 job 已消失)—— 转录记录在 history://${id}`,
		);
	}
	if (agents.length > 0) {
		lines.push("", ...describeAgents(agents));
...
export function nothingToWaitForResult(session: ToolSession): AgentToolResult<CoordinationDetails> {
	const agents = runningAgentsOutsideJobs(session);
	const lines: string[] = ["没有正在运行的后台 job 可等待。"];
	if (agents.length > 0) {
		lines.push("", ...describeAgents(agents));
	}
	return {
...
export async function executeCancel(
	...
	for (const id of ids) {
			...
			cancelOutcomes.push(
				regOutcome.status === "cancelled"
					...
					: {
							...
							message: `后台 job ${id} 已处于 ${existing.status} 状态。`,
						},
			);
			continue;
		}
		...
		cancelOutcomes.push(
			cancelled
				? { id, status: "cancelled", message: `已取消后台 job ${id}。` }
				: { id, status: "already_completed", message: `后台 job ${id} 已完成。` },
		);
	}
...
async function cancelAgentRegistration(
	...
	const ref = registry?.get(id);
	if (ref?.kind !== "sub") {
		return { id, status: "not_found", message: `未找到后台 job: ${id}` };
	}
	if (id === ownerId) {
		return { id, status: "not_found", message: `无法取消自己 (${id})。` };
	}
	if (ownerId && ref.parentId !== ownerId) {
		return { id, status: "not_found", message: `Agent ${id} 不是由你派生的,无法取消。` };
	}
	...
	try {
		...
		return {
			id,
			status: "already_completed",
			message: `Agent ${id} 未能完全取消:${error instanceof Error ? error.message : String(error)}。`,
		};
	}
	return { id, status: "cancelled", message: `已取消 Agent ${id}(已终止会话,已移除注册)。` };
}
...
function describeTarget(args: JobRenderArgs | undefined): string {
	if (args?.list) return "后台 job";
	const poll = args?.poll ?? [];
	const cancel = args?.cancel ?? [];
	const parts: string[] = [];
	if (cancel.length > 0) {
		parts.push(cancel.length === 1 ? `取消 ${cancel[0]}` : `取消 ${cancel.length} 个 job`);
	}
	if (poll.length > 0) {
		parts.push(poll.length === 1 ? `轮询 ${poll[0]}` : `轮询 ${poll.length} 个 job`);
	}
	if (parts.length === 0) return "所有运行中的 job";
	return parts.join(", ");
}
...
export function jobsRenderResult(
	...
	if (jobs.length === 0 && agents.length === 0) {
		const fallback = result.content?.find(c => c.type === "text")?.text || "没有要处理的 job";
	...
	if (agents.length > 0 && jobs.length > 0) {
		meta.push(uiTheme.fg("accent", `${agents.length} 个 Agent`));
	}
	...
	const description =
		jobs.length === 0
			? `${agents.length} 个运行中的 Agent — 无 job`
			: counts.running > 0
				? counts.running === jobs.length
					? `正在等待 ${jobs.length} 个 job`
					: `正在等待 ${jobs.length} 个 job 中的 ${counts.running} 个`
				: `${jobs.length} 个 job 已结束`;
