...
function registerCompletionSink(
	...
	if (!registration) {
		const unregister = client.onCompletion(owner, notification => {
			if (session.isDisposed?.()) throw new Error("会话在 launch 完成投递前已销毁");
			const delivery = session.queueLaunchCompletion?.(notification);
			if (!delivery) throw new Error("会话无法接受 launch 完成投递");
			return delivery;
		});
...
function requiredName(params: LaunchParams): string {
	if (!params.name) throw new ToolError(`${params.op} 需要提供 name`);
	return params.name;
}
...
function commandSpec(params: LaunchParams, session: ToolSession): DaemonSpec {
	...
	if (!params.application) throw new ToolError("start 需要提供 application");
	...
	if (ready?.port !== undefined && (!Number.isInteger(ready.port) || ready.port < 1 || ready.port > 65_535)) {
		throw new ToolError("ready.port 必须是 1 到 65535 之间的整数");
	}
	if (ready && !ready.log && ready.port === undefined) throw new ToolError("ready 需要提供 log 或 port");
	return {
		name,
...
function sendData(params: LaunchParams): string | undefined {
	...
	for (const rawKey of params.keys ?? []) {
		...
		if (input === undefined) throw new ToolError(`不支持的 launch 按键:${rawKey}`);
		data += input;
	}
...
function daemonLabel(daemon: DaemonSnapshot): string {
	...
	return `${daemon.name}: ${daemon.state}${pid}${exit} 运行时长=${formatDuration(
		(daemon.exitedAt ?? Date.now()) - daemon.startedAt,
	)} 重启次数=${daemon.restartCount}${daemon.detached ? " 已分离" : daemon.persist ? " 持久" : ""}`;
}
...
function readyPendingSummary(daemon: DaemonSnapshot, ready?: LaunchParams["ready"]): string[] {
	...
	for (const condition of daemon.readyPending ?? []) {
		if (condition === "log") {
			parts.push(ready?.log ? `日志模式 /${ready.log}/ 从未匹配` : "日志模式从未匹配");
		} else {
			parts.push(
				ready?.port !== undefined
					? `端口 ${ready.port} 在 ${ready.host ?? "127.0.0.1"} 上从未接受连接`
					: "端口从未接受连接",
			);
		}
	}
...
function toolContent(result: DaemonRpcResult, params: LaunchParams): string {
	switch (result.op) {
		case "ping":
		case "shutdown":
			throw new ToolError(`内部 daemon 结果 ${result.op} 对工具不可见`);
		case "start": {
			const daemon = result.daemon;
			const lines = [`${daemon.state === "failed" ? "启动失败" : "已启动"} ${daemonLabel(daemon)}`];
			if (daemon.state === "failed" && daemon.exitReason) lines.push(`原因: ${daemon.exitReason}`);
			if (daemon.readyMatch) lines.push(`就绪日志已匹配: ${daemon.readyMatch}`);
			if (result.readyTimedOut) {
				const pending = readyPendingSummary(daemon, params.ready);
				const cause = pending.length > 0 ? `: ${pending.join("; ")}` : "";
				lines.push(
					`未就绪 —— 就绪检查在 ${params.ready?.timeout ?? 30}s 后超时${cause}。进程仍在运行(state: ${daemon.state});可跟踪其日志或将其停止。`,
				);
			} else if (params.ready && daemon.readyAt === undefined && TERMINAL_STATES[daemon.state]) {
				lines.push("在观察到就绪状态之前进程已退出。");
			}
			return lines.join("\n");
		}
		case "list":
		...
		case "logs": {
			...
			return `${text}${text && !text.endsWith("\n") ? "\n" : ""}[${result.name}: ${result.state}; cursor=${result.cursor}${result.timedOut ? "; 跟踪超时" : ""}]`;
		}
		case "wait": {
			const lines = [daemonLabel(result.daemon)];
			if (result.matched) lines.push(`已匹配: ${result.matched}`);
			if (result.timedOut) {
				const pending = readyPendingSummary(result.daemon);
				lines.push(`等待超时${pending.length > 0 ? ` (仍在等待: ${pending.join("; ")})` : ""}。`);
			}
			return lines.join("\n");
		}
		case "send":
			return `已向 ${daemonLabel(result.daemon)} 发送输入`;
		case "stop":
			return `已停止 ${daemonLabel(result.daemon)}`;
		case "restart":
			return `已重启 ${daemonLabel(result.daemon)}`;
		case "describe":
			return [
				daemonLabel(result.daemon),
				`命令: ${[result.spec.application, ...result.spec.args].join(" ")}`,
				`工作目录: ${shortenPath(result.spec.cwd)}`,
...
/** UI label for daemon lifecycle states (display-only; the raw enum stays in details). */
...
function daemonMeta(daemon: DaemonSnapshot, theme: Theme): string[] {
	const meta = [theme.fg(stateColor(daemon.state), daemonStateLabel(daemon.state))];
	if (daemon.readyPending?.length) meta.push(theme.fg("warning", `等待 ${daemon.readyPending.join("+")}`));
	if (daemon.exitCode !== undefined) {
		meta.push(theme.fg(daemon.exitCode === 0 ? "muted" : "error", `退出码 ${daemon.exitCode}`));
	...
	meta.push(daemon.exitedAt === undefined ? `已运行 ${lifespan}` : `运行了 ${lifespan}`);
	if (daemon.restartCount > 0) meta.push(`重启 ${daemon.restartCount} 次`);
	if (daemon.detached) meta.push("已分离");
	else if (daemon.persist) meta.push("持久");
	return meta;
}
...
function callMeta(args: LaunchRenderArgs): string[] {
	...
	switch (args.op) {
		case "start":
			...
			break;
		case "logs":
			if (args.follow) meta.push("跟踪");
			if (args.grep) meta.push(`grep /${args.grep}/`);
			break;
		case "wait":
			meta.push(args.pattern ? `匹配 /${args.pattern}/` : `等待 ${args.for ?? "exit"}`);
			break;
		case "send":
	...
	if (isError) {
		...
		switch (op) {
			case "start": {
				...
				if (daemon?.readyMatch) body.push(theme.fg("dim", `日志已匹配: ${replaceTabs(daemon.readyMatch)}`));
				...
				if (details?.timedOut) {
					...
					body.push(theme.fg("warning", "在观察到就绪状态之前进程已退出。"));
				}
				break;
			}
			case "send":
			...
			case "wait": {
				meta.push(...callMeta(params));
				if (daemon) meta.push(...daemonMeta(daemon, theme));
				if (details?.matched) body.push(theme.fg("dim", `已匹配: ${replaceTabs(details.matched)}`));
				if (details?.timedOut) {
			...
			case "logs": {
				if (details?.state) meta.push(theme.fg(stateColor(details.state), daemonStateLabel(details.state)));
				if (details?.cursor !== undefined) meta.push(`cursor ${details.cursor}`);
				if (details?.timedOut) meta.push(theme.fg("warning", "跟踪超时"));
			...
			case "describe": {
				...
				if (spec) {
					...
					body.push(theme.fg("dim", `工作目录 ${shortenPath(spec.cwd)}`));
					const flags = [`pty ${spec.pty}`, `restart ${spec.restart}`];
					if (spec.detached) flags.push("已分离");
					else if (spec.persist) flags.push("持久");
		...
		(width, expanded) => {
			...
			if (!expanded && op === "list" && body.length > PREVIEW_LIMITS.COLLAPSED_ITEMS) {
				...
				visible = [
					...
					theme.fg("dim", `… 还有 ${remaining} 个进程 ${formatExpandHint(theme, false, true)}`),
				];
			}
