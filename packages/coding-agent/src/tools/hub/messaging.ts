...
import { formatAge, formatDuration } from "@wxyhgk/pi-utils";
...
export function formatIncoming(msg: IrcMessage): string {
	const replyTag = msg.replyTo ? ` (回复 ${msg.replyTo})` : "";
...
export async function executeList(
	...
	const lines: string[] = [];
	if (peers.length === 0) {
		lines.push("没有其他 Agent。");
	} else {
		lines.push(`${peers.length} 个对等 Agent:`);
		for (const peer of peers) {
			const extras = [
				peer.activity || undefined,
				peer.unread > 0 ? `未读 ${peer.unread}` : undefined,
				peer.parentId ? `父级 ${peer.parentId}` : undefined,
				`${formatDuration(Date.now() - peer.lastActivity)} 前活跃`,
			].filter(Boolean);
			lines.push(`- ${peer.id} [${peer.displayName} · ${peer.kind} · ${peer.status}] — ${extras.join(", ")}`);
		}
		if (peers.some(peer => peer.status === "parked")) {
			lines.push("");
			lines.push("当你给已暂停的 Agent 发消息时,它们会被自动唤醒。");
		}
	}
	return {
...
export async function executeSend(
	...
	if (!to) {
		return hubErrorResult('op="send" 需要提供 `to`。', { op: "send", from: senderId });
	}
	if (!message) {
		return hubErrorResult('op="send" 需要提供 `message`。', { op: "send", from: senderId });
	}
	if (to === senderId) {
		return hubErrorResult("不能给自己发送消息。", { op: "send", from: senderId, to });
	}
	const isBroadcast = to === "all";
	if (isBroadcast && params.await) {
		return hubErrorResult('`await` 与 to:"all" 一起使用无效 —— 广播没有单一的回复者。', {
			op: "send",
			from: senderId,
			to,
		});
	...
	try {
		...
		if (targets.length === 0) {
			lines.push("没有在线的对等 Agent 可广播。");
		} else if (delivered.length === 0) {
			lines.push("没有收件人收到消息。");
		} else {
			lines.push(`已投递给 ${delivered.length} 个对等 Agent:`);
		}
		for (const receipt of receipts) {
			lines.push(
				receipt.outcome === "failed"
					? `- ${receipt.to}: 失败 —— ${receipt.error ?? "未知错误"}`
		...
		if (params.await && waiting && timeoutMs !== undefined) {
			...
			if (delivered.length > 0) {
				...
				if (reply.error) {
					...
					if (waited) {
						lines.push(`来自 ${waited.from} 的回复:`);
						lines.push(waited.body);
					} else {
						lines.push(
							`${formatDuration(timeoutMs)} 内未收到来自 ${to} 的回复。` +
								"对方可能稍后回复 —— 请再次检查 `inbox` 或 `wait`。",
						);
					}
				}
			} else {
...
export async function executeMessageWait(
	...
	try {
		...
		if (!waited) {
			const text = from
				? `在 ${formatDuration(timeoutMs)} 内没有来自 ${from} 的消息。`
				: `在 ${formatDuration(timeoutMs)} 内没有消息。`;
			return {
...
function peerStatusBadge(status: string, theme: Theme): string {
	switch (status) {
		...
		case "parked":
			return theme.fg("muted", `${theme.status.shadowed} 已暂停`);
		default:
...
function renderSendResult(
	...
	if (receipts.length === 0) {
		const text = textContent(result) || (result.isError ? "发送失败。" : "没有可投递的内容。");
		return [
	...
	if (receipts.length === 1) {
		...
		if (delivered.length > 0) meta.push(theme.fg("success", `${delivered.length} 已投递`));
		if (failedCount > 0) meta.push(theme.fg("error", `${failedCount} 失败`));
	}
	if (timedOut) meta.push(theme.fg("warning", "无回复"));
	...
	if (waited) {
		...
		lines.push(`  ${theme.fg("warning", "尚无回复 —— 对方可能稍后回复;请再次检查收件箱或等待。")}`);
	}
	return lines;
}
...
function renderWaitResult(
	...
	const waited = details.waited;
	if (!waited) {
		const text = textContent(result) || "没有消息到达。";
		return [
			renderStatusLine(
				{ icon: "warning", title: `IRC ${theme.nav.back} ${args?.from?.trim() || "任何人"}`, meta: ["已超时"] },
				theme,
			),
...
function renderListResult(details: Partial<CoordinationDetails>, expanded: boolean, theme: Theme): string[] {
	...
	if (peers.length === 0) {
		return [renderStatusLine({ icon: "info", title: "IRC 对等 Agent", meta: ["没有其他 Agent"] }, theme)];
	}
	...
	const meta = [...counts].map(([status, count]) => `${count} ${statusLabels[status] ?? status}`);
	const unreadTotal = peers.reduce((sum, peer) => sum + peer.unread, 0);
	if (unreadTotal > 0) meta.push(theme.fg("warning", `${unreadTotal} 条未读`));
	const header = renderStatusLine({ iconOverride: ircGlyph(theme), title: "IRC 对等 Agent", meta }, theme);
	const items = renderTreeList(
		{
			...
			itemType: "peer",
			renderItem: peer => {
				const kindText = peer.parentId ? `${peer.kind}${theme.sep.dot}的 ${peer.parentId}` : peer.kind;
