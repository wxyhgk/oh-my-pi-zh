...
import { isRecord, prompt, sanitizeText } from "@wxyhgk/pi-utils";
...
import { formatErrorDetail, formatMoreItems, PREVIEW_LIMITS, replaceTabs } from "./render-utils";
...
const InitListEntry = type({
	phase: type("string").describe("阶段名称"),
	items: type("string").describe("任务内容").array().atLeastLength(1).describe("此阶段的任务"),
});

const todoSchema = type({
	op: TodoOp,
	"list?": InitListEntry.array().describe("分阶段任务列表(init)"),
	"task?": type("string").describe("任务内容"),
	"phase?": type("string").describe("阶段名称"),
	...
	"items?": type("string").describe("任务内容").array().describe("要追加的任务"),
	"reason?": type("string").describe("阻塞原因说明(block 操作)"),
}).describe("应用单个 todo 操作");
...
export function selectCollapsedTodos<T extends { status: TodoStatus }>(
	...
	if (active.length > cap) {
		...
		return {
			...
			summary: `… 还有 ${hiddenActive} 个进行中的任务`,
		};
	}
	...
	return { items, summary: hidden > 0 ? formatMoreItems(hidden, "任务") : "" };
}
...
function resolveTaskOrError(
	...
	if (!hit) {
		if (/^task-\d+$/.test(content)) {
			...
			errors.push(`任务 "${content}" 未找到${hint}`);
		}
	}
	return hit;
}
...
function resolvePhaseOrError(phases: TodoPhase[], name: string | undefined, errors: string[]): TodoPhase | undefined {
	...
	if (!phase) errors.push(`阶段 "${name}" 未找到`);
	return phase;
}
...
function appendItems(phases: TodoPhase[], entry: TodoOpEntryValue, errors: string[]): TodoPhase[] {
	if (!entry.phase) {
		errors.push("缺少 append 操作的阶段名称");
		return phases;
	}
	if (!entry.items || entry.items.length === 0) {
		errors.push("缺少 append 操作的任务");
		return phases;
	}
	...
	switch (entry.op) {
		...
		case "block": {
			if (!entry.task && !entry.phase) {
				errors.push("block 操作需要指定任务或阶段目标");
				return phases;
			}
		...
		case "unblock": {
			if (!entry.task && !entry.phase) {
				errors.push("unblock 操作需要指定任务或阶段目标");
				return phases;
			}
			for (const task of getTaskTargets(phases, entry, errors)) {
...
function resolveTodoParams(raw: unknown, hasExistingPhases: boolean): TodoOpEntryValue | string {
	...
	return `无效的 todo 参数: ${direct.summary}`;
}
...
export function markdownToPhases(md: string): { phases: TodoPhase[]; errors: string[] } {
	...
	for (let lineNum = 0; lineNum < lines.length; lineNum++) {
		...
		if (taskMatch) {
			if (!currentPhase) {
				currentPhase = { name: "任务", tasks: [] };
				phases.push(currentPhase);
			}
			const marker = taskMatch[1];
			const status = MARKER_TO_STATUS[marker];
			if (!status) {
				errors.push(`第 ${lineNum + 1} 行:未知状态标记 "[${marker}]"(请使用 [ ]、[x]、[/]、[-]、[!])`);
				continue;
			}
		...
		errors.push(`第 ${lineNum + 1} 行:无法识别的语法 "${trimmed}"`);
	}
...
function formatSummary(phases: TodoPhase[], errors: string[], readOnly = false): string {
	...
	if (tasks.length === 0) {
		if (errors.length > 0) return `错误: ${errors.join("; ")}`;
		return readOnly ? "任务列表为空。" : "任务列表已清空。";
	}
	...
	const lines: string[] = [];
	if (errors.length > 0) lines.push(`错误: ${errors.join("; ")}`);
	if (remainingTasks.length === 0) {
		lines.push("剩余任务:无。");
	} else {
		lines.push(`剩余任务(${remainingTasks.length}):`);
	...
	lines.push(
		`总体: ${closedAll}/${tasks.length} 已完成,${remainingTasks.length} 未完成${blockedAll > 0 ? `,${blockedAll} 被阻塞` : ""}。`,
	);
	lines.push(
		`当前阶段 ${currentIdx + 1}/${phases.length} "${current.name}" (${done}/${current.tasks.length})${
			workedAhead
				? " —— 最早仍有未完成任务阶段的阶段;进行中指针在每次完成任务后自动前移到最早的未完成任务,因此它可能位于乱序完成的工作之后(没有任何任务被恢复为未完成)。"
				: "。"
		}`,
	);
	for (const phase of phases) {
...
export class TodoTool implements AgentTool<typeof todoSchema, TodoToolDetails> {
	...
	readonly label = "任务";
	readonly summary = "写入结构化的任务列表,以在会话中跟踪进度";
...
function formatTodoLine(
	...
	switch (item.status) {
		...
		case "blocked": {
			const note = item.blocker ? `被阻塞: ${forDisplay(item.blocker)}` : "被阻塞";
...
export const todoToolRenderer = {
	renderCall(args: TodoRenderArgs, options: RenderResultOptions, uiTheme: Theme): Component {
		...
		const header = renderStatusLine(
			{ icon: "pending", spinnerFrame: options?.spinnerFrame, title: "任务", meta: ops },
			uiTheme,
		);
	...
	renderResult(
		...
		if (result.isError) {
			const errorText = result.content?.find(content => content.type === "text")?.text ?? "任务操作失败";
			const header = renderStatusLine({ icon: "error", title: "任务" }, uiTheme);
		...
		if (allTasks.length === 0) {
			...
			const fallback = forDisplay(result.content?.find(content => content.type === "text")?.text ?? "暂无任务");
