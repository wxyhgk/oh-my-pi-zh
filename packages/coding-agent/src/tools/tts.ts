...
import { type ApiKey, withAuth } from "@wxyhgk/pi-ai";
...
async function synthesizeXai(
	...
	if (!creds) {
		return {
			...
			content: [
				{
					type: "text",
					text: "没有 xAI 凭据。请运行 /login → xAI Grok OAuth(SuperGrok 或 X Premium+),或设置 XAI_API_KEY。",
				},
			],
		};
	}
		...
		response = await withAuth(
			...
			async key => {
				...
				if (!resp.ok) {
					...
					throw new ProviderHttpError(`xAI TTS 失败(${resp.status}):${detail.slice(0, 300)}`, resp.status, {
	...
	return {
		content: [
			{
				type: "text",
				text: `已将 ${bytes.length} 字节保存到 ${displayPath}(voice=${voiceId}, codec=${codec}, backend=xai)。`,
			},
		],
...
async function synthesizeLocal(
	...
	if (!audio) {
		return {
			...
			content: [
				{
					type: "text",
					text: `本地 TTS 合成失败(model=${modelKey})。设备端 worker 可能不可用,或模型下载被中断。`,
				},
			],
		};
	}
	...
	const note = substituted ? "未内置本地 MP3 编码器,因此写入的是 WAV(PCM16)而非请求的容器格式。" : "";
	return {
		content: [
			{
				type: "text",
				text: `已将 ${wav.length} 字节保存到 ${displayPath}(voice=${modelKey}/${voice}, codec=wav, backend=local, ${audio.sampleRate} Hz)。${note}`,
			},
		],
...
export const ttsTool: CustomTool<typeof ttsSchema, TtsToolDetails> = {
	...
	approval: "write",
	description:
		"根据文本生成语音音频文件并写入 output_path。两个后端,由 providers.tts 设置(auto|local|xai)选择: " +
		`local = 设备端神经 TTS(Kokoro-82M,通过内置 ONNX 运行时,无需网络,输出始终为 WAV/PCM16;语音由 tts.localVoice 设置指定 — ${KOKORO_VOICES.map(v => (v.id === DEFAULT_TTS_VOICE ? `${v.id} (default)` : v.id)).join(", ")}); ` +
		`xai = xAI Grok Voice 云端(内置语音:${formatVoiceList()};接受自定义语音 ID;MP3 或 WAV)。` +
		"auto 优先使用 local,但存在凭据时将 .mp3 请求路由到 xAI(只有云端路径能输出 MP3);" +
		"否则 .mp3 路径会写成同目录的 .wav。xAI 的编码格式由 output_path 后缀推断。" +
		`最长 ${XAI_MAX_TEXT_LENGTH.toLocaleString("en-US")} 个字符。`,
