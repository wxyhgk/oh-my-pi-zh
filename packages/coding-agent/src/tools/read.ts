...
import { formatHashlineHeader, formatNumberedLine, formatNumberedLines } from "@wxyhgk/hashline";
...
import { type SummaryResult, summarizeCode } from "@wxyhgk/pi-natives";
...
async function readHashlineHeaderContext(
	...
	if (!context) throw new ToolError(`无法为非绝对路径记录 hashline 快照:${absolutePath}`);
	return context;
}
...
function formatSummaryElisionFooter(
	...
	const tail = elidedRanges.length > sampleCount ? `,例如 ${example}` : `,可配合 ${example}`;
	return `[…已省略 ${elidedLines} 行;如需重新读取,请使用所需范围${tail}]`;
}
...
function prependSuffixResolutionNotice(text: string, suffixResolution?: { from: string; to: string }): string {
	...
	const notice = `[路径 '${suffixResolution.from}' 未找到;已通过后缀匹配解析为 '${suffixResolution.to}']`;
	return text ? `${notice}\n${text}` : notice;
}
...
function invalidSelector(sel: string): ToolError {
	return new ToolError(
		`选择器 ':${sel}' 无效。请使用 :N、:N-M、:N+K、:N-(开放区间)、逗号分隔的范围列表、:raw,或将范围与 raw 组合(如 :raw:50-100)。`,
	);
}
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	async #tryReadDelimitedPaths(
		...
		const notice = `注意:已按 ${parts.length} 个路径解释:${parts.join(", ")}`;
		const notes = [notice];
		...
		for (const part of parts) {
			try {
				...
				const errorNote = `无法读取 ${part}:${message}`;
	...
	async #extractPdfImages(snapshot: PdfImageSnapshot, imageDir: string, signal: AbortSignal): Promise<string> {
		...
		try {
			...
			if (!result.ok) {
				throw new ToolError(`无法从 PDF 提取图片:${result.error ?? "转换失败"}`);
			}
	...
	async #readPdfImageMember(
		...
		if (member.length === 0) {
			const text =
				members.length === 0
					? "未找到可提取的 PDF 图片成员。"
					: `可提取的 PDF 图片成员:\n${members
							.map(imageMember => `- 读取 \`${pdfDisplayPath}:${imageMember}\``)
							.join("\n")}`;
		...
		if (!members.includes(member)) {
			...
			throw new ToolError(`未找到 PDF 图片成员 '${member}'。可用成员:${available}`);
		}
		...
		if (imageStat.size > MAX_IMAGE_SIZE) {
			...
			throw new ToolError(`图片文件过大:${sizeStr} 超过 ${maxStr} 的限制。`);
		}
		const metadata = await readImageMetadata(imagePath);
		const mimeType = metadata?.mimeType;
		if (!mimeType) throw new ToolError(`PDF 图片成员 '${member}' 不是受支持的图片。`);
		...
		if (!imageInput) {
			throw new ToolError(`读取图片文件 [${mimeType}] 失败:不支持的图片格式。`);
		}
	...
	async #loadImageContent(options: {
		...
		if (this.syncInspectImageState()) {
			...
			const metadataLines = [
				"图片元数据:",
				`- MIME: ${outputMime}`,
				`- 字节数:${fileSize} (${formatBytes(fileSize)})`,
				imageMetadata?.width !== undefined && imageMetadata.height !== undefined
					? `- 尺寸:${imageMetadata.width}x${imageMetadata.height}`
					: "- 尺寸:未知",
				imageMetadata?.channels !== undefined ? `- 通道:${imageMetadata.channels}` : "- 通道:未知",
				imageMetadata?.hasAlpha === true
					...
					: imageMetadata?.hasAlpha === false
						...
						: "- Alpha:未知",
				"",
				`如需分析图片,请调用 inspect_image,传入 path="${formatPathRelativeToCwd(
					absolutePath,
					this.session.cwd,
				)}" 并说明要检查的内容以及期望的输出格式。`,
			];
	...
	#buildInMemoryTextResult(
		...
		if (requestedStart >= allLines.length) {
			const suggestion =
				allLines.length === 0
					? `该 ${options.entityLabel} 为空。`
					: `请使用 :1 从开头读取,或使用 :${allLines.length} 读取最后一行。`;
			return resultBuilder
				.text(
					`第 ${requestedStart + 1} 行超出${options.entityLabel}的末尾(共 ${allLines.length} 行)。${suggestion}`,
				)
				.done();
		}
		...
		if (truncation.firstLineExceedsLimit) {
			...
			if (shouldAddHashLines) {
				outputText = `[第 ${startLineDisplay} 行大小为 ${formatBytes(
					firstLineBytes,
				)},超过 ${formatBytes(DEFAULT_MAX_BYTES)} 的限制。哈希行输出需要完整行;无法为被截断的行生成可编辑的编号预览。]`;
			} else {
			...
			if (snippet.text.length === 0) {
				outputText = `[第 ${startLineDisplay} 行大小为 ${formatBytes(
					firstLineBytes,
				)},超过 ${formatBytes(DEFAULT_MAX_BYTES)} 的限制。无法显示有效的 UTF-8 片段。]`;
			}
			...
			outputText += `\n\n[${options.entityLabel}中还有 ${remaining} 行。请使用 :${nextOffset} 继续]`;
		} else {
			if (options.raw === true) {
	...
	#buildInMemoryMultiRangeResult(
		...
		for (const range of outOfBounds) {
			...
			notices.push(`[范围 ${bound} 超出${options.entityLabel}的末尾(共 ${totalLines} 行);已跳过]`);
		}
		const finalText =
	...
	async #readLocalFileMultiRange(
		...
		if (bridgePromise !== undefined) {
			try {
				...
				logger.warn("ACP fs readTextFile 失败;回退到磁盘读取", { path: absolutePath, error });
			}
		}
		...
		for (const range of ranges) {
			...
			if (rangeStart >= totalFileLines) {
				const bound = range.endLine !== undefined ? `${range.startLine}-${range.endLine}` : `${range.startLine}`;
				notices.push(`[范围 ${bound} 超出文件末尾(共 ${totalFileLines} 行);已跳过]`);
				continue;
			}
	...
	async #readArchive(
		...
		if (!node) {
			throw new ToolError(`归档中未找到路径 '${readPath}'`);
		}

		if (node.isDirectory) {
			if (isMultiRange(sel)) {
				throw new ToolError("归档目录列表不支持多范围行选择器。");
			}
			...
			return toolResult<ReadToolDetails>(details)
				.text(
					prependSuffixResolutionNotice(
						`[无法读取二进制归档条目 '${entry.path}' (${formatBytes(entry.size)})]`,
	...
	async #readSqlite(
		...
		try {
			...
			switch (selector.kind) {
				...
				case "schema": {
					...
					if (sampleRows.rows.length < sampleRows.totalCount) {
						...
						output += `\n[还有 ${remaining} 行;请在数据库路径后追加 :${selector.table}?limit=20&offset=${sampleRows.rows.length} 以继续]`;
					}
					return toolResult<ReadToolDetails>(details)
				...
				case "row": {
					...
					if (!row) {
						return toolResult<ReadToolDetails>(details)
							.text(
								prependSuffixResolutionNotice(
									`在表 '${selector.table}' 中未找到键 '${selector.key}' 对应的行。`,
									resolvedSqlitePath.suffixResolution,
								),
							)
							.sourcePath(resolvedSqlitePath.absolutePath)
				...
				case "raw": {
					...
					if (result.truncated) {
						output += `\n[输出上限为 ${MAX_RAW_QUERY_ROWS} 行;请在查询中添加 LIMIT/OFFSET 子句以分页查看更多]`;
					}
					return toolResult<ReadToolDetails>(details)
						.text(prependSuffixResolutionNotice(output, resolvedSqlitePath.suffixResolution))
						.sourcePath(resolvedSqlitePath.absolutePath)
			...
			throw new ToolError("不支持的 SQLite 选择器");
		} catch (error) {
	...
	async execute(
		...
		if (conflictUri) {
			if (conflictUri.id === "*") {
				throw new ToolError(
					"不支持读取 `conflict://*` — 通配符仅用于写入。如需查看文件中完整的冲突列表,请使用 `<path>:conflicts` 读取选择器,或读取 `conflict://<N>` 检查单个冲突块。",
				);
			}
		...
		if (parsedUrlTarget) {
			if (!this.session.settings.get("fetch.enabled")) {
				throw new ToolError("URL 读取已被设置禁用。");
			}
		...
		if (internalRouter.canResolve(readPath)) {
			...
			if (internalTarget.sel !== undefined && parsed.kind === "none") {
				throw new ToolError(
					`'${internalTarget.path}' 上的选择器 ':${internalTarget.sel}' 无效。请使用 :N、:N-M、:N+K、:N-(开放区间)、逗号分隔的范围列表、:raw,或将范围与 raw 组合(如 :raw:50-100)。`,
				);
			}
		...
		if (!rawPathIsLiteral) {
			...
			if (pdfImageMemberPath) {
				...
				try {
					...
					if (!suffixMatch) throw new ToolError(`未找到路径 '${pdfImageMemberPath.pdfPath}'`);
		...
		try {
			...
			if (isNotFoundError(error)) {
				...
				if (!recoveredApprovedPlan && !suffixResolution) {
					...
					throw new ToolError(`未找到路径 '${localReadPath}'`);
				}
			} else {
				throw error;
			}
		...
		if (isDirectory) {
			if (isMultiRange(parsed)) {
				throw new ToolError("目录列表不支持多范围行选择器。");
			}
		...
		if (mimeType) {
			...
			if (result.ok) {
				...
				content = [{ type: "text", text: `[无法读取 ${ext} 文件:${result.error || "转换失败"}]` }];
			} else {
				content = [{ type: "text", text: `[无法读取 ${ext} 文件:转换失败]` }];
			}
		} else {
			...
			if (!isRawSelector(parsed) && (await isProbablyBinary(absolutePath))) {
				return toolResult<ReadToolDetails>({ resolvedPath: absolutePath, suffixResolution })
					.text(
						prependSuffixResolutionNotice(
							`[无法读取二进制文件 '${formatPathRelativeToCwd(absolutePath, this.session.cwd)}' (${formatBytes(fileSize)});不是有效的 UTF-8 文本。如需按原样读取字节,请使用 ':raw'。]`,
							suffixResolution,
						),
					)
					.sourcePath(absolutePath)
				...
				if (isMultiRange(parsed) && parsed.kind === "lines") {
					...
					if (truncation.firstLineExceedsLimit) {
						...
						if (shouldAddHashLines) {
							outputText = `[第 ${startLineDisplay} 行大小为 ${formatBytes(
								firstLineBytes,
							)},超过 ${formatBytes(maxBytesForRead)} 的限制。哈希行输出需要完整行;无法为被截断的行生成可编辑的编号预览。]`;
						} else {
							outputText = formatText(snippet.text, startLineDisplay);
						}
						if (snippet.text.length === 0) {
							outputText = `[第 ${startLineDisplay} 行大小为 ${formatBytes(
								firstLineBytes,
							)},超过 ${formatBytes(maxBytesForRead)} 的限制。无法显示有效的 UTF-8 片段。]`;
						}
						details = { truncation };
						sourcePath = absolutePath;
						truncationInfo = {
						...
						outputText += reachedEof
							? `\n\n[文件中还有 ${totalFileLines - (startLine + userLimitedLines)} 行。请使用 :${nextOffset} 继续]`
							: `\n\n[文件中还有更多行(共 ${formatBytes(fileSize)},未扫描到文件末尾)。请使用 :${nextOffset} 继续]`;
						details = {};
						sourcePath = absolutePath;
					} else {
	...
	#formatArtifactWorkflowNotice(artifact: ResolvedArtifactFile, artifactUrl: string): string {
		...
		return `产物存储:${displayPath} (${formatBytes(artifact.size)})。请使用 ${artifactUrl}:N-M 分页,${artifactUrl}:raw:N-M 按原样读取块,产物文件路径可用于搜索/复制工作流。`;
	}

	#formatRawArtifactBlockedNotice(artifact: ResolvedArtifactFile, artifactUrl: string): string {
		const displayPath = shortenPath(artifact.path);
		return `已阻止对 ${artifactUrl} (${formatBytes(
			artifact.size,
		)}) 的无限原样读取。按原样读取整个产物可能耗尽内存。请使用 ${artifactUrl}:raw:1-3000 读取有界的原样块,使用 ${artifactUrl}:1-3000 进行带行号的浏览,产物文件路径可用于搜索/复制工作流:${displayPath}`;
	}
	...
	async #readArtifactFile(
		...
		if (requestedStart >= totalFileLines) {
			const suggestion =
				totalFileLines === 0
					? "产物为空。"
					: `请使用 ${artifactUrl}:1 从开头读取,或使用 ${artifactUrl}:${totalFileLines} 读取最后一行。`;
			return toolResult<ReadToolDetails>(details)
				.text(`第 ${requestedStart + 1} 行超出产物末尾(共 ${totalFileLines} 行)。${suggestion}`)
				.sourcePath(artifact.path)
				.sourceInternal(url.href)
				.done();
		}
		...
		if (truncation.firstLineExceedsLimit) {
			...
			outputText =
				snippet.text.length > 0
					...
					: `[第 ${startLineDisplay} 行大小为 ${formatBytes(
							firstLineBytes,
						)},超过 ${formatBytes(maxBytesForRead)} 的限制。无法显示有效的 UTF-8 片段。]`;
			truncationInfo = {
				result: truncation,
			...
			if (truncation.truncated) {
				...
				outputText += reachedEof
					? `\n\n[产物中还有 ${totalFileLines - (startLine + collectedLines.length)} 行。请使用 ${artifactUrl}:${nextOffset} 继续]`
					: `\n\n[产物中还有更多行(共 ${formatBytes(artifact.size)},未扫描到末尾)。请使用 ${artifactUrl}:${nextOffset} 继续]`;
			}
		}
	...
	async #handleInternalUrl(
		...
		if (hasExtraction && parsedSel.kind !== "none" && parsedSel.kind !== "raw") {
			throw new ToolError("不能将查询提取与行选择器组合使用");
		}
		...
		const resource = await internalRouter.resolve(url, {
			...
			xd: {
				read: async name => {
					...
					const xdev = this.session.xdev;
					if (!xdev) throw new ToolError("当前会话未挂载 xd://。");
	...
	async #readDirectory(
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new ToolError(`无法读取目录:${message}`);
		}
		throwIfAborted(signal);

		const output = tree.totalLines <= 1 ? "(空目录)" : tree.rendered;
		const details: ReadToolDetails = {
			isDirectory: true,
		...
		if (wantsSlice) {
			...
			if (end < allLines.length) {
				...
				text += `\n\n[列表中还有 ${remaining} 行。请使用 :${end + 1} 继续]`;
			}
...
export const readToolRenderer = {
	renderCall(args: ReadRenderArgs, _options: RenderResultOptions, uiTheme: Theme): Component {
		...
		const text = renderStatusLine({ icon: "pending", title: "读取", description: pathDisplay }, uiTheme);
	...
	renderResult(
		...
		if (details?.resolvedPath) {
			warningLines.push(uiTheme.fg("dim", wrapBrackets(`解析路径:${details.resolvedPath}`, uiTheme)));
		}
		if (truncation) {
			if (fallback?.firstLineExceedsLimit) {
				let warning = `首行超过 ${formatBytes(fallback.outputBytes ?? fallback.totalBytes)} 限制`;
		...
		if (imageContent) {
			...
			const correction = suffix ? ` ${uiTheme.fg("dim", `(已从 ${shortenPath(suffix.from)} 修正)`)}` : "";
			const header = renderStatusLine(
				{ icon: suffix ? "warning" : "success", title: "读取", description: `${displayPath}${correction}` },
				uiTheme,
			);
					...
					outputBlock.render(
						{
							...
							sections: [
								{
									label: uiTheme.fg("toolTitle", "详情"),
									lines: lines.length > 0 ? lines : [uiTheme.fg("dim", "(图片)")],
								},
							],
							width,
						},
		...
		const correction = suffix ? ` ${uiTheme.fg("dim", `(已从 ${shortenPath(suffix.from)} 修正)`)}` : "";
		let title = displayPath ? `读取 ${displayPath}${correction}` : "读取";
		if (args?.offset !== undefined || args?.limit !== undefined) {
			const startLine = args.offset ?? 1;
			const endLine = args.limit !== undefined ? startLine + args.limit - 1 : "";
			title += `:${startLine}${endLine ? `-${endLine}` : ""}`;
