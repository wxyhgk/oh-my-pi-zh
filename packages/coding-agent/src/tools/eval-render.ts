...
function formatStatusEvent(event: EvalStatusEvent, theme: Theme): string {
	...
	switch (op) {
		...
		case "write":
			parts.push(`${data.chars ?? data.bytes ?? 0} 字符`);
			if (data.path) parts.push(`写入 ${shortenPath(String(data.path))}`);
			break;
		case "cat":
		...
		case "git_status":
			if (data.clean) {
				...
				if (data.staged) statusParts.push(`已暂存 ${data.staged} 项`);
				if (data.modified) statusParts.push(`已修改 ${data.modified} 项`);
				if (data.untracked) statusParts.push(`未跟踪 ${data.untracked} 项`);
		...
		case "completion":
			...
			break;
		case "wc":
			parts.push(`${data.lines} 行 ${data.words} 词 ${data.chars} 字符`);
			break;
		case "cd":
		case "pwd":
		case "mkdir":
...
function formatStatusEventExpanded(event: EvalStatusEvent, theme: Theme): string[] {
	...
	const addItems = (items: unknown[], formatter: (item: unknown) => string, max = 5) => {
		...
		if (arr.length > max) {
			lines.push(`   ${theme.fg("dim", `… 还有 ${arr.length - max} 条`)}`);
		}
	};
	...
	const addPreview = (preview: string, maxLines = 3) => {
		...
		if (totalLines > maxLines) {
			lines.push(`   ${theme.fg("dim", `… 还有 ${totalLines - maxLines} 行`)}`);
		}
	};
	...
	renderResult(
		...
		const timeoutLine =
			typeof timeoutSeconds === "number"
				? uiTheme.fg("dim", wrapBrackets(`超时: ${timeoutSeconds} 秒`, uiTheme))
				: undefined;
		...
		if (!combinedOutput && statusLines.length > 0) {
			const lines = [uiTheme.fg("dim", "状态"), ...statusLines, timeoutLine, noticeLine, warningLine].filter(
				Boolean,
			) as string[];
			return new Text(lines.join("\n"), 0, 0);
		}
			...
			render: (width: number): readonly string[] => {
				...
				if (cachedSkipped && cachedSkipped > 0) {
					...
					const skippedLine = uiTheme.fg(
						"dim",
						`… (之前还有 ${cachedSkipped} 行,显示 ${cachedLines.length}/${cachedSkipped + cachedLines.length}) (ctrl+o 展开)`,
					);
