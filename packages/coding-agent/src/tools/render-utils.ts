...
import type { ToolCallContext } from "@wxyhgk/pi-agent-core";
...
export { Ellipsis } from "@wxyhgk/pi-natives";
export { replaceTabs, truncateToWidth, wrapTextWithAnsi } from "@wxyhgk/pi-tui";
...
export { formatAge, formatBytes, formatCount, formatDuration, pluralize } from "@wxyhgk/pi-utils";
...
export function formatMoreItems(remaining: number, itemType: string): string {
	...
	return `… 还有 ${safeRemaining} 个${itemType}`;
}
...
export function capPreviewLines(
	...
	const marker = `… 前面还有 ${hidden} 行${hint ? ` ${hint}` : ""}`;
...
export function formatDiffStats(added: number, removed: number, hunks: number, theme: Theme): string {
	...
	if (hunks > 0) parts.push(theme.fg("dim", `${hunks} 个差异块`));
...
export function formatScreenshot(opts: {
	...
	if (opts.resized.decodeFailed) {
		lines.push("调整大小: 图像解码失败,已改用原始图像字节");
	}
...
export function appendParseErrorsBulletList(
	...
	if (fullCount > capped.length) {
		lines.push(theme.fg("dim", `  … 还有 ${fullCount - capped.length} 条`));
	}
}
