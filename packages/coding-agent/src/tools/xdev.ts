 ...
 */
import type { AgentToolContext, AgentToolResult, AgentToolUpdateCallback, ToolLoadMode } from "@wxyhgk/pi-agent-core";
import { type Tool as AiTool, jsonSchemaToTypeScript, toolWireSchema, validateToolArguments } from "@wxyhgk/pi-ai";
import { type Component, Container, Text } from "@wxyhgk/pi-tui";
import { parseStreamingJson } from "@wxyhgk/pi-utils";
...
function renderDocs(inst: Tool, heading = "#", descriptionCap?: number): string {
	...
	if (descriptionCap !== undefined && description.length > descriptionCap) {
		description = `${description.slice(0, descriptionCap).trimEnd()}…(完整文档:读取 ${XD_URL_PREFIX}${inst.name})`;
	}
	return [
		...
		`type Args = ${schema};`,
		"```",
		`通过向 ${XD_URL_PREFIX}${inst.name} 写入 JSON 来执行。`,
	].join("\n");
}
...
function parseDeviceArgs(
	...
	try {
		...
		throw new ToolError(
			`${XD_URL_PREFIX}${device.name} 期望 content 为 JSON 参数对象(${error instanceof Error ? error.message : String(error)})。写入 \`?\` 获取文档。`,
		);
	}
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
		throw new ToolError(
			`${XD_URL_PREFIX}${device.name} 的 content 必须是 JSON 对象,实际为 ${Array.isArray(parsed) ? "数组" : typeof parsed}。`,
		);
	}
	...
	try {
		...
		throw new ToolError(`${XD_URL_PREFIX}${device.name} 的参数无效:${message}\n\n${docs()}`);
	}
}
...
export function xdevListing(state: XdevState): string {
	...
	return [
		`${XD_URL_PREFIX} 已挂载 ${state.mountedNames.size} 个工具设备。`,
		...rows,
		"",
		`读取 ${XD_URL_PREFIX}<tool> 获取文档与 JSON schema;将 JSON 参数对象写入 ${XD_URL_PREFIX}<tool> 即可执行。已激活的顶层工具也接受相同的分发方式。`,
	].join("\n");
}
	...
	if (overflow.length > 0) {
		sections.push(
			[
				...
				"",
				`首次使用前,读取 ${XD_URL_PREFIX}<tool> 获取完整文档与 JSON schema。`,
			].join("\n"),
		);
	}
...
function resolveRequiredXdevTool(state: XdevState, name: string): Tool {
	...
	if (!inst) {
		throw new ToolError(
			`不存在此工具:${XD_URL_PREFIX}${name}。已挂载设备:${[...state.mountedNames].join(", ")}。已激活的顶层工具也可通过 ${XD_URL_PREFIX}<tool> 分发调用。`,
		);
	}
	return inst;
}
