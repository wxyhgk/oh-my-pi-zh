...
import { type ApiKey, type FetchImpl, getEnvApiKey, type Model, withAuth } from "@wxyhgk/pi-ai";
...
import { getAntigravityUserAgent } from "@wxyhgk/pi-catalog/wire/gemini-headers";
import {
	$env,
	isEnoent,
	parseImageMetadata,
...
const aspectRatioSchema = type.enumerated(...XAI_IMAGE_ASPECT_RATIOS).describe("宽高比");
const imageSizeSchema = type('"1024x1024" | "1536x1024" | "1024x1536"').describe("图像尺寸");

const inputImageSchema = type({
	"path?": type("string").describe("输入图像路径"),
	"data?": type("string").describe("base64 图像数据"),
	"mime_type?": type("string").describe("MIME 类型"),
});

const imageProviderSchema = type
	.enumerated(...IMAGE_PROVIDER_REQUEST_CHOICES)
	.describe("本次请求的图像提供商;覆盖 providers.imageOrder 设置(默认:使用该设置)");

export const imageGenSchema = type({
	subject: type("string").describe("主要主体"),
	"action?": type("string").describe("主体正在做什么"),
	"scene?": type("string").describe("地点或环境"),
	"composition?": type("string").describe("相机角度与取景"),
	"lighting?": type("string").describe("光照设置"),
	"style?": type("string").describe("艺术风格"),
	"text?": type("string").describe("要渲染的文本"),
	"changes?": type("string[]").describe("要进行的修改"),
	"aspect_ratio?": aspectRatioSchema,
	"image_size?": imageSizeSchema,
	"input?": inputImageSchema.array().describe("输入图像"),
...
async function loadImageFromUrl(
	...
	if (imageUrl.startsWith("data:")) {
		...
		if (!normalized.mimeType) {
			throw new Error("提供原始 base64 数据时必须指定 mime_type。");
		}
		if (!normalized.data) {
	...
	if (!response.ok) {
		...
		throw new Error(`图像下载失败(${response.status}): ${rawText}`);
	}
	const contentType = response.headers.get("content-type")?.split(";")[0];
	if (!contentType?.startsWith("image/")) {
		throw new Error(`URL 中的图像类型不受支持: ${imageUrl}`);
	}
...
function assertImageAspectRatioSupported(provider: ImageProvider, aspectRatio: ImageGenParams["aspect_ratio"]): void {
	...
	throw new Error(
		`宽高比 ${aspectRatio} 仅受 xAI 图像生成支持。请将 providers.image 设置为 xai,或使用以下宽高比之一: ${COMMON_IMAGE_ASPECT_RATIOS.join(", ")}。`,
	);
}
...
async function loadImageFromPath(imagePath: string, cwd: string): Promise<InlineImageData> {
	...
	try {
		...
		if (buffer.length > MAX_IMAGE_SIZE) {
			throw new Error(`图像文件过大: ${imagePath}`);
		}

		const metadata = parseImageMetadata(buffer);
		const mimeType = metadata?.mimeType;
		if (!mimeType) {
			throw new Error(`不支持的图像类型: ${imagePath}`);
		}

		return { data: buffer.toBase64(), mimeType };
	} catch (err) {
		if (isEnoent(err)) throw new Error(`找不到图像文件: ${imagePath}`);
		throw err;
	}
}
...
async function resolveInputImage(input: ImageInput, cwd: string): Promise<InlineImageData> {
	...
	throw new Error("input_images 条目必须包含 path 或 data。");
}
...
function buildResponseSummary(
	...
	const lines = [`提供商: ${provider}`, `模型: ${model}`, `生成了 ${imagePaths.length} 张图像:`];
...
async function parseOpenAIHostedImageSse(response: Response, signal?: AbortSignal): Promise<OpenAIHostedImageResult> {
	...
	for await (const event of readSseJson<OpenAISseEvent>(response.body, signal)) {
		if (event.type === "error") {
			const message = event.error?.message ?? event.message ?? "OpenAI 图像请求失败";
			throw new Error(message);
		}
		if (event.type === "response.failed") {
			const message = event.response?.error?.message ?? "OpenAI 图像请求失败";
			throw new Error(message);
		}
...
async function generateOpenAIHostedImage(
	...
	if (!response.ok) {
		...
		throw new ProviderHttpError(
			`OpenAI 图像请求失败(${response.status}): ${getOpenAIResponseErrorMessage(errorText)}`,
			response.status,
...
export const imageGenTool: CustomTool<typeof imageGenSchema, ImageGenToolDetails> = {
	...
	async execute(_toolCallId, params, _onUpdate, ctx, signal) {
		return untilAborted(signal, async () => {
			...
			for (const preferredProvider of providerOrder) {
				...
				try {
					...
					if (provider === "openai" || provider === "openai-codex") {
						if (!apiKey.model) {
							throw new Error("缺少用于 OpenAI 图像生成的活动 GPT 模型");
						}
						...
						if (parsed.images.length === 0) {
							...
							return {
								content: [{ type: "text", text: `未返回图像数据。${messageText}` }],
								details: {
									provider,
									model,
									imageCount: 0,
					...
					if (provider === "antigravity") {
						if (!apiKey.projectId) {
							throw new Error("antigravity 凭据中缺少 projectId");
						}
						...
						const response = await withAuth(
							...
							async key => {
								...
								for (let i = 0; i < endpoints.length; i++) {
									...
									try {
										...
										lastError = new ProviderHttpError(
											`Antigravity 图像请求失败(${resp.status}): ${message}`,
											resp.status,
								...
								if (!resp?.ok) {
									throw lastError ?? new Error("Antigravity 图像生成失败");
								}
					...
					if (provider === "xai") {
						if (!ctx.modelRegistry) {
							throw new Error("缺少用于 xAI 图像生成的 modelRegistry");
						}
						const xaiCreds = await resolveXAIHttpCredentials(ctx.modelRegistry, resolvedModel);
						if (!xaiCreds) {
							throw new Error(
								"没有 xAI 凭据。请运行 /login → xAI Grok OAuth(SuperGrok 或 X Premium+),或设置 XAI_API_KEY。",
							);
						}
						...
						if (isEdit && resolvedImages.length > XAI_MAX_EDIT_IMAGES) {
							throw new Error(
								`xAI 图像编辑最多接受 ${XAI_MAX_EDIT_IMAGES} 张参考图像;当前收到 ${resolvedImages.length} 张。`,
							);
						}
						...
						if (xaiInlineImages.length === 0) {
							return {
								content: [{ type: "text", text: "未返回图像数据。" }],
								details: {
									provider,
									model: resolvedModel,
									imageCount: 0,
						...
						const rawText = await withAuth(
							...
							async key => {
								...
								if (!resp.ok) {
									...
									throw new ProviderHttpError(
										`OpenRouter 图像请求失败(${resp.status}): ${message}`,
										resp.status,
										{ headers: resp.headers },
									);
								}
					...
					if (inlineImages.length === 0) {
						const blocked = data.promptFeedback?.blockReason
							? `已被拦截: ${data.promptFeedback.blockReason}`
							: "未返回图像数据。";
						return {
			...
			if (!foundCredentials) {
				throw new Error(
					"未找到图像 API 凭据。请连接 Codex(ChatGPT)订阅,使用带 OpenAI 凭据的 GPT Responses/Codex 模型,通过 google-antigravity 或 xAI Grok OAuth 登录,或设置 OPENAI_API_KEY、XAI_API_KEY、OPENROUTER_API_KEY、GEMINI_API_KEY 或 GOOGLE_API_KEY。",
				);
			}
			...
			throw new AggregateError(
				...
				`所有已配置凭据的提供商的图像生成均失败: ${failures.map(failure => failure.provider).join(", ")}`,
			);
		});
	},
};
