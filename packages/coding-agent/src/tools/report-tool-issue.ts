...
import type { FetchImpl } from "@wxyhgk/pi-ai";
import type { Component } from "@wxyhgk/pi-tui";
import { Text } from "@wxyhgk/pi-tui";
import { $env, $flag, getAutoQaDbPath, getInstallId, logger, VERSION } from "@wxyhgk/pi-utils";
...
export function reportIssueDeviceUsage(): string {
	return `以纯文本向 ${REPORT_ISSUE_DEVICE_PATH} 写入 \`<tool>: <简洁描述>\`。两行格式也可用:第一行写工具名,下面写报告正文。`;
}
...
function parseReportIssueBody(text: string): { tool: string; report: string } {
	...
	if (!body) {
		throw new ToolError(`报告为空。${reportIssueDeviceUsage()}`);
	}
	...
	throw new ToolError(`报告格式无效。${reportIssueDeviceUsage()}`);
}
...
function persistConsent(localSettings: Settings | undefined, granted: boolean): void {
	...
	try {
		...
		logger.warn("无法将 auto-QA 同意状态持久化到本地设置快照", { error: String(error) });
	}
	if (persistentConsentSettings && persistentConsentSettings !== localSettings) {
		try {
			...
			logger.warn("无法将 auto-QA 同意状态持久化到持久化设置", { error: String(error) });
		}
	}
}
...
export function openAutoQaDb(): Database | null {
	...
	try {
		...
		logger.warn("无法打开 auto-QA 数据库", { error: String(error) });
		return null;
	}
}
...
export async function flushGrievances(
	...
	const promise = (async () => {
		try {
			...
			lastFailureAt = Date.now();
			logger.warn("autoqa 推送失败", { endpoint: config.endpoint, error: String(error) });
...
function recordToolIssue(session: ToolSession, tool: string, report: string): void {
	...
	lastRecordPipeline = (async () => {
		try {
			...
			logger.debug("autoqa 同意流程失败", { error: String(error) });
		}
	})();
}
