...
import { isClaudeModelId } from "@wxyhgk/pi-catalog/identity";
import type { DesktopCapabilities } from "@wxyhgk/pi-natives";
import { once, prompt } from "@wxyhgk/pi-utils";
...
export class ComputerTool implements AgentTool<ComputerSchema, ComputerToolDetails> {
	...
	readonly summary = "使用持久化 JavaScript 与操作系统无障碍 API 控制宿主桌面";
	...
	readonly formatApprovalDetails = (args: unknown): string[] => {
		...
		return [
			...("read_only" in args && args.read_only === true ? ["只读"] : []),
	...
	async execute(
		...
		if (this.#closed) throw new ToolError("计算机会话已关闭");
