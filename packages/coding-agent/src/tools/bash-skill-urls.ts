...
export function resolveSkillUrlToPath(url: string, skills: readonly Skill[]): string {
	...
	if (!rawSkillSegment) {
		throw new ToolError(`skill:// URL 需要提供技能名称: ${url}`);
	}
	...
	if (!skill) {
		...
		throw new ToolError(`未知技能: ${rawSkillSegment}。可用: ${availableStr}`);
	}
	...
	try {
		...
		throw new ToolError(`skill:// URL 路径编码无效: ${url}`);
	}
	try {
	...
	if (!resolvedPath.startsWith(resolvedBaseDir + path.sep) && resolvedPath !== resolvedBaseDir) {
		throw new ToolError("skill:// URL 中不允许路径穿越");
	}
...
async function resolveInternalUrlToPath(
	...
	if (!scheme) {
		throw new ToolError(`bash 命令中存在不支持的内部 URL: ${url}`);
	}
	...
	try {
		...
		throw new ToolError(`在 bash 命令中解析 ${scheme}:// URL 失败: ${url}\n${message}`);
	}
