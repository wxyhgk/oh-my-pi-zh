 ...
 */
import type { AgentToolResult, CustomMessage } from "@wxyhgk/pi-agent-core";
 ...
 */
export const PREVIEW_PENDING_NOTICE = `已暂存为提案——文件尚未修改。要应用:向 ${RESOLVE_DEVICE_PATH} 写入一句理由。要丢弃:向 ${REJECT_DEVICE_PATH} 写入。`;
...
export function resolutionDeviceUsage(device: ResolutionDeviceName): string {
	switch (device) {
		case RESOLVE_DEVICE_NAME:
			return `以纯文本向 ${RESOLVE_DEVICE_PATH} 写入一句理由,以应用暂存的待处理操作(例如工具预览)。`;
		case REJECT_DEVICE_NAME:
			return `以纯文本向 ${REJECT_DEVICE_PATH} 写入一句理由,以丢弃暂存的待处理操作(例如工具预览)。`;
		case PROPOSE_DEVICE_NAME:
			return `以纯文本向 ${PROPOSE_DEVICE_PATH} 写入你的计划的 <slug>(须与 local://<slug>-plan.md 匹配),以提交计划等待批准。仅在计划模式激活时有效。`;
	}
}
...
async function runResolveInvocation(
	...
	if (params.action === "apply") {
		...
		try {
			...
			throw new ToolError(`应用失败:${message}`);
		}
		return {
			...result,
			details: {
	...
	return {
		content: [{ type: "text" as const, text: `已丢弃:${options.label}。原因:${params.reason}` }],
		details: baseDetails,
	};
}
...
export async function dispatchResolutionDevice(
	...
	if (!invoker) {
		...
		const proposeHint = session.peekPlanProposalHandler?.()
			? ` 若要提交计划等待批准,请改为将其标题写入 ${PROPOSE_DEVICE_PATH}。`
			: "";
		...
		if (action === "discard") {
			...
			return {
				result: {
					content: [{ type: "text", text: `没有可拒绝的内容;当前已无待处理操作。${proposeHint}` }],
					details,
				},
...
export function renderResolutionDeviceCall(device: ResolutionDeviceName, content: unknown, uiTheme: Theme): Component {
	...
	const title = device === PROPOSE_DEVICE_NAME ? "提议" : device === REJECT_DEVICE_NAME ? "拒绝" : "应用";
...
export const resolveRenderer = {
		...
		const text = renderStatusLine(
			{
				...
				badge: {
					label: args.action === "apply" ? "提案 → 已应用" : "提案 → 已拒绝",
	...
	renderResult(
		...
		const verb = isApply ? "接受" : isFailedApply ? "失败" : "丢弃";
		const separator = ": ";
