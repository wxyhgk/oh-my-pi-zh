...
import { type AstFindMatch, astGrep } from "@wxyhgk/pi-natives";
...
const astGrepSchema = type({
	...
	"path?": type("string").describe(
		'要搜索的文件、目录、glob 或内部 URL;可用分号分隔列表传入多个("src; tests")。省略时搜索工作区根目录(".")',
	),
	"skip?": type("number").describe("要跳过的匹配数"),
});
...
export class AstGrepTool implements AgentTool<typeof astGrepSchema, AstGrepToolDetails> {
	...
	readonly label = "AST 搜索";
	readonly summary = "使用 AST 模式搜索代码(结构化搜索)";
	...
	readonly examples: readonly ToolExample<typeof astGrepSchema.inferIn>[] = [
		{
			caption: "在 src 下搜索 TypeScript 文件",
		...
		return untilAborted(signal, async () => {
			...
			if (result.matches.length === 0) {
				const noMatchMessage = cappedParseErrors.length
					? "未找到匹配。解析问题意味着查询范围可能不准确;在得出不存在结论前请缩小 `path` 范围。"
					: "未找到匹配";
			...
			if (result.limitReached) {
				outputLines.push("", "已达到结果上限;请缩小 path 范围或提高上限。");
			}
...
export const astGrepToolRenderer = {
	...
	renderCall(args: AstGrepRenderArgs, _options: RenderResultOptions, uiTheme: Theme): Component {
		...
		if (scopePaths.length) meta.push(`在 ${scopePaths.join(", ")} 中`);
		if (args.skip !== undefined && args.skip > 0) meta.push(`跳过:${args.skip}`);
	...
	renderResult(
		...
		if (matchCount === 0) {
			...
			if (details?.parseErrors?.length) {
				lines.push(uiTheme.fg("warning", "查询范围可能不准确;在得出不存在结论前请缩小 `path` 范围"));
				appendParseErrorsBulletList(lines, details.parseErrors, uiTheme, details.parseErrorsTotal);
			}
			return new Text(lines.join("\n"), 0, 0);
		}

		const summaryParts = [`${matchCount} 个匹配`, `${fileCount} 个文件`];
		...
		const matchGroups = groupLineIndicesByBlank(allLines)
			.filter(indices => {
				...
				return !first.startsWith("已达到结果上限") && !first.startsWith("解析问题:");
			})
			.map(indices => indices.map(index => styledLines[index]!));

		const extraLines: string[] = [];
		if (limitReached) {
			extraLines.push(uiTheme.fg("warning", "已达上限;请缩小 path 范围或提高上限"));
		}
		if (details?.parseErrors?.length) {
			extraLines.push(
