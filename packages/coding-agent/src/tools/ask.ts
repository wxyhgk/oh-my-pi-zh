...
const OptionItem = arkType({
	label: arkType("string").describe("显示标签"),
	"description?": arkType("string").describe("显示在标签下方的可选说明文字"),
	"preview?": arkType("string").describe("用于交互式询问对话框的可选丰富预览内容"),
});

const QuestionItem = arkType({
	id: arkType("string").describe("问题 ID"),
	question: arkType("string").describe("问题文本"),
	"header?": arkType("string").describe("用于交互式询问对话框的可选短标题"),
	options: OptionItem.array().describe("可用选项"),
	"multi?": arkType("boolean").describe("允许选择多个"),
...
const askSchema = arkType({
	questions: QuestionItem.array().atLeastLength(1).describe("要询问的问题"),
});
...
function getDoneOptionLabel(): string {
	return `${theme.status.success} 完成选择`;
}
...
function buildCustomInputRows(
	...
	rows.push({ text: "", priority: -1 });
	rows.push({ text: "输入你的回答:", priority: -1 });
	return rows;
}
...
async function askSingleQuestion(
	...
	const selectOption = async (
		...
		const helpText = navigation
			? "up/down 导航  enter 选择  ←/→ 切换问题  esc 取消"
			: "up/down 导航  enter 选择  esc 取消";
	...
	if (multi) {
		...
		while (true) {
			...
			const prefix = selected.size > 0 ? `(已选 ${selected.size} 项) ` : "";
			const {
				choice,
...
function formatSingleQuestionResponse(result: {
	...
	if (result.selectedOptions.length > 0) {
		const selectedText = result.multi
			? `用户选择: ${result.selectedOptions.join(", ")}`
			: `用户选择: ${result.selectedOptions[0]}`;
		responseParts.push(result.timedOut ? `${selectedText} (超时后自动选择)` : selectedText);
	}
	if (result.customInput !== undefined) {
		responseParts.push(
...
export class AskTool implements AgentTool<typeof askSchema, AskToolDetails> {
	...
	readonly examples: readonly ToolExample<typeof askSchema.infer>[] = [
		{
			...
			call: {
				questions: [
					{
						id: "auth_method",
						question: "该 API 应使用哪种身份验证方式？",
						options: [
							{ label: "JWT", description: "用于无状态 API 客户端的 Bearer token。" },
							{ label: "OAuth2", description: "通过外部身份提供商进行委托授权。" },
							{
								label: "会话 Cookie",
								description: "基于服务器端会话、面向浏览器的身份验证。",
							},
						],
						recommended: 0,
					},
			...
			call: {
				questions: [
					...
					},
					{
						id: "auth_method",
						question: "使用哪种身份验证方式？",
						options: [{ label: "JWT" }, { label: "会话 Cookie" }],
					},
				],
			},
		},
	...
	async execute(
		...
		if (!context?.hasUI || !context.ui) {
			context?.abort();
			throw new ToolAbortError("Ask 工具需要交互模式");
		}
		...
		if (params.questions.length === 0) {
			return {
				content: [{ type: "text" as const, text: "错误:问题不能为空" }],
				details: {},
			};
		}
		...
		if (richAskDialog) {
			try {
				...
				if (!richResult) {
					context.abort();
					throw new ToolAbortError("用户取消了 Ask 工具");
				}
				...
				if (richResult.results.length !== params.questions.length) {
					throw new Error("Ask 对话框返回的结果数量与请求的问题数不匹配");
				}
				...
				for (let index = 0; index < params.questions.length; index++) {
					...
					if (!question || !result || result.id !== question.id) {
						throw new Error("Ask 对话框返回的结果与请求的问题顺序不匹配");
					}
					results.push({
						id: question.id,
				...
				const responseText = `用户回答:\n${results.map(formatQuestionResult).join("\n")}`;
				return { content: [{ type: "text" as const, text: responseText }], details };
			} catch (error) {
				if (error instanceof Error && error.name === "AbortError") {
					throw new ToolAbortError("Ask 输入已被取消");
				}
				throw error;
			}
		}
		...
		while (questionIndex < params.questions.length) {
			...
			if (!q) throw new Error("Ask 问题索引超出请求的问题列表");
...
function renderNoteLines(uiTheme: Theme, note: string, width: number): string[] {
	...
	return replaceTabs(note)
		.split("\n")
		.map((line, index) => {
			const linePrefix = index === 0 ? `${uiTheme.fg("dim", " 备注:")} ` : continuationPrefix;
...
function renderAnswerOptionLines(
	...
	if (selected.size === 0 && customInput === undefined && note === undefined) {
		return [` ${uiTheme.styledSymbol("status.warning", "warning")} ${uiTheme.fg("warning", "已取消")}`];
	}
...
export const askToolRenderer = {
	...
	renderCall(args: AskRenderArgs, _options: RenderResultOptions, uiTheme: Theme): Component {
		const label = formatTitle("询问", uiTheme);
		...
		if (questions && questions.length > 0) {
			const header = `${label} ${uiTheme.fg("muted", `${questions.length} 个问题`)}`;
			return framedBlock(uiTheme, width => {
				const sections = questions.map(q => {
					const meta: string[] = [];
					if (q.multi) meta.push("多选");
					if (q.options?.length) meta.push(`选项:${q.options.length}`);
		...
		if (typeof args.question !== "string" || !args.question) {
			const errorLine = formatErrorMessage("未提供问题", uiTheme);
			return framedBlock(uiTheme, width => ({
				header: errorLine,
				sections: [],
				state: "error",
		...
		const meta: string[] = [];
		if (args.multi) meta.push("多选");
		const questionOptions = normalizeRenderOptions(args.options);
		if (questionOptions?.length) meta.push(`选项:${questionOptions.length}`);
	...
	renderResult(
		...
		if (!details) {
			...
			const header = renderStatusLine({ icon: "warning", title: "询问" }, uiTheme);
		...
		if (details.chatRedirect) {
			const header = renderStatusLine({ icon: "info", title: "询问", meta: ["转为聊天"] }, uiTheme);
		...
		const header = renderStatusLine(
			hasSelection
				? { iconOverride: uiTheme.styledSymbol("tool.ask", "accent"), title: "询问" }
				: { icon: "warning", title: "询问" },
			uiTheme,
		);
		...
		return framedBlock(uiTheme, width => {
			...
			if (dTimedOut) {
				...
				bodyLines.push(uiTheme.fg("dim", "超时后自动选择 — 非用户选择"));
			}
			return {
				header,
				sections: bodyLines.length > 0 ? [{ lines: bodyLines }] : [],
