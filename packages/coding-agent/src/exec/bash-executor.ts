 ...
 */
import { ExponentialYield } from "@wxyhgk/pi-agent-core/utils/yield";
import { type MinimizerOptions, Shell, type ShellRunResult } from "@wxyhgk/pi-natives";
import { isCmdShell, isExecutable, type ShellConfig } from "@wxyhgk/pi-utils/procmgr";
			...
			return {
				...
				...(await sink.dump(
					winner.kind === "timeout" && deadlineTimeoutMs !== undefined
						? `命令在 ${Math.round(deadlineTimeoutMs / 1000)} 秒后超时`
						: "命令已取消",
				)),
			};
		}
