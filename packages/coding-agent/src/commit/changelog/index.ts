...
import type { Api, ApiKey, Model } from "@wxyhgk/pi-ai";
...
export async function runChangelogFlow({
	...
	for (const boundary of boundaries) {
		...
		try {
			...
			logger.warn("提交变更日志解析已跳过", { path: boundary.changelogPath, error: String(error) });
			continue;
		}
...
export async function applyChangelogProposals({
	...
	for (const proposal of proposals) {
		if (
			...
			continue;
		onProgress?.(`正在为 ${proposal.path} 应用条目…`);
		const exists = await Bun.file(proposal.path).exists();
		if (!exists) {
			logger.warn("提交变更日志路径不存在", { path: proposal.path });
			continue;
		}
		...
		try {
			...
			logger.warn("提交变更日志解析已跳过", { path: proposal.path, error: String(error) });
			continue;
		}
