...
export function validateSummaryRules(summary: string): { errors: string[]; warnings: string[] } {
	...
	if (!hasPastTense) {
		errors.push("摘要必须以过去式动词开头");
	}

	const lowerSummary = summary.toLowerCase();
	for (const word of fillerWords) {
		if (lowerSummary.includes(word)) {
			warnings.push(`避免使用填充词:${word}`);
		}
	}
	for (const phrase of metaPhrases) {
		if (lowerSummary.includes(phrase)) {
			warnings.push(`避免使用元短语:${phrase}`);
		}
	}
...
export function capDetails(details: ConventionalDetail[]): { details: ConventionalDetail[]; warnings: string[] } {
	...
	const warnings = [`已根据优先级评分将详情列表上限设为 ${MAX_DETAIL_ITEMS} 条。`];
...
export function validateTypeConsistency(
	...
	switch (type) {
		case "docs":
			if (!hasDocs) errors.push("docs 提交应包含文档文件更改");
			break;
		case "test":
			if (!hasTests) errors.push("test 提交应包含测试文件更改");
			break;
		case "ci":
			if (!hasCI) errors.push("ci 提交应包含 CI 配置更改");
			break;
		case "build":
			if (!hasBuild) errors.push("build 提交应包含与构建相关的文件");
			break;
		case "refactor": {
			const hasNewFiles = options.diffText ? /\nnew file mode\s/m.test(options.diffText) : false;
			if (hasNewFiles) warnings.push("refactor 提交添加了新文件;若包含新功能请考虑 feat");
			break;
		}
		case "perf":
			if (!hasPerfEvidence && !hasPerfKeywords) {
				warnings.push("perf 提交缺少基准测试或性能关键字");
			}
			break;
		default:
			break;
