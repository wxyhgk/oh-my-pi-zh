...
const ADVISOR_QUARANTINE_PREFIX = "Advisor 响应已被隔离";
...
const ADVISOR_OUTPUT_ONLY_HAZARDS: readonly AdvisorOutputHazard[] = [
	{ label: "账号删除声明", pattern: /\buser\b.{0,80}\b(?:deleted|erased)\b.{0,80}\baccount\b/i },
	{
		label: "指令覆盖",
	...
	{ label: "拒绝指令", pattern: /\bdeny\s+(?:this|it|the\s+request)\s+if\s+(?:asked|questioned)\b/i },
];
...
export function quarantineAdvisorUnsafeOutput(
	...
	if (unavailableToolNames.size > 0) {
		...
		reasons.push(`请求了不可用的工具:${names.join(", ")}`);
	}
	...
	if (generatedText) {
		...
		if (
			matchedLabels.includes("破坏性 shell 命令") &&
			labels.includes("指令覆盖") &&
			!labels.includes("破坏性 shell 命令")
		) {
			labels.push("破坏性 shell 命令");
		}
		if (labels.includes("破坏性 shell 命令") || labels.length >= 3) {
			reasons.push(`生成了仅输出的破坏性指令:${labels.join(", ")}`);
		}
	}
...
export class AdvisorRuntime {
	...
	onTurnEnd(messages?: AgentMessage[], opts?: { willContinue?: boolean }): void {
		...
		try {
			...
			this.#failing = true;
			this.#wakeAllWaiters();
			logger.warn("advisor 增量渲染失败", { err: String(err) });
		}
		if (rendered) {
	...
	#noteDroppedBacklog(error: unknown): void {
		...
		this.#pending = [];
		this.#wakeAllWaiters();
		logger.warn("advisor 反复失败后已停止;请使用 /advisor 或重新加载配置以重新启用", {
	...
	#rollbackFailedTurn(snapshot: number): void {
		...
		try {
			...
			logger.debug("advisor 回滚失败", { err: String(err) });
		}
	}
		...
		for (let round = 0; round < MAX_COALESCE_ROUNDS; round++) {
			...
			if (this.host.maintainContext) {
				...
				try {
					...
					logger.debug("advisor 上下文维护失败", { err: String(err) });
				}
	...
	#notifyFailureOnce(error: unknown): void {
		...
		try {
			...
			logger.warn("advisor 失败通知发送失败", { err: String(notifyErr) });
		}
	}
				...
				try {
					...
					if (this.host.onTurnSuccess) {
						try {
							...
							logger.debug("advisor onTurnSuccess 钩子失败", { err: String(hookErr) });
						}
					}
				} catch (err) {
					if (this.#sessionTransitionPaused) {
					...
					this.#rollbackFailedTurn(messageSnapshot);
					logger.debug("advisor 轮次失败", { err: String(err) });
					if (classifierRefusal) {
						if (this.#includeThinking) {
							...
							if (strippedBatch) {
								...
								logger.debug("advisor 拒绝已通过剥离主推理恢复");
								continue;
							}
						}
					...
					try {
						...
						logger.debug("advisor onTurnError 钩子失败", { err: String(hookErr) });
					}
					if (this.#sessionTransitionPaused) {
						this.#pending.unshift(...popped);
						continue;
					...
					if (AIError.isUsageLimit(err)) {
						...
						logger.warn("advisor 额度已耗尽", { err: String(err) });
						...
						try {
							...
							logger.warn("advisor 额度通知发送失败", { err: String(notifyErr) });
						}
						break;
					}
					if (!terminalFailureRetriable) {
						logger.warn("advisor 终端失败不可重试;丢弃有界批次");
						this.#notifyFailureOnce(err);
						this.#consecutiveFailures = 0;
						...
						if (contextWasFresh) {
							...
							logger.warn("advisor 更新在全新上下文下仍溢出;丢弃有界批次");
							this.#notifyFailureOnce(err);
							success = true;
						} else {
							...
							logger.debug("advisor 上下文溢出已在当前主游标处恢复");
						}
					} else {
						this.#consecutiveFailures++;
						if (this.#consecutiveFailures >= 3) {
							logger.warn("advisor 连续失败 3 次;丢弃积压以避免停滞");
							this.#notifyFailureOnce(err);
							this.#consecutiveFailures = 0;
							// The dropped batch may carry primary-context we never delivered; drop
							// the seen-state too so queued raw deltas re-expand before delivery.
...
function getAdvisorTurnError(messages: readonly AgentMessage[]): Error | undefined {
	...
	return new Error("Advisor 轮次结束时未生成助手响应");
}
