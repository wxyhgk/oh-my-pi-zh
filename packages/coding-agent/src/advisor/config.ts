...
export function getOrCreateAdvisorProviderSessionId(
	...
	if (!UUID_V7_PATTERN.test(next)) {
		throw new Error("Advisor 提供商会话 ID 生成器返回了非 UUIDv7 值");
	}
	ids.set(key, next);
	return next;
}
...
function filterAdvisorTools(tools: string[] | undefined, sourcePath: string): string[] | undefined {
	...
	const filtered = normalizeToolNames(tools).filter(name => {
		...
		logger.warn("Advisor 配置:丢弃未知工具", { path: sourcePath, tool: name });
		return false;
	});
...
export async function discoverAdvisorConfigs(cwd: string, agentDir?: string): Promise<DiscoveredAdvisors> {
	...
	for (const item of items) {
		...
		try {
			...
			logger.warn("Advisor 配置:YAML 解析失败", { path: item.path, error: String(err) });
			continue;
		}
		if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
			logger.warn("Advisor 配置:应为 YAML 映射", { path: item.path });
			continue;
		}
		const result = watchdogYamlSchema(parsed);
		if (result instanceof type.errors) {
			logger.warn("Advisor 配置:架构无效", { path: item.path, error: result.summary });
			continue;
		}
...
export async function loadWatchdogConfigFile(filePath: string): Promise<WatchdogConfigDoc> {
	...
	try {
		...
		logger.warn("Advisor 配置:编辑文件解析失败", { path: filePath, error: String(err) });
		return { advisors: [] };
	}
	if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return { advisors: [] };
	const result = watchdogYamlSchema(parsed);
	if (result instanceof type.errors) {
		logger.warn("Advisor 配置:编辑文件架构无效", { path: filePath, error: result.summary });
		return { advisors: [] };
	}
