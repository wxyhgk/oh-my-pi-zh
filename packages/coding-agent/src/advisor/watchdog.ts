...
import { getAgentDir, isEnoent, logger, prompt } from "@wxyhgk/pi-utils";
...
export async function collectConfigCandidates(
	...
	try {
		...
		logger.debug("解析配置发现的 git 根目录失败", { err: String(err) });
	}
	...
	for (const candidate of candidates) {
		try {
			...
			if (!isEnoent(err)) {
				logger.warn("读取配置候选失败", { path: candidate, error: String(err) });
			}
		}
	}
