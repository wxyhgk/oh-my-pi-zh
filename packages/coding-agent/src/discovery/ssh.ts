...
async function loadSshJsonFile(
	...
	for (const [name, rawHost] of Object.entries(config.hosts)) {
		if (!name.trim()) {
			warnings.push(`${filePath} 中的 SSH 主机名无效`);
			continue;
		}
		if (!rawHost || typeof rawHost !== "object") {
			warnings.push(`${filePath} 中的主机条目无效:${name}`);
			continue;
		}
...
registerProvider(sshCapability.id, {
	...
	description: "从受管的 omp 路径及旧版 ssh.json/.ssh.json 文件加载 SSH 主机",
	priority: 5,
	load,
});
