...
async function loadMCPFromSettings(
	...
	for (const [name, config] of Object.entries(servers)) {
		if (!config || typeof config !== "object") {
			warnings.push(`${path} 中服务器"${name}"的配置无效`);
			continue;
		}
...
async function loadExtensionsFromDir(extensionsDir: string, level: "user" | "project"): Promise<LoadResult<Extension>> {
	...
	for (const { entry, extPath, manifestPath, content } of results) {
		...
		if (!manifest) {
			warnings.push(`${manifestPath} 中的 JSON 无效`);
			continue;
		}
	...
	if (userPath) {
		...
		if (content) {
			...
			if (parsed) {
				...
				warnings.push(`${userPath} 中的 JSON 无效`);
			}
		}
	}
	...
	if (projectPath) {
		...
		if (content) {
			...
			if (parsed) {
				...
				warnings.push(`${projectPath} 中的 JSON 无效`);
			}
		}
	}
...
registerProvider(mcpCapability.id, {
	...
	description: "从 ~/.gemini/settings.json 和 .gemini/settings.json 加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
...
registerProvider(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "加载 GEMINI.md 上下文文件",
	priority: PRIORITY,
...
registerProvider<SystemPrompt>(systemPromptCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "加载 system.md 自定义系统提示词文件",
	priority: PRIORITY,
...
registerProvider(extensionCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.gemini/extensions/ 和 .gemini/extensions/ 加载扩展",
	priority: PRIORITY,
	load: loadExtensions,
});
...
registerProvider(extensionModuleCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.gemini/extensions/ 和 .gemini/extensions/ 加载扩展模块",
	priority: PRIORITY,
...
registerProvider(settingsCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.gemini/settings.json 和 .gemini/settings.json 加载设置",
	priority: PRIORITY,
	load: loadSettings,
});
