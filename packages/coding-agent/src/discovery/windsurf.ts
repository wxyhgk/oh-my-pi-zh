...
function parseServerConfig(
	...
	if (typeof serverConfig !== "object" || serverConfig === null) {
		return { warning: `${path} 中"${name}"的服务器配置无效` };
	}
...
registerProvider<MCPServer>(mcpCapability.id, {
	...
	description: "从 Windsurf 配置(mcp_config.json)加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
...
registerProvider<Rule>(ruleCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 Windsurf 加载规则(.windsurf/rules/*.md、memories/global_rules.md、.windsurfrules)",
	priority: PRIORITY,
	load: loadRules,
});
