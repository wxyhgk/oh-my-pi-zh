...
import { getAgentDir, logger, parseFrontmatter, tryParseJson } from "@wxyhgk/pi-utils";
...
const DESCRIPTION = "来自 ~/.omp 和 .omp/ 的原生 OMP 配置";
const PRIORITY = 100;
		...
		for (const [serverName, config] of Object.entries(expanded)) {
			...
			if (serverConfig.enabled === undefined || serverConfig.enabled === null) {
				...
				if (lower === "false" || lower === "0") enabled = false;
					...
					logger.warn(`MCP 服务器"${serverName}":无效的 enabled 值"${serverConfig.enabled}",已忽略`);
					enabled = undefined;
				}
			} else {
				logger.warn(`MCP 服务器"${serverName}":无效的 enabled 类型 ${typeof serverConfig.enabled},已忽略`);
				enabled = undefined;
			}
			...
			if (serverConfig.timeout === undefined || serverConfig.timeout === null) {
				...
				if (Number.isFinite(serverConfig.timeout) && serverConfig.timeout >= 0) {
					...
					logger.warn(`MCP 服务器"${serverName}":无效的 timeout ${serverConfig.timeout},已忽略`);
					timeout = undefined;
				}
				...
				logger.warn(`MCP 服务器"${serverName}":无效的 timeout 类型 ${typeof serverConfig.timeout},已忽略`);
				timeout = undefined;
			}
			...
			if (requestIdFormat === undefined && serverConfig.requestIdFormat != null) {
				logger.warn(
					`MCP 服务器"${serverName}":无效的 requestIdFormat ${JSON.stringify(serverConfig.requestIdFormat)},已忽略`,
				);
			}
...
registerProvider<SystemPrompt>(systemPromptCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "来自 SYSTEM.md 的自定义系统提示词",
	priority: PRIORITY,
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "从 ~/.omp/agent/managed-skills 自动生成的受管技能",
...
async function loadExtensionModules(ctx: LoadContext): Promise<LoadResult<ExtensionModule>> {
	...
	for (let i = 0; i < configDirs.length; i++) {
		...
		for (const entry of extensions) {
			if (typeof entry !== "string") {
				warnings.push(`无效的扩展路径:${settingsPath}(${String(entry)})`);
				continue;
			}
	...
	for (let i = 0; i < settingsExtensions.length; i++) {
		...
		if (entries.length > 0) {
			...
			warnings.push(`扩展路径不存在:${resolvedPath}`);
		}
	}
...
async function loadExtensions(ctx: LoadContext): Promise<LoadResult<Extension>> {
	...
	for (let i = 0; i < manifestCandidates.length; i++) {
		...
		if (!manifest) {
			warnings.push(`解析失败:${manifestPath}`);
			continue;
		}
	...
	for (const { dir, level } of await getConfigDirs(ctx)) {
		...
		if (settingsContent) {
			...
			if (data) {
				...
				warnings.push(`解析失败:${settingsPath}`);
			}
		}
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .omp/ 目录加载 AGENTS.md",
	priority: PRIORITY,
