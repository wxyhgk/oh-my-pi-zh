...
async function loadSettings(ctx: LoadContext): Promise<LoadResult<Settings>> {
	...
	if (userContent && userPath) {
		...
		if (parsed) {
			...
			warnings.push(`${userPath}:JSON 无效`);
		}
	}
	...
	if (projectContent && projectPath) {
		...
		if (parsed) {
			...
			warnings.push(`${projectPath}:JSON 无效`);
		}
	}
...
registerProvider(mcpCapability.id, {
	...
	description: "从 ~/.cursor/mcp.json 和 .cursor/mcp.json 加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
...
registerProvider(ruleCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .cursor/rules/*.mdc 和旧版 .cursorrules 加载规则",
	priority: PRIORITY,
	load: loadRules,
});
...
registerProvider(settingsCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.cursor/settings.json 和 .cursor/settings.json 加载设置",
	priority: PRIORITY,
	load: loadSettings,
});
