...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	...
	if (userResult.status === "fulfilled") {
		...
		warnings.push(`无法扫描 ${userSkillsDir} 中的 Claude 用户技能:${String(userResult.reason)}`);
	}
	...
	for (const projectResult of projectResults) {
		if (projectResult.status === "fulfilled") {
			...
			warnings.push(`无法扫描 Claude 项目技能:${String(projectResult.reason)}`);
		}
	}
...
async function loadSettings(ctx: LoadContext): Promise<LoadResult<Settings>> {
	...
	if (userContent) {
		...
		if (data) {
			...
			warnings.push(`解析 JSON 失败:${userSettingsJson}`);
		}
	}
	...
	if (projectContent) {
		...
		if (data) {
			...
			warnings.push(`解析 JSON 失败:${projectSettingsJson}`);
		}
	}
...
registerProvider<MCPServer>(mcpCapability.id, {
	...
	description: "从 .claude.json 和 .claude/mcp.json 加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/ 目录加载 CLAUDE.md 文件",
	priority: PRIORITY,
...
registerProvider<Skill>(skillCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/skills/*/SKILL.md 加载技能",
	priority: PRIORITY,
	load: loadSkills,
});
...
registerProvider<ExtensionModule>(extensionModuleCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/extensions 加载扩展模块",
	priority: PRIORITY,
...
registerProvider<SlashCommand>(slashCommandCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/commands/*.md 加载斜杠命令",
	priority: PRIORITY,
...
registerProvider<Hook>(hookCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/hooks/pre/ 和 .claude/hooks/post/ 加载钩子",
	priority: PRIORITY,
	load: loadHooks,
});
...
registerProvider<CustomTool>(toolCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/tools/ 加载自定义工具",
	priority: PRIORITY,
	load: loadTools,
});
...
registerProvider<Settings>(settingsCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/settings.json 加载设置",
	priority: PRIORITY,
	load: loadSettings,
});
...
registerProvider<SystemPrompt>(systemPromptCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 .claude/SYSTEM.md 加载系统提示词",
	priority: PRIORITY,
