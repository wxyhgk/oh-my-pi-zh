...
import { FileType, glob } from "@wxyhgk/pi-natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
export async function scanSkillsFromDir(
	...
	try {
		...
		if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
			warnings.push(`无法读取技能目录:${dir}(${String(error)})`);
		}
	...
	const loadSkill = async (skillPath: string) => {
		try {
			...
			warnings.push(`无法读取技能文件:${skillPath}`);
		}
	};
...
export async function loadFilesFromDir<T>(
	...
	for (const { filePath, content } of fileResults) {
		...
		try {
			...
			warnings.push(`解析失败:${filePath}(${err})`);
		}
	}
	return { items, warnings };
}
...
export async function listClaudePluginRoots(
	...
	if (content) {
		...
		if (!registry) {
			warnings.push(`解析 Claude Code 插件注册表失败:${registryPath}`);
		} else {
			for (const [pluginId, entries] of Object.entries(registry.plugins)) {
				...
				if (atIndex === -1) {
					warnings.push(`插件 ID 格式无效(缺少 @marketplace):${pluginId}`);
					continue;
				}
				...
				for (const entry of entries) {
					if (!entry.installPath || typeof entry.installPath !== "string") {
						warnings.push(`插件 ${pluginId} 的条目缺少 installPath`);
						continue;
					}
	...
	if (ompContent) {
		...
		if (ompRegistry) {
			...
			warnings.push(`解析 OMP 插件注册表失败:${ompRegistryPath}`);
		}
	}
	...
	if (resolvedProjectPath) {
		...
		if (projectContent) {
			...
			if (projectRegistry) {
				...
				warnings.push(`解析项目插件注册表失败:${resolvedProjectPath}`);
			}
		}
	}
