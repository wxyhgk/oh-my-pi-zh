...
const DISPLAY_NAME = "Claude Code 市场";
				...
				commandsDirs.map(async dir => {
					try {
						...
						if (stats.isFile()) {
							...
							if (content === null) return { items: [], warnings: [`无法读取文件:${dir}`] };
		...
		if (resolved.inlineServers) {
			...
			if (raw === null) {
				...
				if (resolved.declared) {
					const warning = `[claude-plugins] 缺少 ${root.id} 声明的 mcpServers 文件:${resolved.path}`;
			...
			try {
				...
				warnings.push(`[claude-plugins] ${resolved.path} 中的 JSON 无效`);
				logger.warn(`[claude-plugins] ${resolved.path} 中的 JSON 无效`);
				continue;
			}
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "从 Claude Code 市场插件加载技能(~/.claude/plugins/cache/)",
	priority: PRIORITY,
	load: loadSkills,
});
...
registerProvider<SlashCommand>(slashCommandCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 Claude Code 市场插件加载斜杠命令",
	priority: PRIORITY,
...
registerProvider<Hook>(hookCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 Claude Code 市场插件加载钩子",
	priority: PRIORITY,
	load: loadHooks,
});
...
registerProvider<CustomTool>(toolCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 Claude Code 市场插件加载自定义工具",
	priority: PRIORITY,
	load: loadTools,
});
...
registerProvider<MCPServer>(mcpCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从市场插件的 .mcp.json 文件加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
