...
import { logger, parseFrontmatter } from "@wxyhgk/pi-utils";
...
async function loadTomlConfig(_ctx: LoadContext, path: string): Promise<Record<string, unknown> | null> {
	...
	try {
		...
		logger.warn("解析 TOML 配置失败", { path, error: String(error) });
		return null;
	}
}
...
registerProvider<ContextFile>(contextFileCapability.id, {
	...
	description: "从 ~/.codex/AGENTS.md 加载上下文文件(仅用户级)",
	priority: PRIORITY,
...
registerProvider<MCPServer>(mcpCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 config.toml 的 [mcp_servers.*] 段加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
...
registerProvider<Skill>(skillCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.codex/skills 和 .codex/skills/ 加载技能",
	priority: PRIORITY,
	load: loadSkills,
});
...
registerProvider<ExtensionModule>(extensionModuleCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.codex/extensions 和 .codex/extensions/ 加载扩展模块",
	priority: PRIORITY,
...
registerProvider<SlashCommand>(slashCommandCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.codex/commands 和 .codex/commands/ 加载斜杠命令",
	priority: PRIORITY,
...
registerProvider<Prompt>(promptCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.codex/prompts 和 .codex/prompts/ 加载提示词",
	priority: PRIORITY,
	load: loadPrompts,
});
...
registerProvider<Hook>(hookCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.codex/hooks 和 .codex/hooks/ 加载钩子",
	priority: PRIORITY,
	load: loadHooks,
});
...
registerProvider<CustomTool>(toolCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.codex/tools 和 .codex/tools/ 加载自定义工具",
	priority: PRIORITY,
	load: loadTools,
});
...
registerProvider<Settings>(settingsCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 config.toml 加载设置",
	priority: PRIORITY,
	load: loadSettings,
});
