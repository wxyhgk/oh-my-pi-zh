...
import { logger, parseFrontmatter, tryParseJson } from "@wxyhgk/pi-utils";
...
async function loadJsonConfig(configPath: string): Promise<Record<string, unknown> | null> {
	...
	if (!parsed) {
		logger.warn("解析 OpenCode JSON 配置失败", { path: configPath });
		return null;
	}
	return parsed;
}
...
function extractMCPServers(
	...
	for (const [name, raw] of Object.entries(servers)) {
		if (!raw || typeof raw !== "object") {
			warnings.push(`${configPath} 中"${name}"的 MCP 配置无效`);
			continue;
		}
...
async function loadSettings(ctx: LoadContext): Promise<LoadResult<Settings>> {
	...
	if (userConfigPath) {
		...
		if (content) {
			...
			if (parsed) {
				...
				warnings.push(`${userConfigPath} 中的 JSON 无效`);
			}
		}
	}
	...
	if (content) {
		...
		if (parsed) {
			...
			warnings.push(`${projectConfigPath} 中的 JSON 无效`);
		}
	}
...
registerProvider(contextFileCapability.id, {
	...
	description: "从 ~/.config/opencode/ 加载 AGENTS.md",
	priority: PRIORITY,
...
registerProvider(mcpCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 opencode.json 的 mcp 键加载 MCP 服务器",
	priority: PRIORITY,
	load: loadMCPServers,
});
...
registerProvider(skillCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.config/opencode/skills/ 和 .opencode/skills/ 加载技能",
	priority: PRIORITY,
	load: loadSkills,
});
...
registerProvider(extensionModuleCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.config/opencode/plugins/ 和 .opencode/plugins/ 加载扩展模块",
	priority: PRIORITY,
...
registerProvider(slashCommandCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 ~/.config/opencode/commands/ 和 .opencode/commands/ 加载斜杠命令",
	priority: PRIORITY,
...
registerProvider(settingsCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "从 opencode.json 加载设置",
	priority: PRIORITY,
	load: loadSettings,
});
