import { type Component, matchesKey, parseSgrMouse, replaceTabs, ScrollView, truncateToWidth } from "@wxyhgk/pi-tui";
...
export class RawSseViewerComponent implements Component {
	...
	render(width: number): readonly string[] {
		...
		return [
			topBorder(this.#lastRenderWidth, "原始提供商数据流"),
	...
	#renderRawLines(innerWidth: number): string[] {
		...
		if (snapshot.records.length === 0) {
			return [
				theme.fg("muted", "尚未捕获任何原始 SSE 数据帧。"),
				theme.fg("muted", "模型响应流式传输期间,HTTP SSE 提供商的数据会填充到此视图。"),
			];
		}
	...
	#summaryText(): string {
		...
		const last = snapshot.lastUpdatedAt
			? `${theme.fg("muted", "最后更新")} ${theme.fg("accent", formatRawSseIsoTime(snapshot.lastUpdatedAt))}`
			: theme.fg("muted", "等待首个数据帧");
		const follow = this.#followTail ? theme.fg("success", "跟随开") : theme.fg("warning", "跟随关");
		return `${theme.fg("muted", "事件")} ${theme.fg("accent", String(snapshot.totalEvents))}  ${theme.fg("muted", "记录")} ${theme.fg("accent", String(snapshot.records.length))}  ${last}  ${follow}`;
	}

	#statusText(): string {
		const help = "Esc 关闭 · Ctrl+C 复制原文 · End 跟随尾部 · 滚轮滚动 · 点击摘要行切换跟随";
	...
	#copyAll(): void {
		...
		if (payload.trim().length === 0) {
			const message = "没有可复制的原始 SSE 数据帧";
