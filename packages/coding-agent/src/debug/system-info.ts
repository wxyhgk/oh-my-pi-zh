...
import { formatBytes, getProjectDir, VERSION } from "@wxyhgk/pi-utils";
...
export async function collectSystemInfo(): Promise<SystemInfo> {
	...
	const shell = Bun.env.SHELL ?? Bun.env.ComSpec ?? "未知";
...
export function formatSystemInfo(info: SystemInfo): string {
	const lines = [
		...
		`Arch:    ${info.arch}`,
		`CPU:     ${info.cpu}`,
		`内存:    ${formatBytes(info.memory.total)} (空闲 ${formatBytes(info.memory.free)})`,
		`Bun:     ${info.versions.bun}`,
		`应用:    omp-zh ${info.versions.app}`,
		`Node:    ${info.versions.node} (兼容)`,
		`CWD:     ${info.cwd}`,
		`Shell:   ${info.shell}`,
	];
	if (info.terminal) {
		lines.push(`终端:   ${info.terminal}`);
	}
	return lines.join("\n");
}
