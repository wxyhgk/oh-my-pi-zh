...
export class ProtocolProbeComponent extends Container {
	constructor(options: ProtocolProbeOptions) {
		...
		const yesNo = (on: boolean) => (on ? theme.fg("success", "支持") : theme.fg("muted", "不支持"));

		this.addChild(new DynamicBorder());
		this.addChild(new Text(theme.bold(theme.fg("accent", "终端协议测试")), 1, 0));

		// Styling: SGR attributes, themed foregrounds, and a truecolor sweep.
		const styling = [
			theme.fg("muted", "样式 (SGR)"),
			`  ${theme.bold("加粗")}  ${theme.italic("斜体")}  ${theme.underline("下划线")}  ${theme.strikethrough("删除线")}  ${theme.inverse(" 反色 ")}  ${theme.fg("dim", "弱化")}`,
			`  ${theme.fg("accent", "强调")}  ${theme.fg("success", "成功")}  ${theme.fg("warning", "警告")}  ${theme.fg("error", "错误")}`,
			`  真彩色: ${truecolorBar(32)} (${theme.fg("muted", `24-bit ${TERMINAL.trueColor ? "开" : "关"}`)})`,
		].join("\n");
		...
		this.addChild(
			new Text(
				[
					`${theme.fg("muted", "超链接 (OSC 8)")} — ${yesNo(hyperlinksOn)}`,
					`  \x1b]8;;https://github.com/can1357/oh-my-pi\x07oh-my-pi 仓库\x1b]8;;\x07`,
				].join("\n"),
				1,
				0,
			),
		...
		// Text sizing: OSC 66.
		this.addChild(new Text(`${theme.fg("muted", "文本缩放 (OSC 66)")} — ${yesNo(sizingOn)}`, 1, 0));
		if (sizingOn) {
		...
		this.addChild(new Text(`${theme.fg("muted", "图形")} — ${theme.fg("dim", imageProtocolLabel())}`, 1, 0));
		this.addChild(
			new Image(
