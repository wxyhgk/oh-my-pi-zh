...
export const SESSION_BOUNDARY_WARNING = "### 警告 - 上方日志早于当前会话!";
export const LOAD_OLDER_LABEL = "### 向上移动加载更多...";
...
export class DebugLogViewerComponent implements Component {
	...
	render(width: number): readonly string[] {
		...
		return [
			topBorder(this.#lastRenderWidth, "最近日志"),
	...
	#summaryText(): string {
		...
		return `${theme.fg("muted", "显示")} ${theme.fg("accent", `${this.#model.visibleLogCount}/${this.#model.logCount}`)}  ${theme.fg("muted", "已选")} ${theme.fg(selected > 0 ? "accent" : "muted", String(selected))}  ${theme.fg("muted", "展开")} ${theme.fg(expanded > 0 ? "accent" : "muted", String(expanded))}`;
	}

	#controlsText(): string {
		return "Esc 关闭 · Ctrl+C 复制 · ↑/↓/滚轮移动 · 点击切换 · Shift+↑/↓ 选择 · ←/→ 折叠/展开 · Ctrl+A 全选 · Ctrl+O 更早 · Ctrl+P 按 PID";
	}

	#filterText(): string {
		const sanitized = replaceTabs(sanitizeText(this.#model.filterQuery));
		const query = sanitized.length === 0 ? theme.fg("muted", "输入筛选") : theme.fg("accent", sanitized);
		const pidStatus = this.#model.isProcessFilterEnabled()
			? theme.fg("success", "PID 开")
			: theme.fg("muted", "PID 关");
		const loading = this.#loadingOlder ? `  ${theme.fg("warning", "正在加载更早日志…")}` : "";
		return `${theme.fg("muted", "筛选")} ${query}  ${pidStatus}${loading}`;
	}

	#statusText(): string {
		return this.#statusMessage
			? theme.fg("success", this.#statusMessage)
			: theme.fg("dim", "高亮时按 Enter 加载更早日志;可打印按键更新筛选");
	}
	...
	async #loadOlder(additionalCount: number): Promise<boolean> {
		...
		try {
			...
			this.#statusMessage = `加载更早日志失败:${message}`;
			this.#onError?.(`加载更早日志失败:${message}`);
			this.#onUpdate?.();
			return false;
		} finally {
	...
	#renderVisibleBodyLines(rows: Array<{ lines: string[]; rowIndex: number }>, bodyHeight: number): string[] {
		...
		if (rows.length === 0) {
			...
			lines.push(row(theme.fg("muted", "无匹配"), this.#lastRenderWidth));
		}
	...
	#copySelected() {
		...
		try {
			...
			const message = `已复制 ${selected.length} 条日志`;
			this.#statusMessage = message;
			this.#onStatus?.(message);
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			this.#statusMessage = `复制失败:${message}`;
			this.#onError?.(`复制日志失败:${message}`);
		}
	}
}
