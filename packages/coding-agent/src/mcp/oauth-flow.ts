...
function resolveRedirectUri(redirectUri: string | undefined): string | undefined {
	...
	if (trimmed !== configured) {
		throw new Error("OAuth 重定向 URI 不能包含首尾空白");
	}

	const parsed = new URL(configured);
	if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
		throw new Error("OAuth 重定向 URI 必须使用 http 或 https");
	}
	return configured;
}
...
function resolveResourceUri(resource: string | undefined): string | undefined {
	...
	if (!trimmed) return undefined;
	if (trimmed !== resource) {
		throw new Error("OAuth 资源 URI 不能包含首尾空白");
	}

	const parsed = new URL(trimmed);
	if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
		throw new Error("OAuth 资源 URI 必须使用 http 或 https");
	}
	if (parsed.hash) {
		throw new Error("OAuth 资源 URI 不能包含片段(fragment)");
	}
	return trimmed;
}
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async exchangeToken(code: string, _state: string, redirectUri: string): Promise<OAuthCredentials> {
		...
		if (typeof data.access_token !== "string" || data.access_token.length === 0) {
			...
			throw new Error(`Token 交换未返回访问 token${providerError ? `: ${providerError}` : ""}`);
		}
		...
		const outcome =
			failure.status > 0
				...
				: failure.detail
					...
					: "网络错误";
		return new Error(
			`OAuth 提供商要求 client_id,且动态客户端注册被拒绝` +
				`(POST ${failure.endpoint} → ${outcome})。服务器可能仅允许预先批准的客户端注册。${manualHint}`,
		);
	}
}
...
export async function refreshMCPOAuthToken(
	...
	if (!response.ok) {
		...
		throw new Error(`MCP OAuth 刷新失败: ${response.status} ${text}`);
	}
