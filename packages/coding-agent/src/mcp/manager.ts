...
import { isDefinitiveOAuthFailure, type TSchema } from "@wxyhgk/pi-ai";
...
export class MCPManager {
		...
		if (wasEmpty && this.#pendingNotifications.length > 0) {
			...
			for (const frame of pending) {
				try {
					...
					logger.debug("MCP 通知监听器在缓冲消息排空期间抛出异常", {
	...
	#subscribeAndTrack(name: string, connection: MCPServerConnection, uris: string[], notificationEpoch: number): void {
		void subscribeToResources(connection, uris)
			...
			.catch(error => {
				logger.debug("订阅 MCP 资源失败", { path: `mcp:${name}`, error });
			});
	}
		...
		for (const [name, connection] of this.#connections) {
			...
			if (uris && uris.size > 0) {
				void unsubscribeFromResources(connection, Array.from(uris)).catch(error => {
					logger.debug("取消订阅 MCP 资源失败", { path: `mcp:${name}`, error });
				});
			}
		}
		...
		for (const [name, config] of Object.entries(configs)) {
			...
			const connectionPromise = (async () => {
				...
				connection => {
					...
					connection.transport.onClose = () => {
						logger.debug("MCP 传输连接丢失,触发重连", { path: `mcp:${name}` });
			...
			void toolsPromise
				...
				.catch(error => {
					...
					logger.error("MCP 工具加载失败", { path: `mcp:${name}`, error: message });
				});
		}
	...
	#triggerNotificationRefresh(serverName: string, kind: "tools" | "resources" | "prompts"): Promise<void> {
		...
		return refresh.catch(error => {
			logger.debug("MCP 通知刷新失败", { path: `mcp:${serverName}`, kind, error });
		});
	}
	async #handleServerNotification(serverName: string, method: string, params: unknown): Promise<void> {
		logger.debug("收到 MCP 通知", { path: `mcp:${serverName}`, method });
	...
	#tripReconnectBreaker(name: string): boolean {
		...
		if (recent.length > RECONNECT_BURST_LIMIT) {
			logger.error("MCP 服务器崩溃次数过多;暂停自动重连", {
				path: `mcp:${name}`,
	...
	async #doReconnect(name: string, authChallenge?: MCPAuthChallenge): Promise<MCPServerConnection | null> {
		...
		if (authChallenge) {
			if (!this.#authHandler) {
				logger.error("无法处理 MCP 认证挑战;未配置认证处理器", {
					path: `mcp:${name}`,
				});
				return null;
			}
			try {
				...
				logger.error("MCP 认证挑战处理失败", { path: `mcp:${name}`, error });
				return null;
			}
		}

		logger.debug("MCP 正在重连", { path: `mcp:${name}` });
		...
		for (let attempt = 0; attempt <= delays.length; attempt++) {
			if (this.#epoch !== reconnectEpoch) {
				logger.debug("MCP 重连在尝试前因配置变更而中止", {
					path: `mcp:${name}`,
			...
			try {
				...
				logger.debug("MCP 已重连", { path: `mcp:${name}`, tools: connection.tools?.length ?? 0 });
				return connection;
			} catch (error) {
				if (this.#epoch !== reconnectEpoch) {
					logger.debug("MCP 重连因配置变更而中止", {
						path: `mcp:${name}`,
						storedEpoch: reconnectEpoch,
						currentEpoch: this.#epoch,
					});
				...
				if (attempt < delays.length) {
					...
					logger.error("MCP 多次重试后重连失败", { path: `mcp:${name}`, error: msg });
	...
	async #connectAndWireServer(
		...
		if (!this.#serverConfigs.has(name) || this.#epoch !== reconnectEpoch) {
			...
			throw new Error(`服务器 "${name}" 在重连期间被断开`);
		}
	...
	async #loadServerResourcesAndPrompts(name: string, connection: MCPServerConnection): Promise<void> {
		if (serverSupportsResources(connection.capabilities)) {
			try {
				...
				logger.debug("加载 MCP 资源失败", { path: `mcp:${name}`, error });
			}
		}
		...
		if (serverSupportsPrompts(connection.capabilities)) {
			try {
				...
				logger.debug("加载 MCP 提示失败", { path: `mcp:${name}`, error });
			}
		}
	}
				...
				if (oldUris) {
					...
					if (removed.length > 0) {
						try {
							...
							logger.debug("取消订阅过期的 MCP 资源失败", { path: `mcp:${name}`, error });
						}
					}
				}
				...
				try {
					...
					if (action === "rollback") {
						await unsubscribeFromResources(connection, allUris).catch(error => {
							logger.debug("回滚过期的 MCP 资源订阅失败", { path: `mcp:${name}`, error });
						});
						return;
					}
					if (action === "ignore") {
					...
					logger.debug("重新订阅 MCP 资源失败", { path: `mcp:${name}`, error });
				}
			}
		};
		...
		if (lookup && this.#authStorage) {
			...
			try {
					...
					credentialId,
					{
						...
						refresh: (current, signal) => {
							if (current.refresh === REMOTE_REFRESH_SENTINEL) {
								throw new Error("MCP OAuth 刷新 token 已被 broker 隐藏(broker-redacted);无法进行本地刷新");
							}
							const material = selectMcpOAuthRefreshMaterial(current, auth);
							const tokenUrl = material?.tokenUrl;
							if (!current.refresh || !tokenUrl) {
								throw new Error("MCP OAuth 凭据缺少刷新材料");
							}
						...
						onRefreshFailure: refreshError => {
							...
							logger.warn("MCP OAuth 刷新失败,使用现有 token", {
								credentialId,
								error: refreshError,
							});
						},
				...
				if (refreshResult.removed) {
					logger.warn("MCP OAuth 刷新明确失败;已清除凭据", { credentialId });
				}
				...
				logger.warn("解析 OAuth 凭据失败", { credentialId, error });
			}
		}
