...
async function fetchServerDetailsFromEntry(
	...
	for (const candidate of candidates) {
		try {
			...
			logger.debug("Smithery 详情获取候选失败", { candidate, error: String(error) });
		}
	}
	return null;
}
...
function toSearchResult(entry: SmitherySearchEntry, details: SmitheryServerDetails): SmitherySearchResult | null {
	...
	if (config.type === "stdio") {
		warnings.push("通过 Smithery CLI 在运行时运行(`bunx @smithery/cli run ...`)。");
	}
	if (requiredInputs.length > 0) {
		warnings.push("该提供商需要 Smithery schema 定义的配置输入。");
	}
	const displayName = safeMetadataValue(details.displayName ?? entry.displayName) ?? qualifiedName.replace(/^@/, "");
	const description = safeMetadataValue(details.description ?? entry.description) ?? "无描述";
	const connectionType = safeMetadataValue(selected.connection.type) ?? "未知";
...
export async function searchSmitheryRegistry(
	...
	for (let page = 1; page <= maxPages; page++) {
		...
		try {
			...
			if (isTimeoutError(err)) {
				throw new SmitheryRegistryError("Smithery 搜索超时(超过 10 秒)", 0);
			}
			throw err;
		}
		if (!response.ok) {
			throw new SmitheryRegistryError(`Smithery 搜索失败,状态码 ${response.status}`, response.status);
		}
	...
	if (detailFailures.length > 0) {
		logger.warn("部分条目的 Smithery 详情获取失败", {
			query,
