...
export class MCPToolCache {
	...
	async get(serverName: string, config: MCPServerConfig): Promise<MCPToolDefinition[] | null> {
		...
		try {
			...
			logger.warn("MCP 工具缓存解析失败", { serverName, error: String(error) });
			return null;
		}
		...
		try {
			...
			logger.warn("MCP 工具缓存哈希计算失败", { serverName, error: String(error) });
			return null;
		}
	...
	async set(serverName: string, config: MCPServerConfig, tools: MCPToolDefinition[]): Promise<void> {
		...
		try {
			...
			logger.warn("MCP 工具缓存序列化失败", { serverName, error: String(error) });
			return;
		}
