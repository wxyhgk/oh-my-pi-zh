...
import { getProjectDir, logger, withTimeout } from "@wxyhgk/pi-utils";
...
async function initializeConnection(
	...
	if (options?.signal?.aborted) {
		throw options.signal.reason instanceof Error ? options.signal.reason : new Error("已中止");
	}
...
export async function connectToServer(
	...
	try {
		...
		return await withTimeout(
			connect(),
			timeoutMs,
			`连接 MCP 服务器 "${name}" 超时(${describeMCPTimeout(timeoutMs)})`,
			options?.signal,
		);
	} catch (error) {
...
export async function subscribeToResources(
	...
	for (const result of results) {
		if (result.status === "rejected") {
			logger.warn("订阅 MCP 资源失败", { error: result.reason });
		}
	}
}
...
export async function unsubscribeFromResources(
	...
	for (const result of results) {
		if (result.status === "rejected") {
			logger.warn("取消订阅 MCP 资源失败", { error: result.reason });
		}
	}
}
