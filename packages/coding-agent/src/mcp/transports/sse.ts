...
import { logger, readSseEvents } from "@wxyhgk/pi-utils";
import type {
	JsonRpcError,
	JsonRpcMessage,
	JsonRpcRequest,
...
export class LegacySseTransport implements MCPTransport {
	...
	async connect(): Promise<void> {
		...
		try {
			...
			if (!response.body) {
				throw new Error("旧版 SSE 响应没有响应正文");
			}
			...
			if (operation.isTimeoutAbort(error)) {
				throw new Error(`旧版 SSE 端点超时(${timeout}ms)`);
			}
			throw error;
		}
	}
	...
	async #readSSEStream(
		...
		try {
			for await (const event of readSseEvents(body, signal)) {
				if (event.event === "endpoint") {
					if (!this.#endpointUrl) {
						...
						if (endpointUrl.origin !== configuredUrl.origin) {
							throw new Error(
								`旧版 SSE 端点来源不匹配:预期 ${configuredUrl.origin},实际收到 ${endpointUrl.origin}`,
							);
						}
				...
				try {
					...
					if (error instanceof SyntaxError) {
						throw new Error(`旧版 SSE 消息事件包含非 JSON 数据: ${event.data}`);
					}
					throw error;
				}
			...
			if (!endpointReceived) {
				endpointReady.reject(new Error("未收到旧版 SSE 端点事件"));
			}
		} catch (error) {
			if (!endpointReceived) {
				endpointReady.reject(error);
			} else if (error instanceof Error && error.name !== "AbortError") {
				logger.debug("旧版 SSE 流错误", { url: this.#config.url, error: error.message });
				this.onError?.(error);
			...
			pending.abortHandler = () => {
				...
				deferred.reject(
					options?.signal?.aborted && options.signal.reason instanceof Error
						...
						: new Error(`旧版 SSE 响应超时(${timeout}ms)`),
				);
			};
	...
	async notify(method: string, params?: Record<string, unknown>): Promise<void> {
		...
		try {
			...
			operation.clear();
			if (operation.isTimeoutAbort(error)) {
				throw new Error(`通知超时(${timeout}ms)`);
			}
			throw error;
		}
	}
