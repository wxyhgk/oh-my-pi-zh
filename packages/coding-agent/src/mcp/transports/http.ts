...
import { logger, readSseJson } from "@wxyhgk/pi-utils";
import type {
	JsonRpcError,
	JsonRpcMessage,
	JsonRpcRequest,
	...
	async #readSSEStream(body: ReadableStream<Uint8Array>, signal: AbortSignal): Promise<void> {
		try {
			...
			if (error instanceof Error && error.name !== "AbortError") {
				logger.debug("HTTP SSE 流错误", { url: this.config.url, error: error.message });
				this.onError?.(error);
			}
		}
	}
	...
	#parseSSEResponse<T>(response: Response, expectedId: string | number, options?: MCPRequestOptions): Promise<T> {
		...
		const drain = async (): Promise<void> => {
			try {
				for await (const raw of readSseJson<JsonRpcMessage | JsonRpcMessage[]>(response.body!, signal)) {
					...
					for (const message of messages) {
						if (
							...
							operation.clear();
							if (message.error) {
								reject(new Error(`MCP 错误 ${message.error.code}: ${message.error.message}`));
							} else {
				...
				if (!captured) {
					reject(new Error(`未收到请求 ID ${expectedId} 的响应`));
				}
			} catch (error) {
				if (captured) return;
				if (operation.isTimeoutAbort(error)) {
					reject(new Error(`SSE 响应超时(${timeout}ms)`));
				} else {
