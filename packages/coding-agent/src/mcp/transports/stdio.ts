...
import { getProjectDir, readJsonl } from "@wxyhgk/pi-utils";
...
function assertCmdBatchToken(value: string, kind: "command" | "argument"): void {
	...
	if (/[\0\r\n]/.test(value)) {
		throw new Error(`Windows 批处理 MCP ${kind} 不能包含 NUL、CR 或 LF 字符`);
	}
}
...
export class StdioTransport implements MCPTransport {
	...
	async request<T = unknown>(
		...
		if (signal?.aborted) {
			const reason = signal.reason instanceof Error ? signal.reason : new Error("已中止");
