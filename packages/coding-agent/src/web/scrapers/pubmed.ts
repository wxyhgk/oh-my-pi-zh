...
export const handlePubMed: SpecialHandler = async (
	...
	try {
		...
		if (!summaryResult.ok) {
			return buildFallback(["获取 PubMed 摘要元数据失败"]);
		}
		...
		if (!summaryData) {
			return buildFallback(["解析 PubMed 摘要元数据失败"]);
		}

		const article = summaryData.result?.[pmid];
		if (!article) {
			return buildFallback(["E-utilities 摘要端点未返回 PubMed 记录"]);
		}
		...
		if (abstractResult.ok) {
			...
			notes.push("已通过 NCBI E-utilities 获取摘要");
		}
		...
		try {
			...
			if (meshResult.ok) {
				...
				if (meshTerms.length > 0) {
					...
					notes.push("已通过 NCBI E-utilities 获取 MeSH 术语");
				}
			}
		} catch {
		...
		return buildResult(md, {
			...
			method: "pubmed",
			fetchedAt,
			notes: notes.length > 0 ? notes : ["通过 NCBI E-utilities 获取"],
		});
	} catch {
		return null;
	}
