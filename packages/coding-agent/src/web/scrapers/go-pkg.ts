...
export const handleGoPkg: SpecialHandler = async (
	...
	try {
		...
		if (!pageResult.ok) {
			return buildResult(`获取 pkg.go.dev 页面失败(状态:${pageResult.status ?? "未知"})`, {
				url,
		...
		const license = licenseLink?.textContent?.trim() || "未知";
			...
			if (indexList) {
				...
				if (exported.length > 0) {
					...
					if (exported.length > 50) {
						notes.push(`显示 ${exported.length} 个导出中的前 50 个`);
			...
			if (importsList) {
				...
				if (imports.length > 0) {
					...
					if (imports.length > 20) {
						notes.push(`显示 ${imports.length} 个导入中的前 20 个`);
		...
		if (moduleInfo) {
			notes.push(`发布于 ${moduleInfo.Time}`);
		}
