...
function formatOutput(contentType: string, oEmbed: SpotifyOEmbedResponse, og: OpenGraphData, url: string): string {
	...
	// Title
	const title = og.title || oEmbed.title || "未知";
	sections.push(`# ${title}\n`);
...
export const handleSpotify: SpecialHandler = async (url: string, timeout: number, signal?: AbortSignal) => {
	...
	try {
		...
		if (response.ok) {
			...
			notes.push("已通过 Spotify oEmbed API 获取元数据");
		} else {
			notes.push(`oEmbed API 返回状态 ${response.status || "错误"}`);
		}
	} catch (err) {
		notes.push(`获取 oEmbed 数据失败:${err instanceof Error ? err.message : String(err)}`);
	}
	...
	try {
		...
		if (pageResponse.ok) {
			...
			notes.push("已从页面 HTML 解析出 Open Graph 元数据");
		} else {
			notes.push(`页面获取返回状态 ${pageResponse.status || "错误"}`);
		}
	} catch (err) {
		notes.push(`获取页面 HTML 失败:${err instanceof Error ? err.message : String(err)}`);
	}
