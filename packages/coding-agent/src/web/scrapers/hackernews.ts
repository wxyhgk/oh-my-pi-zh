...
export const handleHackerNews: SpecialHandler = async (url, timeout, signal) => {
	...
	try {
		...
		notes.push(`错误:${errorMsg}`);
		return buildResult(`# 获取 Hacker News 内容失败\n\n${errorMsg}`, {
			url,
			method: "hackernews",
			fetchedAt,
			notes,
