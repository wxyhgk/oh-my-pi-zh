...
async function renderGitLabIssue(
	...
	md += `\n---\n\n## Description\n\n`;
	md += issue.description ? await htmlToBasicMarkdown(issue.description) : "*无描述*";
...
async function renderGitLabMR(
	...
	md += `\n---\n\n## Description\n\n`;
	md += mr.description ? htmlToBasicMarkdown(mr.description) : "*无描述*";
...
export const handleGitLab: SpecialHandler = async (
	...
	switch (gl.type) {
		case "blob": {
			...
			notes.push(`通过 GitLab API 获取原始文件`);
		...
		case "tree": {
			...
			notes.push(`通过 GitLab API 获取目录树`);
		...
		case "merge_request": {
			...
			notes.push(`通过 GitLab API 获取合并请求`);
