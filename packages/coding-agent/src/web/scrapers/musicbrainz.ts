...
function formatTrack(track: MusicBrainzTrack): string {
	const title = track.title || track.recording?.title || "无标题";
	...
	if (media.length) {
		...
		for (const medium of media) {
			...
			if (tracks.length) {
				...
				md += `- ${medium["track-count"]} 个音轨(详细信息不可用)\n\n`;
			}
		}
	}
...
export const handleMusicBrainz: SpecialHandler = async (
	...
	try {
		...
		return buildResult(md, { url, method: "musicbrainz-api", fetchedAt, notes: ["通过 MusicBrainz API 获取"] });
	} catch {}
