...
export const handleOpenLibrary: SpecialHandler = async (
	...
	try {
		...
		return buildResult(md, { url, method: "openlibrary", fetchedAt, notes: ["通过 Open Library API 获取"] });
	} catch {}
...
async function fetchByIsbn(isbn: string, timeout: number, signal?: AbortSignal): Promise<string | null> {
	...
	if (!result.ok) {
		return `# Open Library Book\n\n**ISBN:** ${isbn}\n\n无法从 Open Library 图书 API 获取图书详细信息。\n`;
	}
	...
	if (!book) {
		...
		if (!searchResult.ok) {
			return `# Open Library Book\n\n**ISBN:** ${isbn}\n\n无法从 Open Library 搜索 API 获取图书详细信息。\n`;
		}
