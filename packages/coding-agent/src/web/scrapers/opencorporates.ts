	...
	try {
		...
		if (!data) {
			...
			return buildResult(fallback, {
				...
				method: "opencorporates",
				fetchedAt,
				notes: ["OpenCorporates API 响应解析失败"],
			});
		}
		...
		return buildResult(md, {
			...
			method: "opencorporates",
			fetchedAt,
			notes: ["通过 OpenCorporates API 获取"],
		});
	} catch {}
