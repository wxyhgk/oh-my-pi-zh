...
import { getDocsRsCacheDir, isEnoent, logger, ptree, tryParseJson } from "@wxyhgk/pi-utils";
...
async function readCachedRustdocCrate(
	...
	try {
		...
		logger.warn("读取 docs.rs 缓存失败", { path: cachePath, error: String(err) });
		return null;
	}
}
...
async function writeCachedRustdocCrate(target: DocsRsTarget, json: string): Promise<void> {
	...
	try {
		...
		logger.warn("写入 docs.rs 缓存失败", { path: cachePath, error: String(err) });
	}
}
...
export const handleDocsRs: SpecialHandler = async (
	...
	if (cached) {
		const notes = ["已从 docs.rs rustdoc JSON 缓存加载"];
	...
	const notes = ["通过 docs.rs rustdoc JSON 获取"];
