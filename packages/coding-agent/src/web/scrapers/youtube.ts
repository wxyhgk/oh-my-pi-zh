...
import { ptree, Snowflake } from "@wxyhgk/pi-utils";
...
export const handleYouTube: SpecialHandler = async (
			...
			if (firstDocument) {
				...
				if (content.trim().length > 100) {
					return buildResult(content, {
						...
						method: "parallel",
						fetchedAt,
						notes: ["已使用 Parallel 提取 YouTube 内容"],
					});
				}
			}
		} catch {
	...
	if (!ytdlp) {
		return {
			...
			content: "检测到 YouTube 视频,但无法安装 yt-dlp。",
	...
	throwIfAborted(userSignal);
	if (signal?.aborted) {
		notes.push("获取时间预算已用尽,元数据/字幕可能不完整");
	}
