import { isRecord, ptree } from "@wxyhgk/pi-utils";
	...
	try {
		...
		if (contentLength) {
			...
			if (Number.isFinite(size) && size > MAX_BYTES) {
				return { ok: false, error: `content-length ${size} 超过 ${MAX_BYTES}` };
			}
		}
		...
		if (requestSignal?.aborted) return { ok: false, error: "已中止" };
		return { ok: false, error: err instanceof Error ? err.message : "获取二进制内容失败" };
	}
}
