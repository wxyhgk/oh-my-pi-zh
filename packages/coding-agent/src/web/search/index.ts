...
function formatForLLM(response: SearchResponse, notes: readonly string[] = []): string {
	...
	if (response.answer) {
		...
		if (response.sources.length > 0) {
			parts.push("\n## 来源");
			parts.push(formatCount("来源", response.sources.length));
		}
	}
	...
	if (response.citations && response.citations.length > 0) {
		parts.push("\n## 引用");
		parts.push(formatCount("引用", response.citations.length));
	...
	if (response.relatedQuestions && response.relatedQuestions.length > 0) {
		parts.push("\n## 相关问题");
		parts.push(formatCount("问题", response.relatedQuestions.length));
	...
	if (response.searchQueries && response.searchQueries.length > 0) {
		parts.push(`搜索查询: ${response.searchQueries.length}`);
...
async function executeSearch(
		...
		try {
			...
			if (!available && candidate.explicit) {
				throw new SearchProviderError(
					provider.id,
					`${provider.label} 网页搜索不可用。请配置其凭据,或选择自动提供商链路。`,
				);
			}
			...
			if (parsedQuery.hasConstraints && response.sources.length > 0) {
				...
				for (const label of filtered.dropped) {
					constraintNotes.push(`没有结果匹配 \`${label}\`;该约束已放宽`);
				}
			}

			if (!hasRenderableSearchContent(finalResponse)) {
				throw new SearchProviderError(provider.id, `${provider.label} 未返回可渲染的搜索内容。`, 204);
			}
	...
	const baseMessage = lastFailure
		...
		: `来自 ${lastProvider?.label ?? "网页搜索提供商"} 的未知错误`;
	const message =
		failures.length > 1 ? `所有网页搜索提供商均失败: ${formatSearchProviderFailures(failures)}` : baseMessage;
