...
async function callEcosiaHtml(params: SearchParams): Promise<string> {
	...
	try {
		...
		if (signal.aborted) {
			throw new SearchProviderError("ecosia", "Ecosia 搜索超时。", 504);
		}
		const message = error instanceof Error ? error.message : String(error);
		throw new SearchProviderError("ecosia", `Ecosia 搜索失败: ${message}`, 503);
	}

	if (isBlockedPage(page)) {
		throw new SearchProviderError(
			"ecosia",
			"Ecosia 以 Cloudflare 机器人验证挑战拦截了请求。Ecosia 的防火墙会限制来自数据中心/共享出口 IP 的自动化搜索;请尝试 DuckDuckGo、Brave、Tavily 等其他网页搜索提供商。",
			429,
		);
	}
	if (page.status < 200 || page.status >= 300) {
		...
		throw new SearchProviderError("ecosia", `Ecosia HTML 错误(${page.status})`, page.status);
	}
	return page.html;
}
