...
function parseZaiMcpResponse(rawText: string): unknown {
	...
	if (parsedMessages.length === 0) {
		try {
			...
			throw new SearchProviderError("zai", "无法解析 Z.AI MCP 响应", 500);
		}
	}
...
async function postZaiMcp(
	...
	if (!response.ok) {
		...
		throw new SearchProviderError("zai", `Z.AI MCP 错误(${response.status}): ${errorText}`, response.status);
	}
...
function readJsonRpcPayload(parsed: unknown): JsonRpcPayload {
	...
	if (directErrorSuccess === false && directErrorMessage) {
		throw new SearchProviderError(
			"zai",
			`Z.AI API 错误${directErrorCode ? ` (${directErrorCode})` : ""}: ${directErrorMessage}`,
			directErrorCode,
		);
	}
	...
	if (payload.error) {
		...
		throw new SearchProviderError(
			"zai",
			`Z.AI MCP 错误${payload.error.code ? ` (${payload.error.code})` : ""}: ${payload.error.message ?? "未知错误"}`,
			status,
		);
	}
...
async function callZaiTool(
	...
	if (resultRecord?.isError === true) {
		...
		throw new SearchProviderError("zai", errorText || "Z.AI MCP 工具调用失败", statusCode);
	}
...
async function callZaiSearch(apiKey: string, params: ZaiSearchParams): Promise<unknown> {
	...
	throw lastError ?? new SearchProviderError("zai", "Z.AI 搜索失败", 500);
}
...
export async function searchZai(params: ZaiSearchParams): Promise<SearchResponse> {
	...
	const rawResult = await withAuth(keyOrResolver, key => callZaiSearch(key, params), {
		...
		missingKeyMessage: "未找到 Z.AI 凭据。请设置 ZAI_API_KEY 或运行 'omp /login zai' 登录。",
	});
