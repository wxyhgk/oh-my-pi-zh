...
import { untilAborted } from "@wxyhgk/pi-utils";
...
async function callMojeekHtml(params: SearchParams, numResults: number): Promise<string> {
	...
	try {
		...
		if (signal.aborted) {
			throw new SearchProviderError("mojeek", "Mojeek 搜索超时。", 504);
		}
		const message = error instanceof Error ? error.message : String(error);
		throw new SearchProviderError("mojeek", `Mojeek 搜索失败: ${message}`, 503);
	}
	...
	if (isRobotPage(page)) {
		throw new SearchProviderError(
			"mojeek",
			"Mojeek 以其自动化查询防护墙拦截了请求。Mojeek 会限制来自数据中心/共享出口 IP 的脚本化搜索;请稍后重试,或配置 Brave、Tavily、Exa、Kagi 等其他提供商。",
			429,
		);
	}
	if (page.status < 200 || page.status >= 300) {
		...
		throw new SearchProviderError("mojeek", `Mojeek HTML 错误(${page.status})`, page.status);
	}
	return page.html;
}
