...
async function callGoogleHtml(params: SearchParams, numResults: number): Promise<string> {
	...
	try {
		...
		if (signal.aborted) {
			throw new SearchProviderError("google", "Google 浏览器搜索超时。", 504);
		}
		const message = error instanceof Error ? error.message : String(error);
		throw new SearchProviderError("google", `Google 浏览器搜索失败: ${message}`, 503);
	}
	...
	if (blocked === "traffic") {
		throw new SearchProviderError(
			"google",
			"Google 以自动化流量验证挑战拦截了浏览器搜索。请尝试其他网页搜索提供商或稍后重试。",
			429,
		);
	}
