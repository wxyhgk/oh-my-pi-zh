...
async function callStartpageHtml(params: SearchParams): Promise<string> {
	...
	if (isChallengeResponse(page)) {
		throw new SearchProviderError(
			"startpage",
			"Startpage 以 CAPTCHA 验证挑战拦截了请求。Startpage 会限制来自数据中心/共享出口 IP 的自动化搜索;请尝试 DuckDuckGo、Mojeek 等其他提供商,或稍后重试。",
			429,
		);
	}
	if (page.status < 200 || page.status >= 300) {
		...
		throw new SearchProviderError("startpage", `Startpage HTML 错误(${page.status})`, page.status);
	}
	return page.html;
}
