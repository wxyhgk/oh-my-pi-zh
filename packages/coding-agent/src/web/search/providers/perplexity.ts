...
function throwPerplexityStreamError(message: AssistantMessage): never {
	...
	const details = message.errorMessage ?? "Perplexity API 流式请求失败";
	const classified = classifyProviderHttpError("perplexity", status, details);
	if (classified) throw classified;
	throw new SearchProviderError("perplexity", `Perplexity API 错误(${status}): ${details}`, status);
}
...
async function callPerplexityAsk(
	...
	if (!response.ok) {
		...
		throw new SearchProviderError(
			"perplexity",
			`Perplexity ask API 错误(${response.status}): ${errorText}`,
			response.status,
		);
	}

	if (!response.body) {
		throw new SearchProviderError("perplexity", "Perplexity ask API 未返回响应体", 500);
	}
	...
	for await (const event of readSseJson<PerplexityOAuthStreamEvent>(response.body, params.signal)) {
		if (event.error_code) {
			...
			throw new SearchProviderError("perplexity", `Perplexity ask 流式错误: ${message}`, 400);
		}
...
export async function searchPerplexity(params: PerplexitySearchParams): Promise<SearchResponse> {
	...
	throw new SearchProviderError("perplexity", "没有可用的认证方式。", 401);
}
