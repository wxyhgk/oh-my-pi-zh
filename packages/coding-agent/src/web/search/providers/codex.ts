...
class CodexNoWebSearchError extends SearchProviderError {
	constructor() {
		super(
			"codex",
			"Codex 返回了未运行网页搜索的完成结果(没有 web_search_call 事件);拒绝将非搜索回答视为搜索结果",
			502,
		);
...
async function callCodexSearch(
	...
	if (!response.ok) {
		...
		throw new SearchProviderError("codex", `Codex API 错误(${response.status}): ${errorText}`, response.status);
	}

	if (!response.body) {
		throw new SearchProviderError("codex", "Codex API 未返回响应体", 500);
	}
	...
	for await (const rawEvent of readSseJson<Record<string, unknown>>(response.body, options.signal)) {
		...
		if (eventType === "response.output_text.delta") {
			...
			throw new SearchProviderError("codex", `Codex 错误(${code}): ${message || "未知错误"}`, 500);
		} else if (eventType === "response.failed") {
			const { code, message } = extractCodexSseError(rawEvent);
			const detail = code
				? `Codex 请求失败(${code}): ${message || "请求失败"}`
				: `Codex 请求失败: ${message || "请求失败"}`;
	...
	if (!hasFinalText && !hasStreamedText && sources.length === 0) {
		throw new SearchProviderError("codex", "Codex 返回了仅包含图片的响应", 502);
	}
...
async function runCodexSearchCandidates(options: {
	...
	throw lastError ?? new Error("Codex 搜索失败,未返回结果");
}
...
export async function searchCodex(params: SearchParams): Promise<SearchResponse> {
	...
	if (!firstCandidate) {
		throw new SearchProviderError("codex", "未配置 Codex 网页搜索模型。");
	}
	...
	if (transport.customEndpoint) {
		...
		if (!hasCommandBackedKey && (credentialOrigin?.kind === "oauth" || credentialOrigin?.kind === "env")) {
			throw new SearchProviderError(
				"codex",
				`拒绝将官方 Codex OAuth 凭据发送到自定义端点 ${transport.baseUrl}。请为提供商 "openai-codex" 配置 API 密钥。`,
			);
		}
		...
		result = await withAuth(
				...
				}),
			{
				...
				missingKeyMessage: '未找到 Codex 凭据。请为提供商 "openai-codex" 配置 API 密钥。',
			},
		);
	} else {
		...
		result = await withOAuthAccess(
			...
			access => {
				...
				const accountId = access.accountId ?? getCodexAccountId(access.accessToken);
				if (!accountId) {
					throw new Error("Codex OAuth 凭据缺少 ChatGPT 账户 ID");
				}
