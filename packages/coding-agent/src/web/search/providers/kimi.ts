 ...
 */
import { type ApiKey, type AuthStorage, type FetchImpl, withAuth } from "@wxyhgk/pi-ai";
...
export async function searchKimi(params: KimiSearchParams): Promise<SearchResponse> {
	...
	if (!keyOrResolver) {
		throw new Error(
			"未找到 Kimi 搜索凭据。Kimi 网页搜索使用 Kimi Code 服务(api.kimi.com);请将 MOONSHOT_SEARCH_API_KEY / KIMI_SEARCH_API_KEY 设置为 Kimi Code Console 密钥,或运行 'omp /login kimi-code' 登录。此处不接受 Moonshot Open Platform 密钥(MOONSHOT_API_KEY)。",
		);
	}
