...
import { hasOpus47ApiRestrictions } from "@wxyhgk/pi-catalog/identity/family";
...
export async function searchAnthropic(
	...
	if (!keyOrResolver) {
		throw new Error(
			"未找到 Anthropic 凭据。请设置 ANTHROPIC_SEARCH_API_KEY 或 ANTHROPIC_API_KEY,或配置 Anthropic OAuth。",
		);
	}
