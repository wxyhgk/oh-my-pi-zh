...
export function classifyProviderHttpError(
	...
	if (CREDIT_BODY_PATTERN.test(body)) {
		return new SearchProviderError(provider, `${provider}: 额度已用完`, status);
	}
	if (status === 402) {
		return new SearchProviderError(provider, `${provider}: 402 额度已用完`, status);
	}
	if (status === 401) {
		return new SearchProviderError(provider, `${provider}: 401 未授权`, status);
	}
	if (status === 403) {
		return new SearchProviderError(provider, `${provider}: 403 禁止访问`, status);
	}
	return null;
}
