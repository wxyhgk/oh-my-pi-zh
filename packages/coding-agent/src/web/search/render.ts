...
import { Markdown, Text } from "@wxyhgk/pi-tui";
...
function renderFallbackText(contentText: string, expanded: boolean, theme: Theme): Component {
	...
	let text = `${headerIcon} ${theme.fg("dim", "响应")}${expandHint}`;

	if (displayLines.length === 0) {
		text += `\n ${theme.fg("dim", theme.tree.last)} ${theme.fg("muted", "无响应数据")}`;
	...
	if (!expanded && remaining > 0) {
		text += `\n ${theme.fg("dim", theme.tree.last)} ${theme.fg("muted", formatMoreItems(remaining, "行"))}`;
	}
...
function renderSearchErrorPanel(message: string, providerLabel: string | undefined, theme: Theme): Component {
	const header = renderStatusLine({ icon: "error", title: "网页搜索", description: providerLabel }, theme);
	const body = theme.fg("error", `错误: ${replaceTabs(message)}`);
...
export function renderSearchResult(
	...
	const providerLabel = provider !== "none" ? getSearchProviderLabel(provider) : "无";
	...
	const metaLines: string[] = [`${theme.fg("muted", "提供商:")} ${theme.fg("text", providerInfo)}`];
	if (response.usage) {
		const usageParts: string[] = [];
		if (response.usage.inputTokens !== undefined) usageParts.push(`输入 ${response.usage.inputTokens}`);
		if (response.usage.outputTokens !== undefined) usageParts.push(`输出 ${response.usage.outputTokens}`);
		if (response.usage.totalTokens !== undefined) usageParts.push(`总计 ${response.usage.totalTokens}`);
		if (response.usage.searchRequests !== undefined) usageParts.push(`搜索 ${response.usage.searchRequests}`);
		if (usageParts.length > 0)
			metaLines.push(`${theme.fg("muted", "用量:")} ${theme.fg("text", usageParts.join(theme.sep.dot))}`);
	}
	...
	return markFramedBlockComponent({
		render(width: number): readonly string[] {
			...
			if (renderedAnswer.length === 0) {
				answerLines = [theme.fg("muted", "未返回回答文本")];
				...
				if (remaining > 0) {
					capped.push(theme.fg("muted", formatMoreItems(remaining, "行")));
				}
				answerLines = capped;
			} else {
			...
			return outputBlock.render(
				{
					...
					sections: [
						...(queryPreview
							? [
									{
										lines: [`${theme.fg("muted", "查询:")} ${theme.fg("text", queryPreview)}`],
									},
								]
							: []),
						{
							label: theme.fg("toolTitle", "回答"),
							lines: answerLines,
						},
						{
							label: theme.fg("toolTitle", "来源"),
							lines: sourceTree.length > 0 ? sourceTree : [theme.fg("muted", "未返回来源")],
						},
						{ label: theme.fg("toolTitle", "元数据"), lines: metaLines },
					],
					width,
				},
				theme,
...
export function renderSearchCall(
	...
	const text = renderStatusLine({ icon: "pending", title: "网页搜索", description: query }, theme);
	return new Text(text, 0, 0);
}
