 ...
 */
import { type AuthStorage, type FetchImpl, withAuth } from "@wxyhgk/pi-ai";
...
function createKagiApiError(statusCode: number, detail?: string): KagiApiError {
	return new KagiApiError(
		detail ? `Kagi API 错误(${statusCode}):${detail}` : `Kagi API 错误(${statusCode})`,
		statusCode,
	);
}
...
export async function searchWithKagi(
	...
	const response = await withAuth(
		...
		},
		{
			...
			missingKeyMessage: "未找到 Kagi 凭据。请设置 KAGI_API_KEY,或通过 'omp /login kagi' 登录。",
		},
	);
