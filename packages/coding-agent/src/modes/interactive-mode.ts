...
export function renderSubagentHudLines(sessions: ObservableSession[], columns: number): string[] {
	...
	if (hiddenCount > 0) {
		rows.push(theme.fg("dim", `… 还有 ${hiddenCount} 个正在运行 —— 打开 Agent 中心查看完整列表`));
	}
	return ["", theme.bold(theme.fg("accent", "子 Agent")), ...rows.map(line => ` ${line}`)];
}
...
export class InteractiveMode implements InteractiveModeContext {
	...
	constructor(
		...
		const hookCommands: SlashCommand[] = (
			...
			name: cmd.name,
			description: cmd.description ?? "(钩子命令)",
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		const modelName = this.session.model?.name ?? "未知";
		const providerName = this.session.model?.provider ?? "未知";
		...
		for (const warning of this.session.configWarnings) {
			this.ui.addChild(new Text(theme.fg("warning", `警告:${warning}`), 1, 0));
		...
		if (!startupQuiet) {
			...
			if (this.#startupChangelog && settings.get("startup.changelogMode") !== "hidden") {
				...
				this.ui.addChild(new Text(theme.bold(theme.fg("accent", "新功能")), 1, 0));
				this.ui.addChild(new Spacer(1));
	...
	#submitLoopPromptWhenReady(prompt: string): void {
		...
		if (isLoopDurationExpired(this.loopLimit)) {
			this.disableLoopMode("已达循环时间上限。循环模式已禁用。");
			return;
		}
	...
	async #runLoopIteration(action: "prompt" | "compact" | "reset", prompt: string): Promise<void> {
		...
		if (action === "reset" && this.vibeModeEnabled) {
			this.disableLoopMode("使用重置循环前请先退出 Vibe 模式。循环模式已禁用。");
			return;
		}

		if (!consumeLoopLimitIteration(this.loopLimit)) {
			this.disableLoopMode("已达循环上限。循环模式已禁用。");
			return;
		}
	...
	disableLoopMode(message = "循环模式已禁用。"): void {
	...
	async handleLoopCommand(args = ""): Promise<string | undefined> {
		...
		this.#syncLoopModeStatus();
		const limitSuffix = parsed.limit ? ` 限制为 ${describeLoopLimit(parsed.limit)}.` : "";
		const remainingSuffix = this.loopLimit ? ` ${describeLoopLimitRuntime(this.loopLimit)}.` : "";
		const tail = parsed.prompt ? "每轮后重复。" : "您下一条提示词将在每轮后重复。";
		this.showStatus(
			`循环模式已启用。${limitSuffix}${remainingSuffix} ${tail} 按 Esc 取消当前迭代;/loop 可再次禁用。`,
		);
	...
	#formatTodoLine(todo: TodoItem, prefix: string, matched: boolean): string {
		...
		switch (todo.status) {
			...
			case "blocked":
				return theme.fg("warning", `${prefix}${checkbox.unchecked} ${todo.content}(阻塞中)`) + marker;
			default:
	...
	async #enterPlanMode(options?: {
		...
		if (this.goalModeEnabled || this.goalModePaused) {
			this.showWarning("请先退出目标模式。");
			return;
		}
		if (this.vibeModeEnabled) {
			this.showWarning("请先退出 Vibe 模式。");
			return;
		}
		...
		this.showStatus(`计划模式已启用。计划文件:${planFilePath}`);
	}
	...
	async #exitPlanMode(options?: { silent?: boolean; paused?: boolean; deferModelRestore?: boolean }): Promise<void> {
		...
		if (!options?.silent) {
			this.showStatus(paused ? "计划模式已暂停。" : "计划模式已禁用。");
		}
	}
	...
	async #enterGoalMode(options: { objective?: string; resume?: boolean; silent?: boolean }): Promise<void> {
		...
		if (!options.silent) {
			this.showStatus(options.resume ? "目标模式已恢复。" : "目标模式已启用。");
		}
	}
	...
	async #exitGoalMode(options?: {
		...
		if (!options?.silent) {
			if (options?.reason === "completed") {
				this.showStatus("目标模式已完成。");
			} else if (options?.reason === "dropped") {
				this.showStatus("目标已删除。");
			} else if (options?.paused) {
				this.showStatus("目标模式已暂停。");
			} else {
				this.showStatus("目标模式已禁用。");
			}
		}
	}
	...
	async #openPlanInExternalEditor(planFilePath: string): Promise<void> {
		...
		try {
			...
			if (isEnoent(error)) {
				this.showError(`未找到计划文件:${planFilePath}`);
				return;
			}
			this.showWarning(`打开外部编辑器失败:${error instanceof Error ? error.message : String(error)}`);
			return;
		}
	...
	async handlePlanModeCommand(initialPrompt?: string): Promise<void> {
		...
		if (this.planModePaused && !initialPrompt) {
			...
			this.#updatePlanModeStatus();
			this.sessionManager.appendModeChange("none");
			this.showStatus("计划模式已禁用。");
			return;
		}
		if (!this.session.settings.get("plan.enabled")) {
			this.showWarning("计划模式已禁用。请在设置中启用(plan.enabled)。");
			return;
		}
		await this.#enterPlanMode();
	...
	async #handleGoalBudgetCommand(rawBudget: string): Promise<void> {
		...
		if (!this.goalModeEnabled || !state?.enabled) {
			this.showWarning("没有进行中的目标。");
			return;
		}
		if (state.goal.status === "complete") {
			this.showStatus("目标已完成。");
			return;
		}
		...
		if (trimmed !== "off") {
			...
			if (!Number.isInteger(parsed) || parsed <= 0) {
				this.showError("目标预算必须是正整数或 `off`。");
				return;
			}
			nextBudget = parsed;
		}
		...
		this.#resetGoalContinuationSuppression();
		this.#scheduleGoalContinuation();
		this.showStatus(nextBudget === undefined ? "目标预算已清除。" : `目标预算已设置为 ${nextBudget}。`);
	}
	...
	async handleGoalModeCommand(rest?: string): Promise<void> {
		try {
			...
			if (!this.session.settings.get("goal.enabled")) {
				this.showWarning("目标模式已禁用。请在设置中启用(goal.enabled)。");
				return;
			}
			...
			if (this.goalModeEnabled) {
				if (subRest) {
					this.showStatus("目标模式已处于活动状态。使用 /goal 管理,或使用 /goal drop 重新开始。");
					return;
				}
			...
			if (pausedState) {
				if (subRest) {
					this.showWarning("请先恢复当前目标,或先删除它再设置新目标。");
					return;
				}
	...
	#showGoalDetails(): void {
		...
		if (!goal) {
			this.showStatus("未设置目标。");
			return;
		}
		const used = goal.tokensUsed.toLocaleString();
		const budgetLine =
			goal.tokenBudget !== undefined
				? `${used} / ${goal.tokenBudget.toLocaleString()} (${Math.max(0, goal.tokenBudget - goal.tokensUsed).toLocaleString()} 剩余)`
				: `${used}(无预算)`;
		const lines = [
			`目标:${goal.objective}`,
			`状态:${goal.status}${state?.enabled ? "" : "(已暂停)"}`,
			`Tokens:${budgetLine}`,
			`耗时:${formatDuration(goal.timeUsedSeconds * 1000)}`,
		];
	...
	async #promptGoalBudgetEdit(): Promise<void> {
		...
		const input = (
			await this.showHookEditor("目标预算(数字、`off` 或留空取消)", prefill, undefined, {
				promptStyle: true,
			})
		)?.trim();
		if (!input) return;
	...
	async #resumeGoalAction(): Promise<void> {
		if (!this.#getPausedGoalState()) {
			this.showWarning("没有已暂停的目标可恢复。");
			return;
		}
		await this.#enterGoalMode({ resume: true, silent: true });
		this.showStatus("目标模式已恢复。");
		this.#scheduleGoalContinuation();
	}
	...
	async #handleGoalSetSubcommand(rest: string): Promise<void> {
		...
		const objective = rest.trim()
			? rest.trim()
			: (await this.showHookEditor("目标描述", undefined, undefined, { promptStyle: true }))?.trim();
		if (!objective) return;
		if (this.goalModeEnabled) {
	...
	async openPlanReview(): Promise<void> {
		if (!this.planModeEnabled) {
			this.showWarning("计划模式未激活。");
			return;
		}
		const noPlan = "还没有可审查的计划 —— 请先将计划写入 local://<slug>-plan.md 文件。";
	...
	async handlePlanApproval(details: PlanApprovalDetails): Promise<void> {
		...
		const choice = await this.showPlanReview(
			planContent,
			"计划模式 - 下一步",
			["批准并执行", "批准并压缩上下文", keepContextLabel, "完善计划"],
			{
				helpText,
	...
	static #AUTOQA_CONSENT_PROMPTS: ReadonlyArray<readonly [string, string]> = [
		[
			"😤 您的 Agent 正在为某个工具大动肝火。",
			"想让它向开发者发泄吗?只需工具名称 + 惹恼它的原因,不涉及任何个人隐私。",
		],
		[
	...
	async shutdown(): Promise<void> {
		...
		const stillClosingTimer = setTimeout(() => {
			this.showStatus("仍在关闭中…(正在刷新内存后端/网络)");
		...
		if (sessionId && sessionFile) {
			process.stderr.write(`\n${chalk.dim(`使用 ${APP_NAME} --resume ${sessionId} 恢复此会话`)}\n`);
		}
	...
	#handleLspStartupEvent(event: LspStartupEvent): void {
		...
		if (event.type === "failed") {
			this.showWarning(`LSP 启动失败:${event.error}. 将在写入时惰性重试。`);
			return;
		}
		...
		if (failedServers.length === 1) {
			...
			this.showWarning(`LSP 启动失败:${failedServer.name}${detail}. 将在写入时惰性重试。`);
			return;
		}

		if (failedServers.length > 1) {
			const failedNames = failedServers.map(server => server.name).join(", ");
			this.showWarning(`LSP 启动失败:${failedNames}. 将在写入时惰性重试。`);
		}
	}
	...
	async handleSTTToggle(): Promise<void> {
		if (this.#liveCommandController.active) {
			this.showWarning("使用按住说话语音输入前,请先结束 live 模式。");
			return;
		}
		if (!settings.get("stt.enabled")) {
			this.showWarning("语音转文字已禁用。请在设置中启用:stt.enabled");
			return;
		}
		if (!this.#sttController) {
	...
	async handleLiveCommand(): Promise<void> {
		if (this.#sttController && this.#sttController.state !== "idle") {
			this.showWarning("开始 live 模式前,请先完成当前的语音转文字录制。");
			return;
		}
	...
	async handleResumeSession(sessionPath: string): Promise<void> {
		...
		try {
			...
			this.showError(`保存待处理的设置失败:${err instanceof Error ? err.message : String(err)}`);
			return;
		}
		this.#btwController.dispose();
		this.#omfgController.dispose();
	...
	async handleBtwBranch(
		...
		try {
			...
			if (result.cancelled) {
				this.showStatus("/btw 分支已取消", { dim: true });
				return;
			}
			this.#btwController.dispose();
			this.#omfgController.dispose();
