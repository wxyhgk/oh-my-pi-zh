...
import type { AssistantMessage, ImageContent, Message, Usage } from "@wxyhgk/pi-ai";
import { getStreamingPartialJson } from "@wxyhgk/pi-ai/utils/block-symbols";
import { type Component, Spacer, Text, TruncatedText } from "@wxyhgk/pi-tui";
...
export class UiHelpers {
	...
	renderInitialMessages(options: RenderInitialMessagesOptions = {}): void {
		...
		if (compactionCount > 0) {
			...
			this.ctx.showStatus(`会话已压缩 ${times}`);
		}
	...
	showError(errorMessage: string): void {
		const text = new Text(`错误:${errorMessage}`, 1, 0).setStyleFn(t => theme.fg("error", t));
		this.ctx.present([new Spacer(1), text]);
	}

	showWarning(warningMessage: string): void {
		const text = new Text(`警告:${warningMessage}`, 1, 0).setStyleFn(t => theme.fg("warning", t));
		this.ctx.present([new Spacer(1), text]);
	}
	...
	showNewVersionNotification(newVersion: string): void {
		...
		const title = "有新版本可用";
		const prefix = `新版本 ${newVersion} 已可用。运行:`;
		const command = "omp-zh update";
		block.addChild(
	...
	updatePendingMessagesDisplay(): void {
		...
		const groups = [
			{ label: "转向", messages: steeringMessages },
			{ label: "yield 之后", messages: followUpMessages },
		...
		if (groups.length > 0) {
			...
			const hintText = theme.fg("dim", `  ${theme.tree.hook} 按 ${dequeueKey} 编辑`);
