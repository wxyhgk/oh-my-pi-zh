...
function blockSummaryHint(text: string, codeCount: number, quoteCount: number): string {
	...
	if (codeCount > 0) parts.push(`${codeCount} 个代码块`);
	if (quoteCount > 0) parts.push(`${quoteCount} 条引用`);
	return parts.join(" · ");
}
...
function messageTarget(text: string, rank: number): CopyTarget {
	...
	const messageCopy = rank === 1 ? "已将最后一条消息复制到剪贴板" : "已将消息复制到剪贴板";
	...
	if (codeBlocks.length > 1) {
		...
		children.push({
			id: `${id}:all`,
			label: `全部 ${codeBlocks.length} 个代码块`,
			hint: pluralLines(combined),
			preview: combined,
			content: combined,
			copyMessage: `已将 ${codeBlocks.length} 个代码块复制到剪贴板`,
		});
	}
	if (quoteBlocks.length > 1) {
		...
		children.push({
			id: `${id}:all-quotes`,
			label: `全部 ${quoteBlocks.length} 条引用`,
			hint: pluralLines(combined),
			preview: combined,
			content: combined,
			copyMessage: `已将 ${quoteBlocks.length} 个引用块复制到剪贴板`,
		});
	}
...
function commandTitle(command: LastCommand): string {
	return command.kind === "bash" ? "Bash 命令" : "Eval 代码";
}
...
function commandTarget(command: LastCommand, rank: number): CopyTarget {
	...
	return {
		...
		copyMessage: `已将 ${command.kind === "bash" ? "bash 命令" : "eval 代码"} 复制到剪贴板`,
	};
}
