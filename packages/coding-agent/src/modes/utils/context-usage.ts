import { countTokens } from "@wxyhgk/pi-agent-core";
import type { CompactionSettings } from "@wxyhgk/pi-agent-core/compaction";
import { effectiveReserveTokens, estimateTokens, resolveThresholdTokens } from "@wxyhgk/pi-agent-core/compaction";
import type { Tool as AiTool, Model } from "@wxyhgk/pi-ai";
import { toolWireSchema } from "@wxyhgk/pi-ai/utils/schema";
import { formatNumber } from "@wxyhgk/pi-utils";
...
export function computeContextBreakdown(
	...
	const categories: CategoryInfo[] = [
		{ id: "systemPrompt", label: "系统提示词", tokens: systemPromptTokens, color: "accent", glyph: CELL_FILLED },
		{ id: "systemTools", label: "系统工具", tokens: toolsTokens, color: "warning", glyph: CELL_FILLED },
		{
			id: "systemContext",
			label: "系统上下文",
		...
		{ id: "skills", label: "技能", tokens: skillsTokens, color: "success", glyph: CELL_FILLED },
		{
			id: "messages",
			label: "消息",
...
function buildLegendLines(breakdown: ContextBreakdown, theme: typeof Theme): string[] {
	...
	const modelName = model?.name ?? model?.id ?? "无模型";
	const modelId = model?.id ?? "未知";
	const windowLabel = formatNumber(contextWindow).toLowerCase();

	lines.push(theme.bold(`${modelName}`) + theme.fg("dim", ` (${windowLabel} 上下文)`));
	lines.push(theme.fg("muted", `${modelId}[${windowLabel}]`));
	lines.push(
		`${theme.bold(formatNumber(usedTokens))}${theme.fg("dim", `/${windowLabel} token`)}` +
			theme.fg("muted", ` (${percentString(usedTokens, contextWindow)})`),
	);
	lines.push("");
	lines.push(theme.fg("muted", "按类别估算的用量"));
	...
	for (const category of categories) {
		...
		lines.push(`${dot} ${label}: ${theme.bold(tokens)} ${theme.fg("dim", `token (${pct})`)}`);
	}

	const freeDot = theme.fg("dim", CELL_FREE);
	lines.push(
		`${freeDot} 可用空间:${theme.bold(formatNumber(freeTokens))} ${theme.fg("dim", `(${percentString(freeTokens, contextWindow)})`)}`,
	);

	if (autoCompactBufferTokens > 0) {
		const bufferDot = theme.fg("warning", CELL_BUFFER);
		lines.push(
			`${bufferDot} 自动压缩缓冲:${theme.bold(formatNumber(autoCompactBufferTokens))} ${theme.fg(
				"dim",
				`token (${percentString(autoCompactBufferTokens, contextWindow)})`,
			)}`,
		);
	}
	...
	if (snap) {
		...
		if (!snap.visionCapable) {
			lines.push(theme.fg("muted", "Snapcompact:未启用(模型不支持图像输入)"));
		} else {
			lines.push(theme.fg("muted", "Snapcompact(预估传输节省)"));
			if (snap.systemPrompt) {
				const sp = snap.systemPrompt;
				if (sp.applied) {
					lines.push(
						`  系统提示词(${sp.scope === "agents-md" ? "AGENTS.md" : "全部"}):节省 ${theme.bold(`~${formatNumber(sp.savedTokens)}`)} ` +
							theme.fg(
								"dim",
								`(${formatNumber(sp.textTokens)} 文本 → ${sp.frames} 帧 ≈ ${formatNumber(sp.imageTokens)})`,
							),
					);
				} else {
					const reason =
						sp.reason === "budget"
							...
							: sp.reason === "empty"
								...
								: "帧无法节省 token";
					lines.push(
						`  系统提示词(${sp.scope === "agents-md" ? "AGENTS.md" : "全部"}):${theme.fg("dim", `保持文本(${reason})`)}`,
					);
				}
			}
			if (snap.toolResults) {
				...
				if (tr.swapped > 0) {
					lines.push(
						`  工具结果:节省 ${theme.bold(`~${formatNumber(tr.savedTokens)}`)} ` +
							theme.fg(
								"dim",
								`(${tr.swapped}/${tr.total} 已成像,${formatNumber(tr.textTokens)} 文本 → ${tr.frames} 帧 ≈ ${formatNumber(tr.imageTokens)})`,
							),
					);
				} else {
					lines.push(`  工具结果:${theme.fg("dim", `未成像(历史中有 ${tr.total} 项)`)}`);
				}
			}
			if (snap.savedTokens > 0) {
				lines.push(
					`  下一次请求:${theme.bold(`~${formatNumber(Math.max(0, usedTokens - snap.savedTokens))}`)} ${theme.fg("dim", "个传输 token")}`,
				);
			}
		}
	}
...
export function renderContextUsage(breakdown: ContextBreakdown, theme: typeof Theme): string {
	if (breakdown.contextWindow <= 0) {
		return theme.fg("muted", "上下文用量不可用:当前会话未选择模型。");
	}
