...
import { formatBytes, formatDuration } from "@wxyhgk/pi-utils";
import {
	type CustomMessage,
...
export function buildAsyncResultBlock(message: CustomOrHookMessage): TranscriptBlock {
	...
	for (const job of jobs) {
		const jobId = job.jobId ?? "未知";
		const typeLabel = job.type ? `[${job.type}]` : "[任务]";
		const duration = typeof job.durationMs === "number" ? formatDuration(job.durationMs) : undefined;
		const line = [
			theme.fg("success", `${theme.status.done} 后台任务已完成`),
...
function sanitizeRecoveredRetryNote(note: string): string {
	...
	return truncateToWidth(normalized || "已重试", TRUNCATE_LENGTHS.CONTENT);
}
...
export function resolveAssistantErrorPresentation(
	...
	if (message.stopReason === "error") {
		return { kind: "full", text: message.errorMessage || "错误", isError: true };
	}
