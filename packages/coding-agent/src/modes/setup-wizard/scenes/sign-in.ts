...
import { getAgentDbPath } from "@wxyhgk/pi-utils";
...
function loginUrlLink(url: string): string {
	return `\x1b]8;;${url}\x07打开登录 URL\x1b]8;;\x07`;
}

function loginCopyHint(): string {
	return theme.fg("dim", "(已尝试复制到剪贴板;Alt+C 重试)");
}
...
export class SignInTab implements SetupTab {
	...
	render(width: number, maxLines?: number): readonly string[] {
		...
		if (this.#loggingInProvider) {
			...
			if (maxLines === undefined || maxLines >= 17 + 2) {
				lines.push(theme.fg("muted", "选择提供商登录 — 可以连接多个。"), "");
			}
		...
		if (this.#authUrl) {
			lines.push(
				theme.fg("accent", `浏览器登录:${loginUrlLink(this.#authUrl)} ${loginCopyHint()}`),
				...urlLines.slice(0, 2),
			);
			if (this.#authLaunchUrl) {
				lines.push(theme.fg("dim", `本地快捷方式(仅限本机):${this.#authLaunchUrl}`));
			}
		}
		if (this.#prompt) {
	...
	async #login(providerId: string): Promise<void> {
		...
		try {
			...
			this.#statusLines = [
				theme.fg("success", `${theme.status.success} 已登录 ${providerId}`),
				theme.fg("dim", `凭据已保存到 ${getAgentDbPath()}`),
			];
			this.#authUrl = undefined;
			this.#authLaunchUrl = undefined;
