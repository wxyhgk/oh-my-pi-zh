...
import { logger, sanitizeText } from "@wxyhgk/pi-utils";
	...
	await initializeExtensions(session, {
		reportSendError: (action, err) => {
			process.stderr.write(
				`扩展 ${action === "extension_send" ? "sendMessage" : "sendUserMessage"} 失败:${err.message}\n`,
			);
		},
		reportRuntimeError: err => {
			process.stderr.write(`扩展错误(${err.extensionPath}):${err.error}\n`);
		},
	});
		...
		if (assistantMsg) {
			...
			if (
				...
				const errorLine = sanitizeText(
					assistantMsg.errorMessage || (assistantMsg.stopReason === "aborted" ? "请求已中止" : "请求错误"),
				);
