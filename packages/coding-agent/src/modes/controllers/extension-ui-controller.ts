import type { Component, OverlayHandle, TUI } from "@wxyhgk/pi-tui";
import { Container, Spacer, Text } from "@wxyhgk/pi-tui";
import type { CollabUiRequestDraft, CollabUiSelectItem } from "@wxyhgk/pi-wire";
...
const ASK_OTHER_OPTION = "其他 (自行输入)";
const ASK_CHAT_OPTION = "就此进行对话";
const ASK_NEXT_OPTION = "下一步 →";
...
export class ExtensionUiController {
	...
	async initHooksAndCustomTools(): Promise<void> {
		...
		const uiContext: ExtensionUIContext = {
			...
			setTheme: async themeArg => {
				...
				return Promise.resolve({ success: false, error: "不支持直接传入主题对象" });
			},
			setFooter: () => {},
			setHeader: () => {},
		...
		this.ctx.session.setUsageFallbackConfirmer?.(confirmation => {
			const reserve =
				confirmation.remainingPercent === undefined
					? "在已配置的预留额度内"
					: `剩余 ${confirmation.remainingPercent.toFixed(1)}%`;
			return this.showHookConfirm(
				"已达到编码计划预留额度",
				`${confirmation.from} 的额度为 ${reserve}。切换到 ${confirmation.to}?选择“否”以继续使用当前计划。`,
			);
		});
		...
		const commandActions: ExtensionCommandContextActions = {
			...
			newSession: async options => {
				...
				this.ctx.present([
					new Spacer(1),
					new Text(`${theme.fg("accent", `${theme.status.success} 已开始新会话`)}`, 1, 1),
				]);
				await this.ctx.reloadTodos();
	...
	#createHookWidget(content: ExtensionWidgetContent): ExtensionUiComponent {
		if (Array.isArray(content)) {
			...
			if (content.length > MAX_WIDGET_LINES) {
				container.addChild(new Text(theme.fg("muted", "... (部件已截断)"), 1, 0));
			}
			return container;
		}
		if (content === undefined) {
			throw new Error("部件内容缺失");
		}
		...
		const actions: ExtensionActions = {
			sendMessage: (message, options) => {
				...
				this.ctx.session
					...
					.then(() => this.#applyCustomMessageDisplay(wasStreaming, normalized.display))
					.catch((err: unknown) => {
						const errorText = `扩展 sendMessage 失败:${err instanceof Error ? err.message : String(err)}`;
	...
	showToolError(toolName: string, error: string): void {
		const errorText = new Text(`工具 "${toolName}" 出错:${error}`, 1, 0).setStyleFn(t => theme.fg("error", t));
			...
			while (true) {
				...
				const choice = await this.#requestGuestUiString(
					{
						...
						helpText: hasAnswer
							? "上/下 导航  enter 切换  下一步 → 继续  esc 取消"
							: "上/下 导航  enter 切换  esc 取消",
					},
					signal,
				);
				...
				if (choice.value === ASK_OTHER_OPTION) {
					const input = await this.#requestGuestUiString(
						{ kind: "editor", title: boundPromptTitle("自定义回答: ", question.question) },
						signal,
					);
				...
				const choice = await this.#requestGuestUiString(
					{
						...
						markableCount: question.options.length,
						helpText: "上/下 导航  enter 选择  esc 取消",
					},
					signal,
				);
				if (choice.kind === "unavailable") return "unavailable";
	...
	#sendExtensionUserMessage: SendUserMessageHandler = (content, options) => {
		this.ctx.session.sendUserMessage(content, options).catch((err: unknown) => {
			this.ctx.showError(`扩展 sendUserMessage 失败:${err instanceof Error ? err.message : String(err)}`);
		});
	};
