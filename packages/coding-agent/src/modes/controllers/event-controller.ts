import type { AssistantMessage, ImageContent } from "@wxyhgk/pi-ai";
import * as AIError from "@wxyhgk/pi-ai/error";
import { getStreamingPartialJson } from "@wxyhgk/pi-ai/utils/block-symbols";
import { type Component, Loader, TERMINAL } from "@wxyhgk/pi-tui";
import { logger, prompt, sanitizeText } from "@wxyhgk/pi-utils";
...
export class EventController {
	...
	async #handleAutoCompactionEnd(event: Extract<AgentSessionEvent, { type: "auto_compaction_end" }>): Promise<void> {
		...
		if (event.aborted) {
			...
			if (event.errorMessage) {
				...
				this.ctx.statusLine.invalidate();
				this.ctx.ui.requestRender();
				this.ctx.showStatus("自动 Shake 已完成");
			}
			...
			this.ctx.showWarning("自动快照压缩维护失败;继续运行(不进行维护)");
		} else {
			this.ctx.showWarning("自动上下文占满维护失败;继续运行(不进行维护)");
		}
	...
	async #handleAutoRetryStart(event: Extract<AgentSessionEvent, { type: "auto_retry_start" }>): Promise<void> {
		...
		this.ctx.retryLoader = new Loader(
			...
			text => theme.fg("muted", text),
			`正在重试 (${event.attempt}/${event.maxAttempts}),${delaySeconds} 秒后…${this.#maintenanceEscHint()}`,
			getSymbolTheme().spinnerFrames,
		);
	...
	async #handleAutoRetryEnd(event: Extract<AgentSessionEvent, { type: "auto_retry_end" }>): Promise<void> {
		...
		if (event.success) {
			...
			this.#clearRetrySupersededAssistantComponents();
			this.ctx.showError(`重试 ${event.attempt} 次后失败:${event.finalError || "未知错误"}`);
		}
		this.#ensureWorkingLoaderWhileStreaming();
		this.ctx.ui.requestRender();
	}
	...
	async #runIdleRecap(): Promise<void> {
		...
		try {
			...
			if (!recap) return;
			this.ctx.showStatus(theme.fg("dim", theme.italic(`※ 回顾:${recap}`)), { dim: false });
		} catch (error) {
