...
import { prompt, Snowflake } from "@wxyhgk/pi-utils";
...
export class TanCommandController {
	...
	async start(work: string): Promise<void> {
		...
		if (!model) {
			this.ctx.showError("没有可用于 /tan 的活动模型。");
			return;
		}

		const manager = session.asyncJobManager;
		if (!manager) {
			this.ctx.showError("后台任务已被禁用;请启用异步任务以使用 /tan。");
			return;
		}

		const parentFile = this.ctx.sessionManager.getSessionFile();
		if (!parentFile) {
			this.ctx.showError("/tan 需要已持久化的会话。");
			return;
		}
		...
		try {
			...
			jobId = manager.register(
				...
				label,
				async ({ signal }) => {
					if (signal.aborted) throw new Error("执行前已中止");
					...
					try {
						...
						try {
							...
							return extractAssistantText(clone.getLastAssistantMessage()) || "(无输出)";
						} finally {
		...
		this.ctx.showStatus(`已派发后台 tan 任务 ${jobId}`);
	}
}
