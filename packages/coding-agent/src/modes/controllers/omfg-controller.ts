...
const PROJECT_OPTION = "此项目 (.omp/rules)";
const GLOBAL_OPTION = "全局 — 所有项目 (~/.omp/agent/rules)";
const AMEND_OPTION = "修改并附上反馈…";
...
export class OmfgController {
	...
	async start(complaint: string): Promise<void> {
		...
		if (!trimmedComplaint) {
			this.ctx.showStatus("用法:/omfg <问题>");
			return;
		}

		const model = this.ctx.session.model;
		if (!model) {
			this.ctx.showError("没有可用于 /omfg 的活动模型。");
			return;
		}
	...
	async #runRequest(request: OmfgRequest): Promise<void> {
		try {
			...
			for (;;) {
				...
				if (!candidate) {
					request.component.markError("模型未返回有效的 TTSR 规则。");
					return;
				}
	...
	async #generateCandidate(
		...
		for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
			...
			request.component.setStatus("generating", `第 ${attempt}/${MAX_ATTEMPTS} 次尝试 · 正在生成…`);
			...
			if ("error" in parsed) {
				...
				request.component.setStatus("validating", `第 ${attempt}/${MAX_ATTEMPTS} 次尝试 · ${parsed.error}`);
				continue;
			}

			request.component.setRule(parsed.fileContent);
			request.component.setStatus("validating", `第 ${attempt}/${MAX_ATTEMPTS} 次尝试 · 正在验证…`);
	...
	async #saveCandidate(request: OmfgRequest, candidate: OmfgCandidate): Promise<SaveCandidateResult> {
		...
		for (;;) {
			request.component.setStatus("saving", "选择保存位置或修改 TTSR 规则…");
			const location = await this.ctx.showHookSelector("将 TTSR 规则保存到哪里?", [
				PROJECT_OPTION,
				GLOBAL_OPTION,
				AMEND_OPTION,
			]);
			...
			if (location === AMEND_OPTION) {
				request.component.setStatus("confirming", "请描述如何修改该规则…");
				const amendment = await this.ctx.showHookInput(
					"修改 TTSR 规则",
					"例如:使其特定于 tool:write(*.rb) 中的 Ruby 字符串求值",
				);
				if (!this.#isActiveRequest(request)) return { kind: "aborted" };
			...
			if (await Bun.file(target.filePath).exists()) {
				const shouldOverwrite = await this.ctx.showHookConfirm(
					"覆盖 TTSR 规则?",
					`${shortenPath(target.filePath)} 已存在。要覆盖它吗?`,
				);
				if (!this.#isActiveRequest(request)) return { kind: "aborted" };
				if (!shouldOverwrite) {
					request.component.markRejected();
			...
			request.component.setStatus("saving", `正在保存 ${candidate.rule.name}…`);
