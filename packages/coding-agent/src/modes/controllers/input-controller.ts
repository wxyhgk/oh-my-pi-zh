...
export class InputController {
		...
		this.#enhancedPaste = new EnhancedPasteController({
			...
			pasteImage: async image => {
				...
				if (focused && focused !== this.ctx.editor && hasPasteText(focused)) {
					this.ctx.showStatus("此提示词中不支持粘贴图片");
					return;
				}
				await this.#normalizeAndInsertPastedImage(image, `不支持的粘贴图片格式:${image.mimeType}`);
			},
			...
			if (text.startsWith("!")) {
				...
				if (command) {
					if (this.ctx.session.isBashRunning) {
						this.ctx.showWarning("已有 Bash 命令正在运行。请先按 Esc 取消。");
			...
			if (pythonCommand) {
				...
				if (code) {
					if (this.ctx.session.isEvalRunning) {
						this.ctx.showWarning("已有 Python 执行正在运行。请先按 Esc 取消。");
						this.ctx.editor.setText(text);
						return;
					}
					this.ctx.editor.addToHistory(text);
	...
	async #submitToFocusedSession(text: string, streamingBehavior: "steer" | "followUp"): Promise<void> {
		...
		if (text && (text.startsWith("/") || text.startsWith("!") || parsePythonCommandInput(text))) {
			this.ctx.showStatus("命令在主会话中运行 — 请先按 ←← 返回");
	...
	handleCtrlZ(): void {
		...
		if (process.platform === "win32") {
			this.ctx.showStatus("此平台不支持挂起 (Ctrl+Z)");
			return;
		}
		...
		try {
			...
			this.ctx.showError(`挂起失败:${reason}`);
		}
	}
	...
	handleDequeue(): void {
		...
		if (restored === 0) {
			this.ctx.showStatus("没有可恢复的排队消息");
		} else {
			this.ctx.showStatus(`已将 ${restored} 条排队消息恢复到编辑器`);
		}
	}
	...
	async handleRetry(): Promise<void> {
		if (this.ctx.collabGuest) {
			this.ctx.showStatus("/retry 在协作会话中仅限主机使用");
			return;
		}
	...
	async #queueForYield(
		...
		if (splitMessages.length === 0 && !options.images?.length) {
			this.ctx.editor.clearDraft();
			this.ctx.showWarning("用法:/queue <message>(或以 -> / => 开始提示词)");
			return;
		}
		...
		if (queuedCount === messages.length) {
			this.ctx.showStatus(
				startImmediately
					? queuedCount === 1
						? "已发送排队的消息"
						: `已发送第一条消息;已将 ${queuedCount - 1} 条排队供后续让出时发送`
					: queuedCount === 1
						? "已排队,将在 Agent 让出时发送"
						: `已为 Agent 让出时排队 ${queuedCount} 条消息`,
			);
		}
		this.ctx.ui.requestRender();
	}
	...
	async handleImagePathPaste(path: string): Promise<void> {
		try {
			...
			if (!image) {
				...
				this.ctx.ui.requestRender();
				this.ctx.showStatus("粘贴的路径不是受支持的图片");
				return;
			}
			await this.#normalizeAndInsertPastedImage(
				{ type: "image", data: image.data, mimeType: image.mimeType },
				`不支持的粘贴图片格式:${image.mimeType}`,
			);
		} catch (error) {
			...
			if (isEnoent(error)) {
				...
				this.ctx.showStatus(
					overSsh
						? `在 ${displayPath} 未找到图片。通过 SSH 时,此路径仅在你的终端本地 — 请直接粘贴图片(剪贴板图片粘贴快捷键)以发送其内容。`
						: `在 ${displayPath} 未找到图片`,
				);
				return;
			}
			...
			this.ctx.editor.pasteText(path);
			this.ctx.ui.requestRender();
			this.ctx.showStatus("读取粘贴的图片路径失败");
		}
	}
	...
	async handleClipboardTextRawPaste(): Promise<void> {
		try {
			...
			if (text) {
				...
				this.ctx.showStatus("剪贴板中没有可原样粘贴的文本");
			}
		} catch {
			this.ctx.showStatus("从剪贴板粘贴原始文本失败");
		}
	}
	...
	async presentLargePasteMenu(text: string, lineCount: number): Promise<void> {
		const WRAPPED_BLOCK = "作为包裹块附加";
		const LOCAL_FILE = "作为本地文件附加";
		const INLINE = "直接粘贴";
		...
		try {
			choice = await this.ctx.showHookSelector(
				`已粘贴 ${lineCount} 行`,
				[
					{ label: WRAPPED_BLOCK, description: "将文本包裹在 <attachment> 标签中,折叠为标记" },
					{ label: LOCAL_FILE, description: "将文本保存到 local://paste 文件" },
					{ label: INLINE, description: "将文本折叠为内联粘贴标记" },
				],
				{ helpText: "Esc 直接粘贴" },
			);
		} catch (error) {
	...
	async #attachPasteAsFile(text: string, lineCount: number): Promise<void> {
		try {
			...
			this.ctx.showStatus(`已将 ${lineCount} 行粘贴内容保存到 local://${name}`);
		} catch (error) {
			logger.warn("failed to save large paste to file", {
				error: error instanceof Error ? error.message : String(error),
			});
			this.ctx.editor.insertPaste(text);
			this.ctx.showError("保存粘贴内容到文件失败 — 已改为直接粘贴");
		}
	}
	...
	handleCopyCurrentLine(): void {
		...
		try {
			...
			this.ctx.showStatus(`已复制行:${preview}`);
		} catch {
			this.ctx.showWarning("复制到剪贴板失败");
		}
	}
	...
	handleCopyPrompt(): void {
		...
		try {
			...
			const sanitized = sanitizeText(text);
			const preview = sanitized.length > 30 ? `${sanitized.slice(0, 30)}...` : sanitized;
			this.ctx.showStatus(`已复制:${preview}`);
		} catch {
			this.ctx.showWarning("复制到剪贴板失败");
		}
	}
	...
	cycleThinkingLevel(): void {
		if (this.ctx.focusedAgentId) {
			this.ctx.showStatus("模型/思考设置应用于主会话 — 请先按 ←← 返回");
			return;
		}
		const newLevel = this.ctx.session.cycleThinkingLevel();
		if (newLevel === undefined) {
			this.ctx.showStatus("当前模型不支持思考");
		} else {
	...
	async cycleRoleModel(direction: "forward" | "backward" = "forward"): Promise<void> {
		...
		try {
			...
			if (!result) {
				this.ctx.showStatus("只有一个角色模型可用");
				return;
			}
	...
	toggleToolOutputExpansion(): void {
		if (this.ctx.hideToolActivity) {
			...
			const visibilityHint = visibilityKey ? `${visibilityKey} 或 /settings` : "/settings";
			this.ctx.showStatus(`工具活动已隐藏 — 展开前请使用 ${visibilityHint} 显示它`);
			return;
		}
	...
	toggleToolActivityVisibility(): void {
		...
		this.ctx.ui.resetDisplay();
		this.ctx.showStatus(`工具活动:${this.ctx.hideToolActivity ? "已隐藏" : "可见"}`);
	}
	...
	toggleThinkingBlockVisibility(): void {
		...
		if (thinkingOff && !this.ctx.hasDisplayableThinkingContent) {
			this.ctx.showStatus("思考已关闭 — 请启用思考以显示内容块");
			return;
		}
		...
		this.ctx.showStatus(`思考块:${this.ctx.hideThinkingBlock ? "已隐藏" : "可见"}`);
	}
