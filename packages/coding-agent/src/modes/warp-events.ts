...
import { isInsideTmux, wrapTmuxPassthrough } from "@wxyhgk/pi-tui/terminal-capabilities";
import { VERSION } from "@wxyhgk/pi-utils/dirs";
	...
	return api => {
		...
		api.on("tool_approval_requested", event => {
			emitter?.emit({
				...
				summary: `omp 想要运行 ${event.toolName}`,
			});
		});
		...
		api.on("tool_execution_start", event => {
			if (event.toolName === "ask") {
				emitter?.emit({ event: "question_asked", summary: "等待你的回答" });
			}
		});
