...
const LOOP_USAGE = "用法:/loop [次数|时长]。示例:/loop 10、/loop 10m、/loop 10min。";
...
export function describeLoopLimitRuntime(limit: LoopLimitRuntime): string {
	if (limit.kind === "iterations") {
		return `剩余 ${limit.remaining}/${limit.initial} 次迭代`;
	}
	return `${formatDuration(limit.durationMs)} 限制`;
}
