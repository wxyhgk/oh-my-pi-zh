...
export class PluginListComponent extends Container {
	...
	constructor(
		...
		this.addChild(new Text(theme.bold(theme.fg("accent", "  插件")), 0, 0));
		this.addChild(new Spacer(1));

		if (entries.length === 0) {
			this.addChild(new Text(theme.fg("muted", "  未安装任何插件"), 0, 0));
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("dim", "  安装 npm 插件:        omp plugin install <package>"), 0, 0));
			this.addChild(
				new Text(theme.fg("dim", "  安装市场插件:         omp plugin install <name>@<marketplace>"), 0, 0),
			);
			this.addChild(new Spacer(1));
			this.addChild(new DynamicBorder());
		...
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", "  按 Enter 配置 · 按 Esc 返回"), 0, 0));
		this.addChild(new DynamicBorder());
	}

	#renderItem(entry: PluginListEntry): SelectItem {
		const kindBadge = theme.fg("dim", entry.kind === "npm" ? "[npm]" : "[市场]");
		...
		if (entry.kind === "npm") {
			...
			if (featureCount > 0) {
				details += ` ${theme.sep.dot} ${enabledCount}/${featureCount} 个功能`;
			}
		...
		if (summary.shadowedBy) {
			details += ` ${theme.sep.dot} 被 ${summary.shadowedBy} 遮蔽`;
		}
...
export class PluginDetailComponent extends Container {
	...
	async #rebuild(): Promise<void> {
		...
		if (manifest.features && Object.keys(manifest.features).length > 0) {
			...
			for (const [featName, feat] of Object.entries(manifest.features)) {
				...
				items.push({
					id: `feature:${featName}`,
					label: `  ${featName}`,
					description: feat.description || `启用 ${featName} 功能`,
			...
			for (const [key, schema] of Object.entries(manifest.settings)) {
				...
				if (schema.type === "boolean") {
					items.push({
						id: `config:${key}`,
						label: `  ${key}`,
						description: schema.description || `配置 ${key}`,
		...
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", "  按 Enter 编辑 · 按 Esc 返回"), 0, 0));
		this.addChild(new DynamicBorder());
	}
...
export class MarketplacePluginDetailComponent extends Container {
	...
	constructor(
		...
		if (plugin.shadowedBy) subtitleParts.push(`${theme.status.shadowed} 被 ${plugin.shadowedBy} 遮蔽`);
		...
		this.addChild(new Text(theme.fg("dim", `  版本          ${entry?.version ?? "未知"}`), 0, 0));
		this.addChild(new Text(theme.fg("dim", `  作用域          ${plugin.scope}`), 0, 0));
		this.addChild(
			new Text(
				theme.fg("dim", `  安装路径        ${entry?.installPath ? shortenPath(entry.installPath) : "未知"}`),
				0,
				0,
			),
		);
		this.addChild(new Text(theme.fg("dim", `  安装时间        ${entry?.installedAt ?? "未知"}`), 0, 0));
		this.addChild(new Text(theme.fg("dim", `  更新时间        ${entry?.lastUpdated ?? "未知"}`), 0, 0));
		if (entry?.gitCommitSha) {
			this.addChild(new Text(theme.fg("dim", `  Git 提交        ${entry.gitCommitSha}`), 0, 0));
		}

		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", "  按 Enter 切换 · 按 Esc 返回"), 0, 0));
		this.addChild(new DynamicBorder());
	}
...
class ConfigEnumSubmenu extends Container {
	...
	constructor(
		...
		this.addChild(this.#selectList);
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", "  按 Enter 选择 · 按 Esc 取消"), 0, 0));
	}
...
class ConfigInputSubmenu extends Container {
	...
	constructor(
		...
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", "  按 Enter 保存 · 按 Esc 取消"), 0, 0));
	}
