...
const LIST_FOOTER =
	" ↑/↓: 导航  Space: 切换  Enter: 模型覆盖  P: prewalk  N: 新建 Agent  ←/→: 来源  Ctrl+R: 重新加载  Esc: 关闭";

const IDENTIFIER_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+){1,5}$/;
function joinPatterns(patterns: string[]): string {
	if (patterns.length === 0) return "(会话模型)";
...
function parseGeneratedAgentSpec(raw: string): GeneratedAgentSpec {
	...
	if (
		...
		throw new Error("模型输出缺少必填字段 (identifier, whenToUse, systemPrompt)");
	}
	...
	if (!IDENTIFIER_PATTERN.test(identifier)) {
		throw new Error("生成的 identifier 无效(必须是小写 kebab-case,至少 2 个词)");
	}
	if (!whenToUse.toLowerCase().startsWith("use this agent when")) {
		throw new Error("生成的 whenToUse 必须以 'Use this agent when...' 开头");
	}
	if (!systemPrompt) {
		throw new Error("生成的 systemPrompt 为空");
	}
...
class AgentListPane implements Component {
	...
	render(width: number): readonly string[] {
		const lines: string[] = [];
		const searchPrefix = theme.fg("muted", "搜索: ");
		const searchText = this.searchQuery || theme.fg("dim", "输入以筛选");
		lines.push(`${searchPrefix}${searchText}`);
		lines.push("");

		if (this.agents.length === 0) {
			lines.push(theme.fg("muted", "  未找到 Agent。"));
			return lines;
		}
		...
		for (let i = start; i < end; i++) {
			...
			const override = agent.overrideModel ? ` ${theme.fg("warning", "(覆盖)")}` : "";
...
class AgentInspectorPane implements Component {
	...
	render(width: number): readonly string[] {
		if (!this.agent) {
			return [theme.fg("muted", "选择一个 Agent"), theme.fg("dim", "以查看设置")];
		}

		const lines: string[] = [];
		const state = this.agent.disabled
			? theme.fg("dim", `${theme.status.disabled} 已禁用`)
			: theme.fg("success", `${theme.status.enabled} 已启用`);

		lines.push(theme.bold(theme.fg("accent", replaceTabs(this.agent.name))));
		lines.push("");
		lines.push(`${theme.fg("muted", "状态:")} ${state}`);
		lines.push(`${theme.fg("muted", "来源:")} ${SOURCE_LABEL[this.agent.source]}`);
		lines.push("");

		lines.push(`${theme.fg("muted", "默认 pattern:")} ${replaceTabs(joinPatterns(this.defaultPatterns))}`);
		lines.push(
			`${theme.fg("muted", "默认解析结果:")} ${this.defaultResolution ? this.#formatResolution(this.defaultResolution) : theme.fg("dim", "(未解析)")}`,
		);
		lines.push(
			`${theme.fg("muted", "覆盖:")} ${this.agent.overrideModel ? theme.fg("warning", replaceTabs(this.agent.overrideModel)) : theme.fg("dim", "(无)")}`,
		);
		lines.push(`${theme.fg("muted", "生效 pattern:")} ${replaceTabs(joinPatterns(this.effectivePatterns))}`);
		lines.push(
			`${theme.fg("muted", "生效:")} ${this.effectiveResolution ? this.#formatResolution(this.effectiveResolution) : theme.fg("dim", "(未解析)")}`,
		);
		lines.push(`${theme.fg("muted", "Prewalk:")} ${this.#prewalkLabel()}`);

		if (this.agent.filePath) {
			lines.push("");
			lines.push(theme.fg("muted", "路径:"));
			lines.push(theme.fg("dim", `  ${replaceTabs(shortenPath(this.agent.filePath))}`));
		}

		if (this.agent.description) {
			lines.push("");
			lines.push(theme.fg("muted", "描述:"));
	...
	#prewalkLabel(): string {
		if (!this.agent) return theme.fg("dim", "关");
		...
		const sourceTag = override
			...
			: this.agent.prewalk !== undefined && this.agent.prewalk !== false
				? theme.fg("dim", " (Agent 默认)")
				: "";
		if (!this.prewalkPattern) {
			return `${theme.fg("dim", "关")}${override ? sourceTag : ""}`;
		}
		const target = this.prewalkResolution
			? this.#formatResolution(this.prewalkResolution)
			: theme.fg("dim", "(未解析)");
		return `${theme.fg("success", "开")} ${theme.fg("dim", `${replaceTabs(this.prewalkPattern)} →`)} ${target}${sourceTag}`;
	}
...
export class AgentDashboard extends Container {
	...
	#tabs: SourceTab[] = [{ id: "all", label: "全部", count: 0 }];
	#activeTabIndex = 0;
	#selectedIndex = 0;
	#scrollOffset = 0;
	#searchQuery = "";
	...
	async #reloadData(): Promise<void> {
		...
		try {
			...
			this.#allAgents = [];
			this.#filteredAgents = [];
			this.#tabs = [{ id: "all", label: "全部", count: 0 }];
	...
	#buildTabs(agents: DashboardAgent[]): SourceTab[] {
		const tabs: SourceTab[] = [{ id: "all", label: "全部", count: agents.length }];
	...
	#cyclePrewalkOverride(): void {
		...
		const state = selected.prewalkOverride ?? "Agent 默认";
		this.#notice = `已为 ${selected.name} 设置 Prewalk:${state}${pattern ? ` (使用 ${pattern})` : ""}`;
		this.#buildLayout();
	}
	...
	#saveModelOverride(rawValue: string): void {
		...
		this.#editInput = null;
		this.#applyFilters();
		this.#notice = `已更新 ${selected.name} 的模型覆盖`;
		this.#buildLayout();
	}
	...
	async #runAgentCreationArchitect(description: string): Promise<GeneratedAgentSpec> {
		...
		if (!selectedModel) {
			throw new Error("没有可用的模型来生成 Agent 规格。");
		}
		...
		try {
			...
			if (!raw) {
				throw new Error("Agent 创建流程未返回任何响应。");
			}
	...
	async #saveGeneratedAgent(): Promise<void> {
		...
		if (!targetDir) {
			throw new Error(`无法解析 ${this.#createScope} 的 agents 目录。`);
		}

		const filePath = path.join(targetDir, `${spec.identifier}.md`);
		try {
			await fs.stat(filePath);
			throw new Error(`Agent 文件已存在:${shortenPath(filePath)}`);
		} catch (error) {
			if (!isEnoent(error)) {
				throw error;
			}
		...
		this.#notice = `已在 ${shortenPath(filePath)} 创建 Agent ${spec.identifier}`;
		this.#rebuildAndRender();
	}
	...
	#renderCreateInput(): void {
		this.addChild(new Text(theme.bold(theme.fg("accent", " 新建 Agent")), 0, 0));
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("muted", "描述新 Agent 应该做什么:"), 0, 0));
		this.addChild(new Spacer(1));
		if (this.#createInput) {
		...
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("muted", `范围:${this.#createScope}`), 0, 0));
		if (this.#createGenerating) {
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("accent", "正在生成 Agent 规格..."), 0, 0));
			if (this.#createStreamingText) {
				...
				if (wrappedLines.length > maxPreview) {
					this.addChild(new Text(theme.fg("dim", `  ... 上方还有 ${wrappedLines.length - maxPreview} 行`), 0, 0));
				}
	...
	#renderCreateReview(): void {
		...
		this.addChild(new Text(theme.bold(theme.fg("accent", " 审核生成的 Agent")), 0, 0));
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("muted", `标识符:${spec.identifier}`), 0, 0));
		this.addChild(new Text(theme.fg("muted", `范围:${this.#createScope}`), 0, 0));
		this.addChild(new Spacer(1));
		...
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("muted", "systemPrompt 预览:"), 0, 0));
		...
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", " Enter: 保存  Tab: 切换范围  R: 重新生成  Esc: 取消"), 0, 0));
	}
	...
	#buildLayout(): void {
		...
		this.addChild(new Text(theme.bold(theme.fg("accent", " Agent 控制中心")), 0, 0));
		...
		if (this.#loading) {
			this.addChild(new Text(theme.fg("muted", "正在加载 Agents..."), 0, 0));
			this.addChild(new Spacer(1));
		} else if (this.#loadError) {
			this.addChild(new Text(theme.fg("error", `加载 Agents 失败:${replaceTabs(this.#loadError)}`), 0, 0));
			this.addChild(new Spacer(1));
			...
			this.addChild(
				new Text(theme.bold(theme.fg("accent", `模型覆盖:${replaceTabs(this.#editingAgentName)}`)), 0, 0),
			);
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("muted", "输入模型 pattern(留空则清除覆盖)"), 0, 0));
			this.addChild(new Spacer(1));
			...
			this.addChild(
				new Text(
					`${theme.fg("muted", "预览生效:")} ${previewResolution ? formatResolution(previewResolution) : theme.fg("dim", "(未解析)")}`,
					0,
					0,
				),
			);
			...
			if (suggestions.length > 0) {
				this.addChild(new Spacer(1));
				this.addChild(new Text(theme.fg("muted", "建议:"), 0, 0));
			...
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("dim", " Enter: 保存  Esc: 取消"), 0, 0));
		} else {
			const selected = this.#selectedAgent();
