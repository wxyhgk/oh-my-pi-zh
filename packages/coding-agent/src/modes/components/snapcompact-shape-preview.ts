 ...
 */
import { type Component, type ImageBudget, renderImage, TERMINAL } from "@wxyhgk/pi-tui";
import {
	DIM_OFF,
	DIM_ON,
	geometry,
...
export class SnapcompactShapePreview implements Component {
	...
	render(width: number): readonly string[] {
		...
		const stats = `整帧 ${geo.cols}×${geo.rows} 格 ≈ ${chars} 字符 ≈ ${tokens} tokens`;
		const lines: string[] = [theme.fg("muted", `  示例(放大) · ${label} · ${stats}`), ""];

		if (!this.#budget || !TERMINAL.imageProtocol) {
			lines.push(theme.fg("dim", "  (图形示例需要 Kitty 图形终端)"));
			return lines;
		}

		const entry = this.#ensureEntry(name, shape);
		if (entry.state === "rendering") {
			lines.push(theme.fg("dim", "  正在渲染示例…"));
			return lines;
		}
		if (entry.state === "failed") {
			lines.push(theme.fg("dim", "  (示例渲染失败)"));
			return lines;
		}
		...
		if (!result?.lines) {
			lines.push(theme.fg("dim", "  (图形示例需要 Kitty unicode-placeholder 图形)"));
			return lines;
		}
		if (result.transmit) {
