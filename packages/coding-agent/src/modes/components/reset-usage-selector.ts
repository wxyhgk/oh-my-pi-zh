...
export class ResetUsageSelectorComponent extends Container {
	...
	constructor(accounts: ResetUsageAccount[], onSelect: (account: ResetUsageAccount) => void, onCancel: () => void) {
		...
		this.addChild(new TruncatedText(theme.bold("消耗一个已保存的速率限制重置:")));
		this.addChild(new Spacer(1));
	...
	#updateList(): void {
		...
		const hint = pending
			? theme.fg("warning", `  再次按 Enter 为 ${pending.label} 消耗 1 个重置,Esc 取消`)
			: theme.fg("muted", "  ↑/↓ 选择 · ↵ 消耗重置 · Esc 取消");
	...
	handleInput(keyData: string): void {
		...
		if (matchesSelectUp(keyData)) {
			...
			if (account.availableCount <= 0) {
				this.#statusMessage = "该账户没有可消耗的已保存重置。";
				this.#updateList();
				return;
			}
