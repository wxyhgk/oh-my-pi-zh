...
import type { OAuthProviderInfo } from "@wxyhgk/pi-ai/oauth/types";
import {
	Container,
...
export class OAuthSelectorComponent extends Container {
	...
	constructor(
		...
		// Add title
		const title = mode === "login" ? "选择要登录的提供商：" : "选择要退出登录的提供商：";
	...
	#getSourceLabel(providerId: string): string {
		...
		if (!origin) return "";
		const detail = origin.kind === "env" && origin.envVar ? `环境变量: ${origin.envVar}` : ORIGIN_LABELS[origin.kind];
	...
	#renderStatusLine(_total: number): string {
		...
		const suffix = query ? `搜索: ${this.#searchQuery}` : "输入以搜索";
