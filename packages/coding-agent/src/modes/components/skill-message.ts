import type { TextContent } from "@wxyhgk/pi-ai";
import type { Component } from "@wxyhgk/pi-tui";
import { Box, Container, Markdown, Spacer, Text } from "@wxyhgk/pi-tui";
...
export class SkillMessageComponent extends Container {
	...
	#rebuild(): void {
		...
		const name = details?.name?.trim() || "未知";
		...
		const tag = theme.fg("customMessageLabel", theme.bold(`${theme.icon.extensionSkill} 技能`));
		...
		this.#box.addChild(new Text(theme.fg("muted", "提示词"), 0, 0));
		this.#box.addChild(new Spacer(1));
	...
	#metaLine(details: SkillPromptDetails | undefined): string | undefined {
		...
		if (typeof details?.lineCount === "number") {
			parts.push(theme.fg("muted", `${details.lineCount} 行`));
		}
