...
import { type Component, padding, truncateToWidth, visibleWidth } from "@wxyhgk/pi-tui";
...
export class FooterComponent implements Component {
	...
	#getCurrentBranch(): string | null {
		...
		this.#cachedBranch =
			headState === null ? null : headState.kind === "ref" ? (headState.branchName ?? headState.ref) : "游离";
		return this.#cachedBranch;
	}
	...
	render(width: number): readonly string[] {
		...
		if (state.model?.thinking) {
			if (this.session.isAutoThinking) {
				...
				rightSide = `${modelName} • ${resolved ? resolved : `${theme.thinking.autoPending} 自动`}`;
			} else {
