import { Box, Container, Spacer, Text } from "@wxyhgk/pi-tui";
...
export class TtsrNotificationComponent extends Container {
	...
	#rebuildSingle(rule: Rule): void {
		const header = `${theme.icon.warning} 注入规则: ${theme.bold(rule.name)}  ${theme.icon.rewind}`;
		...
		if (truncated) {
			this.#box.addChild(new Text(theme.italic(" (按 ctrl+o 展开)"), 0, 0));
		}
	}

	#rebuildMulti(): void {
		const header = `${theme.icon.warning} 注入 ${this.#rules.length} 条规则:  ${theme.icon.rewind}`;
		this.#box.addChild(new Text(header, 0, 0));
		this.#box.addChild(new Spacer(1));
		...
		if (hidden > 0) {
			this.#box.addChild(new Text(theme.italic(`… 还有 ${hidden} 条 (按 ctrl+o 展开)`), 0, 0));
