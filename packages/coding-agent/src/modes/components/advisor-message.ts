import { type Component, visibleWidth } from "@wxyhgk/pi-tui";
...
export function createAdvisorMessageCard(
	...
	const meta: string[] = [`${notes.length} 条建议`];
	if (blockers > 0) meta.push(uiTheme.fg("error", `${blockers} 个阻塞项`));
	...
	return createCachedComponent(
		...
		(width, expanded) => {
			...
			if (hidden > 0) {
				...
				lines.push(`  ${rail} ${uiTheme.fg("dim", `… 还有 ${hidden} 条建议`)}`);
			}
