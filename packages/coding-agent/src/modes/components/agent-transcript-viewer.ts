...
import { type Component, Editor, matchesKey, routeSgrMouseInput, ScrollView, type TUI } from "@wxyhgk/pi-tui";
import { formatDuration, formatNumber, logger } from "@wxyhgk/pi-utils";
...
function statusBadge(status: AgentStatus): string {
	switch (status) {
		...
		case "parked":
			return theme.fg("muted", "已暂停");
		case "aborted":
			return theme.fg("error", "已中止");
	}
}
...
export class AgentTranscriptViewer implements Component {
	...
	#headerLines(status: AgentStatus | undefined, kind: string | undefined, parentId: string | undefined): string[] {
		const lines = [theme.fg("accent", `Agent 中心 ${theme.sep.dot} ${this.deps.agentId}`)];
		if (status && kind) {
	...
	#footerLines(): string[] {
		...
		const hint = this.#editor
			? `Enter:发送  Esc:关闭  ${this.deps.expandKeys[0] ?? "ctrl+o"}:展开  输入为空 → j/k:滚动  g/G:顶部/底部`
	...
	#placeholder(maxWidth: number): string {
		if (this.deps.remote) {
			...
			if (this.#remoteUnavailable) return "转录内容保存在主机上,暂不可用。";
			return this.#hasRemoteData ? "暂无消息。" : "正在从主机加载转录…";
		}
		if (!this.deps.registry.get(this.deps.agentId)?.sessionFile) return "尚无会话文件。";
		return "暂无消息。";
	}
}
