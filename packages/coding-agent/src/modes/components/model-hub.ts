...
import { getCatalogProviderEntry } from "@wxyhgk/pi-catalog/provider-models";
import {
	type Component,
...
export class ModelHubComponent implements Component {
	...
	#emptyStateMessage(): string | undefined {
		...
		const entry = this.#activeEntry();
		if (entry.kind === "recent") return "  暂无最近使用的模型";
		if (entry.kind !== "provider" || entry.locked) return undefined;
		if (this.#browser.query.trim()) {
			return `  ${entry.label} 中没有匹配的模型。切换到全部模型可搜索所有提供商。`;
		}
		const providerId = entry.providerId ?? "";
		...
		switch (state.status) {
			case "cached":
				return age
					? `  正在使用 ${age} 的缓存模型列表。实时刷新仍在进行中。`
					: "  正在使用缓存模型列表。实时刷新仍在进行中。";
			case "unavailable": {
				const httpMatch = state.error?.match(/^HTTP (\d+) from (.+)$/);
				if (httpMatch?.[1] === "404") {
					return `  发现端点 ${httpMatch[2]} 返回 404。请将 baseUrl 指向提供 /models 服务的主机(通常是 .../v1)。`;
				}
				if (state.error) return `  发现失败:${state.error}`;
				return age ? `  提供商不可用。正在使用 ${age} 的缓存模型列表。` : "  提供商不可用。";
			}
			case "unauthenticated":
				return "  提供商需要先进行身份验证才能发现模型。";
			case "idle":
				return "  提供商尚未刷新。";
			case "empty":
				return "  发现成功但返回了 0 个模型。请检查 /models 是否返回 { data: [{ id }] }。";
			case "ok":
				return undefined;
		}
	}
	...
	#openRoleStrip(item: ModelBrowserItem): void {
		...
		chips.push({
			label: `回退:${item.model.id}`,
			styled: theme.fg("muted", `回退:${item.model.id}`),
			action: "fallbackModel",
		});
		chips.push({
			label: `回退:${item.model.provider}/*`,
			styled: theme.fg("muted", `回退:${item.model.provider}/*`),
			action: "fallbackProvider",
		});
		chips.push({ label: "重试回退", styled: theme.fg("muted", "重试回退"), action: "fallback" });
	...
	#openFallbackKeyStrip(item: ModelBrowserItem): void {
		const chips: StripChip[] = [
			{
				label: `用于 ${item.selector}`,
				styled: theme.fg("muted", `用于 ${item.selector}`),
				action: "fallbackModel",
			},
			{
				label: `用于 ${item.model.provider}/*`,
				styled: theme.fg("muted", `用于 ${item.model.provider}/*`),
				action: "fallbackProvider",
			},
		];
		this.#strip = { kind: "role", item, chips, index: 0, returnToRoles: false };
	...
	#statusRow(width: number): string {
		if (this.#assigning !== null) {
			...
			if (this.#assigning.kind === "fallback") {
				const verb = this.#assigning.index === null ? "添加回退" : "替换回退";
				return truncateToWidth(
					theme.fg("accent", ` ${verb} ${theme.bold(label)} — 按 Enter 选择回退模型,Esc 取消`),
					width,
				);
			}
			return truncateToWidth(
				theme.fg("accent", ` 正在为 ${theme.bold(label)} 分配 — 按 Enter 确认,Esc 取消`),
				width,
			);
		}
		const entry = this.#activeEntry();
		...
		switch (entry.kind) {
			case "recent":
				...
				break;
			case "roles":
				text = "模型角色 — f 添加重试回退,被清除的角色回退到自动选择";
				break;
			case "provider":
				if (entry.locked) {
	...
	#renderRolesView(width: number, rows: number): string[] {
		...
		for (let i = 0; i < this.#rolesRows.length && lines.length < rows - 2; i++) {
			...
			if (rowDef.kind === "newRole" || rowDef.kind === "newFallback") {
				const label = rowDef.kind === "newRole" ? "+ 新建角色…" : "+ 新建回退…";
	...
	#renderLockedView(entry: SidebarEntry, width: number, rows: number): string[] {
		...
		if (entry.oauth) {
			...
			lines.push(truncateToWidth(theme.fg("accent", `  ${theme.nav.cursor} 使用 OAuth 登录(按 Enter)`), width));
		}
		lines.push("");
	...
	#footerHint(): string {
		...
		if (strip) {
			if (strip.kind === "roleName") {
				return "Enter 新建并选择模型 · Esc 取消";
			}
			if (strip.kind === "role") return "←/→ 选择 · Enter 分配/清除 · Esc 取消";
			if (strip.kind === "scope") return "←/→ 保存范围 · Enter 选择 · Esc 取消";
			return "←/→ 思考级别 · Enter 应用 · Esc 保留";
		}
		if (this.#assigning !== null) {
			switch (this.#assigning.kind) {
				case "fallback":
					return "Enter 选择回退 · ↑/↓ 提供商 · 输入搜索 · Esc 取消";
				case "fallbackKey":
					return "Enter 选择受保护的模型 · ↑/↓ 提供商 · 输入搜索 · Esc 取消";
				default:
					return "Enter 分配 · ↑/↓ 提供商 · 输入搜索 · Esc 取消";
			}
		}
		const entry = this.#activeEntry();
		if (entry.kind === "roles") {
			if (this.#focus !== "list") {
				return "↑/↓ 提供商 · → 角色 · Esc 关闭";
			}
			const row = this.#rolesRows[this.#roleIndex];
			if (row?.kind === "fallback") {
				return "↑/↓ 行 · Enter 替换 · f 再添加 · x 移除 · [/] 排序 · ← 提供商";
			}
			if (row?.kind === "chainKey") {
				return "↑/↓ 行 · Enter/f 添加回退 · x 清空链 · ← 提供商";
			}
			if (row?.kind === "newFallback") {
				return "↑/↓ 行 · Enter 新建模型/提供商回退链 · ← 提供商";
			}
			return "↑/↓ 行 · Enter 选择 · f 回退 · x 清除 · t 思考 · c 循环 · [/] 排序 · n 新建";
		}
		if (entry.kind === "provider" && entry.locked) {
			return entry.oauth ? "Enter 登录 · ↑/↓ 提供商 · Esc 关闭" : "↑/↓ 提供商 · Esc 关闭";
		}
		const arrows = this.#focus === "scope" ? "↑/↓ 提供商 · → 模型" : "↑/↓ 模型 · ← 提供商";
		const refresh = entry.kind === "provider" ? " · F5 刷新" : "";
		return `Enter 分配角色 · ${arrows} · 输入搜索${refresh} · Esc 关闭`;
	}
