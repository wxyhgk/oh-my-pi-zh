...
import type { ShapeTarget } from "@wxyhgk/snapcompact";
import {
	getDefault,
	getType,
...
class TextInputSubmenu extends Container {
	...
	constructor(
		...
		this.addChild(new Text(theme.fg("dim", "  回车保存 · Esc 取消 · 清空字段以取消设置"), 0, 0));
	}
	...
	#showProviderList(): void {
		...
		const providerItems = this.#providerIds().map((provider): SelectItem => {
			...
			return {
				value: provider,
				label: provider,
				description: limit === undefined ? "无限制" : `限制: ${limit}`,
			};
		});
		const clearItem: SelectItem[] =
			Object.keys(limits).length === 0
				? []
				: [{ value: "__clear_all", label: "清除所有限制", description: "让所有提供商无限制" }];
		...
		this.addChild(this.#selectList);
		this.addChild(new Spacer(1));
		this.addChild(new Text(theme.fg("dim", "  回车编辑提供商 · Esc 返回"), 0, 0));
	}
			...
			new TextInputSubmenu(
				...
				value => {
					...
					if (trimmed === "") {
						...
						if (!Number.isFinite(limit) || limit <= 0) throw new Error("限制必须是正数。");
...
export class SettingsSelectorComponent implements Component {
	...
	#footerHintText(): string {
		if (this.#searchList) {
			return "回车修改 · Tab 跳转标签页 · Esc 退出搜索";
		}
		if (this.#currentTabId === "plugins") {
			return "Tab 切换标签页 · Esc 关闭";
		}
		if (this.#currentList?.sectionFocused) {
			return "↑/↓ 跳转分组 · Tab/回车进入设置 · ←/→ 切换标签页 · Esc 关闭";
		}
		const nav = this.#hasSectionJump ? "Tab 跳转分组 · ←/→ 切换标签页" : "Tab 切换标签页";
		return `回车/空格修改 · ${nav} · 输入以搜索 · Esc 关闭`;
	}
	...
	#formatProviderLimitsValue(value: unknown): string {
		...
		if (entries.length === 0) return "无限制";
	...
	#setSettingValue(path: SettingPath, value: string): void {
		...
		if (path === "compaction.thresholdPercent" && value === "default") {
			...
			try {
				...
				throw new Error(`记录 JSON 无效: ${path}`);
			}
