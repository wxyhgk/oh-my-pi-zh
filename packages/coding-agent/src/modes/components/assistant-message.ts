...
export class AssistantMessageComponent extends Container {
	...
	#thinkingDotsLabel(): string {
		...
		const thinkingLabel = theme.fg("muted", " 思考");
	...
	#appendErrorBlock(message: string): void {
		if (this.#errorExpanded) {
			const [first = "未知错误", ...rest] = replaceTabs(message.replace(/\s+$/, "")).split("\n");
			this.#contentContainer.addChild(new Text(theme.fg("error", `错误: ${first}`), 1, 0));
		...
		if (lines.length === 0) lines.push("未知错误");
		// The caller owns the separating Spacer; adding one here doubled the gap.
		this.#contentContainer.addChild(new Text(theme.fg("error", `错误: ${lines[0]}`), 1, 0));
	...
	#renderImageEntries(entries: Array<{ image: ImageContent; key: string }>, withLeadingSpacer: boolean): void {
		...
		for (const { image, key } of entries) {
			...
			this.#contentContainer.addChild(new Text(theme.fg("toolOutput", `[图片: ${image.mimeType}]`), 1, 0));
		}
	}
