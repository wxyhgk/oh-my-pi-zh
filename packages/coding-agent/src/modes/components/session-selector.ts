...
function formatSessionStatus(status: SessionStatus | undefined): string | undefined {
	switch (status) {
		...
		case "pending":
			return theme.fg("accent", `${theme.status.pending} 待处理`);
		default:
			return undefined;
	}
}
...
export class SessionSelectorComponent extends Container {
	...
	#headerLabel(): string {
		...
		const scopeLabel = this.#scopeLabel ?? (this.#scope === "all" ? "全部项目" : "当前文件夹");
	...
	async #toggleScope(): Promise<void> {
		...
		if (this.#scope === "folder") {
			...
			if (!global) {
				...
				this.#messageContainer.addChild(new Text(theme.fg("muted", "  正在加载全部项目…"), 1, 0));
				this.#onRequestRender?.();
				try {
	...
	#showError(message: string): void {
		this.#messageContainer.clear();
		this.#messageContainer.addChild(new Text(theme.fg("error", `错误: ${replaceTabs(message)}`), 1, 0));
	...
	#footerLines(width: number): string[] {
		const scopeHint = this.#scope === "all" ? "当前文件夹" : "全部项目";
