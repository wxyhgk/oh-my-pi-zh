...
function formatCostPair(model: Model): string {
	...
	if (!cost || (cost.input <= 0 && cost.output <= 0)) return "免费";
...
export class ModelBrowser implements Component {
	...
	#detailLines(width: number): [string, string] {
		...
		facts.push(`${formatCostPair(model)} 每 M`);
		...
		if (perf) {
			...
			if (perf.ttftMs !== null) facts.push(`${formatTtft(perf.ttftMs)} 首字延迟`);
		}
		const line1 = truncateToWidth(theme.fg("muted", `  ${facts.join(" · ")}`), width);

		if (this.isOverContext(selected)) {
			const warning = `  ${theme.status.disabled} 上下文 ${formatNumber(this.#currentContextTokens).toLowerCase()} 超过 ${formatNumber(model.contextWindow ?? 0).toLowerCase()} 限制 · 先用当前模型压缩,再切换`;
	...
	render(width: number): string[] {
		...
		if (total === 0) {
			const message = this.#emptyText?.() ?? (this.query.trim() ? "  没有匹配的模型" : "  此范围内没有可用模型");
