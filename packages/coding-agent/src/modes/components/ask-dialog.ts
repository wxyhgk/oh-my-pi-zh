...
function questionTabLabel(question: ExtensionAskDialogQuestion, index: number): string {
	const base = question.header?.trim() || question.id || `问题${index + 1}`;
...
export class AskDialogComponent implements Component {
	...
	#renderHeader(width: number): string[] {
		...
		if (this.#isSubmitTab()) {
			lines.push(theme.bold(theme.fg("accent", "检查答案")));
			return lines;
		}
	...
	#footerHintText(indicator: string): string {
		...
		if (this.#isSubmitTab()) {
			...
			return `Enter 提交 · ↑/↓ 滚动 ·${scroll} ${cancel}`;
		}
		const question = this.#questions[this.#currentQuestionIndex()];
		const action = question?.multi ? "Space/Enter 切换 · n 备注" : "Enter 选择 · n 备注";
