...
const DEFAULT_HELP_SUFFIX = "esc 取消";
...
export class PlanReviewOverlay implements Component {
	...
	#buildHelp(): string {
		...
		switch (this.#focus) {
			case "actions":
				parts.push("↑↓ 选择", "⏎ 确认");
				if (this.#slider) parts.push("◂▸ 模型");
				break;
			case "toc":
				parts.push("↑↓ 章节", "⏎ 打开", "a 批注", "d 删除", "u 撤销");
				break;
			case "body":
				parts.push("↑↓ 滚动", "⇧ 加速", "pgup/pgdn", "g/G 两端", "a 批注");
				break;
		}
		if (this.callbacks.onCopyPlan) parts.push("c 复制");
		parts.push("tab 区域");
		if (this.#externalEditorLabel && this.#focus !== "toc") parts.push(`${this.#externalEditorLabel} 编辑器`);
	...
	#appendAnnotationCallout(
		...
		for (let i = 0; i < noteLines.length; i++) {
			const prefix =
				i === 0
					? `${theme.fg("warning", "▎ ")}${theme.fg("dim", "备注: ")}`
	...
	#renderTocEntry(p: number, width: number): string {
		...
		const title = truncateToWidth(section.title || "(无标题)", avail, Ellipsis.Unicode);
	...
	#renderFooterLines(innerWidth: number): string[] {
		if (this.#annotating) {
			const target = this.#annotationTarget;
			const section = target ? this.#sections[target.sectionIndex] : undefined;
			const title = sanitizeStatusText(section?.title || "计划前言");
			const location =
				target?.row === null
					? `‹${title}›`
					: `‹${title}› · ${truncateToWidth(target?.context ?? "", Math.max(1, innerWidth - 16), Ellipsis.Unicode)}`;
			const caption = truncateToWidth(
				`${theme.fg("dim", "批注")} ${theme.fg("accent", location)}`,
				innerWidth,
				Ellipsis.Unicode,
			);
			const hintParts = ["enter 保存", "esc 取消"];
			if (this.#externalEditorLabel) hintParts.push(`${this.#externalEditorLabel} 编辑器`);
	...
	render(width: number): readonly string[] {
		...
		const submittingLabel = this.#committedLabel ? `${this.#committedLabel} — 提交中…` : "提交中…";
		...
		const footerLines = committed
			? [theme.fg("dim", "正在应用你的选择——准备上下文可能需要一点时间。")]
