...
import { formatDuration, formatNumber, getProjectDir, pathIsWithin, relativePathWithinRoot } from "@wxyhgk/pi-utils";
...
function formatLoopLimit(limit: NonNullable<SegmentContext["loopMode"]>["limit"]): string | undefined {
	...
	if (hours > 0) return `剩余 ${hours}h${minutes > 0 ? `${minutes}m` : ""}`;
	if (minutes > 0) return `剩余 ${minutes}m${seconds > 0 ? `${seconds}s` : ""}`;
	return `剩余 ${seconds}s`;
}
...
const modeSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		if (loop) {
			...
			const stateLabel = loop.state === "running" ? "运行中" : loop.state === "paused" ? "已暂停" : "等待中";
			const parts = [withIcon(icon, `循环 ${stateLabel}`)];
	...
	render(ctx) {
		...
		const label =
			ctx.collab.role === "host"
				? `⇄ 协作:${ctx.collab.participantCount}`
				: `⇄ 协作访客:${ctx.collab.participantCount}`;
