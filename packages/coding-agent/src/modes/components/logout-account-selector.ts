import { Container, matchesKey, ScrollView, Spacer, TruncatedText } from "@wxyhgk/pi-tui";
...
export class LogoutAccountSelectorComponent extends Container {
	...
	constructor(
		...
		this.addChild(new TruncatedText(theme.bold(`选择要退出登录的 ${providerName} 账户:`)));
		this.addChild(new Spacer(1));
	...
	#updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			if (!account) continue;
			const activeTag = account.active ? theme.fg("muted", " (当前)") : "";
		...
		if (total === 0) {
			this.#listContainer.addChild(new TruncatedText(theme.fg("muted", "  没有已保存的账户可退出登录"), 0, 0));
		}

		this.#listContainer.addChild(
			new TruncatedText(theme.fg("muted", "  ↑/↓ 选择 · ↵ 退出登录账户 · Esc 取消"), 0, 0),
		);
