import { type Component, matchesKey, padding, Text, truncateToWidth, visibleWidth } from "@wxyhgk/pi-tui";
...
export class CopySelectorComponent implements Component {
	...
	#renderPreview(width: number, target: CopyTarget | undefined, rows: number): string[] {
		const out: string[] = [];
		const hint = target?.hint;
		out.push(row(theme.fg("dim", `预览${hint ? ` · ${hint}` : ""}`), width));
		...
		for (let k = 0; k < contentRows; k++) {
			if (k < visibleCount) {
				...
				out.push(row(theme.fg("dim", `… 还有 ${wrapped.length - visibleCount} 行`), width));
			} else {
				out.push(row("", width));
			}
		}
	...
	render(width: number): readonly string[] {
		...
		const footer = [
			rawKeyHint("↑↓", "移动"),
			keyHint("tui.select.confirm", "复制"),
			keyHint("tui.select.cancel", "退出"),
		].join(theme.fg("dim", " · "));

		return [
			topBorder(width, "复制到剪贴板"),
