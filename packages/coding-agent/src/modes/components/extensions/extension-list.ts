 ...
 */
import { type Component, matchesKey, padding, truncateToWidth, visibleWidth } from "@wxyhgk/pi-tui";
...
export class ExtensionList implements Component {
	...
	render(width: number): readonly string[] {
		...
		const searchText = this.#searchQuery || (this.#focused ? "" : theme.fg("dim", "输入以筛选"));
		const cursor = this.#focused ? theme.fg("accent", "_") : "";
		lines.push(searchPrefix + searchText + cursor);
		lines.push("");

		if (this.#listItems.length === 0) {
			lines.push(theme.fg("muted", "  此提供商下未找到扩展。"));
			return lines;
		}
	...
	#renderMasterSwitch(item: ListItem & { type: "master" }, isSelected: boolean, width: number): string {
		...
		const label = `启用 ${item.providerName}`;
		const badge = theme.fg("warning", "(总开关)");
