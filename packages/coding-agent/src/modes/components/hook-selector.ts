...
export class HookSelectorComponent extends Container {
	...
	constructor(
		...
		this.addChild(new Spacer(1));
		const controlsHint = opts?.helpText ?? "up/down 导航  enter 选择  esc 取消";
	...
	#updateList(renderWidth = this.#lastRenderWidth): void {
		...
		if (total === 0) {
			rows.push({ text: theme.fg("dim", "  无匹配选项"), highlight: false });
		}
	...
	#renderStatusLine(total: number): string {
		...
		const count =
			this.#searchQuery.trim() && total !== this.#options.length
				? `${selectedCount}/${total} 共 ${this.#options.length}`
				: `${selectedCount}/${total}`;
		const suffix = this.#searchQuery.trim() ? `  搜索: ${this.#searchQuery}` : "  输入以搜索";
