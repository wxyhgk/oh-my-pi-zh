	...
	render(width: number): readonly string[] {
		...
		if (this.#filteredNodes.length === 0) {
			...
			if (this.#flatNodes.length === 0) {
				...
				lines.push(
					truncateToWidth(
						theme.fg("muted", `  当前筛选隐藏了 ${this.#flatNodes.length} 个条目 ${filterLabel}`),
						width,
					),
				);
	...
	constructor(
		...
		this.addChild(
			new TruncatedText(
				theme.fg(
					"muted",
					"Enter: 切换。Alt+↑/↓: 上一/下一轮次。PgUp/PgDn (←/→): 翻页。Home/End: 首项/末项。Shift+Enter: 生成摘要并切换。Shift+L: 标签。Ctrl+O: 筛选。Alt+D/T/U/L/A: 筛选。输入以搜索",
				),
				0,
				0,
			),
