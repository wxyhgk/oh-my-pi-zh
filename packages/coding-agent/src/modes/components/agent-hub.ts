 ...
 */
import { type AgentTool, ThinkingLevel } from "@wxyhgk/pi-agent-core";
import { Container, Ellipsis, matchesKey, type OverlayHandle, padding, type TUI, visibleWidth } from "@wxyhgk/pi-tui";
import { formatAge, getProjectDir, logger } from "@wxyhgk/pi-utils";
...
function modelBadge(ref: AgentRef, observed: ObservableSession | undefined): string | undefined {
	...
	if (fallbackSelector) {
		return `${theme.fg("warning", "回退 →")} ${formatResolvedModelBadge(fallbackSelector, true)}`;
	}
...
export class AgentHubOverlayComponent extends Container {
	...
	#renderTable(width: number): string[] {
		...
		lines.push(` ${theme.fg("accent", "Agent 中心")}${counts ? theme.fg("dim", `${theme.sep.dot}${counts}`) : ""}`);
		lines.push(...new DynamicBorder().render(width));

		if (this.#rows.length === 0) {
			lines.push(` ${theme.fg("dim", "尚无子 Agent — 派生的任务 Agent 会显示在这里")}`);
		} else {
			...
			if (start > 0) {
				lines.push(` ${theme.fg("dim", `… 还有 ${start} 条`)}`);
			}
			for (let i = start; i < end; i++) {
				lines.push(...entries[i]);
			}
			if (end < this.#rows.length) {
				lines.push(` ${theme.fg("dim", `… 还有 ${this.#rows.length - end} 条`)}`);
			}
		}
		...
		lines.push("");
		lines.push(` ${theme.fg("dim", "j/k:选择  Enter:打开  r:恢复  x:终止  Esc/←←:关闭")}`);
		lines.push(...new DynamicBorder().render(width));
		return lines;
	}
	...
	#statusSummary(): string {
		...
		for (const status of ["running", "idle", "parked", "aborted"] as const) {
			...
			if (count > 0) parts.push(`${count} ${STATUS_LABELS[status]}`);
		}
	...
	#reviveSelected(): void {
		...
		if (!ref) return;
		if (ref.kind === "advisor") {
			this.#notice = `"${ref.id}" 是只读的 advisor 转录 — 没有可恢复的内容。`;
			this.#requestRender();
			return;
		}
		if (ref.status !== "parked") {
			this.#notice = `Agent "${ref.id}" 当前状态为 ${ref.status} — 只有已暂停的 Agent 可以恢复。`;
			this.#requestRender();
			return;
		}
		this.#notice = undefined;
	...
	#killSelected(): void {
		...
		if (!ref) return;
		if (ref.kind === "advisor") {
			this.#notice = `"${ref.id}" 是只读的 advisor 转录 — 无法终止。`;
			this.#requestRender();
			return;
		}
		this.#notice = undefined;
