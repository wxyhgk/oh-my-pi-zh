...
import { AstMatchStrictness, astMatch } from "@wxyhgk/pi-natives";
...
export class TtsrManager {
	...
	#compileConditions(rule: Rule): RegExp[] {
		...
		for (const pattern of rule.condition ?? []) {
			try {
				...
				logger.warn("TTSR 条件包含无效的正则表达式,已跳过该条件", {
					ruleName: rule.name,
					pattern,
	...
	#buildScope(rule: Rule): TtsrScope {
		...
		for (const rawToken of rule.scope) {
			...
			if (!toolScope) {
				logger.warn("TTSR 作用域标记无效,已跳过该标记", {
					ruleName: rule.name,
					token: rawToken,
				});
				continue;
	...
	addRule(rule: Rule): boolean {
		...
		if (!this.#hasReachableScope(scope)) {
			logger.warn("TTSR 作用域排除了所有流,已跳过该规则", {
				ruleName: rule.name,
				scope: rule.scope,
			});
			return false;
	...
	async #astConditionsMatch(patterns: string[], source: string, lang: string): Promise<boolean> {
		try {
			...
			logger.warn("TTSR ast 匹配失败,视为无匹配", {
				patterns,
				lang,
				error: error instanceof Error ? error.message : String(error),
			});
	...
	restoreInjected(ruleNames: string[]): void {
		...
		if (ruleNames.length > 0) {
			logger.debug("TTSR 注入状态已恢复", { ruleNames });
		}
	}
