    (function() {
        ...
        switch (entry.type) {
          case 'message': {
            ...
            if (msg.role === 'developer') {
              const content = truncate(normalize(extractContent(msg.content)));
              return labelHtml + `<span class="tree-role-developer">开发者:</span> ${escapeHtml(content)}`;
            }
          ...
          case 'compaction':
            return labelHtml + `<span class="tree-compaction">[上下文压缩: ${Math.round(entry.tokensBefore/1000)}k tokens]</span>`;
          case 'branch_summary': {
          ...
          case 'model_change':
            return labelHtml + `<span class="tree-muted">[模型: ${escapeHtml(entry.model)}]</span>`;
          case 'thinking_level_change':
            return labelHtml + `<span class="tree-muted">[思考: ${entry.thinkingLevel}]</span>`;
          case 'mode_change':
            return labelHtml + `<span class="tree-muted">[模式: ${escapeHtml(entry.mode)}]</span>`;
          default:
      ...
      function renderTree() {
        ...
        document.getElementById('tree-status').textContent = `${filtered.length} / ${flatNodes.length} 条条目`;
      ...
      function ensureSubOverlay() {
        ...
        subOverlayEl.innerHTML = `
          ...
          <div class="subsession-panel" role="dialog" aria-modal="true" aria-label="子代理会话" tabindex="-1">
            <div class="subsession-header">
              <nav class="subsession-breadcrumb" aria-label="子代理面包屑导航"></nav>
              <button type="button" class="subsession-close" title="关闭 (Esc)" aria-label="关闭子代理视图">&times;</button>
            </div>
      ...
      function subSessionMetaText(key) {
        ...
        parts.push(sub.entries.length + ' 条条目');
        return parts.join(' · ');
      }
      ...
      function renderCopyLinkButton(entryId) {
        return `<button class="copy-link-btn" data-entry-id="${entryId}" title="复制此消息的链接">
      ...
      function renderEntry(entry, sctx) {
        ...
        if (entry.type === 'model_change') {
          const html = `<div class="model-change" id="${entryId}">${tsHtml}已切换模型: <span class="model-name">${escapeHtml(entry.model)}</span></div>`;
          return html;
        }

        if (entry.type === 'thinking_level_change') {
          const html = `<div class="thinking-change" id="${entryId}">${tsHtml}思考级别: <span class="thinking-level">${escapeHtml(entry.thinkingLevel)}</span></div>`;
          return html;
        }
        ...
        if (entry.type === 'compaction') {
          return `<div class="compaction" id="${entryId}" onclick="this.classList.toggle('expanded')">
            <div class="compaction-label">[上下文压缩]</div>
      ...
      function renderHeader() {
        ...
        const msgParts = [];
        if (globalStats.userMessages) msgParts.push(`${globalStats.userMessages} 条用户`);
        if (globalStats.developerMessages) msgParts.push(`${globalStats.developerMessages} 条开发者`);
        if (globalStats.assistantMessages) msgParts.push(`${globalStats.assistantMessages} 条助手`);
        if (globalStats.toolResults) msgParts.push(`${globalStats.toolResults} 条工具结果`);
        if (globalStats.customMessages) msgParts.push(`${globalStats.customMessages} 条自定义`);
        ...
        let html = `
          <div class="header">
            ...
            <div class="help-bar">T 切换思考 · O 切换工具</div>
            <div class="header-info">
              ...
              <div class="info-item"><span class="info-label">模型:</span><span class="info-value">${globalStats.models.join(', ') || '未知'}</span></div>
        ...
        if (tools && tools.length > 0) {
          ...
          html += `<div class="tools-list collapsed" onclick="this.classList.toggle('collapsed')">
            <div class="tools-header">可用工具(${tools.length})</div>
      ...
      function showLoadError(err) {
        ...
        div.textContent = '加载会话失败: ' + (err && err.message ? err.message : String(err));
