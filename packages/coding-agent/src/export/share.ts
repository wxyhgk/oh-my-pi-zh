...
export async function sealToFit(key: CryptoKey, data: SessionData, maxBytes: number): Promise<SealedSession> {
	...
	throw new Error(`会话过大,无法分享:密封后 ${sealed.byteLength} 字节超过了 ${maxBytes} 字节的限制`);
}
...
async function tryCreateGist(sealed: Uint8Array): Promise<{ id: string; url: string } | null> {
	...
	if (auth.exitCode !== 0) {
		logger.debug("share:已安装 gh 但未认证;回退到分享服务器");
		return null;
	}
	...
	try {
		...
		if (result.exitCode !== 0) {
			logger.warn("share:gist 创建失败;回退到分享服务器", {
		...
		if (!GIST_ID_RE.test(id)) {
			logger.warn("share:无法从 gh 输出解析 gist id", { url });
			return null;
		}
		return { id, url };
	} finally {
...
async function uploadToServer(sealed: Uint8Array, base: string): Promise<string> {
	...
	try {
		...
		throw new Error(`分享上传到 ${base} 失败:${err instanceof Error ? err.message : String(err)}`);
	}
	if (!res.ok) {
		const detail = (await res.text().catch(() => "")).trim().slice(0, 200);
		throw new Error(`分享上传到 ${base} 失败:HTTP ${res.status}${detail ? ` (${detail})` : ""}`);
	}
	const body = (await res.json().catch(() => null)) as { id?: unknown } | null;
	const id = body && typeof body.id === "string" ? body.id : "";
	if (!/^[A-Za-z0-9_-]{10,64}$/.test(id)) {
		throw new Error(`分享上传到 ${base} 失败:服务器未返回可用的 id`);
	}
	return id;
}
