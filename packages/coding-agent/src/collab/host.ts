...
const TRANSCRIPT_ENTRY_TOO_LARGE_ERROR = `转录条目超过转录获取上限(${TRANSCRIPT_READ_CAP} 字节)`;
/**
...
export class CollabHost {
	...
	async start(relayUrl: string, webUrl = ""): Promise<void> {
		...
		socket.onClose = (reason, willReconnect) => {
			...
			if (willReconnect) {
				this.#ctx.showStatus(`协作中继连接已断开(${reason}),正在重新连接…`, { dim: true });
			} else {
				void this.#teardown();
				this.#ctx.session.emitNotice("warning", `协作已结束:${reason}`, "collab");
			}
		};
		socket.connect();
	...
	#rejectReadOnly(action: string, fromPeer: number): void {
		this.#socket?.send({ t: "error", message: `在只读链接上禁用 ${action}` }, fromPeer);
	}

	#handleHello(name: string, proto: number, writeToken: string | undefined, fromPeer: number): void {
		if (proto !== COLLAB_PROTO) {
			this.#socket?.send(
				{ t: "error", message: `协议不匹配:主机使用 v${COLLAB_PROTO},访客发送了 v${proto}` },
				fromPeer,
			);
			return;
		}
	...
	#handlePrompt(text: string, images: ImageContent[] | undefined, fromPeer: number): void {
		...
		this.#ctx.session
			...
			.catch(err => {
				...
				this.#socket?.send({ t: "error", message: `提示发送失败:${String(err)}` }, fromPeer);
			});
	}
	...
	#handleAgentCmd(cmd: "chat" | "kill" | "revive", agentId: string, text: string | undefined, fromPeer: number): void {
		...
		if (AgentRegistry.global().get(agentId)?.kind === "advisor") {
			this.#socket?.send({ t: "error", message: `Agent ${agentId}:顾问转录记录为只读` }, fromPeer);
			return;
		}
		...
		switch (cmd) {
			case "chat": {
				...
				if (!trimmed) {
					this.#socket?.send({ t: "error", message: `Agent ${agentId}:聊天消息为空` }, fromPeer);
					return;
				}
