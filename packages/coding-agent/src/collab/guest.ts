...
import { getConfigRootDir, logger } from "@wxyhgk/pi-utils";
...
export class CollabGuestLink {
	...
	async join(link: string): Promise<void> {
			...
			this.#applyChain = this.#applyChain
				.then(async () => {
					...
					if (frame.t === "error" && !this.#welcomed && !this.#left) {
						...
						this.#clearWelcomeTimer();
						if (joined) this.#ctx.showError(`协作主机:${frame.message}`);
		...
		socket.onClose = (reason, willReconnect) => {
			...
			if (willReconnect) {
				this.#ctx.showStatus(`协作连接已断开(${reason}),正在重新连接…`, { dim: true });
				return;
			}
			this.#ctx.showStatus(`协作会话已结束(${reason})`);
	...
	#armWelcomeTimer(): void {
		...
		this.#welcomeTimer = setTimeout(() => {
			...
			this.#joinReject?.(new Error("等待主机欢迎消息超时"));
		}, WELCOME_TIMEOUT_MS);
	}
	...
	#armSnapshotProgressTimer(): void {
		...
		this.#snapshotProgressTimer = setTimeout(() => {
			...
			this.#joinReject?.(new Error("等待主机会话快照超时"));
	...
	#applyFrame(frame: CollabFrame): void {
		switch (frame.t) {
			...
			case "bye": {
				this.#ctx.showStatus(`协作会话已结束(${frame.reason})`);
				this.#socket?.close();
				void this.#restoreLocalSession();
				break;
			}
			case "error":
				this.#ctx.showError(`协作主机:${frame.message}`);
				break;
			default:
