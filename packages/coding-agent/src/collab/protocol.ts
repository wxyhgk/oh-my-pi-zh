...
import type { ImageContent, Model } from "@wxyhgk/pi-ai";
import type {
	BusChannel,
	CollabUiRequest,
	GuestFrame,
...
export { COLLAB_PROMPT_MESSAGE_TYPE, COLLAB_PROTO } from "@wxyhgk/pi-wire";
...
function normalizeRelayOrigin(relayUrl: string): { origin: string } | { error: string } {
	...
	try {
		...
		return { error: `无效的中继 URL:${relayUrl}` };
	}
	let scheme: string;
	switch (url.protocol) {
		case "wss:":
		...
		case "http:":
			...
			break;
		default:
			return { error: `不支持的中继 URL 协议:${url.protocol}` };
	}
	if (scheme === "ws:" && !isLocalHostname(url.hostname)) {
		return { error: "中继链接必须为 wss://(纯 ws:// 仅允许用于 localhost)" };
	}
...
function normalizeCollabWebBaseUrl(relayUrl: string, webUrl?: string): string {
	...
	try {
		...
		throw new Error("collab.webUrl 必须以 http:// 或 https:// 开头");
	}
	if (url.protocol !== "http:" && url.protocol !== "https:") {
		throw new Error("collab.webUrl 必须以 http:// 或 https:// 开头");
	}
	if (url.protocol === "http:" && !isLocalHostname(url.hostname)) {
		throw new Error("collab.webUrl 必须使用 https://,除非目标是 localhost");
	}
	if (url.search || url.hash) {
		throw new Error("collab.webUrl 不能包含查询字符串或片段");
	}
...
export function parseCollabLink(link: string): ParsedCollabLink | { error: string } {
	...
	if (!match) {
		...
		return { error: "协作链接必须包含 /r/<roomId> 路径" };
	}
	...
	if (!secret || (secret.byteLength !== ROOM_KEY_BYTES && secret.byteLength !== ROOM_KEY_BYTES + WRITE_TOKEN_BYTES)) {
		return { error: "协作链接密钥必须为 32(只读)或 48(完整)个 base64url 字节" };
	}
