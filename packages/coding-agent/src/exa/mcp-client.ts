...
export function formatSearchResults(data: ExaSearchResponse): string {
	...
	if (data.costDollars) {
		output += `\n**费用：** $${data.costDollars.total.toFixed(4)}`;
	}
	if (data.searchTime) {
		output += `\n**搜索时间：** ${data.searchTime.toFixed(2)}s`;
	}
...
export function formatGenericResponse(data: unknown): string {
	if (data === null || data === undefined) return "无结果。";
	...
	if (Array.isArray(data)) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			if (typeof item === "object" && item !== null) {
				...
				const title = (record.title ?? record.name ?? record.id ?? `条目 ${i + 1}`) as string;
				parts.push(`\n### ${title}`);
	...
	async execute(
		...
		try {
			...
			if (!apiKey && this.config.isWebsetsTool) {
				return {
					content: [{ type: "text" as const, text: "错误：EXA_API_KEY 是 Websets 工具所必需的" }],
					details: { error: "EXA_API_KEY 是 Websets 工具所必需的", toolName: this.config.name },
				};
			}
