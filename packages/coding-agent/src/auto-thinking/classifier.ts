 ...
 */
import { type AssistantMessage, completeSimple, Effort, type Model } from "@wxyhgk/pi-ai";
...
async function classifyOnline(input: string, deps: ClassifyDifficultyDeps, ceiling: Effort): Promise<Effort> {
	...
	if (!model) {
		throw new Error("auto-thinking:没有可用于分类的 tiny/smol 模型");
	}
	const apiKey = await deps.registry.getApiKey(model, deps.sessionId);
	if (!apiKey) {
		throw new Error(`auto-thinking:${model.provider}/${model.id} 没有 API 密钥`);
	}
	...
	if (response.stopReason === "error") {
		throw new Error(`auto-thinking:在线分类失败:${response.errorMessage ?? "未知错误"}`);
	}

	const text = extractText(response.content);
	const effort = parseDifficultyLevel(text);
	if (!effort) {
		throw new Error(`auto-thinking:无法解析在线分类结果:${JSON.stringify(text)}`);
	}
	return effort;
}
...
async function classifyLocal(input: string, modelKey: string, deps: ClassifyDifficultyDeps): Promise<Effort> {
	if (!isTinyMemoryLocalModelKey(modelKey)) {
		throw new Error(`auto-thinking:不支持的本地分类模型:${modelKey}`);
	}
	...
	if (!text) {
		throw new Error("auto-thinking:本地分类未返回输出");
	}
