...
function initialCoverage(plan: SecurityScanPlan): SecurityCoverage {
	return {
		...
		surfaces: [],
		explicitExclusions: [],
		deferred: [{ id: "scan-pending", reason: "安全审查尚未完成" }],
	};
}
...
function terminalText(snapshot: SecurityOperationSnapshot): string {
	return [
		`安全扫描 ${snapshot.scanId}:${snapshot.phase}。`,
		`操作:${snapshot.operationId}`,
		`计划:${snapshot.planId}`,
		`发现数:${snapshot.findingCount}`,
...
async function prepareSecurityExecutionTarget(
	...
	if (!headRevision || !baseRevision) throw new Error("ref_diff 安全计划缺少已解析的版本");
...
export class SecurityCoordinator {
	...
	async preflight(input: SecurityPreflightInput = {}): Promise<SecurityScanPlan> {
		if (!this.#host.settings.get("security.enabled")) {
			throw new Error("安全功能已禁用;规划扫描前请启用 security.enabled");
		}
		const model = input.model ?? this.#host.activeModel;
		if (!model) throw new Error("安全扫描 preflight 需要活动模型");
	...
	async start(input: SecurityStartInput): Promise<SecurityOperationSnapshot> {
		if (!this.#host.settings.get("security.enabled")) {
			throw new Error("安全功能已禁用;开始扫描前请启用 security.enabled");
		}
		await this.#ensureRecovered();
		const store = await this.#openStore(this.#host.cwd);
		const plan = await store.getPlan(input.planId);
		if (!plan) throw new Error(`未知的安全扫描计划:${input.planId}`);
	...
	async cancel(operationId: string): Promise<boolean> {
		...
		record.abortController?.abort(new Error("安全扫描已取消"));
		return true;
	}
	...
	async wait(operationId: string): Promise<SecurityOperationSnapshot> {
		await this.#ensureRecovered();
		const record = this.#operations.get(operationId);
		if (!record) throw new Error(`未知的安全操作:${operationId}`);
	...
	async #run(
		...
		try {
			...
			const abortSession = (): void => {
				void session?.abort({ reason: "安全扫描已取消" });
			};
			signal.addEventListener("abort", abortSession, { once: true });
			try {
				if (signal.aborted) throw signal.reason ?? new Error("安全扫描已取消");
				this.#update(record, "reviewing");
				await reportProgress?.("正在使用 OMP 安全 worker 审查仓库");
			...
			if (publishedBundle) {
				...
				await reportProgress?.(`已发布 ${publishedBundle.findings.length} 条安全发现`);
				return;
			}
			...
			partial.scan.error = "扫描会话在未发布规范结果的情况下结束";
