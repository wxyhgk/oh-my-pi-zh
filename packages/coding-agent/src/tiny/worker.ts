...
import { getTinyModelsCacheDir, prompt } from "@wxyhgk/pi-utils";
...
async function loadPipelineWithDeviceFallback(
	...
	if (devices[0] !== tinyModelDevicePreference.device) {
		sendLog(transport, "warn", "tiny-model:请求的设备在 worker 中不安全;改用 CPU", {
			modelKey,
			repo: spec.repo,
	...
	for (let i = 0; i < devices.length; i += 1) {
		...
		try {
			...
			sendLog(transport, "warn", "tiny-model:加速设备加载失败;正在回退", meta);
		}
	}
	throw new Error("未配置任何 tiny 模型设备");
}
...
async function loadPipeline(
	...
	if (!spec) throw new Error(`未知的 tiny 本地模型:${modelKey}`);
	if (spec.unsupportedReason) throw new Error(`${modelKey} 不可用:${spec.unsupportedReason}`);
	...
	const loaded = loadPipelineWithDeviceFallback(transformers, spec, modelKey, transport, requestId).then(
		({ generator, device }) => {
			sendLog(transport, "debug", "tiny-model:本地模型已加载", {
				modelKey,
				repo: spec.repo,
				device,
				requestedDevice: tinyModelDevicePreference.device,
