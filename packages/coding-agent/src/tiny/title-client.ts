...
export class TinyTitleClient {
	...
	async generate(
		...
		try {
			...
			logger.debug("tiny-title:本地生成失败", {
				modelKey,
				error: error instanceof Error ? error.message : String(error),
			});
			return null;
	...
	async complete(
		...
		try {
			...
			logger.debug("tiny-model:本地补全失败", {
				modelKey,
				error: error instanceof Error ? error.message : String(error),
			});
			return null;
	...
	async downloadModel(modelKey: string, options: TinyTitleDownloadOptions = {}): Promise<TinyTitleDownloadResult> {
		...
		try {
			...
			logger.debug("tiny-title:本地模型下载失败", {
				modelKey,
				error: message,
			});
	...
	#handleMessage(message: TinyTitleWorkerOutbound): void {
		...
		logger.debug("tiny-title:worker 返回错误", { error: message.error });
	...
	#handleWorkerError(error: Error): void {
		logger.warn("tiny-title:worker 错误", { error: error.message });
		for (const pending of this.#pending.values()) {
			this.#emitProgress({ modelKey: pending.modelKey, status: "error" });
			if (pending.kind === "generate" || pending.kind === "complete") pending.resolve(null);
...
export async function smokeTestTinyTitleWorker({
	...
	await smokeTestWorker(wrapSubprocess(createTinyTitleSubprocess()), "tiny 标题 worker", timeoutMs);
}
