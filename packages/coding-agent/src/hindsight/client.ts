	...
	async #request<T>(method: string, path: string, operation: string, opts?: RequestOptions): Promise<T> {
		...
		try {
			...
			const message = isTimeoutError(err)
				? `${operation} 请求在 ${Math.round(timeoutMs / 1000)} 秒后超时`
				: `${operation} 请求失败:${err instanceof Error ? err.message : String(err)}`;
		...
		if (!response.ok) {
			...
			throw new HindsightError(
				`${operation} 失败:${typeof details === "string" ? details : JSON.stringify(details)}`,
				response.status,
				details,
			);
		}
