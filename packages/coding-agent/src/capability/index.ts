...
export function registerProvider<T>(capabilityId: string, provider: Provider<T>): void {
	...
	if (!capability) {
		throw new Error(`未知能力:“${capabilityId}”。请先使用 defineCapability() 定义。`);
	}
...
async function loadImpl<T>(
	...
	for (const entry of results) {
		...
		if ("error" in entry) {
			allWarnings.push(`[${provider.displayName}] 加载失败:${entry.error}`);
			continue;
		}
		...
		for (const item of result.items) {
			...
			if (!itemWithSource._source) {
				allWarnings.push(`[${provider.displayName}] 条目缺少 _source 元数据,已跳过`);
				continue;
			}
