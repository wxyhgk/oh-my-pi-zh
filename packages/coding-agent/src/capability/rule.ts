...
export const ruleCapability = defineCapability<Rule>({
	id: "rules",
	displayName: "规则",
	description: "项目特定的规则与约束(Cursor MDC、Windsurf、Cline 格式)",
	key: rule => rule.name,
	toExtensionId: rule => `rule:${rule.name}`,
	validate: rule => {
		if (!rule.name) return "缺少规则名称";
		if (!rule.path) return "缺少规则路径";
		if (!rule.content || typeof rule.content !== "string") return "规则必须包含内容";
		return undefined;
	},
});
