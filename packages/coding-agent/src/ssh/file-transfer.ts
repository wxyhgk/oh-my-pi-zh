...
async function ensurePosixRemote(target: SSHConnectionTarget): Promise<"sh" | "bash" | "zsh"> {
	...
	if (info.os === "windows") {
		throw new Error(
			`ssh://:${target.name} 是 Windows 主机;ssh:// 仅支持 POSIX 远程主机(head/cat/mv)——Windows 主机请使用 ssh 工具`,
		);
	}
	if (!info.transferShell) {
		throw new Error(
			`ssh://:${target.name} 没有可用于 ssh:// 读写且已验证的 POSIX shell——sh/bash/zsh 均未通过能力探测(此主机请使用 ssh 工具)`,
		);
	}
...
export async function writeRemoteFile(
	...
	if (remotePath.endsWith("/")) {
		throw new Error("ssh://:目标路径是目录(以“/”结尾);ssh:// 写入需要文件路径");
	}
	...
	const command =
		...
		`mkdir -p -- "$(dirname "$t")" && ` +
		`cat > "$t" && { ` +
		`if [ -d ${dest} ]; then echo 'ssh://:目标路径是目录' >&2; exit 1; ` +
		`elif [ -f ${dest} ] && [ ! -L ${dest} ]; then cat "$t" > ${dest} || exit 1; ` +
		`elif [ -e ${dest} ] && [ ! -L ${dest} ]; then echo 'ssh://:目标路径是特殊文件(非普通文件)' >&2; exit 1; ` +
		`else mv "$t" ${dest}; fi; ` +
		`}`;
