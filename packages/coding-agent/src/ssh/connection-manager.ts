...
import { $which, getRemoteHostDir, getSshControlDir, isEnoent, logger, postmortem, ptree } from "@wxyhgk/pi-utils";
...
function ensureControlDir() {
	...
	try {
		...
		logger.debug("SSH 控制目录权限修改失败", { path: CONTROL_DIR, error: String(err) });
	}
}
...
async function deleteHostInfoFromDisk(hostName: string): Promise<void> {
	...
	try {
		...
		logger.warn("删除 SSH 主机信息失败", { host: hostName, error: String(err) });
	}
}
...
async function validateKeyPermissions(keyPath?: string, platform: SshPlatform = process.platform): Promise<void> {
	...
	try {
		...
		if (isEnoent(err)) {
			throw new Error(`未找到 SSH 密钥:${keyPath}`);
		}
		throw err;
	}
	if (!stats.isFile()) {
		throw new Error(`SSH 密钥不是文件:${keyPath}`);
	}
	if (platform === "win32") return;
	const mode = stats.mode & 0o777;
	if ((mode & 0o077) !== 0) {
		throw new Error(`SSH 密钥权限必须为 600 或更严格:${keyPath}`);
	}
}
...
function ensureSshBinary(): void {
	if (!$which("ssh")) {
		throw new Error("PATH 中未找到 ssh 命令");
	}
}
...
function applyCompatOverride(host: SSHConnectionTarget, info: SSHHostInfo): SSHHostInfo {
	...
	if (host.compat === true && !compatShell) {
		logger.warn("已请求 SSH 兼容模式,但未检测到兼容的 shell", {
			host: host.name,
			shell: info.shell,
		});
	}
...
async function loadHostInfoFromDisk(host: SSHConnectionTarget): Promise<SSHHostInfo | undefined> {
	...
	try {
		...
		logger.warn("加载 SSH 主机信息失败", { host: host.name, error: String(err) });
		return undefined;
	}
}
...
async function loadHostInfoFromDiskByName(hostName: string): Promise<SSHHostInfo | undefined> {
	...
	try {
		...
		if (isEnoent(err)) return undefined;
		logger.warn("加载 SSH 主机信息失败", { host: hostName, error: String(err) });
		return undefined;
	}
}
...
async function persistHostInfo(host: SSHConnectionTarget, info: SSHHostInfo): Promise<void> {
	try {
		...
		logger.warn("持久化 SSH 主机信息失败", { host: host.name, error: String(err) });
	}
}
...
async function probeHostInfo(host: SSHConnectionTarget): Promise<SSHHostInfo> {
	...
	if (payload === null) {
		logger.debug("SSH 主机探测失败", { host: host.name, error: result.stderr });
...
export async function ensureConnection(host: SSHConnectionTarget): Promise<void> {
	...
	const promise = (async () => {
		...
		if (start.exitCode !== 0) {
			...
			throw new Error(`为 ${target} 启动 SSH master 失败${detail}`);
		}
