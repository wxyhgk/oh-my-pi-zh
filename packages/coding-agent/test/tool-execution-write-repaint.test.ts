...
describe("ToolExecutionComponent write repaint seam", () => {
	...
	it("removes stale pending tail rows from the terminal buffer when the first partial result arrives", async () => {
		...
		try {
			...
			expect(pendingRows.some(row => row.includes("… (前面还有 8 行)"))).toBe(true);
			expect(pendingRows.some(row => row.includes("… (流式)"))).toBe(true);
			...
			expect(rows.some(row => row.includes("… (流式)"))).toBe(false);
			expect(rows.some(row => row.includes("前面还有"))).toBe(false);
			...
			expect(rows.some(row => row.includes("… 还有 14 行"))).toBe(true);
		} finally {
			tui.stop();
			await term.flush();
		}
