...
import { completionBudgetReport, GoalRuntime } from "@wxyhgk/pi-coding-agent/goals/runtime";
import type { Goal, GoalModeState, GoalTokenUsage } from "@wxyhgk/pi-coding-agent/goals/state";
import { GoalTool } from "@wxyhgk/pi-coding-agent/goals/tools/goal-tool";
...
describe("GoalTool", () => {
	it("routes create/get/complete operations and returns completion budget details", async () => {
		...
		expect(completed.content[0]).toEqual({
			type: "text",
			text: "目标:Complete route\n状态:complete\n已用 tokens:7 / 10 预算\n剩余 tokens:3\n\n目标已完成。请向用户报告最终预算使用情况:已用 tokens:7 / 10; 已用时间:3 秒。",
		});
	});
	...
	it("rejects create when a goal already exists", async () => {
		...
		).rejects.toThrow("当前会话已有一个目标,无法创建新目标");
	});
	...
	it("rejects op=create when the objective is missing or only whitespace", async () => {
		...
		).rejects.toThrow("op=create 时必须提供 objective");
