...
import {
	...
	parseLoopLimitArgs,
} from "@wxyhgk/pi-coding-agent/modes/loop-limit";
import type { BuiltinSlashCommandRuntime } from "@wxyhgk/pi-coding-agent/slash-commands/builtin-registry";
...
describe("loop limit parsing", () => {
	...
	test("rejects zero, negative, and unknown limit-shaped tokens", () => {
		...
		expect(parseLoopLimitArgs("-1")).toContain("用法:/loop");
		expect(parseLoopLimitArgs("10fortnights")).toBe("循环时长单位必须是秒、分钟或小时。");
	});
});
