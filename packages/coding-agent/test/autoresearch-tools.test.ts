...
import { createSessionRuntime } from "@wxyhgk/pi-coding-agent/autoresearch/state";
import {
	type AutoresearchStorage,
	closeAllAutoresearchStorages,
	openAutoresearchStorage,
	type SessionRow,
} from "@wxyhgk/pi-coding-agent/autoresearch/storage";
import { createInitExperimentTool } from "@wxyhgk/pi-coding-agent/autoresearch/tools/init-experiment";
import { createLogExperimentTool } from "@wxyhgk/pi-coding-agent/autoresearch/tools/log-experiment";
import { createRunExperimentTool } from "@wxyhgk/pi-coding-agent/autoresearch/tools/run-experiment";
import { createUpdateNotesTool } from "@wxyhgk/pi-coding-agent/autoresearch/tools/update-notes";
import type { ASIData, LogDetails, NumericMetricMap, RunDetails } from "@wxyhgk/pi-coding-agent/autoresearch/types";
import type { ExtensionAPI, ExtensionContext } from "@wxyhgk/pi-coding-agent/extensibility/extensions";
...
describe("init_experiment", () => {
	...
	it("opens a new session and persists scope and metric metadata", async () => {
		...
		expect(firstTextBlockText(result.content)).toContain("已开始会话");
	...
	it("auto-commits pending harness changes on an autoresearch branch", async () => {
		...
		expect(message).toContain("autoresearch: 环境设置");
	});
...
describe("run_experiment", () => {
	...
	it("rejects when no session is active", async () => {
		...
		expect(firstTextBlockText(result.content)).toContain("没有活动的 autoresearch 会话");
	});
...
describe("log_experiment", () => {
	...
	it("rejects when no pending run exists", async () => {
		...
		expect(firstTextBlockText(result.content)).toContain("没有可用的待处理运行");
	});
	...
	it("flags scope deviations and warns when justification is missing", async () => {
		...
		expect(firstTextBlockText(result.content)).toContain("未说明理由");
	});
