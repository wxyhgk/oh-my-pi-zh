...
import { callSessionTool } from "@wxyhgk/pi-coding-agent/eval/js/tool-bridge";
...
describe("callSessionTool", () => {
	it("injects js intent and summarizes text results", async () => {
		...
		expect(execute).toHaveBeenCalledWith(
			...
			{ path: "/tmp/demo.txt", [INTENT_FIELD]: "js 预加载" },
			undefined,
		);
	...
	it("throws when the requested tool is not available in the session registry", async () => {
		...
		await expect(callSessionTool("missing", {}, { session })).rejects.toThrow("JS 运行时中的未知工具:missing");
	});
});
