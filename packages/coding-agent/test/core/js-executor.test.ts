...
import { disposeAllVmContexts } from "@wxyhgk/pi-coding-agent/eval/js/context-manager";
import { executeJs, type JsResult } from "@wxyhgk/pi-coding-agent/eval/js/executor";
...
describe("executeJs", () => {
	...
	it("delegates URI reads through the read tool with positional slicing", async () => {
		...
		expect(execute).toHaveBeenNthCalledWith(
			...
			{ path: "artifact://15:raw:1-1400", [INTENT_FIELD]: "js 预加载" },
			expect.any(AbortSignal),
		);
		expect(execute).toHaveBeenNthCalledWith(
			2,
			expect.stringMatching(/^js-read-/),
			{ path: "artifact://15:raw:1-2", [INTENT_FIELD]: "js 预加载" },
			expect.any(AbortSignal),
		);
	});
	...
	it("rejects directory reads from native read()", async () => {
		...
		expect(directoryResult.output).toContain("read() 不支持目录路径");
	});
	...
	it("routes output() through tool.read and keeps tool.* results normalized", async () => {
		...
		expect(execute.mock.calls[0]?.[1]).toEqual({ path: "package.json", [INTENT_FIELD]: "js 预加载" });
		expect(execute.mock.calls[1]?.[1]).toEqual({ path: "agent://agent-42", [INTENT_FIELD]: "js 预加载" });
	});
