...
import { githubToolRenderer } from "@wxyhgk/pi-coding-agent/tools/gh-renderer";
import { ToolError } from "@wxyhgk/pi-coding-agent/tools/tool-errors";
import { WriteTool, writeToolRenderer } from "@wxyhgk/pi-coding-agent/tools/write";
import {
	listXdevTools,
	...
	xdevDocsAll,
	xdevEntries,
} from "@wxyhgk/pi-coding-agent/tools/xdev";
...
describe("read and write route xd:// device URLs", () => {
	...
	it("renders device execution errors as the mounted tool instead of write", async () => {
		...
		expect(rendered).toContain("GitHub 仓库");
	...
	it("docsAll inlines small device docs and falls back to a listing past the caps", async () => {
		...
		try {
			...
			expect(docs).toContain("## 其他设备(按需文档)");
	...
	it("docsAll supports inline, builtins, and catalog prompt modes", async () => {
		...
		try {
			...
			expect(inlineDocs).toContain("…(完整文档:读取 xd://mcp_external_tool)");
