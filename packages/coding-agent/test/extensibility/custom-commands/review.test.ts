...
import { ReviewCommand } from "@wxyhgk/pi-coding-agent/extensibility/custom-commands/bundled/review";
import type { CustomCommandAPI } from "@wxyhgk/pi-coding-agent/extensibility/custom-commands/types";
import type { HookCommandContext } from "@wxyhgk/pi-coding-agent/extensibility/hooks/types";
import type { SessionEntry } from "@wxyhgk/pi-coding-agent/session/session-entries";
import type { PrDiffPayload, ViewLookupResult } from "@wxyhgk/pi-coding-agent/tools/gh";
import * as gh from "@wxyhgk/pi-coding-agent/tools/gh";
...
describe("ReviewCommand", () => {
	...
	it("notifies and stops when explicit PR diff fetching fails", async () => {
		...
		expect(notifications).toEqual([
			{
				message: "获取 PR owner/repo#123 的差异失败:authentication required",
				type: "error",
			},
		]);
	});
	...
	it("notifies and stops when explicit PR diff content is empty", async () => {
		...
		expect(notifications).toEqual([
			{
				message: "PR owner/repo#123 没有可用的差异内容",
				type: "warning",
			},
		]);
	});
	...
	it("reviews a detected PR from recent conversation context", async () => {
		...
		const command = new ReviewCommand({ cwd: dir } as unknown as CustomCommandAPI);
		const ctx = createContext({
			selectedMode: "从对话中审查 PR owner/example#77",
	...
	it("does not detect PR URLs from entries outside the current branch", async () => {
		...
		const ctx = createContext({
			...
			branchEntries: [],
			onSelectCall: call => {
				if (call.title === "审查模式") reviewModeOptions = call.options;
			},
		});
		...
		expect(result).toBeDefined();
		expect(reviewModeOptions).not.toContain("从对话中审查 PR owner/example#77");
	});
	...
	it("detects only PR URLs from the active branch path", async () => {
		...
		expect(result).toBeDefined();
		expect(reviewModeOptions).toContain("从对话中审查 PR owner/example#78");
		expect(reviewModeOptions).not.toContain("从对话中审查 PR owner/example#77");
	});
	...
	it("orders detected PR menu entries by most recent mention", async () => {
		...
		expect(reviewModeOptions.slice(0, 2)).toEqual([
			"从对话中审查 PR owner/example#78",
			"从对话中审查 PR owner/example#77",
		]);
	});
	...
	it("keeps base branch review mode working", async () => {
		...
		const command = new ReviewCommand({ cwd: dir } as unknown as CustomCommandAPI);
		const ctx = createContext({
			selectResults: ["1. 对照基础分支审查(PR 风格)", "main"],
		});
	...
	it("keeps specific commit review mode working", async () => {
		...
		const command = new ReviewCommand({ cwd: dir } as unknown as CustomCommandAPI);
		const ctx = createContext({
			selectResults: ["3. 审查指定提交", "abc1234 Fix review command"],
		});
