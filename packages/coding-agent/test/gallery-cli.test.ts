...
import {
	...
	renderGalleryState,
	resolveFixture,
} from "@wxyhgk/pi-coding-agent/cli/gallery-cli";
import type { GalleryFixture } from "@wxyhgk/pi-coding-agent/cli/gallery-fixtures";
...
describe("gallery harness", () => {
	...
	it("routes customRendered tools (task) through the custom-tool branch", async () => {
		...
		expect(stripped.some(line => line.startsWith(theme.boxRound.topLeft) && line.includes("任务"))).toBe(true);
		// ...but no standalone "任务" label line precedes it.
		expect(stripped.some(line => line === "任务")).toBe(false);
	});

	it("renders curated failed states as failures", async () => {
		const cases = [
			["hub_inbox", "IRC 收件箱失败:消息存储不可用。", "2 条未读消息:"],
			["hub_list", "IRC 列表失败:Agent 中心不可用。", "2 个对等 Agent:"],
			["hub_jobs", "子 Agent 以退出码 1 结束:缺少 Redis 连接字符串。", "已取消"],
		] as const;
