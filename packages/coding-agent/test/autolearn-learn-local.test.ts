...
import { localBackend } from "@wxyhgk/pi-coding-agent/memory-backend/local-backend";
...
describe("learn tool (local backend)", () => {
	...
	it("execute throws when the lesson is empty after sanitization", async () => {
		await expect(new LearnTool(localSession()).execute("2", { memory: "   " })).rejects.toThrow(/经清理后为空/);
		expect(await Bun.file(learnedFile).exists()).toBe(false);
	});
});
