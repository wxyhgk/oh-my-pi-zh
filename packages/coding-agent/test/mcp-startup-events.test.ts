...
} from "@wxyhgk/pi-coding-agent/mcp/startup-events";
...
describe("mcp/startup-events — connection-status cross-module contract", () => {
	...
	it("formats failures with server names and errors", () => {
		...
		).toBe("MCP 连接完成但存在失败。已连接: alpha。失败: broken: missing command");
	});
	...
	it("sanitizes server names before rendering them in status text", () => {
		...
		expect(message).not.toContain("\n");
		expect(message).not.toContain("\t");
		expect(message).toContain("已连接: ~/.omp/connected   server。");
		expect(message).toContain("失败: ~/.omp/broken server: missing command。");
		expect(message).toContain("仍在连接: ~/.omp/pending");
	});
	...
	it("keeps pending servers visible while other servers settle", () => {
		...
		).toBe("已连接: alpha。 失败: broken: missing command。 仍在连接: slow…");
	});
