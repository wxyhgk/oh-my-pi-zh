...
import type { OAuthLoginCallbacks, OAuthProviderId } from "@wxyhgk/pi-ai/oauth/types";
import { SignInTab } from "@wxyhgk/pi-coding-agent/modes/setup-wizard/scenes/sign-in";
import type { SetupSceneHost } from "@wxyhgk/pi-coding-agent/modes/setup-wizard/scenes/types";
...
describe("SignInTab", () => {
	it("keeps the OSC8 login link and manual-code prompt above clipped wizard rows", async () => {
		...
		try {
			...
			expect(rendered.join("\n")).toContain(`\x1b]8;;${url}\x07打开登录 URL\x1b]8;;\x07`);
			...
			expect(clippedBody.some(line => line.startsWith("浏览器登录:打开登录 URL"))).toBe(true);
			expect(plainUrlIndex).toBeGreaterThanOrEqual(0);
			expect(clippedBody).toContain("粘贴授权代码(或完整重定向 URL):");
