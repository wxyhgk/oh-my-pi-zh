...
import { ErrorBannerComponent } from "@wxyhgk/pi-coding-agent/modes/components/error-banner";
...
describe("ErrorBannerComponent", () => {
	it("renders the provider error message", () => {
		...
		expect(rendered).toContain("发送下一条消息后自动消失。");
	});
...
describe("AssistantMessageComponent error pinning", () => {
	it("hides the inline error while pinned and restores it afterwards", () => {
		...
		expect(Bun.stripANSI(component.render(120).join("\n"))).toContain("错误: 400 invalid reasoning value");

		component.setErrorPinned(true);
		expect(Bun.stripANSI(component.render(120).join("\n"))).not.toContain("错误: 400 invalid reasoning value");
