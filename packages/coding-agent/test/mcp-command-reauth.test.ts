...
import * as mcpClient from "@wxyhgk/pi-coding-agent/mcp/client";
...
import { MCPCommandController } from "@wxyhgk/pi-coding-agent/modes/controllers/mcp-command-controller";
...
describe("/mcp auth commands", () => {
	...
	test("Esc aborts the OAuth flow during /mcp reauth", async () => {
		...
		expect(showError).not.toHaveBeenCalled();
		expect(showStatus).toHaveBeenCalledWith(expect.stringMatching(/已取消对 "envserver" 的重新授权/));
	...
	test("OAuth deadline still surfaces as a reauthorization error, not a cancellation", async () => {
		...
		expect(showStatus).not.toHaveBeenCalledWith(expect.stringMatching(/已取消/));
		expect(showError).toHaveBeenCalledWith(expect.stringMatching(/OAuth 流程超时/));
	});
