...
import { tryRunRpcSkillCommand } from "@wxyhgk/pi-coding-agent/modes/rpc/rpc-mode";
import { type CustomMessage, SKILL_PROMPT_MESSAGE_TYPE } from "@wxyhgk/pi-coding-agent/session/messages";
...
describe("tryRunRpcSkillCommand", () => {
	test("dispatches registered /skill commands as skill prompt messages", async () => {
		...
		expect(message?.content).toContain(`[技能目录:${dir}]`);
