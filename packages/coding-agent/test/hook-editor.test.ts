...
import { HookEditorComponent } from "@wxyhgk/pi-coding-agent/modes/components/hook-editor";
...
import { CURSOR_MARKER, isFocusable, setKeybindings, type TUI } from "@wxyhgk/pi-tui";
...
describe("HookEditorComponent prompt-style mode", () => {
	...
	it("renders prompt-style editor with legacy ask chrome", () => {
		...
		expect(rendered).toContain("enter 或 ctrl+q 提交  esc 取消");
		expect(rendered).not.toContain("shift+enter 换行");
		expect(rendered).toContain("ctrl+g 外部编辑器");
	});
	...
	it("aligns the title and hint with the editor prompt gutter at column zero (#5313)", () => {
		...
		const titleRow = lines.find(line => line.includes("输入你的回答:"));
		const gutterRow = lines.find(line => line.startsWith("> "));
		const hintRow = lines.find(line => line.includes("esc 取消"));
		...
		expect(titleRow!.startsWith("输入你的回答:")).toBe(true);
