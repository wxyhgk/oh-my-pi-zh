...
describe("extractProfileFlags", () => {
	...
	it("rejects --profile without a value", () => {
		expect(() => extractProfileFlags(["--profile"])).toThrow("--profile 需要提供配置档名称");
		expect(() => extractProfileFlags(["--profile", "--version"])).toThrow("--profile 需要提供配置档名称");
		expect(() => extractProfileFlags(["--profile="])).toThrow("--profile 需要提供配置档名称");
	});

	it("rejects --alias without a value", () => {
		expect(() => extractProfileFlags(["--alias"])).toThrow("--alias 需要提供命令名称");
		expect(() => extractProfileFlags(["--alias", "--profile"])).toThrow("--alias 需要提供命令名称");
		expect(() => extractProfileFlags(["--alias="])).toThrow("--alias 需要提供命令名称");
	});
