...
import { Agent, type AgentMessage, RESCUE_SHAKE_CONFIG } from "@wxyhgk/pi-agent-core";
import * as compactionModule from "@wxyhgk/pi-agent-core/compaction";
import type { AssistantMessage, ImageContent, ToolResultMessage } from "@wxyhgk/pi-ai";
...
describe("AgentSession shake", () => {
	...
	describe("auto-shake strategy", () => {
		...
		it("falls back to context-full when shake cannot drop context below the threshold (regression #2119)", async () => {
			...
			expect(shakeEnd?.errorMessage).toMatch(/回退到 context-full/);
