...
import { mnemopiBackend } from "@wxyhgk/pi-coding-agent/mnemopi/backend";
import { loadMnemopiConfig, type MnemopiBackendConfig } from "@wxyhgk/pi-coding-agent/mnemopi/config";
import {
...
} from "@wxyhgk/pi-coding-agent/mnemopi/state";
import type { AgentSessionEventListener } from "@wxyhgk/pi-coding-agent/session/agent-session";
import type { ToolSession } from "@wxyhgk/pi-coding-agent/tools/index";
import { MemoryEditTool } from "@wxyhgk/pi-coding-agent/tools/memory-edit";
import { MemoryRecallTool } from "@wxyhgk/pi-coding-agent/tools/memory-recall";
import { MemoryReflectTool } from "@wxyhgk/pi-coding-agent/tools/memory-reflect";
import { MemoryRetainTool } from "@wxyhgk/pi-coding-agent/tools/memory-retain";
import { resetMemoryForTests } from "@wxyhgk/pi-mnemopi";
...
describe("retain.execute", () => {
	...
	it("queues the memory and reports success without calling the API", async () => {
		...
		expect(result.content[0]).toEqual({ type: "text", text: "1 条记忆已加入队列。" });
	...
	it("flushes a multi-item tool call as a single retainBatch call with per-item context", async () => {
		...
		expect(result.content[0]).toEqual({ type: "text", text: "2 条记忆已加入队列。" });
	...
	it("emits a UI-only warning notice when the batch flush fails", async () => {
		...
		expect(message).toContain("1 条");
	});

	it("throws when no per-session state is registered", async () => {
		const settings = Settings.isolated({ "memory.backend": "hindsight" });
		const tool = MemoryRetainTool.createIf(makeSession(settings))!;
		await expect(tool.execute("call-2", { items: [{ content: "x" }] })).rejects.toThrow(/尚未初始化/);
	});
});
...
describe("retain.execute (Mnemopi backend)", () => {
	...
	it("writes memories synchronously and returns a stored success message", async () => {
		...
		expect(result.content[0]).toEqual({ type: "text", text: "1 条记忆已存储。" });
	...
	it("stores multiple memories and returns correct count", async () => {
		...
		expect(result.content[0]).toEqual({ type: "text", text: "3 条记忆已存储。" });
	...
	it("isolates memories between projects when scoping is per-project", async () => {
		...
		expect(betaRecall.content[0]).toEqual({ type: "text", text: "未找到相关记忆。" });
		await registeredMnemopiState?.dispose();
		registerMnemopiState(alphaConfig, { cwd: "/work/project-alpha" });
...
describe("recall.execute", () => {
	...
	it("returns the no-results sentinel when recall yields empty", async () => {
		...
		expect(result.content[0]).toEqual({ type: "text", text: "未找到相关记忆。" });
	});
	...
	it("formats non-empty results with count + UTC timestamp header", async () => {
		...
		expect(block).toMatch(/^找到 2 条相关记忆\(截至 \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC\)/);
...
describe("recall.execute (Mnemopi backend)", () => {
	...
	it("throws when no per-session Mnemopi state is registered", async () => {
		const settings = Settings.isolated({ "memory.backend": "mnemopi" });
		const tool = MemoryRecallTool.createIf(makeSession(settings))!;
		await expect(tool.execute("call-mnemopi-no-state", { query: "anything" })).rejects.toThrow(/尚未初始化/);
	});
});
...
describe("memory_edit.execute (Mnemopi backend)", () => {
	...
	it("reports not_found for unknown ids", async () => {
		...
		expect((result.content[0] as { text: string }).text).toContain("未找到记忆");
	});
	...
	it("renders backend stats and diagnostics for scoped banks", async () => {
		...
		expect(stats).toContain("# Mnemopi 记忆统计");
		expect(stats).toContain("test-bank");
		expect(diagnose).toContain("# Mnemopi 记忆诊断");
...
describe("reflect.execute", () => {
	...
	it("falls back to a sentinel when reflect returns blank text", async () => {
		...
		expect((result.content[0] as { text: string }).text).toBe("未找到可据以思考的相关信息。");
	});
});
...
describe("reflect.execute (Mnemopi backend)", () => {
	...
	it("returns a synthesized text block based on recalled memories when data exists", async () => {
		...
		const text = (result.content[0] as { text: string }).text;
		expect(text).toContain("基于回忆到的记忆");
