...
import type { Model, TextContent } from "@wxyhgk/pi-ai";
import * as codexResponses from "@wxyhgk/pi-ai/providers/openai-codex-responses";
...
import { TodoTool, type ToolSession, USER_TODO_EDIT_CUSTOM_TYPE } from "@wxyhgk/pi-coding-agent/tools";
...
const CONTINUE_MARKER = "继续处理用户最近的意图";
...
describe("AgentSession eager prelude re-injection after compaction", () => {
	...
	it("re-injects the eager task reminder on the auto-continuation turn (task.eager always)", async () => {
		...
		const reminder = continuation.messageTexts.find(text => text.includes("任务委派已启用"));
	...
	it("does not re-inject the eager task reminder when task.eager is default", async () => {
		...
		expect(continuation.messageTexts.some(text => text.includes("任务委派已启用"))).toBe(false);
	});
	...
	it("re-injects the eager todo reminder on the auto-continuation turn (todo.eager preferred)", async () => {
		...
		expect(continuation.messageTexts.some(text => text.includes("可以考虑先调用"))).toBe(true);
		expect(continuation.toolChoice).toBeUndefined();
	});
	...
	it("re-injects the eager todo reminder reminder-only for todo.eager always (no forced tool)", async () => {
		...
		expect(continuation.messageTexts.some(text => text.includes("你必须在本轮首先调用"))).toBe(true);
