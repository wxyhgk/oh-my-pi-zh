...
import { connectToServer, listTools } from "@wxyhgk/pi-coding-agent/mcp/client";
import { isRetriableConnectionError } from "@wxyhgk/pi-coding-agent/mcp/tool-bridge";
import type { JsonRpcMessage } from "@wxyhgk/pi-coding-agent/mcp/types";
...
describe("legacy MCP HTTP+SSE transport", () => {
	...
	it("rejects endpoint events that point at another origin", async () => {
		...
		).rejects.toThrow("旧版 SSE 端点来源不匹配");
	});
