...
import type { AgentToolContext } from "@wxyhgk/pi-agent-core";
import { AsyncJobManager } from "@wxyhgk/pi-coding-agent/async";
import { DEFAULT_BASH_INTERCEPTOR_RULES, Settings } from "@wxyhgk/pi-coding-agent/config/settings";
...
import { wrapToolWithMetaNotice } from "@wxyhgk/pi-coding-agent/tools/output-meta";
import { ReadTool } from "@wxyhgk/pi-coding-agent/tools/read";
import * as toolTimeouts from "@wxyhgk/pi-coding-agent/tools/tool-timeouts";
import { WriteTool } from "@wxyhgk/pi-coding-agent/tools/write";
import { unzip } from "@wxyhgk/pi-coding-agent/utils/zip";
import { $which, removeSyncWithRetries, Snowflake } from "@wxyhgk/pi-utils";
...
describe("Coding Agent Tools", () => {
	...
	describe("read tool", () => {
		it("should read file contents that fit within limits", async () => {
			...
			expect(getTextOutput(result)).not.toContain("请使用 :");
		...
		it("truncates lines wider than the read column cap, leaving narrow lines untouched", async () => {
			...
			expect(output).toContain("部分行被截断至 768 个字符");
		});
		...
		it("returns wide lines verbatim with the :raw selector", async () => {
			...
			expect(output).not.toContain("部分行被截断至 768 个字符");
		});
		...
		it("should handle non-existent files", async () => {
			...
			await expect(readTool.execute("test-call-2", { path: testFile })).rejects.toThrow(/ENOENT|未找到/i);
		});
		...
		it("should truncate files exceeding line limit", async () => {
			...
			expect(output).toContain(`[显示第 1-${defaultLimit} 行(共 3500 行)。请使用 :${defaultLimit + 1} 继续]`);
		});
		...
		it("should truncate when byte limit exceeded", async () => {
			...
			expect(output).toMatch(/\[显示第 1-\d+ 行\(共 1000 行\) \(\d+(\.\d+)?\s*KB 限制\)。请使用 :\d+ 继续\]/);
		});
		...
		it("should handle offset parameter (with leading context expansion)", async () => {
			...
			// No truncation message since file fits within limits
			expect(output).not.toContain("请使用 :");
		});
		...
		it("should handle limit parameter (with trailing context expansion)", async () => {
			...
			expect(output).toContain("[显示第 1-13 行(共 100 行)。请使用 :14 继续]");
		});
		...
		it("does not expand on the leading side when offset is 1 or unspecified", async () => {
			...
			expect(output).toContain("[显示第 1-8 行(共 50 行)。请使用 :9 继续]");
		});
		...
		it("should handle offset + limit together (1 leading + 3 trailing)", async () => {
			...
			expect(output).toContain("[显示第 40-63 行(共 100 行)。请使用 :64 继续]");
		});
		...
		it("should show error when offset is beyond file length", async () => {
			...
			expect(output).toContain("第 100 行超出文件末尾(共 3 行)");
			expect(output).toContain("请使用 :1 从开头读取,或使用 :3 读取最后一行。");
		});
		...
		it("should refuse binary files (NUL or invalid UTF-8) instead of emitting mojibake", async () => {
			...
			for (const file of [nulFile, invalidUtf8File]) {
				...
				expect(output).toContain("无法读取二进制文件");
		...
		it("reads a binary file verbatim when :raw is requested", async () => {
			...
			expect(output).not.toContain("无法读取二进制文件");
		});
		...
		it("should spill oversized read output to an artifact", async () => {
			...
			try {
				...
				expect(output).toContain(`请使用 :${defaultLimit + 1} 继续`);
		...
		it("omits inspect_image from the description when the tool is disabled", () => {
			...
			expect(disabled.description).toContain("内联解码");
		});
	...
	describe("write tool", () => {
		it("should write file contents", async () => {
			...
			expect(getTextOutput(result)).toContain("已成功向");
		...
		it.skipIf(process.platform === "win32")(
			...
			async () => {
				...
				expect(getTextOutput(result)).toContain("[注意:已通过 chmod +x 设为可执行]");
		...
		it("should write to a new local:// path under the session local root", async () => {
			...
			expect(getTextOutput(result)).toContain(
				`已成功向 session/local/handoffs/new-output.json 写入 ${content.length} 字节`,
			);
		...
		it("should write to an existing archive entry", async () => {
			...
			expect(getTextOutput(result)).toContain(
				`已成功向 ${path.basename(archivePath)}:pkg/README.md 写入 ${content.length} 字节`,
			);
		...
		it("should create a new archive when writing to an archive subpath", async () => {
			...
			expect(getTextOutput(result)).toContain(
				`已成功向 nested/${path.basename(archivePath)}:pkg/new.txt 写入 ${content.length} 字节`,
			);
...
`,
	...
	describe("bash tool", () => {
		...
		it("should record wall time in text content and details", async () => {
			...
			expect(output).toMatch(/实际耗时:\d+\.\d{2} 秒/);
		...
		it("should surface non-zero exits as an error result", async () => {
			...
			expect(getTextOutput(result)).toContain("命令以退出码 1 结束");
		});
		...
		it("should auto-background long-running commands when enabled", async () => {
			...
			expect(getTextOutput(result)).toContain("已转入后台");
		...
		it("backgrounds a running command when the steering signal fires mid-wait", async () => {
			...
			expect(result.details?.async?.state).toBe("running");
			expect(getTextOutput(result)).toContain("已提前转入后台");
			const jobId = result.details?.async?.jobId;
			if (!jobId) {
		...
		it("should background instead of timing out when auto-background wait exceeds the effective timeout", async () => {
			...
			expect(deliveries).toHaveLength(1);
			expect(deliveries[0]?.jobId).toBe(jobId);
			expect(deliveries[0]?.text).toContain("秒后超时");
			await asyncJobManager.dispose();
		});
		...
		it("should surface clamped timeout in results", async () => {
			...
			expect(output).toContain("超时已被限制为 3600s(请求 7200s;允许范围 1-3600s)。");
		...
		it("should abort and recover for subsequent commands", async () => {
			...
			await expect(promise).rejects.toThrow(/中止|取消|超时/i);
	...
	describe("HubTool", () => {
		it("should wait for jobs and acknowledge deliveries to prevent race conditions", async () => {
			...
			expect(getTextOutput(result)).toContain("已完成");
		...
		it("flags still-waiting polls and all-running snapshots as contextually useless", async () => {
			...
			expect(getTextOutput(settled)).toContain("已完成");
			...
			expect(getTextOutput(idle)).toContain("没有正在运行的后台 job");
			...
			expect(getTextOutput(missing)).toContain("未找到与 ID 匹配的 job");
	...
	describe("search tool", () => {
		...
		it("flags a zero-match search as contextually useless", async () => {
			...
			expect(getTextOutput(result)).toContain("未找到匹配项");
		...
		it("flags a zero-match search useless even when it reports missing paths", async () => {
			...
			expect(getTextOutput(result)).toContain("已跳过缺失路径");
			expect(result.useless).toBe(true);
		});
		...
		it("should group multi-file matches", async () => {
			...
			expect(output).not.toContain("# .");
			expect(output).not.toContain("达到上限");
		...
		it("should cap distinct files and surface pagination", async () => {
			...
			expect(output).toContain(`正在显示文件 1-${DEFAULT_FILE_LIMIT},共 ${totalFiles} 个。`);
			expect(output).toContain(`使用 skip=${DEFAULT_FILE_LIMIT} 查看下一页`);
		});
	...
	describe("find tool", () => {
		...
		it("should not disable gitignore after an ignored broad hidden-file search finds no matches", async () => {
			...
			const output = getTextOutput(result);
			expect(output).toContain("未找到与模式匹配的文件");
