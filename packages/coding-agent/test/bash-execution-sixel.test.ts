...
import { BashExecutionComponent } from "@wxyhgk/pi-coding-agent/modes/components/bash-execution";
import { getThemeByName, setThemeInstance } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import { sanitizeWithOptionalSixelPassthrough } from "@wxyhgk/pi-coding-agent/utils/sixel";
...
describe("BashExecutionComponent SIXEL sanitization", () => {
	...
	it("does not truncate long SIXEL payload lines", () => {
		...
		expect(output).not.toContain("个可见列");
	});
...
describe("BashExecutionComponent expand footer", () => {
	...
	it("advertises hidden lines while collapsed", () => {
		...
		expect(rendered).toContain("还有 7 行");
		expect(rendered).toContain("ctrl+o 展开");
	});
	...
	it("drops the hidden-lines footer once expanded", () => {
		...
		expect(rendered).not.toContain("还有 7 行");
