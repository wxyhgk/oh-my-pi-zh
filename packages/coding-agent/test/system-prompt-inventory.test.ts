...
import { buildSystemPrompt as buildSdkSystemPrompt } from "@wxyhgk/pi-coding-agent/sdk";
import {
	buildSystemPrompt,
...
} from "@wxyhgk/pi-coding-agent/system-prompt";
import { createTools, type Tool, type ToolSession } from "@wxyhgk/pi-coding-agent/tools";
...
describe("system prompt tool inventory", () => {
	...
	it("omits eval prompt guidance when every eval backend is disabled", async () => {
		...
		expect(bash?.description).toContain("用专门的工具或已入库的脚本");
		expect(bash?.description).not.toContain("→ `eval`");
		expect(bash?.description).not.toContain("增量工作");
		expect(bash?.description).not.toContain("内置 `grep`");
		expect(bash?.description).not.toContain("用 `read` 列目录");
		expect(bash?.description).not.toContain("用 `glob` 找路径");
		...
		expect(text).not.toContain("在持久内核中运行一步代码");
		expect(text).not.toContain("增量工作");
	});
	...
	it("SDK wrapper preserves an explicit empty tool list", async () => {
		...
		expect(text).not.toContain("# 工具清单");
	...
	it("omits the read-only scout delegation gate when scout is unavailable", async () => {
		...
		expect(withScout).toContain("你继续工作时,单个只读 scout 是可以的");
		expect(withoutScout).not.toContain("只读 scout");
	});
});
