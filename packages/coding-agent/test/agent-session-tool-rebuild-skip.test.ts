...
import type { Message, Model } from "@wxyhgk/pi-ai";
import { createMockModel, type MockResponseSource } from "@wxyhgk/pi-ai/providers/mock";
import { buildModel } from "@wxyhgk/pi-catalog/build";
import { Settings } from "@wxyhgk/pi-coding-agent/config/settings";
import type { CustomTool } from "@wxyhgk/pi-coding-agent/extensibility/custom-tools/types";
...
import { type CustomMessage, convertToLlm } from "@wxyhgk/pi-coding-agent/session/messages";
...
} from "@wxyhgk/pi-coding-agent/session/session-tools";
import { listXdevTools, XDEV_EXTERNAL_DESCRIPTION_CAP, type XdevState } from "@wxyhgk/pi-coding-agent/tools/xdev";
...
describe("AgentSession refreshMCPTools rebuild skipping", () => {
	...
	it("warns and keeps the stable winner when distinct MCP tools mint the same name", async () => {
		...
		expect(warn).toHaveBeenCalledWith("MCP 工具名冲突;保留稳定的胜者", {
	...
	it("waits for the next user prompt before delivering xd:// mount notices", async () => {
		...
		expect(allNotices[1]).toContain("已不再挂载");
		expect(allNotices[1]).toContain("xd://mcp__nucleus_fetch");
		expect(allNotices[1]).not.toContain("变为可用");
	});
	...
	it("drops a mount delta that cancels out before the next prompt", async () => {
		...
		expect(notices[0]).not.toContain("已不再挂载");
	});
