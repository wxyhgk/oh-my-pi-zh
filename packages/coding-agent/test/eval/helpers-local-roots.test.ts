...
describe("eval js helpers internal-url resolution", () => {
	...
	it("rejects traversal and schemes without an injected root", async () => {
		...
		await expect(helpers.writeFile("local://../escape.md", "x")).rejects.toThrow(/不允许路径穿越/);
		await expect(helpers.writeFile("memory://x.md", "x")).rejects.toThrow(/不支持协议路径/);
		await expect(helpers.read("https://example.com/page")).rejects.toThrow(/不支持协议路径/);
	});
