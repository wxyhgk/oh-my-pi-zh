...
import type { ClientBridge, ClientBridgeTerminalHandle } from "@wxyhgk/pi-coding-agent/session/client-bridge";
...
describe("BashTool ACP terminal routing", () => {
	...
	it("kills and releases the client terminal when the caller aborts", async () => {
		...
		await expect(executePromise).rejects.toThrow(/命令已中止/);
	...
	it("kills and releases the client terminal when the command times out", async () => {
		...
		await expect(executePromise).rejects.toThrow(/命令在 1 秒后超时/);
