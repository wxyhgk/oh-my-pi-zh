...
import * as dapModule from "@wxyhgk/pi-coding-agent/dap";
import { connectSocket, DapClient, waitForTcpServerListening } from "@wxyhgk/pi-coding-agent/dap/client";
import { DapSessionManager } from "@wxyhgk/pi-coding-agent/dap/session";
import type {
	DapCapabilities,
	DapClientState,
	DapEventMessage,
	DapResolvedAdapter,
} from "@wxyhgk/pi-coding-agent/dap/types";
import type { ToolSession } from "@wxyhgk/pi-coding-agent/tools";
import { DebugTool } from "@wxyhgk/pi-coding-agent/tools/debug";
...
describe("DAP launch failure handling", () => {
	...
	it("times out promptly and does not emit an unhandled rejection when the stdin flush is wedged", async () => {
		...
		try {
			...
			await expect(client.sendRequest("initialize", {}, undefined, 50)).rejects.toThrow(/在 50ms 后超时/);
	...
	it("throws targeted 'python not found in PATH' when adapter:'debugpy' is unresolvable for launch", async () => {
		...
		try {
			...
			try {
				...
				).rejects.toThrow(/未在 PATH 中找到 python/);
			} finally {
				await removeWithRetries(cwd);
			}
		} finally {
