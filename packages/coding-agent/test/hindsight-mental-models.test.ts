...
import type { BankScope } from "@wxyhgk/pi-coding-agent/hindsight/bank";
import {
	type HindsightApi,
...
} from "@wxyhgk/pi-coding-agent/hindsight/mental-models";
...
describe("diffMentalModelContent", () => {
	...
	it("caps long diffs and emits an elision marker so the TUI stays readable", () => {
		...
		expect(lines[lines.length - 1]).toMatch(/\[…\d+ 行已省略…\]$/);
	});
	...
	it("caps LCS input lines so a huge curated model cannot hang the diff", () => {
		...
		expect(out).toContain("计算差异前,每侧输入最多截取 1000 行");
