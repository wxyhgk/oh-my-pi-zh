...
import { Agent, RESCUE_SHAKE_CONFIG } from "@wxyhgk/pi-agent-core";
...
import type { CompactionEntry } from "@wxyhgk/pi-coding-agent/session/session-entries";
...
describe("AgentSession snapcompact frame dead-end rescue", () => {
	...
	const NO_PROGRESS_FRAGMENT = "压缩释放的上下文太少";
	...
	it("rebuilds a stale trailing snapcompact archive and skips the no-progress warning", async () => {
		...
		const recovery = notices.filter(n => n.source === NOTICE_SOURCE && n.message.includes("压缩死胡同恢复"));
