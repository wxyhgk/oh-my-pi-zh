...
import * as connectionManager from "@wxyhgk/pi-coding-agent/ssh/connection-manager";
...
describe("buildRemoteCommand", () => {
	...
	it("rejects group/world-readable identity files on Unix-like platforms", async () => {
		await withLooseKey(async keyPath => {
			...
			).rejects.toThrow("SSH 密钥权限必须为 600 或更严格");
		});
	});
});
