...
describe("ssh file-transfer POSIX guard", () => {
	...
	it("rejects a confirmed Windows remote before running any POSIX command", async () => {
		...
		await expect(readRemoteFile(target, "C:/x.txt", { maxBytes: 1024 })).rejects.toThrow(/Windows 主机/);
		await expect(writeRemoteFile(target, "C:/x.txt", new Uint8Array([1]), {})).rejects.toThrow(/Windows 主机/);
