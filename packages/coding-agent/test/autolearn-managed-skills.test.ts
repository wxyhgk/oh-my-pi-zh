...
import {
	...
	toSkillFrontmatter,
	writeManagedSkill,
} from "@wxyhgk/pi-coding-agent/autolearn/managed-skills";
import { parseFrontmatter, removeWithRetries } from "@wxyhgk/pi-utils";
...
describe("managed-skills primitives", () => {
	...
	describe("writeManagedSkill", () => {
		...
		it("caps on the FINAL serialized size (body under cap but description pushes it over)", async () => {
			...
			await expect(writeManagedSkill({ action: "create", name: "fin", description, body })).rejects.toThrow(/字节/);
		...
		it("lets exactly one of two concurrent creates win", async () => {
			...
			expect(String(rejected[0]?.reason)).toMatch(/已存在/);
		});
	...
	describe("deleteManagedSkill", () => {
		it("removes an existing skill and throws for a missing one", async () => {
			...
			await expect(deleteManagedSkill("gone")).rejects.toThrow(/不存在/);
		});
		...
		it("refuses to delete through a symlinked skill directory", async () => {
			...
			try {
				...
				await expect(deleteManagedSkill("linked")).rejects.toThrow(/符号链接/);
