...
import type { MCPReconnect } from "@wxyhgk/pi-coding-agent/mcp/tool-bridge";
import {
	DeferredMCPTool,
	deduplicateMCPToolsByName,
	isRetriableConnectionError,
	MCPTool,
} from "@wxyhgk/pi-coding-agent/mcp/tool-bridge";
import type { MCPServerConnection, MCPToolCallResult, MCPTransport } from "@wxyhgk/pi-coding-agent/mcp/types";
...
describe("MCPTool.execute retry on connection error", () => {
	...
	it("returns error result when reconnect returns null", async () => {
		...
		expect(result.content[0]).toEqual({ type: "text", text: "MCP 错误: ECONNRESET" });
	});
	...
	it("does not retry when no reconnect callback", async () => {
		...
		expect(result.details?.isError).toBe(true);
		expect(result.content[0]).toEqual({ type: "text", text: "MCP 错误: ECONNREFUSED" });
	});
	...
	it("returns error from retry when retry also fails", async () => {
		...
		expect(result.details?.isError).toBe(true);
		expect(result.content[0]).toEqual({ type: "text", text: "MCP 错误: HTTP 503: Service Unavailable" });
	});
