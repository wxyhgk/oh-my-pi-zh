...
describe("status line loop mode segment", () => {
	it("shows that a bounded loop is waiting for its first prompt", () => {
		...
		expect(Bun.stripANSI(rendered.content)).toBe(withIcon(theme.icon.loop, "循环 等待中 10/10"));
	});
	...
	it("shows the live remaining duration while a loop is running", () => {
		...
		expect(Bun.stripANSI(rendered.content)).toBe(withIcon(theme.icon.loop, "循环 运行中 剩余 1m30s"));
	});
	...
	it("distinguishes a paused loop from an active loop", () => {
		...
		expect(Bun.stripANSI(rendered.content)).toBe(withIcon(icon, "循环 已暂停"));
		expect(rendered.content).toBe(theme.fg("warning", withIcon(icon, "循环 已暂停")));
	});
});
