...
import { registerPersistedSubagents } from "@wxyhgk/pi-coding-agent/registry/persisted-agents";
...
describe("AgentLifecycleManager", () => {
	...
	it("tombstoning a parked agent during revive prevents the stale session from attaching", async () => {
		...
		gate.resolve();
		await expect(revival).rejects.toThrow(/进入终态/);
	...
	it("ensureLive on a parked agent without a reviver throws as not revivable", async () => {
		...
		await expect(lifecycle.ensureLive("5-Sub")).rejects.toThrow(/无法恢复.*未注册恢复器/);
	});
	...
	it("a persisted factory that declines leaves the parked ref transcript-only", async () => {
		...
		await expect(lifecycle.ensureLive("7-Sub")).rejects.toThrow(/无法恢复.*未注册恢复器/);
	});
