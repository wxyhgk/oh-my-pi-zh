...
import { Agent, type AgentMessage, type StreamFn } from "@wxyhgk/pi-agent-core";
import * as compactionModule from "@wxyhgk/pi-agent-core/compaction";
import type { AssistantMessage, Model, ToolCall } from "@wxyhgk/pi-ai";
...
import { SecretObfuscator } from "@wxyhgk/pi-coding-agent/secrets";
	...
	it("downgrades auto snapcompact to context-full when local preflight rejects the transcript", async () => {
		...
		const downgradeNotice = events.find(
			(event): event is Extract<AgentSessionEvent, { type: "notice" }> =>
				...
				event.message.startsWith("snapcompact 已禁用：所选 snapcompact 字体包含不支持的字符"),
		);
		expect(downgradeNotice?.message).toContain("改用 context-full 自动压缩");
	});
