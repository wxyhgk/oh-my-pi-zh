...
import type { CmuxKind } from "@wxyhgk/pi-coding-agent/tools/browser/cmux/rpc";
import { CmuxSocketClient } from "@wxyhgk/pi-coding-agent/tools/browser/cmux/socket-client";
import { acquireBrowser, getBrowsersMapForTest } from "@wxyhgk/pi-coding-agent/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
	releaseTabsForOwner,
} from "@wxyhgk/pi-coding-agent/tools/browser/tab-supervisor";
...
describe("browser lifecycle — close deadlines", () => {
	...
	it("rejects a stuck close with the backend, tab, and pending resource", async () => {
		...
		await expect(close).rejects.toThrow(
			'关闭 cmux 浏览器标签页 "probe" 在 100ms 后超时;待清理资源:cmux surface "probe-surface" (surface.close)',
		);
