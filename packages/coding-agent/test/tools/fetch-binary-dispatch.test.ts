...
import { zip } from "@wxyhgk/pi-coding-agent/utils/zip";
import * as scrapers from "@wxyhgk/pi-coding-agent/web/scrapers/types";
import * as scraperUtils from "@wxyhgk/pi-coding-agent/web/scrapers/utils";
...
describe("read URL binary dispatch", () => {
	...
	it("lists a remote zip instead of dumping decoded bytes", async () => {
		...
		expect(text).toContain("方式: archive");
	...
	it("returns a metadata notice when a hinted binary refetch fails", async () => {
		...
		expect(result.details?.notes).toContain("二进制下载失败: content-length 52428801 exceeds 52428800");
		expect(text).toContain("[二进制内容: application/octet-stream");
		expect(text).not.toContain("PK\u0003\u0004");
	});
	...
	it("renders a remote sqlite database through the sqlite reader", async () => {
		...
		expect(text).toContain("方式: sqlite");
		expect(text).toContain("notes (2 行)");
	});
	...
	it("renders a remote notebook as editable cells", async () => {
		...
		expect(text).toContain("方式: notebook");
	...
	it("returns a metadata notice for unrenderable binary bytes", async () => {
		...
		expect(result.details?.method).toBe("binary");
		expect(text).toContain("方式: binary");
		expect(text).toContain("[二进制内容: application/octet-stream");
		expect(text).not.toContain("\u0000");
		expect(text).not.toContain("�");
	});
