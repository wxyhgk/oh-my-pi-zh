...
describe("apply_patch rendering", () => {
	...
	it("does not show missing end-marker errors while apply_patch input is streaming", async () => {
		...
		expect(rendered).not.toContain("补丁的最后一行必须是");
	});
	...
	it("shows an apply_patch parse error preview for malformed input", async () => {
		...
		expect(rendered).toContain("更新文件区块为空");
	});
	...
	it("shows apply_patch preview diffs after args complete", async () => {
		...
		try {
			...
			expect(before).not.toContain("(预览)");

			component.setArgsComplete();
			const after = await waitForRenderedText(component, 160, "(预览)");
			expect(after).toContain("(预览)");
		...
		component.updateResult(
			{
				...
				details: {
					...
					diff: [
						" 10|}",
						'+11|import { CODEX_INSTRUCTIONS } from "@wxyhgk/pi-ai/providers/openai-codex-responses";',
						" 12|\t$env,",
