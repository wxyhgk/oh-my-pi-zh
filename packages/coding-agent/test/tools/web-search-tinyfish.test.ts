...
import { searchTinyFish } from "@wxyhgk/pi-coding-agent/web/search/providers/tinyfish";
...
describe("TinyFish web search provider", () => {
	...
	it("propagates second-page HTTP errors", async () => {
		...
		try {
			...
			expect(error).toMatchObject({ provider: "tinyfish", status: 402, message: "tinyfish: 402 额度已用完" });
		}
	});

	it.each([
		[401, "tinyfish: 401 未授权"],
		[402, "tinyfish: 402 额度已用完"],
