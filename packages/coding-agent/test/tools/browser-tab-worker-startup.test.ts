...
import type { ReadyInfo, WorkerInbound, WorkerOutbound } from "@wxyhgk/pi-coding-agent/tools/browser/tab-protocol";
import { initializeTabWorkerForTest } from "@wxyhgk/pi-coding-agent/tools/browser/tab-supervisor";
...
describe("browser tab worker startup", () => {
	it("surfaces worker startup errors instead of waiting for the generic init timeout", async () => {
		...
		await expect(pending).rejects.toThrow("标签页 Worker 启动失败:Cannot find tab-worker-entry.ts");
