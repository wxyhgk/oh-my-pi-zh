...
import type { DaemonSnapshot } from "@wxyhgk/pi-coding-agent/launch/protocol";
import { renderTerminalOutput } from "@wxyhgk/pi-coding-agent/launch/terminal-output";
import { getThemeByName } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import { hubToolRenderer, type LaunchToolDetails } from "@wxyhgk/pi-coding-agent/tools/hub";
import { toolRenderers } from "@wxyhgk/pi-coding-agent/tools/renderers";
...
describe("hub launch rendering", () => {
	...
	it("folds a stop result into one header with op, name, and exit state", async () => {
		...
		expect(rendered[0]).toContain("启动 stop");
		expect(rendered[0]).toContain("pyc-profile-run");
		expect(rendered[0]).toContain("已退出");
		expect(rendered[0]).toContain("退出码 0");
	});
	...
	it("renders log lines without the trailing cursor-status suffix, surfacing it as header meta", async () => {
		...
		expect(rendered[0]).toContain("启动 logs");
		...
		expect(rendered.some(line => line.includes("输出"))).toBe(true);
	...
	it("marks a failed start with the daemon's exit reason even though the result is not an error", async () => {
		...
		expect(rendered[0]).toContain("启动 start");
		expect(rendered[0]).toContain("失败");
	...
	it("surfaces a pre-ready exit in the interactive start result", async () => {
		...
		expect(rendered[0]).toContain("启动 start");
		expect(rendered[0]).toContain("已退出");
		expect(rendered.some(line => line.includes("在观察到就绪状态之前进程已退出。"))).toBe(true);
	});
	...
	it("names the unmet readiness condition instead of a contradictory Ready + timed-out pair", async () => {
		...
		expect(rendered[0]).toContain("等待 port");
		expect(rendered.some(line => line.includes("日志已匹配: Local: http://localhost:3100"))).toBe(true);
		expect(rendered.some(line => line.includes("端口 3100 在 127.0.0.1 上从未接受连接"))).toBe(true);
		// The old render labeled the log match a bare "ready:" while also saying readiness timed out.
		expect(rendered.some(line => line.includes("就绪: Local:"))).toBe(false);
	});
});
