...
import { serializeEvalWithEnvelope, unwrapEvalEnvelope } from "@wxyhgk/pi-coding-agent/tools/browser/cmux/rpc";
...
describe("cmux eval envelope", () => {
	...
	it("flags Promise returns with an actionable error instead of an unsupported-type failure", () => {
		...
		const value = runOnWire(script);
		expect(() => unwrapEvalEnvelope(value, "tab.evaluate()")).toThrow(/同步求值/);
	});
