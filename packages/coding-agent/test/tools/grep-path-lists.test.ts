...
import { canonicalSnapshotKey } from "@wxyhgk/pi-coding-agent/edit/file-snapshot-store";
import type { RenderResultOptions } from "@wxyhgk/pi-coding-agent/extensibility/custom-tools/types";
import { AgentTranscriptViewer } from "@wxyhgk/pi-coding-agent/modes/components/agent-transcript-viewer";
import { TreeSelectorComponent } from "@wxyhgk/pi-coding-agent/modes/components/tree-selector";
import type {
	ObservableSession,
	SessionObserverRegistry,
} from "@wxyhgk/pi-coding-agent/modes/session-observer-registry";
import type { Theme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
...
import type { SessionEntry, SessionTreeNode } from "@wxyhgk/pi-coding-agent/session/session-entries";
import { ToolChoiceQueue } from "@wxyhgk/pi-coding-agent/session/tool-choice-queue";
...
describe("tool path arrays", () => {
	...
	it("search keeps comma-delimited surviving entries when peers are missing", async () => {
		...
		expect(text).toContain("已跳过缺失路径: missing.txt");
	...
	it("grep pending renderer accepts a single string path", () => {
		...
		expect((component as Text).getText()).toContain("在 folder with spaces/ 中");
	});
	it("agent hub chat renders a single-string grep path summary", async () => {
		...
		expect(rendered).toContain("在 folder with spaces/ 中");
		await removeWithRetries(tmp);
	});
	...
	it("tree selector renders a single-string grep path summary", () => {
		...
		expect(rendered).toContain("[grep: /space-needle/ 于 folder with spaces/]");
	...
	it("write reports absolute in-cwd targets relative to cwd", async () => {
		...
		expect(text).toContain("已成功向 written.txt 写入 8 字节");
	...
	it("read expands comma-delimited paths", async () => {
		...
		expect(text).toContain("注意:已按 2 个路径解释:apps/grep.txt, packages/grep.txt");
		expect(text).toContain("shared-needle apps");
		expect(text).toContain("shared-needle packages");
		expect(details?.notes).toEqual(["注意:已按 2 个路径解释:apps/grep.txt, packages/grep.txt"]);
	});
	...
	it("read keeps readable delimited paths when peers are missing", async () => {
		...
		expect(text).toContain("注意:已按 2 个路径解释:missing.txt, packages/grep.txt");
		expect(text).toContain("shared-needle packages");
		expect(text).toContain("[无法读取 missing.txt:未找到路径 'missing.txt']");
		expect(details?.notes).toEqual([
			"注意:已按 2 个路径解释:missing.txt, packages/grep.txt",
			"无法读取 missing.txt:未找到路径 'missing.txt'",
		]);
	});
