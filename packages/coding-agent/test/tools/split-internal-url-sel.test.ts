...
import { pathTargetsSsh, peelWriteUrlSelector, splitInternalUrlSel } from "@wxyhgk/pi-coding-agent/tools/path-utils";
...
describe("peelWriteUrlSelector (write/read selector parity)", () => {
	...
	it("applies the same peel to every write-capable internal scheme (intentional, matches read's target)", () => {
		...
		expect(() => peelWriteUrlSelector("vault://note:1-20")).toThrow(/写入整个文件/);
	});

	it("rejects line-range and malformed selectors instead of silently stripping them", () => {
		expect(() => peelWriteUrlSelector("ssh://h/f:1-20")).toThrow(/写入整个文件/);
		expect(() => peelWriteUrlSelector("ssh://h/f:-10")).toThrow(/写入整个文件/);
		expect(() => peelWriteUrlSelector("ssh://h/f:raw:1-20")).toThrow(/写入整个文件/);
		expect(() => peelWriteUrlSelector("ssh://h/f:conflicts:1-20")).toThrow(/写入整个文件/);
	});
});
