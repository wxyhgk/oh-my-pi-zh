...
import type { AgentToolResult, RenderResultOptions } from "@wxyhgk/pi-agent-core";
import { arkToWireSchema } from "@wxyhgk/pi-ai/utils/schema";
import { Settings } from "@wxyhgk/pi-coding-agent/config/settings";
import { preloadPluginRoots } from "@wxyhgk/pi-coding-agent/discovery/helpers";
import { LspTool } from "@wxyhgk/pi-coding-agent/lsp";
import * as lspClient from "@wxyhgk/pi-coding-agent/lsp/client";
import * as lspConfig from "@wxyhgk/pi-coding-agent/lsp/config";
import { getServersForFile, type LspConfig, loadConfig } from "@wxyhgk/pi-coding-agent/lsp/config";
import {
	applyTextEditsToString,
	applyWorkspaceEdit,
	sortAndValidateTextEdits,
} from "@wxyhgk/pi-coding-agent/lsp/edits";
import { renderCall, renderResult } from "@wxyhgk/pi-coding-agent/lsp/render";
import {
	type CodeAction,
	type CreateFile,
	type DeleteFile,
	...
	type WorkspaceEdit,
} from "@wxyhgk/pi-coding-agent/lsp/types";
import {
	applyCodeAction,
	collectGlobMatches,
	...
	resolveSymbolColumn,
	uriToFile,
} from "@wxyhgk/pi-coding-agent/lsp/utils";
...
import { ToolAbortError } from "@wxyhgk/pi-coding-agent/tools/tool-errors";
import { clampTimeout } from "@wxyhgk/pi-coding-agent/tools/tool-timeouts";
import * as piUtils from "@wxyhgk/pi-utils";
import { sanitizeText, TempDir } from "@wxyhgk/pi-utils";
...
describe("lsp regressions", () => {
	...
	it("supports long LSP timeouts up to the advertised ceiling", () => {
		...
		expect(arkToWireSchema(lspSchema)).toMatchObject({
			properties: {
				timeout: {
					description: "超时秒数(默认 20;范围 5–300)。",
					maximum: 300,
					minimum: 5,
				},
			},
	...
	it("opens rust-analyzer Cargo workspace files before polling workspace readiness", async () => {
		...
		try {
			...
			expect(output).toContain("找到 1 处定义");
	...
	it("sanitizes symbol metadata in renderer output", async () => {
		...
		expect(normalizedResultText).toContain("符号:foo bar baz");
	...
	it("treats a go.work-only root as a Go workspace for workspace diagnostics", async () => {
		...
		try {
			...
			expect(output).toContain("工作区诊断(");
			expect(output).toContain("go build");
			expect(output).toContain("未发现问题");
	...
	it("builds every go.work use module when go.work and go.mod coexist", async () => {
		...
		try {
			...
			expect(textResult(result)).toContain("工作区诊断(");
	...
	it("flushes pending descendant text edits before a folder rename", async () => {
		...
		try {
			...
			expect(applied[0]).toContain("应用 1 处编辑");
			expect(applied[0]).toContain("src/a.ts");
			expect(applied[1]).toContain("重命名");
	...
	it("validates every file's edits before writing any workspace-edit file", async () => {
		...
		try {
			...
			expect(applyWorkspaceEdit(workspaceEdit, tempDir.path())).rejects.toThrow(/LSP 编辑范围重叠/);
	...
	it("applies a create op followed by a text edit for the same URI in declared order", async () => {
		...
		try {
			...
			expect(applied).toHaveLength(2);
			expect(applied[0]).toContain("已创建");
			expect(applied[0]).toContain("extracted.ts");
			expect(applied[1]).toContain("应用 1 处编辑");
	...
	it("flushes pending descendant text edits before a folder delete", async () => {
		...
		try {
			...
			expect(applied[0]).toContain("应用 1 处编辑");
			expect(applied[0]).toContain("src/a.ts");
			expect(applied[1]).toContain("已删除");
			expect(applied[1]).toContain("src");
		} finally {
			tempDir.removeSync();
		}
	...
	it("sendRequest respects an explicit timeoutMs and reports it in the error", async () => {
		...
		expect(lspClient.sendRequest(client, "test/method", {}, undefined, 25)).rejects.toThrow(/在 25ms 后超时/);
	});
	...
	it("workspace reload rediscovers LSP servers after an empty config was cached", async () => {
		...
		try {
			...
			const initialOutput = initial.content
				...
				.map(block => block.text)
				.join("\n");
			expect(initialOutput).toContain("此项目未配置语言服务器");
			...
			expect(starOutput).toContain("已重新加载 test-lsp");
			expect(omittedOutput).toContain("已重新加载 test-lsp");
	...
	it("status distinguishes configured servers from started clients", async () => {
		...
		expect(output).toContain("rust-analyzer (已配置,未启动)");
	...
	it("reload * invalidates the per-cwd config cache so newly written .omp/lsp.json is observed", async () => {
		...
		try {
			...
			const text1 = status1.content
				...
				.map(b => b.text)
				.join("\n");
			expect(text1).toContain("此项目未配置语言服务器");
			...
			const text3 = status3.content
				...
				.map(b => b.text)
				.join("\n");
			expect(text3).toContain("fake-pylsp (已配置,未启动)");
			expect(loadConfigSpy).toHaveBeenCalledTimes(2);
		} finally {
			tempDir.removeSync();
		}
	...
	describe("reload cancellation and truthful teardown (#6369)", () => {
		...
		it("still falls back to the generic reload on method-not-found without killing the server", async () => {
			...
			try {
				...
				expect(textResult(result)).toContain("已重新加载 fake");
				expect(server.killed).toBe(false);
			} finally {
				vi.restoreAllMocks();
				await lspClient.shutdownAll();
	...
	describe("reader exit ordering and initialization backoff (#7041)", () => {
		it("surfaces the process diagnostic when stdout closes before exit publication", async () => {
			...
			try {
				...
				await expect(lspClient.getOrCreateClient(config, tempDir.path())).rejects.toThrow(
					"LSP 服务器已退出(代码 23):simulated rust-analyzer crash",
				);
			} finally {
				await lspClient.shutdownAll();
				tempDir.removeSync();
		...
		it("kills and evicts a client whose stdout reader fails while the process is alive", async () => {
			...
			try {
				...
				await expect(lspClient.getOrCreateClient(config, tempDir.path())).rejects.toThrow(
					"LSP 连接已关闭:Error: simulated reader failure",
				);
