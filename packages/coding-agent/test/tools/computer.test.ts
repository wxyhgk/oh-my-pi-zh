...
import { ComputerTool, computerApproval, type ToolSession } from "@wxyhgk/pi-coding-agent/tools";
import type {
	ComputerSessionSnapshot,
	ComputerWorkerInbound,
	ComputerWorkerOutbound,
	ComputerWorkerTransport,
} from "@wxyhgk/pi-coding-agent/tools/computer/protocol";
import { ComputerSupervisor, type ComputerWorkerHandle } from "@wxyhgk/pi-coding-agent/tools/computer/supervisor";
import { ComputerWorkerCore, type NativeDesktopSession } from "@wxyhgk/pi-coding-agent/tools/computer/worker";
import type {
	AxNode,
	AxQuery,
	AxSnapshotOptions,
...
describe("computer worker round trips", () => {
	it("lists windows and returns screenshot caption, image, and detail through a fake native session", async () => {
		...
		expect(texts[0]?.text).toMatch(/^截图 desktop 64×32 → .*omp-computer-.*\.png$/);
	...
	it("reports source dimensions when a screenshot is scaled", async () => {
		...
		expect(result.payload.displays[0]).toEqual(
			expect.objectContaining({
				type: "text",
				text: expect.stringMatching(/^截图 desktop 64×32 \(由 128×64 缩放\) → .*omp-computer-.*\.png$/),
			}),
		);
	...
	it("blocks read-only click after capture before invoking native input", async () => {
		...
		expect(result.error.message).toBe("只读运行:'click' 需要 read_only: false");
	...
	it("reports the worker watchdog timeout budget explicitly", async () => {
		...
		expect(result.error).toMatchObject({
			isToolError: true,
			message: "computer 代码执行超过 10ms 后超时",
		});
	});
	...
	it("applies the current read-only policy to a retained writable window", async () => {
		...
		if (second.ok) return;
		expect(second.error.message).toBe("只读运行:'click' 需要 read_only: false");
		expect(native.clickCount).toBe(0);
	});
	...
	it("denies async continuations leaked from an ended run the next run's authority", async () => {
		...
		expect(second.ok).toBe(true);
		if (second.ok) expect(String(second.payload.returnValue)).toContain("computer 运行已结束");
		expect(native.clickCount).toBe(0);
	});
...
describe("computer supervisor recovery", () => {
	it("surfaces a timeout ToolError and creates a fresh worker for the next run", async () => {
		...
		await expect(supervisor.run("await new Promise(() => {})", 5, snapshot())).rejects.toEqual(
			expect.objectContaining({
				name: "ToolError",
				message: "computer worker 已重启;截图与 AX 引用已重置",
			}),
		);
