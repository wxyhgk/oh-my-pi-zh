...
import {
	...
	resolveOpTimeouts,
	resolveWaitTimeout,
} from "@wxyhgk/pi-coding-agent/tools/browser/tab-worker";
import { resolvePredicateTimeout } from "@wxyhgk/pi-coding-agent/tools/run-scope";
...
describe("browser selector guard", () => {
	it("rejects Playwright-only selector engines with an actionable message", () => {
		expect(() => normalizeSelector('button:has-text("Allow all")')).toThrow(/Playwright 专用/);
		expect(() => normalizeSelector("div:visible")).toThrow(/不支持 Playwright 专用选择器/);
		expect(() => normalizeSelector(':text("Login")')).toThrow(/Playwright 专用/);
	});
	...
	it("rejects non-string selectors (handle/number) instead of crashing on .startsWith", () => {
		...
		expect(() => normalizeSelector(handle as never)).toThrow(/必须是字符串;实际传入 一个 ElementHandle/);
		expect(() => normalizeSelector(23 as never)).toThrow(/必须是字符串;实际传入 一个 number/);
	});
});
