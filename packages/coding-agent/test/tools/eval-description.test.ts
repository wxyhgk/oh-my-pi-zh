...
import type { Tool as AiTool } from "@wxyhgk/pi-ai";
...
import { EvalTool, getEvalToolDescription } from "@wxyhgk/pi-coding-agent/tools/eval";
...
describe("eval tool dynamic schema", () => {
	...
	it("hides rb/jl from the wire schema, summary, description, and examples by default", () => {
		...
		expect(fields.languageDescription).toBe('运行时: "py" 用于 IPython 内核, "js" 用于持久化 JS 虚拟机');
		expect(fields.codeDescription).toBe("要在此 eval 调用中原样运行的代码。可随意使用顶层 await。");
		expect(tool.summary).toBe("在进程内 eval 后端中执行 Python 或 JavaScript 代码");
	...
	it("advertises rb/jl across enum, descriptions, summary, and prelude once enabled", () => {
		...
		expect(fields.languageDescription).toBe(
			'运行时: "py" 用于 IPython 内核, "js" 用于持久化 JS 虚拟机, "rb" 用于持久化 Ruby 内核, "jl" 用于持久化 Julia 内核',
		);
		expect(fields.codeDescription).toContain(
			"要在此 eval 调用中原样运行的代码。顶层 `await` 可在 py/js 中使用; rb/jl 会像 REPL 一样自动显示最后一个表达式。",
		);
		expect(tool.summary).toBe("在持久化 eval 后端中执行 Python、JavaScript、Ruby 或 Julia 代码");
	...
	it("advertises only the enabled subset of optional backends", () => {
		...
		expect(tool.summary).toBe("在持久化 eval 后端中执行 Python、JavaScript 或 Ruby 代码");
		expect(tool.description).toMatch(/ruby/i);
