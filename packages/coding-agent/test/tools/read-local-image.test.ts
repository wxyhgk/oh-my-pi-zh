...
import { InternalUrlRouter, LocalProtocolHandler, parseInternalUrl } from "@wxyhgk/pi-coding-agent/internal-urls";
...
describe("read local:// images", () => {
	...
	it("rejects a local:// non-image binary without emitting decoded bytes", async () => {
		...
		expect(text).toContain("无法读取二进制文件");
	...
	it("does not materialize local:// binary resources in the protocol handler", async () => {
		...
		expect(resource.content).toContain("无法读取二进制 local:// 文件");
