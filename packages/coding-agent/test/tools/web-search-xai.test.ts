...
import type { AuthStorage, CredentialOriginKind, FetchImpl } from "@wxyhgk/pi-ai";
...
import { searchXAI, XAIProvider } from "@wxyhgk/pi-coding-agent/web/search/providers/xai";
import { SearchProviderError } from "@wxyhgk/pi-coding-agent/web/search/types";
...
describe("xAI web search provider", () => {
	...
	it("never sends official xAI OAuth credentials to a configured custom endpoint", async () => {
		...
		try {
			...
			expect(error).toHaveProperty(
				"message",
				'拒绝将官方 xAI OAuth 凭据发送到自定义端点 https://proxy.example/v1。请为提供商 "xai-oauth" 配置 API 密钥。',
			);
		}
	...
	it("rejects deprecated live-search 410 responses without retrying", async () => {
		...
		try {
			...
			expect(error).toMatchObject({
				provider: "xai",
				status: 410,
				message: "xAI Responses API 错误(410): Live search is deprecated. Please use the Agent Tools API.",
			});
		}
	...
	it("throws a clear missing-key error before fetch when credentials are unavailable", async () => {
		...
		try {
			...
			expect(error).toHaveProperty(
				"message",
				'未找到 xAI 凭据。请设置 XAI_API_KEY 或为提供商 "xai" 配置 API 密钥。',
			);
		}
