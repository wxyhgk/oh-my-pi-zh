...
import * as reportIssue from "@wxyhgk/pi-coding-agent/tools/report-tool-issue";
import {
...
} from "@wxyhgk/pi-coding-agent/tools/report-tool-issue";
...
describe("dispatchReportIssueDevice", () => {
	...
	it("records a grievance from `<tool>: <report>` text", async () => {
		...
		try {
			...
			if (first?.type === "text") expect(first.text).toBe("已记录,谢谢!");
