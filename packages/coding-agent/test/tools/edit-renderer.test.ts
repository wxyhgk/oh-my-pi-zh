...
import { InMemorySnapshotStore } from "@wxyhgk/hashline";
import type { AgentTool } from "@wxyhgk/pi-agent-core";
import { renderGalleryState, resolveFixture } from "@wxyhgk/pi-coding-agent/cli/gallery-cli";
import { resetSettingsForTest, Settings } from "@wxyhgk/pi-coding-agent/config/settings";
import { editToolRenderer } from "@wxyhgk/pi-coding-agent/edit/renderer";
import { renderDiff } from "@wxyhgk/pi-coding-agent/modes/components/diff";
...
import { Text, type TUI, visibleWidth } from "@wxyhgk/pi-tui";
...
describe("editToolRenderer", () => {
	...
	it("windows the expanded streaming diff to the viewport tail", async () => {
		...
		try {
			...
			expect(collapsed).toContain("上方");
			expect(collapsed).toContain("(预览)");
			...
			expect(expanded).not.toContain("上方");
			expect(expanded).not.toContain("(预览)");
			...
			expect(expandedTall).toContain("上方");
		} finally {
	...
	it("uses the result's own path for a genuine no-op, not the shared preview", async () => {
		...
		const rendered = Bun.stripANSI(component.render(160).join("\n"));
		expect(rendered).toContain("未对 scripts/real.ts 产生任何更改。");
