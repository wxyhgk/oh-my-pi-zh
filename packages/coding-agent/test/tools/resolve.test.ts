...
import {
	...
	resolveRenderer,
	writeDeviceDispatch,
} from "@wxyhgk/pi-coding-agent/tools/resolve";
...
describe("dispatchResolutionDevice", () => {
	...
	it("errors and clears stale pending markers when resolve has no invoker", async () => {
		...
		await expect(dispatchResolutionDevice(session, RESOLVE_DEVICE_NAME, "looks correct")).rejects.toThrow(
			`没有可应用的操作——仅当存在暂存预览时,${RESOLVE_DEVICE_PATH} 才有效。`,
		);
	...
	it("treats reject with no pending action as a successful cancellation and clears stale markers", async () => {
		...
		expect(getText(result)).toContain("没有可拒绝的内容");
	...
	it("rejects through the pending invoker", async () => {
		...
		const handler = async (input: unknown) => {
			...
			return {
				content: [{ type: "text", text: "已拒绝待处理的预览。" }],
				details: {
					action,
					reason,
					sourceToolName: "ast_edit",
					label: "AST 编辑:2 处替换,共 1 个文件",
				},
			};
		};
		const { result, xdev } = await dispatchResolutionDevice(
		...
		expect(getText(result)).toContain("已拒绝待处理的预览。");
	...
	it("applies through the pending invoker and carries dispatch metadata", async () => {
		...
		const handler = async (input: unknown) => {
			...
			return {
				content: [{ type: "text", text: "已在 1 个文件中应用 1 处替换。" }],
				details: {
					action,
					reason,
					sourceToolName: "ast_edit",
					label: "AST 编辑:1 处替换,共 1 个文件",
				},
			};
		};
		const { result, xdev } = await dispatchResolutionDevice(
		...
		expect(getText(result)).toContain("已在 1 个文件中应用 1 处替换。");
		expect(result.details).toEqual({
			action: "apply",
	...
	it("routes propose to the plan proposal handler", async () => {
		...
		const proposalHandler: PlanProposalHandler = async (title: string) => {
			proposedTitle = title;
			return {
				content: [{ type: "text", text: "计划已就绪,等待批准。" }],
		...
		expect(getText(result)).toContain("计划已就绪,等待批准。");
...
it("renders a highlighted apply summary", async () => {
	...
	const component = resolveRenderer.renderResult(
		{
			content: [{ type: "text", text: "已在 1 个文件中应用 2 处替换。" }],
			details: {
				action: "apply",
	...
	expect(rendered).toContain("接受: AST 编辑:2 处替换,共 1 个文件");
	expect(rendered).toContain("AST 编辑");
	expect(rendered).toContain("All replacements are correct");
	expect(rendered).not.toContain("已在 1 个文件中应用 2 处替换。");
	expect(rendered).not.toContain("决定");
