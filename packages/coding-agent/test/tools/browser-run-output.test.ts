...
import { RunOutput } from "@wxyhgk/pi-coding-agent/tools/browser/run-output";
import { formatSelectorMatchHint, toActionableHandle } from "@wxyhgk/pi-coding-agent/tools/browser/tab-worker";
...
describe("browser selector timeout hint", () => {
	it("points at observe/ariaSnapshot when nothing matches", () => {
		expect(formatSelectorMatchHint(0)).toContain("未匹配到任何元素");
	...
	it("reports the match count when elements exist but the action stalled", () => {
		...
		expect(hint).toContain("匹配到 3 个元素");
		expect(hint).toContain("被隐藏或被遮挡");
	});
});
