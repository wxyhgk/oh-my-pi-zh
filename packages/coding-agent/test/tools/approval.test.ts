...
import type { AgentTool, ToolApproval } from "@wxyhgk/pi-agent-core";
import { LSP_READONLY_ACTIONS } from "@wxyhgk/pi-coding-agent/lsp";
import {
	type ApprovalMode,
	...
	resolveApproval,
	truncateForPrompt,
} from "@wxyhgk/pi-coding-agent/tools/approval";
import { BashTool } from "@wxyhgk/pi-coding-agent/tools/bash";
import { DEBUG_READONLY_ACTIONS } from "@wxyhgk/pi-coding-agent/tools/debug";
...
describe("MCP fallback and prompt formatting", () => {
	...
	it("formats MCP origin, reason, and per-tool details", () => {
		...
		expect(formatApprovalPrompt(subject, {}, "Needs confirmation").split("\n")).toEqual([
			"允许工具: mcp__server__dangerous",
			"来源: MCP 服务器工具",
			"原因: Needs confirmation",
			"Path: /tmp/out",
			"Content:",
			"hello",
		]);
	...
	it("does not add MCP origin for annotated MCP tools", () => {
		...
		expect(formatApprovalPrompt(subject, {}, undefined)).toBe("允许工具: mcp__server__safe");
	});

	it("truncates prompt details without touching short strings", () => {
		expect(truncateForPrompt("hello", 10)).toBe("hello");
		expect(truncateForPrompt("abcdefgh", 5)).toBe("abcde[…省略 3 字符…]");
	});
});
...
describe("tool-owned dynamic approval declarations", () => {
	it("classifies critical bash patterns through BashTool.approval", () => {
		for (const command of [
			...
			expect(bashApproval(command)).toEqual({ tier: "exec", override: true, reason: "检测到关键危险模式" });
		}
	});
