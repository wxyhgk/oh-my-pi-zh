...
import { getThemeByName, initTheme, type Theme, theme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import {
	dedupeParseErrors,
	expandKeyHint,
	formatCodeFrameLine,
	...
	shortenPath,
	truncateDiffByHunk,
} from "@wxyhgk/pi-coding-agent/tools/render-utils";
...
describe("formatScreenshot", () => {
	...
	it("formats full-res save with home-relative path", () => {
		...
		expect(
			...
			"已捕获截图",
			"已保存: image/png (2.00 KB) 至 ~/screenshots/capture.png",
	...
	it("formats temp-only screenshot without save line", () => {
		...
		).toEqual(["已捕获截图", "格式: image/webp (3.00 KB)", "尺寸: 800x600"]);
	});
	...
	it("surfaces screenshots that could not be resized", () => {
		...
		).toContain("调整大小: 图像解码失败,已改用原始图像字节");
	});
...
describe("formatExpandHint / expandKeyHint", () => {
	...
	it("tracks a user remap of the expand binding", () => {
		...
		expect(formatExpandHint(plainTheme, false, true)).toBe("[Alt+E: 展开]");
	});
