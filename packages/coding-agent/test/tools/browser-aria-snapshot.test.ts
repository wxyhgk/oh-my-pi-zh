...
import { buildAriaSnapshotScript, parseAriaRefSelector } from "@wxyhgk/pi-coding-agent/tools/browser";
...
describe("parseAriaRefSelector", () => {
	...
	it("rejects non-string selectors (handle/Promise) with a recovery-naming ToolError", () => {
		...
		expect(() => parseAriaRefSelector(handle as never)).toThrow(/必须是字符串;实际传入 一个 ElementHandle/);
		expect(() => parseAriaRefSelector(handle as never)).toThrow(/\(await tab\.id\(n\)\)\.click\(\)/);
		const promise = Promise.resolve(handle);
		expect(() => parseAriaRefSelector(promise as never)).toThrow(/实际传入 一个 Promise\(缺少 await\?\)/);
		promise.catch(() => {});
	});
});
