...
import { AuthStorage, type completeSimple, Effort, type ImageContent, type Model } from "@wxyhgk/pi-ai";
...
import { InspectImageTool } from "@wxyhgk/pi-coding-agent/tools/inspect-image";
import { inspectImageToolRenderer } from "@wxyhgk/pi-coding-agent/tools/inspect-image-renderer";
import { toolRenderers } from "@wxyhgk/pi-coding-agent/tools/renderers";
import { removeSyncWithRetries, sanitizeText } from "@wxyhgk/pi-utils";
...
describe("InspectImageTool", () => {
	...
	it("registers custom renderer and shows question in terminal output", async () => {
		...
		expect(callOutput).toContain("检查图片");
		expect(callOutput).toContain("问题:");
	...
	it("surfaces manual abort as aborted, not as timed out", async () => {
		...
		await expect(pending).rejects.toThrow(/inspect_image 请求已中止/);
