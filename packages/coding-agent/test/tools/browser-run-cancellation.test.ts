...
describe("browser run cancellation", () => {
	...
	it("fails wait(predicate) with a named timeout error instead of stalling", async () => {
		...
		await expect(wait).rejects.toThrow("wait(predicate) 在 50ms 后超时");
	});
	...
	it("aborts run-scoped wait() before a stale continuation can mutate the tab", async () => {
		...
		timeoutSignal.addEventListener("abort", () => reject(new Error("浏览器代码执行超时(超过 20ms")), {
			once: true,
		});
		runtime.setRunScope({
		...
		await expect(run).rejects.toThrow("浏览器代码执行超时(超过 20ms");
