...
import * as imageResize from "@wxyhgk/pi-coding-agent/utils/image-resize";
...
import { ptree, removeSyncWithRetries, Snowflake } from "@wxyhgk/pi-utils";
...
describe("read tool URL handling", () => {
	...
	it("uses section-scoped llms.txt fallback without requesting the site-wide file", async () => {
		...
		expect(result.details?.notes).toContain("已使用 llms.txt 回退: https://example.com/docs/llms.txt");
		expect(textBlock?.type).toBe("text");
