...
import { getThemeByName, initTheme, type Theme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import { readToolRenderer } from "@wxyhgk/pi-coding-agent/tools/read";
import { writeToolRenderer } from "@wxyhgk/pi-coding-agent/tools/write";
...
describe("tool path renderers with invalid provider arguments", () => {
	for (const invalid of invalidPathCases) {
		it(`read renderer does not throw for ${invalid.name}`, () => {
			...
			expect(renderPlain(callComponent!)).toContain("读取");
		...
		it(`write renderer does not throw for ${invalid.name}`, () => {
			...
			expect(resultText).toContain("写入");
		...
		it(`edit renderer does not throw for ${invalid.name}`, () => {
			...
			expect(renderPlain(callComponent!)).toContain("编辑");
