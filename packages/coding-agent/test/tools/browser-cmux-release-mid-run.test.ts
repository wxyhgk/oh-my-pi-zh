...
import { acquireBrowser } from "@wxyhgk/pi-coding-agent/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
...
describe("browser tab-supervisor — cmux tab close mid-run (#4499)", () => {
	...
	it("releaseTab() during an in-flight cmux run rejects the run and never emits unhandledRejection", async () => {
		...
		try {
			...
			await expect(runPromise).rejects.toThrow(/标签页 "docfinal" 已关闭/);
	...
	it("releaseTab() unblocks a cmux run that is not making any socket request (wait(...) mid-flight)", async () => {
		...
		try {
			...
			expect((capturedCloseAc!.signal.reason as Error).message).toMatch(/标签页 "docfinal" 已关闭/);
