...
import { type ResolveContext, resolveLocalUrlToPath } from "@wxyhgk/pi-coding-agent/internal-urls";
import { expandInternalUrls, expandSkillUrls } from "@wxyhgk/pi-coding-agent/tools/bash-skill-urls";
...
describe("expandSkillUrls", () => {
	...
	it("throws ToolError for unknown skills with available names", () => {
		...
		expect(() => expandSkillUrls("python skill://missing/run.py", skills)).toThrow(
			"未知技能: missing。可用: first-skill, second-skill",
		);
	});
