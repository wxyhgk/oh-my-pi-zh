...
import { enforcePlanModeWrite, resolvePlanPath } from "@wxyhgk/pi-coding-agent/tools/plan-mode-guard";
...
describe("enforcePlanModeWrite (working tree read-only, local:// sandbox writable)", () => {
	...
	it("rejects writes to the working tree", () => {
		const session = makeSession({ artifactsDir: ARTIFACTS_DIR, cwd: REPO_ROOT, planMode });
		expect(() => enforcePlanModeWrite(session, "src/foo.ts", { op: "update" })).toThrow(/工作区为只读/);
		expect(() => enforcePlanModeWrite(session, "PLAN.md", { op: "create" })).toThrow(/工作区为只读/);
	});
