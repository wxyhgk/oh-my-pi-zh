...
import { createLspWritethrough, type FileDiagnosticsResult, FileFormatResult } from "@wxyhgk/pi-coding-agent/lsp";
import * as lspClient from "@wxyhgk/pi-coding-agent/lsp/client";
import * as lspConfig from "@wxyhgk/pi-coding-agent/lsp/config";
import type { Diagnostic, LinterClient, LspClient, ServerConfig } from "@wxyhgk/pi-coding-agent/lsp/types";
import { fileToUri } from "@wxyhgk/pi-coding-agent/lsp/utils";
import type { DeferredDiagnosticsEntry, ToolSession } from "@wxyhgk/pi-coding-agent/tools";
import { WriteTool } from "@wxyhgk/pi-coding-agent/tools/write";
import { type ptree, TempDir } from "@wxyhgk/pi-utils";
...
describe("LSP diagnostics freshness", () => {
	...
	it("announces batched sibling writes before syncing the diagnostic target", async () => {
		...
		expect(result?.summary).toBe("无问题");
