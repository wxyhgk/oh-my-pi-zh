...
import { searchWithParallel } from "@wxyhgk/pi-coding-agent/web/parallel";
import { searchParallel } from "@wxyhgk/pi-coding-agent/web/search/providers/parallel";
...
describe("Parallel web search", () => {
	...
	it("surfaces plain-text Parallel API errors", async () => {
		...
		await expect(searchParallel({ query: "broken", fetch: fetchMock }, fakeAuthStorage)).rejects.toMatchObject({
			provider: "parallel",
			status: 503,
			message: "Parallel API 错误(503):upstream unavailable",
		});
	});
});
