...
import { getThemeByName, initTheme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import type { ToolSession } from "@wxyhgk/pi-coding-agent/tools";
import { AskTool, askToolRenderer } from "@wxyhgk/pi-coding-agent/tools/ask";
...
describe("AskTool cancellation", () => {
	...
	it("auto-selects the recommended option on ask timeout", async () => {
		...
		expect(result.content[0].text).toContain("用户选择: no");
	...
	it("auto-selects the first option when timeout elapses without a selected option", async () => {
		...
		expect(result.content[0].text).toContain("用户选择: yes");
...
describe("AskTool option descriptions", () => {
	it("passes descriptions to the selector while returning selected labels", async () => {
		...
		expect(result.content[0].text).toContain("用户选择: Set up in terminal");
	...
	it("forwards descriptions through multi-select and returns bare labels", async () => {
		...
		const context = createContext({
			select: async (_prompt, options) => {
				...
				return selectItemLabel(options.find(o => selectItemLabel(o)?.includes("完成选择")));
			},
			editor,
		});
		...
		expect(result.content[0].text).toContain("用户选择: alpha, beta");
...
describe("AskTool custom input", () => {
	it("routes custom input through editor and preserves raw multiline strings", async () => {
		...
		expect(result.content[0].text).toBe("用户提供的自定义输入:\n  first line\n  second line");
	...
	it("treats explicit empty-string custom input as submitted input", async () => {
		...
		expect(result.content[0].text).toContain("用户提供的自定义输入:");
	...
	it("returns to the option selector when multi-select custom input is dismissed", async () => {
		...
		const context = createContext({
			select: async (_prompt, options) => {
				...
				const doneOption = options.find(option => selectItemLabel(option)?.includes("完成选择"));
		...
		expect(result.content[0].text).toContain("用户选择: alpha");
		expect(step).toBe(2);
		expect(editor).toHaveBeenCalledTimes(1);
	});
});
...
describe("AskTool rich ask dialog", () => {
	...
	it("returns chat redirect result when askDialog returns kind chat", async () => {
		...
		expect(result.content[0]?.type).toBe("text");
		expect((result.content[0] as { text: string }).text).toContain("聊天而不是回答问题");
	});
