...
import { loadCapability } from "@wxyhgk/pi-coding-agent/capability";
...
import { loadSlashCommands } from "@wxyhgk/pi-coding-agent/extensibility/slash-commands";
import { discoverAgents } from "@wxyhgk/pi-coding-agent/task/discovery";
import { removeWithRetries } from "@wxyhgk/pi-utils";
import "@wxyhgk/pi-coding-agent/discovery/claude-plugins";
import { type MCPServer, mcpCapability } from "@wxyhgk/pi-coding-agent/capability/mcp";
import type { Skill } from "@wxyhgk/pi-coding-agent/capability/skill";
import type { SlashCommand } from "@wxyhgk/pi-coding-agent/capability/slash-command";
...
describe("listClaudePluginRoots", () => {
	...
	test("warns on invalid plugin ID format", async () => {
		...
		expect(result.warnings[0]).toContain("插件 ID 格式无效");
	});
	...
	test("warns on entry without installPath", async () => {
		...
		expect(result.roots).toHaveLength(0);
		expect(result.warnings).toHaveLength(1);
		expect(result.warnings[0]).toContain("缺少 installPath");
	});
	...
	test("warns when a manifest mcpServers pointer names a missing file", async () => {
		...
		expect(result.warnings).toEqual([
			`[Claude Code 市场] [claude-plugins] 缺少 broken-pointer@market 声明的 mcpServers 文件:${path.join(pluginPath, "mcp-omp.json")}`,
		]);
	});
	...
	test("ignores manifest skills directory that resolves outside plugin root", async () => {
		...
		const result = await loadCapability<Skill>("skills", { cwd: tempDir });
		expect(result.warnings[0]).toContain("插件根目录之外的 skills 路径");
	...
	test("ignores manifest slash commands directory that resolves outside plugin root", async () => {
		...
		const result = await loadCapability<SlashCommand>("slash-commands", { cwd: tempDir });
		expect(result.warnings[0]).toContain("插件根目录之外的 slash-commands 路径");
	...
	test("array-form commands warns on out-of-root entries while loading valid ones", async () => {
		...
		const result = await loadCapability<SlashCommand>("slash-commands", { cwd: tempDir });
		expect(result.warnings.some(w => w.includes("插件根目录之外的 commands 路径"))).toBe(true);
