...
import type { AgentHubRemote } from "@wxyhgk/pi-coding-agent/modes/components/agent-hub";
...
describe("AgentTranscriptViewer", () => {
	...
	it("aligns the title and body content on the same gutter", () => {
		withViewer(viewer => {
			...
			const titleLine = lines.find(l => l.includes("Agent 中心"));
	...
	it("collapses synthetic advisor inputs on cold open and expands their body on ctrl+o", () => {
		...
		try {
			...
			expect(collapsed.some(l => /Session update .* 行/.test(l))).toBe(true);
		...
		try {
			const body = () =>
				viewer
					...
					.join("\n");
			const deadline = Date.now() + 5000;
			while (body().includes("正在从主机加载转录") && Date.now() < deadline) {
				await Bun.sleep(10);
			}
			expect(body()).toContain("暂无消息。");
		} finally {
			viewer.dispose();
		}
	});
