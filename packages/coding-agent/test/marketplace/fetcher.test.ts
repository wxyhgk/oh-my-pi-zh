...
describe("classifySource", () => {
	...
	it("throws on bare name with suggestion", () => {
		expect(() => classifySource("just-a-name")).toThrow(
			"无法识别的源格式。您的意思是 './just-a-name' (本地)或 'owner/repo' (GitHub)吗?",
		);
	});
});
...
describe("parseMarketplaceCatalog", () => {
	...
	it("throws on missing name", () => {
		...
		expect(() => parseMarketplaceCatalog(bad, "/f.json")).toThrow(/字段 “name” 缺失/);
	});
	...
	it("throws on missing plugins", () => {
		...
		expect(() => parseMarketplaceCatalog(bad, "/f.json")).toThrow(/字段 “plugins” 缺失/);
	});

	it("throws on missing owner", () => {
		const bad = JSON.stringify({ name: "valid-name", plugins: [] });
		expect(() => parseMarketplaceCatalog(bad, "/f.json")).toThrow(/字段 “owner” 缺失/);
	});
...
describe("fetchMarketplace", () => {
	...
	it("throws a clear error for nonexistent local directory", async () => {
		...
		await expect(fetchMarketplace(missing, tmpDir)).rejects.toThrow(/处找到市场目录/);
	});
	...
	it("throws a clear error for relative nonexistent path", async () => {
		...
		await expect(fetchMarketplace(fakeSrc, tmpDir)).rejects.toThrow(/处找到市场目录/);
	});
	...
	it("hides temp clone paths in cloned catalog validation errors", async () => {
		...
		try {
			await expect(fetchMarketplace("kubeshark/kubeshark", tmpDir)).rejects.toThrow(
				"克隆仓库 https://github.com/kubeshark/kubeshark.git 时出错:目录中字段 “owner” 缺失或无效:.claude-plugin/marketplace.json(来源:kubeshark/kubeshark)",
			);
		} finally {
