...
import { listOmpExtensionRoots } from "@wxyhgk/pi-coding-agent/discovery/omp-extension-roots";
...
describe("MarketplaceManager", () => {
	...
	it("addMarketplace with duplicate name → throws", async () => {
		...
		await expect(ctx.manager.addMarketplace(FIXTURE_DIR)).rejects.toThrow(/已存在/);
	});
	...
	it("updateMarketplace on nonexistent marketplace → throws", async () => {
		await expect(ctx.manager.updateMarketplace("ghost")).rejects.toThrow(/不存在/);
	});
	...
	it("listAvailablePlugins(unknown) → throws", async () => {
		await ctx.manager.addMarketplace(FIXTURE_DIR);
		await expect(ctx.manager.listAvailablePlugins("no-such")).rejects.toThrow(/不存在/);
	});
	...
	it("installPlugin already installed → throws without force", async () => {
		await ctx.manager.addMarketplace(FIXTURE_DIR);
		await ctx.manager.installPlugin("hello-plugin", "test-marketplace");
		await expect(ctx.manager.installPlugin("hello-plugin", "test-marketplace")).rejects.toThrow(/已安装/);
	});
	...
	it("installPlugin with nonexistent plugin in catalog → clear error", async () => {
		...
		await expect(ctx.manager.installPlugin("ghost-plugin", "test-marketplace")).rejects.toThrow(
			/在市场 “test-marketplace” 中未找到插件 “ghost-plugin”/,
		);
	});
	...
	it("uninstallPlugin nonexistent → throws", async () => {
		await expect(ctx.manager.uninstallPlugin("ghost-plugin@nowhere")).rejects.toThrow(/未安装/);
	});

	it("uninstallPlugin with invalid ID format → throws clear error", async () => {
		await expect(ctx.manager.uninstallPlugin("no-at-sign")).rejects.toThrow(/无效的插件 ID 格式/);
	});
	...
	it("setPluginEnabled on nonexistent plugin → throws", async () => {
		await expect(ctx.manager.setPluginEnabled("ghost@nowhere", true)).rejects.toThrow(/未安装/);
	});
	...
	describe("auto-update", () => {
		...
		it("upgradePlugin rejects invalid plugin ID", async () => {
			await expect(ctx.manager.upgradePlugin("no-at-sign")).rejects.toThrow(/无效的插件 ID/);
		});
