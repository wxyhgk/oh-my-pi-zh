...
describe("getCachedPluginPath", () => {
	it("throws on invalid marketplace name (uppercase)", () => {
		expect(() => getCachedPluginPath("/cache", "My-Market", "plugin", "1.0.0")).toThrow(/用于缓存的市场名称无效/);
	});
	...
	it("throws on invalid plugin name (uppercase)", () => {
		expect(() => getCachedPluginPath("/cache", "market", "My-Plugin", "1.0.0")).toThrow(/用于缓存的插件名称无效/);
	});

	it("throws on invalid version containing ..", () => {
		expect(() => getCachedPluginPath("/cache", "market", "plugin", "..")).toThrow(/用于缓存的版本无效/);
	});
