...
import type { ApiKeyResolveContext, AssistantMessage, ToolCall, ToolResultMessage } from "@wxyhgk/pi-ai";
...
describe("AgentSession retry delay cap", () => {
	...
	it("bails immediately when retry-after exceeds retry.maxDelayMs", async () => {
		...
		expect(retryEndEvents[0].finalError).toContain("超过 retry.maxDelayMs");
