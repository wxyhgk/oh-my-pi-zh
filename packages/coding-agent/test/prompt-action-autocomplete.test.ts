...
import { KeybindingsManager, setKeybindings, TUI_KEYBINDINGS } from "@wxyhgk/pi-tui";
...
describe("prompt action autocomplete", () => {
	...
	it("shows prompt actions with configured shortcut hints", async () => {
		...
		expect(suggestions?.items.find(item => item.label === "复制当前行")?.description).toBe("Ctrl+Shift+L");
	...
	it("passes the typed trigger to undo and leaves text removal to the editor", async () => {
		...
		const item = suggestions?.items.find(entry => entry.label === "撤销");
	...
	it("returns # prompt-action completions for matched slash commands that reject arguments", async () => {
		...
		expect(suggestions?.items.map(item => item.label)).toEqual(["复制当前行", "复制整个提示词"]);
	});
