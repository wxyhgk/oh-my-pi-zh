...
import { renderWelcomeTip } from "@wxyhgk/pi-coding-agent/modes/components/welcome";
import { initTheme, setTheme, theme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
...
describe("renderWelcomeTip", () => {
	...
	it("wraps long tips under the label instead of truncating", () => {
		...
		expect(plain[0]).toStartWith(" 提示: Next time");
	...
	it("leaves tips without the marker untouched", () => {
		...
		expect(plain).toContain("提示: Plain old tip");
	});
