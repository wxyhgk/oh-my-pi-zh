...
import { type Args, parseArgs } from "@wxyhgk/pi-coding-agent/cli/args";
import * as modelResolverModule from "@wxyhgk/pi-coding-agent/config/model-resolver";
import { Settings } from "@wxyhgk/pi-coding-agent/config/settings";
import * as pluginHelpers from "@wxyhgk/pi-coding-agent/discovery/helpers";
import { createSessionManager, runRootCommand } from "@wxyhgk/pi-coding-agent/main";
...
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@wxyhgk/pi-utils";
	...
	it("throws the move-specific error when unavailable in non-interactive mode", async () => {
		...
		try {
			...
			await expect(createSessionManager(buildArgs("019e84ed"), "/current/project", stubSettings)).rejects.toThrow(
				`会话 “019e84ed” 所属的目录已不存在(${missingProject});请以交互模式运行以将其移动到当前项目。`,
			);
		} finally {
