...
import type { AssistantMessage, StopReason, ToolCall } from "@wxyhgk/pi-ai";
...
import * as autoGeneratedGuard from "@wxyhgk/pi-coding-agent/tools/auto-generated-guard";
...
it("auto-generated streaming abort fires even when edit.streamingAbort is disabled", async () => {
	...
	const checkSpy = vi
		...
		.mockRejectedValue(new ToolError("无法修改自动生成的文件"));
...
it("multi-entry edit on an auto-generated file surfaces isError + error text instead of silently faking success", async () => {
	...
	try {
		...
		const occurrences = text.match(/无法修改自动生成的文件/g) ?? [];
		expect(occurrences.length).toBe(1);
		expect(text).toContain("(第 1/2 项)");
		expect(text).toContain("第 2 项未应用");
