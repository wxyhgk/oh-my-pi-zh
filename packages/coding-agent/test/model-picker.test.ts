...
import { ModelPickerComponent, type ModelPickerOptions } from "@wxyhgk/pi-coding-agent/modes/components/model-picker";
import { resolveSegmentPalette } from "@wxyhgk/pi-coding-agent/modes/components/segment-track";
import { getThemeByName, setThemeInstance, theme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import type { ResolvedRoleModel } from "@wxyhgk/pi-coding-agent/session/agent-session";
...
describe("ModelPicker", () => {
	...
	test("flags over-context models but keeps them selectable, reporting overContext on pick", () => {
		...
		expect(normalize(picker.render(220))).toContain("仅切换当前会话");

		picker.handleInput("small");
		const rendered = normalize(picker.render(220));
		expect(rendered).toContain("上下文>4.1k");
		expect(rendered).toContain("先用当前模型压缩,再切换");
	...
	test("highlights and preselects the session's current model", () => {
		...
		expect(normalize(picker.render(220))).toContain("当前");
