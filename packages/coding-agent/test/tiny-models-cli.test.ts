...
import { resolveModels, runTinyModelsCommand } from "@wxyhgk/pi-coding-agent/cli/tiny-models-cli";
import { TINY_LOCAL_MODELS } from "@wxyhgk/pi-coding-agent/tiny/models";
import { tinyTitleClient } from "@wxyhgk/pi-coding-agent/tiny/title-client";
...
describe("tiny-models download model resolution", () => {
	...
	it("includes worker error details in JSON failures", async () => {
		...
		).rejects.toThrow("一个或多个微型标题模型下载失败");
	...
	it("includes worker error details in text failures", async () => {
		...
		expect(output.join("")).toContain("下载 LFM2 700M 失败:runtime install failed。");
	});
	...
	it("prints actionable CUDA provider diagnostics from worker errors", async () => {
		...
		expect(text).toContain("下载 LFM2 700M 失败:");
