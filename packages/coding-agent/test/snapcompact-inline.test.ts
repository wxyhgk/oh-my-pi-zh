...
import type { Context, ImageContent, Message, TextContent, ToolResultMessage } from "@wxyhgk/pi-ai";
...
} from "@wxyhgk/pi-coding-agent/session/snapcompact-inline";
...
describe("SnapcompactInlineTransformer", () => {
	...
	it("moves only loaded context-file instructions when AGENTS.md mode is selected", async () => {
		...
		const carrier = result.messages[0] as { content: (TextContent | ImageContent)[] };
		expect((carrier.content[0] as TextContent).text).toContain("上下文文件指令");
