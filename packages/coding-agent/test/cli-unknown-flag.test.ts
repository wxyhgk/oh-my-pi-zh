...
import { parseArgs, reportUnrecognizedFlags } from "@wxyhgk/pi-coding-agent/cli/args";
import { applyExtensionFlags } from "@wxyhgk/pi-coding-agent/cli/extension-flags";
...
describe("reportUnrecognizedFlags — stderr error surface", () => {
	...
	it("returns true and writes a singular `unknown flag` line for one entry", () => {
		...
		expect(text).toContain("错误:未知标志: --list-models");
	...
	it("uses the plural form and joins multiple flags when several are unrecognized", () => {
		...
		expect(reported).toBe(true);
		const text = wrote.join("");
		expect(text).toContain("错误:未知标志: --foo, --bar");
	});
});
