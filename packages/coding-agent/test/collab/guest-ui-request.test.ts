...
import { generateRoomKey, importRoomKey } from "@wxyhgk/pi-coding-agent/collab/crypto";
import { CollabGuestLink } from "@wxyhgk/pi-coding-agent/collab/guest";
import { CollabHost } from "@wxyhgk/pi-coding-agent/collab/host";
import {
	COLLAB_PROTO,
	type CollabFrame,
	...
	formatCollabLink,
	parseCollabLink,
} from "@wxyhgk/pi-coding-agent/collab/protocol";
import { CollabSocket } from "@wxyhgk/pi-coding-agent/collab/relay-client";
import type {
	ExtensionAskDialogQuestion,
	ExtensionUIDialogOptions,
	ExtensionUISelectItem,
} from "@wxyhgk/pi-coding-agent/extensibility/extensions/types";
import { ExtensionUiController } from "@wxyhgk/pi-coding-agent/modes/controllers/extension-ui-controller";
import type { InteractiveModeContext, InteractiveSelectorDialogOptions } from "@wxyhgk/pi-coding-agent/modes/types";
...
describe("collab proto handshake (#4049)", () => {
	it("host rejects a stale-proto hello with a protocol-mismatch error and never welcomes or admits the guest", async () => {
		...
		try {
			...
			expect(reply.message).toContain("协议不匹配");
			expect(reply.message).toContain(`主机使用 v${COLLAB_PROTO}`);
			expect(reply.message).toContain(`访客发送了 v${COLLAB_PROTO - 1}`);
...
describe("guest ask multi-select Next gating (#4375 PRRT_kwDOQxs0bc6OFbDW)", () => {
	...
	it("omits Next from the first ui-request, includes it after a toggle", async () => {
		...
		try {
			...
			expect(firstLabels).not.toContain("下一步 →");
			expect(firstLabels).toContain("Option A");
			expect(firstLabels).toContain("其他 (自行输入)");
			expect(firstLabels).toContain("就此进行对话");
			...
			expect(secondLabels).toContain("下一步 →");
			expect(secondLabels).toContain("Option A");

			// Guest selects Next to submit.
			guest.socket.send({ t: "ui-response", reqId: second.request.reqId, value: "下一步 →" });
