...
import type { ApiKeyResolveContext, AssistantMessage, AssistantRetryRecovery, Usage } from "@wxyhgk/pi-ai";
...
import { resolveAssistantErrorPresentation } from "@wxyhgk/pi-coding-agent/modes/utils/transcript-render-helpers";
...
import type { SessionMessageEntry } from "@wxyhgk/pi-coding-agent/session/session-entries";
...
describe("AgentSession retry recovery", () => {
	...
	it("marks a recovered retry error, emits it, persists it, and excludes only model-context replay", async () => {
		...
		expect(recoveredEvent.note).toBe("被限流; 已切换账户; 已重试");
