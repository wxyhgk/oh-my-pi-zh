...
import { createAcpConnection } from "@wxyhgk/pi-coding-agent/modes/acp/acp-mode";
...
describe("ACP lazy startup", () => {
	it("applies schema defaults for ACP background jobs and preserves explicit overrides", async () => {
		const { runRootCommand } = await import("@wxyhgk/pi-coding-agent/main");
...
`,
		...
		try {
			...
			const { runRootCommand } = await import("@wxyhgk/pi-coding-agent/main");
			const { createAgentSession } = await import("@wxyhgk/pi-coding-agent/sdk");
