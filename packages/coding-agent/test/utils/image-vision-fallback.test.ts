...
import type { AssistantMessage, completeSimple, Model } from "@wxyhgk/pi-ai";
...
} from "@wxyhgk/pi-coding-agent/utils/image-vision-fallback";
...
describe("describeAttachedImagesForTextModel", () => {
	...
	it("saves the image but emits a no-vision note when no vision model is available", async () => {
		...
		expect(blocks[0]!.text).toContain("未配置支持视觉的模型");
	...
	it("falls back to a note when the vision model returns no text", async () => {
		...
		expect(stub.calls).toHaveLength(1);
		expect(blocks[0]!.text).toContain("图片描述不可用");
	});
