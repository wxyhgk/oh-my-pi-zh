...
import { TASK_SUBAGENT_LIFECYCLE_CHANNEL } from "@wxyhgk/pi-coding-agent/task";
import type { TodoPhase } from "@wxyhgk/pi-coding-agent/tools/todo";
import { EventBus } from "@wxyhgk/pi-coding-agent/utils/event-bus";
import type { NativeScrollbackLiveRegion } from "@wxyhgk/pi-tui";
...
describe("InteractiveMode todo HUD anchor", () => {
	...
	it("renders a Todos tree: stage progression header, active stage expanded, others collapsed", () => {
		...
		const root = lines.find(line => line.includes("待办"));
