...
import type { ModelRegistry, ProviderDiscoveryState } from "@wxyhgk/pi-coding-agent/config/model-registry";
import { ModelRegistry as ModelRegistryImpl } from "@wxyhgk/pi-coding-agent/config/model-registry";
import { Settings } from "@wxyhgk/pi-coding-agent/config/settings";
import { ModelHubComponent } from "@wxyhgk/pi-coding-agent/modes/components/model-hub";
...
describe("issue #970 custom provider discovery", () => {
	...
	test("shows a provider-tab hint when discovery succeeds but returns zero models", async () => {
		...
		expect(rendered).toContain("发现成功但返回了 0 个模型");
		expect(rendered).toContain("/models 是否返回 { data: [{ id }] }");
		hub.dispose();
	});
	...
	test("shows a provider-tab hint when the discovery endpoint returns 404", async () => {
		...
		const rendered = normalizeRenderedText(hub.render(200).join("\n"));
		expect(rendered).toContain("发现端点 http://192.168.5.3:8085/v1/models 返回 404");
