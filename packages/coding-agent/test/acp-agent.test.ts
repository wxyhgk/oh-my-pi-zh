...
} from "@wxyhgk/pi-coding-agent/modes/acp/acp-agent";
import type { PlanModeState } from "@wxyhgk/pi-coding-agent/plan-mode/state";
import type {
	AgentSession,
	AgentSessionEvent,
	UsageFallbackConfirmation,
} from "@wxyhgk/pi-coding-agent/session/agent-session";
import { SILENT_ABORT_MARKER } from "@wxyhgk/pi-coding-agent/session/messages";
import { SessionManager } from "@wxyhgk/pi-coding-agent/session/session-manager";
import { DEFAULT_STT_MODEL_KEY, STT_MODEL_OPTIONS } from "@wxyhgk/pi-coding-agent/stt/models";
import {
	DEFAULT_TTS_LOCAL_MODEL_KEY,
	DEFAULT_TTS_VOICE,
	TTS_LOCAL_MODELS,
	TTS_LOCAL_VOICE_OPTIONS,
} from "@wxyhgk/pi-coding-agent/tts/models";
import { getConfigRootDir, setAgentDir } from "@wxyhgk/pi-utils";
...
describe("ACP agent", () => {
	...
	it("plan-proposal handler errors when the plan file is missing", async () => {
		...
		await expect(handler!("demo")).rejects.toThrow(/未找到计划文件/);
	...
	it("executes skill commands through custom skill messages", async () => {
		...
		expect(customMessage.content).toContain(`[技能目录:${skillDir}]`);
		expect(customMessage.content).toContain("用户:extra context");
