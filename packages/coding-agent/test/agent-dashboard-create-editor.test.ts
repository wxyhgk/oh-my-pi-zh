...
import { AgentDashboard } from "@wxyhgk/pi-coding-agent/modes/components/agent-dashboard";
import { initTheme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import * as discovery from "@wxyhgk/pi-coding-agent/task/discovery";
...
describe("AgentDashboard create editor", () => {
	test("keeps carriage return as multiline editor text", async () => {
		...
		expect(rendered).toContain("Ctrl+Q/Ctrl+Enter: 生成");
		expect(rendered).toContain("Enter: 换行");
	...
	test("submits multiline new-agent descriptions on CSI-u Ctrl+Enter", async () => {
		...
		expect(rendered).toContain("当前会话中模型注册表不可用。");
		expect(rendered).not.toContain("Description is required.");
	});
	...
	test("Ctrl+Q still works after pressing Enter for a newline (Windows Terminal)", async () => {
		...
		expect(afterSubmit).toContain("当前会话中模型注册表不可用。");
	});
});
...
describe("AgentDashboard layout", () => {
	test("fills the terminal height exactly and keeps the footer visible", async () => {
		...
		try {
			...
			expect(plain).toContain("Agent 控制中心");
...
describe("AgentDashboard prewalk", () => {
	test("shows the bundled task prewalk default when task.prewalk is enabled", async () => {
		...
		expect(rendered).toContain("Prewalk: 开");
		expect(rendered).not.toContain("Prewalk: 关");
	});
});
