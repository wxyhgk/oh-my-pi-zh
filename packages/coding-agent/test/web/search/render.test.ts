...
import { renderSearchResult, type SearchRenderDetails } from "@wxyhgk/pi-coding-agent/web/search/render";
import type { SearchResponse } from "@wxyhgk/pi-coding-agent/web/search/types";
...
function answerSection(lines: string[]): string {
	const start = lines.findIndex(l => / 回答 /.test(l));
	const end = lines.findIndex((l, i) => i > start && / 来源 /.test(l));
...
describe("renderSearchResult", () => {
	...
	it("shows the full answer when expanded — no answer truncation summary", async () => {
		...
		expect(answer).not.toMatch(/还有 \d+ 个行/);
	});
