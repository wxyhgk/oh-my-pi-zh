...
import { type AsyncJob, AsyncJobManager } from "@wxyhgk/pi-coding-agent/async/job-manager";
...
describe("task spawn routing", () => {
	...
	it("returns immediately on spawn and delivers the follow-up hint when the job completes", async () => {
		...
		expect(text).toContain("已派生 Agent `Spawnling`");
		const jobId = result.details?.async?.jobId;
		expect(jobId).toBeTruthy();
		expect(text).toContain(`作业 \`${jobId}\``);
		...
		expect(job!.resultText).toContain("Spawnling 现已空闲");
		expect(job!.resultText).toContain("通过 `hub` 发送消息以跟进");
