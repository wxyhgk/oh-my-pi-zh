...
import { AgentBusyError, type AgentTelemetryConfig, type Tracer } from "@wxyhgk/pi-agent-core";
import { type AssistantMessage, Effort } from "@wxyhgk/pi-ai";
import { Settings } from "@wxyhgk/pi-coding-agent/config/settings";
import type { ExtensionActions, LoadExtensionsResult } from "@wxyhgk/pi-coding-agent/extensibility/extensions/types";
...
import type { AuthStorage } from "@wxyhgk/pi-coding-agent/session/auth-storage";
import {
	finalizeSubprocessOutput,
	runSubprocess,
	SUBAGENT_WARNING_MISSING_YIELD,
} from "@wxyhgk/pi-coding-agent/task/executor";
...
describe("runSubprocess yield reminders", () => {
	...
	const baseOptions = {
		...
		modelRegistry: {
			refresh: async () => {},
		} as unknown as import("@wxyhgk/pi-coding-agent/config/model-registry").ModelRegistry,
		enableLsp: false,
	};
	...
	it("splices the subagent role prompt before the trailing system section", async () => {
		...
		expect(systemPrompt?.[2]).toMatch(/角色\n=+\n\ntest/);
	...
	it("keeps null yield warning when subagent submits success without data", async () => {
		...
		expect(result.output).toContain("SYSTEM WARNING: 子代理以 null 数据调用了 yield。");
	});
	...
	it("fails instead of waiting forever when yield submit errors repeat", async () => {
		...
		expect(result.stderr).toContain("子代理连续 6 次提交无效的 yield 结果");
		expect(result.stderr).toContain("为防止无限提交循环而停止");
	...
	it("marks pre-aborted subprocess with a concrete reason", async () => {
		...
		expect(result.aborted).toBe(true);
		expect(result.abortReason).toBe("启动前已取消");
		expect(result.stderr).toBe("启动前已取消");
	});
	...
	it("logs reminder-loop aborts at debug, not error (issue #1623)", async () => {
		...
		expect(result.aborted).toBe(true);
		expect(errorSpy).not.toHaveBeenCalledWith("子代理提示词失败", expect.anything());
		expect(debugSpy).toHaveBeenCalledWith("子代理提示词已中止");
	});
});
