...
import { formatResultOutputFallback } from "@wxyhgk/pi-coding-agent/task";
...
describe("runSubprocess request guards", () => {
	...
	it("still force-stops at 1.5x the soft budget when budget notices are disabled", async () => {
		...
		expect(result.abortReason).toContain("软请求预算超限");
	...
	it("salvages the last assistant text for an aborted child with no completed output", async () => {
		...
		expect(result.output).toContain("已取消:1 次请求");
		expect(result.output).toContain("150 tok");
		expect(result.output).toContain("最后活动:");
	...
	it("formats the (no output) fallback with the request count", () => {
		expect(formatResultOutputFallback({ output: "", stderr: "", requests: 7 })).toBe("(无输出) 已消耗 7 次请求");
		expect(formatResultOutputFallback({ output: "  ", stderr: "", requests: 0 })).toBe("(无输出)");
