...
import { TaskTool, taskSchema } from "@wxyhgk/pi-coding-agent/task";
import * as discoveryModule from "@wxyhgk/pi-coding-agent/task/discovery";
import { getTaskSchema, oneLineLabel } from "@wxyhgk/pi-coding-agent/task/types";
...
describe("task approval details surface the dispatch", () => {
	...
	it("surfaces agent, name, and task for a flat spawn", async () => {
		...
		expect(lines).toContain("名称:ReviewAuth");
	...
	it("summarizes a homogeneous batch whose agents use the session default", async () => {
		...
		expect(lines).toContain("上下文:\nshared background");
		expect(lines).toContain("批量 Agent:scout ×2");
		expect(lines).toContain("名称:DbMigrator");
		expect(lines).toContain("Agent:scout");
		expect(lines).toContain("任务:\nmigrate the schema");
		expect(lines).toContain("+1 个其他任务");
	});
	...
	it("summarizes mixed effective agents and safely renders partial batch items", async () => {
		...
		expect(lines).toContain("批量 Agent:scout ×1, reviewer ×1");
		expect(lines).toContain("名称:DefaultScout");
		expect(lines).toContain("Agent:scout");
