...
import type { AgentProgress, SingleResult, TaskToolDetails } from "@wxyhgk/pi-coding-agent/task";
import { taskToolRenderer } from "@wxyhgk/pi-coding-agent/task/renderer";
import { formatDuration, formatNumber } from "@wxyhgk/pi-utils";
...
describe("task renderer: nested live rendering", () => {
	...
	it("does not recurse forever when a nested task snapshot points at itself", async () => {
		...
		expect(text).toContain("… 嵌套任务进度已在上方显示");
	});
	...
	it("wraps the completed run summary in bracket glyphs, dropping the Total: label", async () => {
		...
		const expected = `${theme.format.bracketLeft}1 已完成${theme.sep.dot}1 失败${theme.sep.dot}${formatDuration(39_400)}${theme.format.bracketRight}`;
