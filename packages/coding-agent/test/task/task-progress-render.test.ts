...
import type { SettingPath, SettingValue } from "@wxyhgk/pi-coding-agent/config/settings";
...
import { taskToolRenderer } from "@wxyhgk/pi-coding-agent/task/renderer";
import type { AgentProgress, SingleResult, TaskToolDetails } from "@wxyhgk/pi-coding-agent/task/types";
...
describe("task progress rendering", () => {
	...
	it("shows the dispatch glyph in the header while agents run, not a spinner", async () => {
		...
		expect(stripped).toContain(`${theme.symbol("tool.task")} 任务`);
	...
	it("folds collapsed progress lists to the live edge with a status summary", async () => {
		...
		expect(collapsed).toContain("… 还有 3 个Agent");
		// The summary line sits above the visible rows (live edge at the bottom).
		expect(collapsed.indexOf("还有")).toBeLessThan(collapsed.indexOf("LiveOne"));
	...
	it("keeps problem rows visible when the collapsed result list folds", async () => {
		...
		expect(collapsed).toContain("… 还有 2 个Agent");
		// The run summary footer still counts the full batch.
		expect(collapsed).toContain("5 已完成");
		expect(collapsed).toContain("1 失败");
	});
});
