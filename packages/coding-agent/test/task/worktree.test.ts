...
import {
	...
	mergeTaskBranches,
	parseIsolationMode,
} from "@wxyhgk/pi-coding-agent/task/worktree";
...
import { removeWithRetries, setWorktreesDir } from "@wxyhgk/pi-utils";
...
describe("getRepoRoot", () => {
	...
	it("rejects pure jj workspaces with an actionable Jujutsu message", async () => {
		...
		await expect(getRepoRoot(dir)).rejects.toThrow(/纯 Jujutsu/);
	...
	it("preserves the generic git-not-found error for directories without any repo", async () => {
		...
		tempDirs.push(dir);
		await expect(getRepoRoot(dir)).rejects.toThrow("未找到 Git 仓库,无法执行隔离任务。");
	});
	...
	it("rejects a pure jj workspace nested inside an unrelated outer git checkout", async () => {
		...
		await expect(getRepoRoot(inner)).rejects.toThrow(/纯 Jujutsu/);
...
describe("applyNestedPatches", () => {
	...
	it("returns a stash-restore warning when pop conflicts with the agent commit", async () => {
		...
		expect(warnings[0]).toContain("无法自动恢复");
