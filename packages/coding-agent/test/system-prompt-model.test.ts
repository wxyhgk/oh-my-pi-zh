...
import { usesCodexTaskPrompt } from "@wxyhgk/pi-coding-agent/task/prompt-policy";
...
it("renders the prompt date in the startup timezone", async () => {
	...
	try {
		...
		expect(rendered).toContain(\`今天是 \${process.env.OMP_EXPECTED_DATE}\`);
		expect(rendered).not.toContain(\`今天是 \${process.env.OMP_REJECTED_DATE}\`);
	} finally {
		setSystemTime();
	}
});
...
describe("system prompt model identifier", () => {
	...
	it("renders the model identifier into the workstation block when provided", async () => {
		...
		expect(systemPrompt.join("\n\n")).toContain("- 模型:anthropic/claude-opus-4");
	});
