...
import { TodoCommandController } from "@wxyhgk/pi-coding-agent/modes/controllers/todo-command-controller";
import type { InteractiveModeContext } from "@wxyhgk/pi-coding-agent/modes/types";
import { type TodoPhase, USER_TODO_EDIT_CUSTOM_TYPE } from "@wxyhgk/pi-coding-agent/tools";
...
describe("TodoCommandController", () => {
	...
	it("exports the default TODO.md under the active session cwd", async () => {
		...
		expect(ctx.showStatus).toHaveBeenCalledWith(`已将待办事项写入 ${target}`);
	...
	it("imports the default TODO.md under the active session cwd", async () => {
		...
		expect(ctx.showStatus).toHaveBeenCalledWith(`已从 ${target} 导入 1 个阶段,1 项任务。`);
		expect(ctx.showError).not.toHaveBeenCalled();
	});
	...
	it("reports import parse errors without committing", async () => {
		...
		expect(ctx.showError).toHaveBeenCalledWith(expect.stringContaining(`无法解析 ${target}:`));
	...
	it("reports invalid internal-scheme import paths without committing", async () => {
		...
		expect(ctx.showError).toHaveBeenCalledWith(expect.stringContaining("读取待办事项失败:"));
		expect(ctx.showError).toHaveBeenCalledWith(expect.stringContaining("内部协议"));
		expect(ctx.session.setTodoPhases).not.toHaveBeenCalled();
		expect(ctx.setTodos).not.toHaveBeenCalled();
	});
	...
	it("reports invalid internal-scheme export paths", async () => {
		...
		expect(ctx.showError).toHaveBeenCalledWith(expect.stringContaining("写入待办事项失败:"));
		expect(ctx.showError).toHaveBeenCalledWith(expect.stringContaining("内部协议"));
	});
	...
	it("tells the model not to recreate the list after /todo rm (all)", async () => {
		...
		expect(text).toContain("用户有意清空了待办事项列表");
		expect(text).toMatch(/请勿重新创建或填充/);
	});
	...
	it("tells the model not to re-add a removed phase after /todo rm <phase>", async () => {
		...
		expect(reminderTextFrom(ctx)).toMatch(/请勿重新添加/);
	});
