...
import { MCPAuthorizationLinkPrompt } from "@wxyhgk/pi-coding-agent/modes/controllers/mcp-command-controller";
...
const COPY_URL_LABEL = "复制 URL:";
const SHORTCUT_LABEL = "本地快捷方式(仅限本机):";
...
describe("MCPAuthorizationLinkPrompt", () => {
	...
	it("renders a clickable hyperlink label and inline Copy URL row when the row fits the viewport", () => {
		...
		expect(plainLines[1]).toContain("点击此处授权");
