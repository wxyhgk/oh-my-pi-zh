...
import type { Rule } from "@wxyhgk/pi-coding-agent/capability/rule";
import { OmfgController } from "@wxyhgk/pi-coding-agent/modes/controllers/omfg-controller";
...
import { Container, type TUI } from "@wxyhgk/pi-tui";
...
describe("OmfgController", () => {
	it("saves a matching generated rule under project rules and registers it live", async () => {
		...
		expect(rendered).toContain("已实时注册");
	...
	it("asks before saving when validation never confirms a match", async () => {
		...
		expect(harness.showHookConfirm.mock.calls[0]?.[0]).toBe("验证");
	...
	it("returns to save selection when amendment input is cancelled", async () => {
		...
		expect(harness.showHookSelector.mock.calls).toEqual([
			["将 TTSR 规则保存到哪里?", [PROJECT_OPTION, GLOBAL_OPTION, AMEND_OPTION]],
			["将 TTSR 规则保存到哪里?", [PROJECT_OPTION, GLOBAL_OPTION, AMEND_OPTION]],
		]);
	...
	it("guards empty complaints and missing models before model calls", async () => {
		...
		expect(emptyHarness.ctx.showStatus).toHaveBeenCalledWith("用法:/omfg <问题>");
		...
		expect(runEphemeralTurn).not.toHaveBeenCalled();
		expect(missingModelHarness.ctx.showError).toHaveBeenCalledWith("没有可用于 /omfg 的活动模型。");
	});
