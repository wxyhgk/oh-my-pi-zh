...
import { LoginDialogComponent } from "@wxyhgk/pi-coding-agent/modes/components/login-dialog";
...
describe("SelectorController login", () => {
	it("awaits a provider-scoped online refresh, then presents OAuth success", async () => {
		...
		expect(renderPresented(presentedBlocks)).toContain("已成功登录 xai-oauth");
	...
	it("Esc during a pending login aborts the flow and restores the editor", async () => {
		...
		expect(ctx.showError).not.toHaveBeenCalled();
		expect(ctx.showStatus).toHaveBeenCalledWith("登录已取消");
		expect(editorSlot).toEqual([editor]);
		expect(renderPresented(presentedBlocks)).not.toContain("已成功登录");
	});
