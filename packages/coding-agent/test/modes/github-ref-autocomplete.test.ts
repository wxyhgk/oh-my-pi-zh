...
import { KeybindingsManager as AppKeybindingsManager } from "@wxyhgk/pi-coding-agent/config/keybindings";
import { getGithubRefContext, getGithubRefSuggestions } from "@wxyhgk/pi-coding-agent/modes/github-ref-autocomplete";
import { createPromptActionAutocompleteProvider } from "@wxyhgk/pi-coding-agent/modes/prompt-action-autocomplete";
import type { SlashCommand } from "@wxyhgk/pi-tui";
...
describe("github-ref autocomplete — suggestions", () => {
	it("offers both candidates when no qualifier is given", () => {
		...
		expect(result!.items).toEqual([
			{ value: "pr://3164", label: "PR #3164", description: "GitHub 拉取请求" },
	...
	it("offers only the PR candidate for a pr/pull qualifier", () => {
		...
		expect(result!.items).toEqual([{ value: "pr://3164", label: "PR #3164", description: "GitHub 拉取请求" }]);
	});
...
describe("github-ref autocomplete — provider integration", () => {
	...
	it("offers GitHub references inside prompt-bearing skill command arguments", async () => {
		...
		expect(suggestions).toEqual({
			prefix: "#123",
			items: [
				{ value: "pr://123", label: "PR #123", description: "GitHub 拉取请求" },
