...
import { CopySelectorComponent } from "@wxyhgk/pi-coding-agent/modes/components/copy-selector";
import { getThemeByName, setThemeInstance, theme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import type { CopyTarget } from "@wxyhgk/pi-coding-agent/modes/utils/copy-targets";
...
describe("CopySelectorComponent", () => {
	...
	it("renders human-readable keybinding hints in the footer", () => {
		...
		expect(out).toContain("Enter 复制");
		expect(out).toContain("Ctrl+G 退出");
	});
