...
} from "@wxyhgk/pi-coding-agent/modes/components/assistant-message";
import { initTheme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import { setTerminalImageProtocol, TERMINAL } from "@wxyhgk/pi-tui";
...
describe("AssistantMessageComponent error rendering", () => {
	...
	it("width-truncates an overlong error line", () => {
		const lines = renderLines(erroredMessage(proxy502));
		const head = lines.find(line => line.trim().startsWith("错误:"));
	...
	it("renders a short single-line error unchanged", () => {
		...
		expect(lines.some(line => line.includes("错误: overloaded_error: Overloaded"))).toBe(true);
	});
});
