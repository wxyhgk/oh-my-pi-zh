...
import type { ExtensionAskDialogQuestion } from "@wxyhgk/pi-coding-agent/extensibility/extensions/types";
import { AskDialogComponent } from "@wxyhgk/pi-coding-agent/modes/components/ask-dialog";
...
describe("AskDialogComponent", () => {
	...
	it("n on an option calls onPrompt and stores note with marker", async () => {
		...
		expect(onPrompt.mock.calls[0][0]).toBe("Option A 的备注: Choose one?");
	...
	it("bounds custom input prompt title for long multi-line questions", async () => {
		...
		expect(stripVTControlCharacters(title)).toContain("自定义回答:");
	});
	...
	it("bounds note prompt title for long multi-line questions", async () => {
		...
		expect(stripVTControlCharacters(title)).toContain("Option A 的备注:");
	});
	...
	it("multi-select: Enter on a checked option toggles it off; empty answer submits from Submit tab", () => {
		...
		component.handleInput(TAB);
		expect(render(component)).toContain("未回答");
		component.handleInput(ENTER);
