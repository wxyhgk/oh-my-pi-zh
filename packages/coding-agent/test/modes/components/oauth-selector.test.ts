...
import { OAuthSelectorComponent } from "@wxyhgk/pi-coding-agent/modes/components/oauth-selector";
...
describe("OAuthSelectorComponent", () => {
	it("fuzzy-filters overflowing provider lists from typed input", () => {
		...
		expect(rendered).toContain(`搜索: ${target.id}`);
	...
	it("does not offer env-only providers as logout targets", () => {
		...
		const rendered = component
			...
			.map(line => Bun.stripANSI(line))
			.join("\n");
		expect(rendered).toContain("没有已存储的提供商凭据可退出登录");
