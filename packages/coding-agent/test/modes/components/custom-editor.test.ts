...
import { CURSOR_MARKER } from "@wxyhgk/pi-tui";
...
describe("CustomEditor queue shorthand decoration", () => {
	...
	it("renders the reserved line as a dim Queueing header", () => {
		for (const prefix of ["->", "=>"]) {
			...
			expect(editor.decorateText(prefix)).toBe(theme.fg("dim", `排队中 ${theme.nav.selected}`));
			editor.focused = true;
			const rendered = editor.render(40).map(line => Bun.stripANSI(line.replace(CURSOR_MARKER, "")));
			expect(rendered.some(line => line.includes(`排队中 ${theme.nav.selected}`))).toBe(true);
