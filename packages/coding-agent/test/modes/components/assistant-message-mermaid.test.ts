...
import type { AssistantThinkingRenderer } from "@wxyhgk/pi-coding-agent/extensibility/extensions";
import { AssistantMessageComponent } from "@wxyhgk/pi-coding-agent/modes/components/assistant-message";
import { clearMermaidCache } from "@wxyhgk/pi-coding-agent/modes/theme/mermaid-cache";
import { initTheme } from "@wxyhgk/pi-coding-agent/modes/theme/theme";
import { ImageProtocol, setTerminalImageProtocol, TERMINAL, Text } from "@wxyhgk/pi-tui";
...
describe("AssistantMessageComponent images", () => {
	it("renders native assistant images in content order and honors image visibility", () => {
		...
		expect(rendered.indexOf("Before image")).toBeLessThan(rendered.indexOf("[图片: image/png]"));
		expect(rendered.indexOf("[图片: image/png]")).toBeLessThan(rendered.indexOf("After image"));
		component.setImagesVisible(false);
		expect(Bun.stripANSI(component.render(80).join("\n"))).not.toContain("[图片: image/png]");
	});
	...
	it("converts WebP tool images for Kitty terminal rendering", async () => {
		...
		expect(rendered).not.toContain("[图片: image/webp]");
	});
});
