...
import type { HistoryEntry, HistoryStorage } from "@wxyhgk/pi-coding-agent/session/history-storage";
...
describe("HistorySearchComponent", () => {
	...
	it("distinguishes an empty query from an unmatched query", () => {
		...
		expect(render(empty).plain).toContain("暂无历史");
		...
		type(unmatched, "zzzz");
		expect(render(unmatched).plain).toContain("没有匹配的历史");
	});
});
