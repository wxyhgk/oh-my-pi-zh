...
import type { CompactionQueuedMessage, InteractiveModeContext } from "@wxyhgk/pi-coding-agent/modes/types";
...
import { SKILL_PROMPT_MESSAGE_TYPE, type SkillPromptDetails } from "@wxyhgk/pi-coding-agent/session/messages";
import { SessionManager } from "@wxyhgk/pi-coding-agent/session/session-manager";
import { Container } from "@wxyhgk/pi-tui";
...
describe("compaction skill re-invocation", () => {
	...
	it("re-invokes a queued skill as a user-attributed skill prompt", async () => {
		...
		expect(renderedText.text).toContain(`[技能目录:${tempDir.path()}]`);
...
describe("UiHelpers / InputController against derived queued custom display", () => {
	...
	it("renders the compact slash form for queued skills", async () => {
		...
		expect(rendered).toContain("转向 · 1");
	...
	it("groups yield follow-ups under one heading", async () => {
		...
		const rendered = Bun.stripANSI(pendingMessagesContainer.render(120).join("\n"));
		expect(rendered).toContain("yield 之后 · 3");
