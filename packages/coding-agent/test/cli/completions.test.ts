...
import { buildSpec, type CompletionSpec, generateCompletion } from "@wxyhgk/pi-coding-agent/cli/completion-gen";
import type { CliConfig, CommandCtor } from "@wxyhgk/pi-utils/cli";
...
describe("omp completions (integration / drift)", () => {
	it("emits a zsh script reflecting the live command + flag surface", async () => {
		...
		expect(stdout).toContain("command omp-zh __complete $kind");
