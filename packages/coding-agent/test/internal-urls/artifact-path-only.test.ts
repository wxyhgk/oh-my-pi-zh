...
import { ArtifactProtocolHandler } from "@wxyhgk/pi-coding-agent/internal-urls/artifact-protocol";
...
import { resolveToolSearchScope } from "@wxyhgk/pi-coding-agent/tools/path-utils";
...
describe("artifact:// path-only resolution", () => {
	...
	it("still rejects full content resolution for large artifacts (existing OOM guard)", async () => {
		const url = parseInternalUrl("artifact://0");
		await expect(handler.resolve(url)).rejects.toThrow(/无法进行完整内部解析/);
	});
