...
describe("SshProtocolHandler", () => {
	...
	it("rejects user/port overrides on a configured host", async () => {
		...
		mockReadBytes("x");
		await expect(handler.resolve(parseInternalUrl("ssh://user@icaro:22/x"))).rejects.toThrow(/用户\/端口覆盖/);
	});
	...
	it("rejects a binary / non-UTF-8 file instead of returning a resource", async () => {
		...
		await expect(handler.resolve(parseInternalUrl("ssh://icaro/bin/true"))).rejects.toThrow(/二进制或非 UTF-8/);
	});
	...
	it("rejects a file that exceeds the size cap", async () => {
		...
		await expect(handler.resolve(parseInternalUrl("ssh://icaro/big.log"))).rejects.toThrow(/超过 1 MiB 限制/);
	});
	...
	it("rejects a remote special file (FIFO/device) without reading it", async () => {
		...
		await expect(handler.resolve(parseInternalUrl("ssh://icaro/dev/zero"))).rejects.toThrow(/不是普通文件/);
	...
	it("shows a helpful message when no hosts are configured", async () => {
		...
		expect(res.content).toMatch(/未配置任何 SSH 主机/);
	});

	it("rejects a host-less ssh:// URL that carries a path", async () => {
		mockHosts();
		await expect(handler.resolve(parseInternalUrl("ssh:///etc/hosts"))).rejects.toThrow(/需要在路径前指定主机/);
	});

	it("rejects an explicit ssh:// port 0 before connecting", async () => {
		mockHosts();
		await expect(handler.resolve(parseInternalUrl("ssh://icaro:0/etc/hostname"))).rejects.toThrow(/端口 0/);
	});
	...
	it("rejects a malformed or out-of-range ssh:// port before connecting", async () => {
		mockHosts();
		await expect(handler.resolve(parseInternalUrl("ssh://prod:abc/etc"))).rejects.toThrow(/主机或端口无效/);
		await expect(handler.resolve(parseInternalUrl("ssh://prod:65536/etc"))).rejects.toThrow(/主机或端口无效/);
	});

	it("rejects an empty ssh:// port before connecting", async () => {
		mockHosts();
		await expect(handler.resolve(parseInternalUrl("ssh://prod:/etc/hosts"))).rejects.toThrow(/端口为空/);
		await expect(handler.resolve(parseInternalUrl("ssh://user@prod:/etc/hosts"))).rejects.toThrow(/端口为空/);
		await expect(handler.resolve(parseInternalUrl("ssh://[::1]:/etc/hosts"))).rejects.toThrow(/端口为空/);
		await expect(handler.resolve(parseInternalUrl("ssh://prod%2Dblue:/etc/hosts"))).rejects.toThrow(/端口为空/);
		await expect(handler.resolve(parseInternalUrl("ssh://u%2Dname@prod:/etc/hosts"))).rejects.toThrow(/端口为空/);
	});
	...
	it("rejects ssh:// URL queries and fragments instead of operating on the truncated path", async () => {
		...
		await expect(handler.resolve(parseInternalUrl("ssh://h/tmp/a?draft"))).rejects.toThrow(/查询字符串/i);
		await expect(handler.resolve(parseInternalUrl("ssh://h/tmp/a#draft"))).rejects.toThrow(/URL 片段/i);
