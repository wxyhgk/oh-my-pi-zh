...
import { createAutoresearchExtension } from "@wxyhgk/pi-coding-agent/autoresearch";
import {
...
} from "@wxyhgk/pi-coding-agent/autoresearch/state";
import { AutoresearchStorage, closeAllAutoresearchStorages } from "@wxyhgk/pi-coding-agent/autoresearch/storage";
import type { ExperimentResult } from "@wxyhgk/pi-coding-agent/autoresearch/types";
import type {
	ExtensionAPI,
...
describe("autoresearch slash command", () => {
	...
	it("enables autoresearch with a notify when invoked bare in a clean repo", async () => {
		...
		expect(harness.notifications.some(n => n.message.includes("Autoresearch 已启用"))).toBe(true);
	});
	...
	it("aborts with an error when the worktree is dirty", async () => {
		...
		await harness.command.handler("", harness.ctx);
		expect(harness.notifications.some(n => n.type === "error" && n.message.includes("工作区不干净"))).toBe(true);
		// Should abort: no enabled notification, no checkout, no message sent
		expect(harness.notifications.some(n => n.message.includes("Autoresearch 已启用"))).toBe(false);
