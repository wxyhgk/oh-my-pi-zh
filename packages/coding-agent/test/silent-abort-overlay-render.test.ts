...
import type { ObservableSession } from "@wxyhgk/pi-coding-agent/modes/session-observer-registry";
...
function makeSubagentRegistry(sessions: ObservableSession[]) {
	...
	} as unknown as import("@wxyhgk/pi-coding-agent/modes/session-observer-registry").SessionObserverRegistry;
}
...
describe("Agent hub silent-abort regression", () => {
	...
	it("renders normal error messages with an Error: line", () => {
		...
		// AssistantMessageComponent renders the error as "错误: <message>"
		expect(renderedText).toContain("错误: Connection timed out");
	});
});
