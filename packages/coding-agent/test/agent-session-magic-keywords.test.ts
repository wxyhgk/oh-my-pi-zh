...
import * as autoThinkingClassifier from "@wxyhgk/pi-coding-agent/auto-thinking/classifier";
...
describe("AgentSession magic keyword settings", () => {
	...
	it("renders the eval-specific workflowz notice", async () => {
		...
		expect(notice?.content).toContain("**Python(`eval`,Python 后端):**");
		expect(notice?.content).toContain("**JavaScript(`eval`,JavaScript 后端):**");
	});
