...
import { Agent, AgentBusyError, ThinkingLevel } from "@wxyhgk/pi-agent-core";
import type { AssistantMessage, Usage } from "@wxyhgk/pi-ai";
import * as AIError from "@wxyhgk/pi-ai/error";
import { KeybindingsManager } from "@wxyhgk/pi-coding-agent/config/keybindings";
...
import { resolveLocalUrlToPath } from "@wxyhgk/pi-coding-agent/internal-urls";
import { AssistantMessageComponent } from "@wxyhgk/pi-coding-agent/modes/components/assistant-message";
import type { HookSelectorSlider } from "@wxyhgk/pi-coding-agent/modes/components/hook-selector";
import {
	type PlanReviewAnnotationState,
	PlanReviewOverlay,
} from "@wxyhgk/pi-coding-agent/modes/components/plan-review-overlay";
...
import type { SubmittedUserInput } from "@wxyhgk/pi-coding-agent/modes/types";
...
import { SILENT_ABORT_MARKER, USER_INTERRUPT_LABEL } from "@wxyhgk/pi-coding-agent/session/messages";
...
import { type OverlayHandle, type OverlayOptions, setKeybindings, Text } from "@wxyhgk/pi-tui";
import { formatNumber, TempDir } from "@wxyhgk/pi-utils";
...
describe("InteractiveMode plan review rendering", () => {
	...
	it("keeps confirmation before exiting a non-empty plan", async () => {
		...
		expect(confirm).toHaveBeenCalledWith("退出计划模式?", "这将退出计划模式,且不会批准计划。");
	...
	it("forwards each submitted plan to the review overlay", async () => {
		...
		mode.planModeEnabled = true;
		mode.planModePlanFilePath = planFilePath;
		const review = vi.spyOn(mode, "showPlanReview").mockResolvedValue("完善计划");
	...
	it("dismisses Plan Review and restores input when a provider error is pinned", async () => {
		...
		const choice = mode.showPlanReview("# Plan\n\nReady for approval.", "计划模式 - 下一步", ["Approve"]);
	...
	it("copies the overlay's current edited plan markdown from the real plan review overlay", async () => {
		...
		expect(statusSpy).toHaveBeenCalledWith("计划已复制到剪贴板");
	...
	it("Refine with no annotations silently aborts approval and returns to the editor", async () => {
		...
		expect(statusSpy).toHaveBeenCalledWith("完善计划:请输入后续提示词。");
		expect(errorSpy).not.toHaveBeenCalledWith(expect.stringContaining("完善计划失败"));
	...
	it("approves with in-overlay edits and mirrors them to the plan file", async () => {
		...
		const promptSpy = vi.spyOn(session, "prompt").mockImplementation(async promptText => {
			if (typeof promptText === "string" && promptText.startsWith("计划已批准。")) {
	...
	it("offers approve-and-keep-context as a distinct plan approval path", async () => {
		...
		const selector = vi.spyOn(mode, "showPlanReview").mockResolvedValue("完善计划");
	...
	it("measures keep-context approval against the execution model restored after plan mode", async () => {
		...
		expect(selector.mock.calls[0]?.[2]).toEqual([
			"批准并执行",
			"批准并压缩上下文",
			`批准并保留上下文(~${compactNumber(tokens)} / ${compactNumber(executionModel.contextWindow)})`,
			"完善计划",
		]);
	});
	...
	it("approves a plan without clearing the session when keeping context", async () => {
		...
		mode.planModePlanFilePath = planFilePath;
		vi.spyOn(session, "getContextUsage").mockReturnValue(undefined);
		vi.spyOn(mode, "showPlanReview").mockResolvedValue("批准并保留上下文");
	...
	it("hides the review overlay before the blocking execution turn resolves", async () => {
		...
		// "批准并保留上下文" (index 2) — that branch keeps the session, so no
	...
	it("queues the approved plan as a synthetic follow-up when a turn is already in flight", async () => {
		...
		expect(errorSpy).not.toHaveBeenCalledWith(expect.stringContaining("完成批准计划失败"));
		expect(promptSpy).not.toHaveBeenCalled();
	...
	it("lands the approved plan behind a user turn queued during approve-and-compact", async () => {
		// End-to-end contract: choosing "批准并压缩上下文" runs
	...
	it("compaction runs on the plan model and restores the pre-plan model after success", async () => {
		...
		vi.spyOn(session, "getContextUsage").mockReturnValue(undefined);
		vi.spyOn(mode, "showPlanReview").mockResolvedValue("批准并压缩上下文");
		vi.spyOn(session, "prompt").mockResolvedValue(undefined as never);
	...
	it("cancelled compaction restores the pre-plan model before exiting", async () => {
		...
		// #planModePreviousModelState, so an aborted "批准并压缩上下文"
	...
	/**
	 * Drives `handlePlanApproval` with the "批准并压缩上下文"
	...
	it("re-enters plan mode on the approved titled artifact after approval", async () => {
		...
		vi.spyOn(session, "getContextUsage").mockReturnValue(undefined);
		const selector = vi.spyOn(mode, "showPlanReview").mockResolvedValue("批准并保留上下文");
	...
	describe("openPlanReview (manual /plan-review)", () => {
		...
		it("warns and does not start approval when plan mode is inactive", async () => {
			...
			expect(warn).toHaveBeenCalledWith("计划模式未激活。");
		});
		...
		it("warns when no plan file has been written yet", async () => {
			...
			expect(approval).not.toHaveBeenCalled();
			expect(warn).toHaveBeenCalledWith(expect.stringContaining("还没有可审查的计划"));
		});
	});
});
