...
import { computeFileHash } from "@wxyhgk/hashline";
...
import type { ClientBridge } from "@wxyhgk/pi-coding-agent/session/client-bridge";
...
describe("write tool ACP fs routing", () => {
	...
	it("emits a progress snapshot before filesystem writes complete", async () => {
		...
		expect(updates[0]?.content).toEqual([
			{ type: "text", text: `正在向 progress.txt 写入 ${FILE_CONTENT.length} 字节...` },
		]);
		expect(updates[0]?.details).toEqual({ resolvedPath: filePath });
		expect(resultText(result)).toContain(`已成功向 progress.txt 写入 ${FILE_CONTENT.length} 字节`);
	});
	...
	it("rejects read-only internal URLs without creating scheme-looking paths on disk", async () => {
		...
		expect(rejection.message).toContain("memory:// URL 为只读,不能写入;如需修改请使用该协议对应的工具。");
	});
});
