...
import { type BenchModelRegistry, runBenchCommand } from "@wxyhgk/pi-coding-agent/cli/bench-cli";
...
describe("bench cache mode", () => {
	it("splits the cache breakpoint prefix from each variable suffix with native OMP messages", async () => {
		...
		expect(coldMessages?.[1]?.content).toBe("缓存基准测试后缀 A。");
		expect(warmMessages?.[1]?.content).toBe("缓存基准测试后缀 B。");
		...
		expect(stdout).not.toContain("缓存基准测试后缀");
	});
	...
	it("gives every pair a private stable namespace and flags a prewarmed cold request", async () => {
		...
		expect(stdout).toContain("cold (已预热)");
	});
		...
		const benchmark = runBenchCommand(
			...
			},
			{
				...
				streamSimple: (_model, context, options) => {
					...
					const phase = context.messages[1]?.content === "缓存基准测试后缀 B。" ? "warm" : "cold";
					calls.push({
						phase,
						cacheKey,
	...
	it("prints cold and warm cache token breakdowns and cost for human output", async () => {
		...
		expect(stdout).toContain(
			"cold prompt_cache_write_observed 输入 20 缓存读取 0 缓存写入 100 输出 2 总计 122 费用 $4.00",
		);
		expect(stdout).toContain(
			"warm prompt_cache_read_observed 输入 20 缓存读取 100 缓存写入 0 输出 2 总计 122 费用 $4.00",
		);
		expect(stdout).toContain("TTFT");
		expect(stdout).toContain("耗时");
		expect(stdout).toContain("吞吐量");
	});
