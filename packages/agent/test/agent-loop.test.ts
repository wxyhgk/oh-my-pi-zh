...
} from "@wxyhgk/pi-agent-core/agent-loop";
import type {
	AgentContext,
	AgentEvent,
	AgentLoopConfig,
	...
	AgentToolContext,
	ToolCallContext,
} from "@wxyhgk/pi-agent-core/types";
import { ASIDE_MESSAGE_COMMIT, ASIDE_MESSAGE_DISCARD } from "@wxyhgk/pi-agent-core/types";
import type { AssistantMessage, AssistantMessageEvent, Context, Message, ToolResultMessage } from "@wxyhgk/pi-ai";
import { createMockModel, type MockResponse } from "@wxyhgk/pi-ai/providers/mock";
...
declare module "@wxyhgk/pi-agent-core/types" {
...
describe("agentLoop kCursorExecResolved (issue #4348)", () => {
	it("skips execute for a toolCall block marked as already run by Cursor's exec channel", async () => {
		const { kCursorExecResolved } = await import("@wxyhgk/pi-ai/utils/block-symbols");
